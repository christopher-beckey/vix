...
namespace gov.va.med.imaging.exchange.VixInstaller.business
{
    public static class ZipUtilities
    {
        ...
        // Obtains the umber of files within the specified zip archive
        ...
        ///  </param>
        public static void CreateNewZipFile(string sourceDirectoryName, string destinationArchiveFileName)
        {
        ...
        /// The directory to extract the zip file to
        /// </param>
        /// /// <param name="destinationSubFolderName">
        /// The subfolder directory to extract the zip file to
        /// </param>
        /// <param name="overwriteMethod">
        /// Specifies how we are going to handle an existing file.
        /// The default is Always.
        /// </param>
        public static void ImprovedExtractSubFolderToDirectory(string sourceArchiveFileName,
                                                      string destinationDirectoryName,
                                                      string destinationSubFolderName,
                                                      Overwrite overwriteMethod = Overwrite.Always)
        {
            //Opens the zip file up to be read
            using (ZipArchive archive = System.IO.Compression.ZipFile.OpenRead(sourceArchiveFileName))
            {
                var subFolderResult = from currEntry in archive.Entries
                             where Path.GetDirectoryName(currEntry.FullName) == destinationSubFolderName
                             where !String.IsNullOrEmpty(currEntry.Name)
                             select currEntry;

                //Loops through each file in the zip file
                foreach (ZipArchiveEntry file in subFolderResult)
                {
                    //ZipArchiveEntry propertyExternalAttributes is low order byte,
                    //cast it as FileAttributes and check the attributes if it is a "Directory" or not.
                    var lowerByte = (byte)(file.ExternalAttributes & 0x00FF);
