...
namespace ECGConversion.DICOM
{
	...
	public sealed class DICOMFormat : IECGFormat, ISignal, IDemographic, IDiagnostic, IGlobalMeasurement
	{
		...
		private int _StartPerBeatMeasurement
		{
			get
			{
				int ret = 100;
		...
		public bool _ConfigurationWorks()
		{
			try
			{
				...
				return (_GenerateSequenceNr != GenerateSequenceNr.Error)
					&& (_MortaraDiagCompat != MortaraDiagCompat.Error)
					&& (spbm >= 0)
		...
		public int setDiagnosticStatements(Statements stat)
		{
			...
			&&  (stat.statement.Length > 0))
			{
				...
				else
				{
					...
					if ((mdc & MortaraDiagCompat.HistoryOnly) == 0)
						itemsToKeep = new Dataset[nr];
				...
				else if ((mdc & MortaraDiagCompat.HistoryOnly) == 0)
				{
					_DICOMData.Remove(Tags.VerificationDateTime);
					_DICOMData.Remove(Tags.ObservationDateTime);
				}

				if ((mdc & MortaraDiagCompat.StateOnly) != 0)
				{
					if (stat.confirmed)
					{
						Dataset ds = element.AddNewItem();
		...
		public int getGlobalMeasurements(out GlobalMeasurements mes)
		{
			...
			&&	(resultMeasurements != null))
			{
				try
				{
					int spbm = _StartPerBeatMeasurement;
					...
					for (int n=resultAvgRR_PP.GetLength(0)-1;n >= 0;n--)
					{
						...
						if (!string.IsNullOrEmpty(resultAvgRR_PP[n, 4]))
							mes.PRint = ParseUShort(resultAvgRR_PP[n, 4]);

                        if (!string.IsNullOrEmpty(resultAvgRR_PP[n, 5]))
							mes.QRSdur = ParseUShort(resultAvgRR_PP[n, 5]);

                        if (!string.IsNullOrEmpty(resultAvgRR_PP[n, 6]))
		...
		public int setGlobalMeasurements(GlobalMeasurements mes)
		{
			...
			&&	(mes.measurment != null))
			{
				...
				else
				{
					...
					if ((_MortaraDiagCompat & MortaraDiagCompat.HistoryOnly) == 0)
						itemsToKeep = new Dataset[nr];
