...
{
    public class DSRProcedureLogConstraintChecker : DSRIODConstraintChecker
    {
        /// <summary>
        /// constructor
        /// </summary>
        public DSRProcedureLogConstraintChecker()
        {
        }
        ...
        {
            return E_DocumentType.DT_ProcedureLog;
        }
                                                     ...
                                                     bool byReference = false)
        {
            ...
            if (!byReference)
            {
                ...
                /* row 2 of the table */
                else if (relationshipType == E_RelationshipType.RT_hasObsContext)
                {
                    result = (targetValueType == E_ValueType.VT_Text) || (targetValueType == E_ValueType.VT_Code) || (targetValueType == E_ValueType.VT_Num) ||
                ...
                /* row 5 of the table */
                else if ((relationshipType == E_RelationshipType.RT_hasProperties) && (sourceValueType != E_ValueType.VT_Container))
                {
                    result = (targetValueType == E_ValueType.VT_Text) || (targetValueType == E_ValueType.VT_Code) || (targetValueType == E_ValueType.VT_Num) ||
                             (targetValueType == E_ValueType.VT_DateTime) || (targetValueType == E_ValueType.VT_UIDRef) || (targetValueType == E_ValueType.VT_PName);
                }
                /* row 6 of the table */
                else if ((relationshipType == E_RelationshipType.RT_inferredFrom) &&
                    ((sourceValueType == E_ValueType.VT_Text) || (sourceValueType == E_ValueType.VT_Code) || (sourceValueType == E_ValueType.VT_Num)))
                {
                    result = (targetValueType == E_ValueType.VT_Composite) || (targetValueType == E_ValueType.VT_Image) || (targetValueType == E_ValueType.VT_Waveform);
                }
            }
