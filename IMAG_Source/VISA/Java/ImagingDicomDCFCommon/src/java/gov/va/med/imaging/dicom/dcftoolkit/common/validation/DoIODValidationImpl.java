...
public class DoIODValidationImpl implements IDoIODValidation {
	...
	public int doIODValidation(DicomAE dicomAE,	IDicomDataSet dataset) throws IODViolationException {
		...
		if (dicomAE.isValidateIODs()) {
			try {
                ...
                logger.error("Unknown SOP Class for IOD Validation: {}", dataset.getSOPClass());
    ...
    public  IIODViolationList validateIOD(IDicomDataSet dds)
		...
		try{
            ...
            logger.error("{}: Exception thrown while validating object IOD. /n{}", this.getClass().getName(), viodX.getMessage());
    	...
    	if(ae.isValidateVRs()){
        	...
        	if(ae.isRelaxValidation()){
            	if(vrList.hasErrors()){
                    ...
                    logger.warn("{}: VR Violation Error(s) in {} message from {}.\nHowever, an attempt will still be made to process the Dimse Message.\nVR Violations= {}", this.getClass().getName(), DimseMessage.translateCommand(commandField), ae.getRemoteAETitle(), str);
