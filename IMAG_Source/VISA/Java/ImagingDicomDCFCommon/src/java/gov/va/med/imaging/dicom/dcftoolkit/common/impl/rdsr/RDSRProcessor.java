...
public class RDSRProcessor
{
	...
	public List<Dose> getDose(SOPInstance sopInstance)
	{
		...
		logger.info("Parsing the DICOM header and building the object graph...");
		parser.parseRDSR();
	...
	{
		logger.info("Checking for CT Dose using the template identifier...");
		...
		{
			logger.info("No data found using the template identifer. Looking for CT Acquisition nodes instead.");
		...
		{
			int counter = 1;
			logger.info("Found CT dose data. Details to follow...");
			...
			for (Vertex doseTemplate : doseTemplates)
			{
				...
				doseList.add(ctDose);
                logger.info("\n* CT dose object {}:\n{}", counter, ctDose.toString());
				counter++;
			}
		}
		else
		{
			logger.info("No CT dose data found in this DICOM object.");
		}
	}

	private void extractFluoroDoses(List<Dose> doseList, SOPInstance sopInstance)
	{
		logger.info("Checking for X-Ray (Fluoro) Dose data using the template identifier...");
		...
		if (doseTemplates == null || !doseTemplates.iterator().hasNext())
		{
			logger.info("No data found using the template identifer. Looking for Accumulated X-Ray Dose Data nodes instead.");
		...
		{
			int counter = 1;
			logger.info("Found X-Ray (Fluoro) dose data. Details to follow...");
			for (Vertex doseTemplate : doseTemplates)
			{
                ...
                logger.info("\n* X-Ray (Fluoro) dose object {}:\n{}", counter, fluoroDose.toString());
				counter++;
			}
		}
		else
		{
			logger.info("No X-Ray (Fluoro) dose data found in this DICOM object.");
		}
	}
}
