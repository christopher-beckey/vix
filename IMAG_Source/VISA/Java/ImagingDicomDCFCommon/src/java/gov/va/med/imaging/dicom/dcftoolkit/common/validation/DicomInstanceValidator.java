...
public class DicomInstanceValidator {
	...
	private class OurURIResolver implements URIResolver {
		...
		public Source resolve(String href,String base) throws TransformerException {
            logger.debug("OurURIResolver.resolve() href={} base = {}", href, base);
			
			// Fortify wanted to close this stream.  Shouldn't.
	...
	private DicomInstanceValidator() throws javax.xml.transform.TransformerConfigurationException {
		...
		// Fortify change: used "new" try-with-resources to see if Fortify still complains
		try (InputStream transformStream = DicomInstanceValidator.class.getResourceAsStream(compiledXSLFile)) {
			...
			// this helps us find the common rules in the jar file
			tf.setURIResolver(new OurURIResolver());
			...
			tf.setAttribute("http://apache.org/xml/features/disallow-doctype-decl", true);

			transformer = tf.newTransformer(transformSource);
		} catch (IOException e) {
			logger.debug("DicomInstanceValidator() --> Initialization failed with IOException....");
			throw new javax.xml.transform.TransformerConfigurationException(e.getMessage(), e);
		}
		
		/* No longer needed per Fortify change above
	...
	private DicomInstanceValidator(String path) throws javax.xml.transform.TransformerConfigurationException {
		logger.debug("Initializing DicomInstanceValidator(String)...");
		...
		try (InputStream transformStream = DicomInstanceValidator.class.getResourceAsStream(compiledXSLFile)) {
			...
			logger.debug("DicomInstanceValidator(String) --> Initialization failed with IOException....");
			throw new javax.xml.transform.TransformerConfigurationException(e.getMessage(), e);
		}
