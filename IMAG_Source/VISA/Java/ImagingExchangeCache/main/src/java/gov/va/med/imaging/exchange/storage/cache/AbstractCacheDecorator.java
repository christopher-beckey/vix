...
implements RealizedCache
{
	...
	public static String[] createInternalInstanceGroupKeys(GlobalArtifactIdentifier gaid, boolean documentIdIsGroup )
	{
		if(gaid instanceof ImageURN)
		{
	...
	throws CacheException
	{
		...
		{
			// Fortify change: added to close resource
			try { if(metadataWritable != null) { metadataWritable.close(); } } catch(Exception exc) {/*unrecoverable so do nothing*/}
			
			try{ if (metadataOutStream != null) metadataOutStream.close(); } catch(Throwable t) {}

			try{ if (innerStream != null) innerStream.close(); } catch(Throwable t) {}
