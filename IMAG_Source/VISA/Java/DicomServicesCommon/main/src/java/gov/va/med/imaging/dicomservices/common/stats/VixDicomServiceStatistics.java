...
implements VixDicomServiceStatisticsMBean {
	...
	protected synchronized VixDicomServicesStoreSCUStatistics getFromStorageSCUList(String aet, String sopClassUID){
		...
		if(vixStorageSCUStatistics.isEmpty()){
			statEntry = new VixDicomServicesStoreSCUStatistics(aet, sopClassUID);
	...
	public void incrementVixSendToAEFailureCount(String aet, String sopClassUID){
		try {
			VixDicomServicesStoreSCUStatistics sendFailure  = getFromStorageSCUList(aet, sopClassUID);
	...
	public void addToVixSendToAEFailureCount(String aet, String sopClassUID, int failuresCount){
		try {
			VixDicomServicesStoreSCUStatistics sendFailure  = getFromStorageSCUList(aet, sopClassUID);
	...
	private synchronized static void registerResourceMBeans()
    {
		...
		if(vixDicomServiceStatisticsMBeanName == null)
		{
			try
            {
				...
				mBeanProperties.put( "type", "VixDicomServicesStats" );
				mBeanProperties.put( "name", "VixDicomServicesActivity");
