...
implements Observer
{
	...
	public <T extends Object> T createInstance(
		...
		Object ... optionalParameters )
	{
        getLogger().info("createInstance({}, {}, {}, {})", resultClass.getName(), instancePopulation == null ? "null instances" : instancePopulation.toString(), aggregates == null ? "null aggregates" : aggregates.toString(), components == null ? "null components" : components.toString()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		{
            getLogger().warn("No create method found, creating instance of '{}' using default constructor.", resultClass.getSimpleName());
			try
			{
				T instance = resultClass.newInstance();
                getLogger().warn("Created instance of '{}' using default constructor.", resultClass.getSimpleName());
				populateChildren(instance, instancePopulation, aggregates, components, references);
				return instance;
			}
			catch (Throwable x)
			{
                getLogger().error("Unable to create instance of '{}' using default constructor.", resultClass.getSimpleName(), x);
				while((x = x.getCause()) != null)
                    getLogger().error("ROOT CAUSE of inability to create instance of '{}' using default constructor follows.", resultClass.getSimpleName(), x);
				x.printStackTrace();
			}
		}
		...
		)
	{
		...
		if(dataGenerationAnnotation != null)
		{
			...
			if(valueType == null)
			{
                getLogger().error("Collection derived class '{}' must specify a componentValueType in the annotation.", resultClass.getName()); //$NON-NLS-1$ //$NON-NLS-2$
				return;
			}
		...
		ReferenceMap references)
	{
		...
		if(keyType == null)
		{
            getLogger().error("Map derived field '{}' must specify a componentKeyType in the annotation.", resultClass.getName()); //$NON-NLS-1$ //$NON-NLS-2$
			return;
		}
		...
		if(valueType == null)
		{
            getLogger().error("Map derived class '{}' must specify a componentValueType in the annotation.", resultClass.getName()); //$NON-NLS-1$ //$NON-NLS-2$
			return;
		}
	...
	{
		// if the field is not required or the data generation mode is not FULL the field does not need to be populated
		if( !(dataGenerationAnnotation.required() || InstancePopulation.FULL == instancePopulation) )
			return;
		
		Class<?> fieldType = field.getType();
		MockSizedInputStream inStream = null;

		try
		{
			if( isSimpleType(fieldType) )
			{
				String mockData = String.class == field.getType() ? createStringData(			// creates the data as a string according to the annotation
												...
												: Date.class == field.getType() ? createDateData(			// creates the data as a string according to the annotation
																		dataGenerationAnnotation.defaultValue(),
				...
				setSimpleField(instance, field, mockData);
			}
			...
			{
				AccessUtilities.setAccessible(field);
				inStream = createInputStream(instancePopulation, dataGenerationAnnotation.mediaTypes(),	dataGenerationAnnotation.defaultMediaType());
			...
			else
			{
				...
				if( Map.class.isAssignableFrom(fieldType) )
				{
					...
					if(map == null)
                        getLogger().error("Map derived field '{}' must be initialized to an empty instance.", name); //$NON-NLS-1$ //$NON-NLS-2$
					else if(map.size() > 0)
                        getLogger().error("Map derived field '{}' must be empty.", name); //$NON-NLS-1$ //$NON-NLS-2$
						
					populateMapField(map, dataGenerationAnnotation.componentKeyType(), 
									 dataGenerationAnnotation.componentValueType(), 
					...
					else if( Collection.class.isAssignableFrom(fieldType) )
					{
						...
						String name = field.getName();
						if(collection == null)
                            getLogger().error("Collection derived field '{}' must be initialized to an empty instance.", name); //$NON-NLS-1$ //$NON-NLS-2$
						else if(collection.size() > 0)
                            getLogger().error("Collection derived field '{}' must be empty.", name); //$NON-NLS-1$ //$NON-NLS-2$
					...
					{
						// the createInstance() will return true if it can create the instance.
						// This gives application code an opportunity to create instances.
						Object obj = createInstance(fieldType, instancePopulation, aggregates, components, references);
						if(obj != null)
		...
		{
			AccessUtilities.setAccessible(field, field.isAccessible());
			try { if (inStream != null) {inStream.close();} } catch (IOException ioe) {/* unrecoverable, do nothing */}
		}
	}
	...
	{
		if(keyType == null)
            getLogger().error("Map derived field '{}' must specify a componentKeyType in the annotation.", name); //$NON-NLS-1$ //$NON-NLS-2$
		if(valueType == null)
            getLogger().error("Map derived field '{}' must specify a componentValueType in the annotation.", name); //$NON-NLS-1$ //$NON-NLS-2$
		
		if(map == null)
            getLogger().error("Collection derived field '{}' must be initialized to an empty instance.", name); //$NON-NLS-1$ //$NON-NLS-2$
		else if(map.size() > 0)
            getLogger().error("Collection derived field '{}' has {} contained instances and must be empty.", name, map.size()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	throws IllegalAccessException
	{
		if(valueType == null || valueType.length() == 0)
            getLogger().error("Collection derived field '{}' must specify a componentValueType in the annotation.", name); //$NON-NLS-1$ //$NON-NLS-2$
		
		if(collection == null)
            getLogger().error("Collection derived field '{}' must be initialized to an empty instance.", name); //$NON-NLS-1$ //$NON-NLS-2$
		else if(collection.size() > 0)
            getLogger().error("Collection derived field '{}' has {} contained instances and must be empty.", name, collection.size()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		else
		{
			for(int index = 0; index < desiredInstanceCount; ++index)
			{
				...
				{
                    getLogger().error("No class named '{}' found.", valueType); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
		}
	}
	...
	throws IllegalArgumentException, IllegalAccessException
	{
		...
		{
			AccessUtilities.setAccessible(field, accessible);
		}
	}
		...
		String maximumValue)
	{
		try
		{
			...
			Date randomDate = new Date((long) (new SecureRandom().nextDouble() * (maximumDate.getTime() - minimumDate.getTime()) ) + minimumDate.getTime());
	...
	public MockSizedInputStream getInputStream(String mediaType)
	{
		...
		{
            logger.error("Media type '{}' is not valid when creating an input stream.", mediaType);
			return null;
		}
	...
	protected int createManyChildCount()
	{
		...
		case RANDOMIZE:
			return (int)( (new SecureRandom().nextDouble() * (MAXIMUM_MANY_CHILD_COUNT - MINIMUM_MANY_CHILD_COUNT)) + MINIMUM_MANY_CHILD_COUNT );
		case DEFAULT:
		default:
	...
	{
		return new SecureRandom().nextDouble() >= .5;
	}
	...
	throws NumberFormatException
	{
		return (char)(((double)(maximum-minimum)) * new SecureRandom().nextDouble() + (double)minimum);
	}

	public byte createRandomByte(byte minimum, byte maximum)
	throws NumberFormatException
	{
		return (byte)(((double)(maximum-minimum)) * new SecureRandom().nextDouble() + (double)minimum);
	}

	public short createRandomShort(short minimum, short maximum)
	throws NumberFormatException
	{
		return (short)(((double)(maximum-minimum)) * new SecureRandom().nextDouble() + (double)minimum);
	}

	public int createRandomInt(int minimum, int maximum)
	throws NumberFormatException
	{
		return (int)(((double)(maximum-minimum)) * new SecureRandom().nextDouble() + (double)minimum);
	}

	public long createRandomLong(long minimum, long maximum)
	throws NumberFormatException
	{
		return (long)(((double)(maximum-minimum)) * new SecureRandom().nextDouble() + (double)minimum);
	}

	public float createRandomFloat(float minimum, float maximum)
	throws NumberFormatException
	{
		return (float)(((double)(maximum-minimum)) * new SecureRandom().nextDouble() + (double)minimum);
	}

	public double createRandomDouble(double minimum, double maximum)
	throws NumberFormatException
	{
		return ((double)(maximum-minimum)) * new SecureRandom().nextDouble() + (double)minimum;
	}
	...
	public <T extends Enum> T selectEnum(Class<T> enumerationClass)
	{
		try
		{
			...
			int index = (Mode.DEFAULT == getConfiguration().getMode()) ? 0 :
				...
				(int)(new SecureRandom().nextDouble() * values.length);
	...
	public String createRandomString(String regex)
	{
		...
		while( matcher.find() )
		{
			...
			else			// not a literal group, need to generate a string by pattern
			{
				...
				int actualCharCount = (int)( (new SecureRandom().nextDouble() * (maxCharCount - minCharCount)) + minCharCount );
	...
	protected char createRandomChar(String pattern)
	{
		...
		int random = (int)(new SecureRandom().nextDouble() * sb.length());
