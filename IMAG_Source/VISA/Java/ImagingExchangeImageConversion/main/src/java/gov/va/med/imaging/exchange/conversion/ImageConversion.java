...
//			HashMap theHISChanges)			// it was used for VA2DoD only, not for thumbnails
			...
			{	
						...
						&& (txtObject == null)) 
				{
					...
					{	// DoD2VA DICOM decompress to raw DICOM format (optional in future),
						...
						return new ByteBufferBackedImageInputStream(sanitizeDicomData(convertedObject)); //pass through dicom data
					}
				}
			} 
			else 
	...
	{       
		...
		{ 
			...
			if ((targetImageFormat==ImageFormat.DICOMJPEG) ||
				(targetImageFormat==ImageFormat.DICOMJPEG2000)) {
                LOGGER.error("CompressImage cannot create DICOM compressed output for {} input!", sourceImageFormat.getType());
				throw new ImageConversionInvalidInputException(
		...
		else if(sourceImageFormat==ImageFormat.DICOMJPEG2000){
			ByteBufferBackedImageInputStream outStream = null;
			ByteBufferBackedObject outObject = null;
			//decompress the stream.
			try {
				outStream = 
						DecompressImage(sourceImageFormat, ImageFormat.DICOM, imageObject);
				outObject = outStream.toBufferedObject();
			} catch (ImageConversionDecompressionException icdX) {
		        LOGGER.error("Image Decompression Exception: ", icdX);
                LOGGER.error("Failed to decompress image from [{}] to [{}]", sourceImageFormat, targetImageFormat);
				throw new ImageConversionCompressionException(
					"Failed to decompress image from [" + sourceImageFormat + "] to [" + targetImageFormat +"]");
			} catch (IOException ioX) {
                LOGGER.error("Failed to decompress image from [{}] to [{}]", sourceImageFormat, targetImageFormat);
		        LOGGER.error("IO Exception while Image Decompression: ", ioX);
				throw new ImageConversionCompressionException(
					"Failed to decompress image from [" + sourceImageFormat + "] to [" + targetImageFormat +"]");
			}
			//if target is DICOM, return the decompress stream.
			if(targetImageFormat == ImageFormat.DICOM){
				return outStream;
			}
			//if target is JPEG,
				//convert DICOM stream to a JPEG stream and return
			else if(targetImageFormat == ImageFormat.JPEG){
				return Compress(outObject, quality, targetImageFormat, modality, false);
			}
			else{
                LOGGER.error("Invalid targetImageFormat {}", targetImageFormat.getType());
				throw new ImageConversionInvalidInputException(
					"Invalid targetImageFormat " + targetImageFormat.getType());				
			}
		}
		else 
		{	// invalid input type
            LOGGER.error("Invalid sourceImageFormat {}", sourceImageFormat.getType());
			throw new ImageConversionInvalidInputException(
				"Invalid sourceImageFormat " + sourceImageFormat.getType());
		}        
	}
	...
	ImageConversionDecompressionException
	{
		...
		if (badCompany) {
            LOGGER.error("DecompressImage got ImageFormat pair -- {} in & {} out!", sourceImageFormat, targetImageFormat);
			throw new ImageConversionInvalidInputException(
	...
	throws ImageConversionIOException, ImageConversionCompressionException
	{
	    ...
	    {
            LOGGER.error("ThumbnailCompress - Compression Exception: {} ({} bytes in)", e.getMessage(), imageObject.getSize());
...
//				mutex.unlock();
			...
			{
                LOGGER.error("ThumbnailCompress - AWJ2K Destroy Exception: {} ({} bytes in)", ex.getMessage(), imageObject.getSize());
//				mutex.unlock();
	...
	throws ImageConversionIOException, ImageConversionCompressionException 
	{
        ...
        LOGGER.info("Compress Dicom Object({}) and stream it", imageObject.getSize());
	...
	throws ImageConversionIOException, ImageConversionCompressionException 
	{
        ...
        LOGGER.info("Compress Object({}) and stream it", imageObject.getSize());
		int autoLevel=-1; // Automatically determines the best number of compression levels for J2K
		float lossyCompressionRatio;
...
//			        	}
		        ...
		        { // ImageQuality.DIAGNOSTIC
		            codec.AwJ2kSetOutputJpegOptions(quality.getCanonical()); // lossless compression
		        }
		        codec.AwJ2kSetOutputType(AwJ2kParameters.AW_J2K_FORMAT_JPG);	        	
	        }
	        AwOutputImageValue valueObj = codec.AwJ2kGetOutputImage();
	...
	throws ImageConversionIOException 
	{
        ...
        LOGGER.info("Update Dicom Stream ({}) with HIS/TXT changes ({})", imageStream.getSize(), textStream.getSize());
		byte[] data=null;
		...
		try 
		{
			...
			data = dicomUtilities.updateDicomStream(sizedDicomStream, sizedTextStream);
	...
	throws ImageConversionIOException 
	{
        ...
        LOGGER.info("Assemble Dicom Object Stream from Text ({}) and TGA ({}) streams", tgaStream.getSize(), txtStream.getSize());
		byte[] data=null;
		...
		try 
		{
			...
			data = dicomUtilities.assembleDicomStream(sizedTextStream, sizedtgaStream);
