...
implements EvictionStrategy, StorageThresholdEvictionStrategyMBean
{
	...
	private final static Logger LOGGER = Logger.getLogger(StorageThresholdEvictionStrategy.class);
	...
	public void sweep()
	{
		...
		else
		{
			...
			// Fortify change: check for null first.  Not on list but fixed anyway
			if(firstRegion != null)
			{
				freeSpace = firstRegion.getFreeSpace();
			}
		...
		if(freeSpace != 0L && getMaxUsedSpaceThreshold() > 0)
		{
            ...
            LOGGER.warn("Managed regions do not support free space reporting and no maximum size was specified.   The eviction strategy is '{}', which requires it.  No eviction will occur in these regions.", this.getClass().getSimpleName());
	...
	{
        LOGGER.info("Region '{}' has {} bytes free, maximum used threshold is {} bytes free, minimum threshold is {} bytes free, target threshold is {} bytes free.", region.getName(), freeSpace, getMaxUsedSpaceThreshold(), getMinFreeSpaceThreshold(), getTargetFreeSpaceThreshold());
	}
	...
	implements Callable<TargetSizeGroupPathSet>
	{
		...
		throws Exception
		{
            ...
            LOGGER.info("Region '{} bytes free, minimum threshold is {} bytes free, target threshold is {} bytes free.", region.getName(), getMinFreeSpaceThreshold(), getTargetFreeSpaceThreshold());
	...
	implements Runnable
	{
		...
		public void run()
		{
			...
			for(Future<TargetSizeGroupPathSet> future : this.regionTaskFutures)
			{
				...
				{
                    LOGGER.warn("EvictorTask.run() --> Encountered InterruptedException in region eviction thread while getting result: {}", x.getMessage());
				} 
				catch (ExecutionException x)
				{
                    LOGGER.warn("EvictorTask.run() --> Encountered ExecutionException in region eviction thread while getting result: {}", x.getMessage());
				}
			}

            LOGGER.info("EvictorTask.run() --> There are [{}] cache entries to evict.", evictableGroups.size());
			...
			if(evictableGroups.size() > 0)
			{
				for( GroupPath groupPath:evictableGroups )
				{
					...
					{
                        LOGGER.error("EvictorTask.run() --> Unable to remove group [{}]: {}", groupPath.getGroup().getName(), x.getMessage());
					}
				}
			}
