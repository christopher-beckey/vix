...
{
	private static final Logger LOGGER = Logger.getLogger(RecursiveGroupParent.class);
	...
	throws CacheException
	{
		...
		if(childGroup == null)
		{
            LOGGER.info("RecursiveGroupParent.deleteInstance() --> Unable to delete instance [{}], ancestor group [{}] does not exist", key, groupName[0]);
			return;
		}
	...
	throws CacheException
	{
		Group childGroup = allowCreate ? getOrCreateChildGroup(groupName[0]) : getChildGroup(groupName[0]);
		
		if(childGroup == null)
		{
			if(allowCreate)
                LOGGER.error("RecursiveGroupParent.getOrCreateInstance() --> Unable to create child group [{}]", groupName[0]);
			return null;
		}
	...
	throws CacheException
	{
		...
		if(childGroup == null)
		{
            LOGGER.error("RecursiveGroupParent.getOrCreateInstance() --> Unable to find group [{}]", groupName[0]);
			throw new InstanceInaccessibleException("RecursiveGroupParent.getOrCreateInstance() --> Unable to find group [" + groupName[0] + "]");
		}
