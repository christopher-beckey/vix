...
public class CommonStudyFunctions
{
	...
	public static ArtifactResults mergeWithMuseStudySetResult(ArtifactResults results, 
			ArtifactResults museResults)
	{
		
		//SortedSet<Study> updatedStudies = null;
		
		if(results == null || museResults == null || !museResults.containsStudySetResult() 
				|| results.getStudySetResult().isErrorResult() 
				|| museResults.getStudySetResult().isErrorResult()){
			return results;
		}
		
		StudySetResult primaryStudySetResult = results.getStudySetResult();
		DocumentSetResult primaryDocumentSetResult = results.getDocumentSetResult();
		StudySetResult museStudySetResult = museResults.getStudySetResult();

		if(primaryStudySetResult == null){
			return ArtifactResults.create(museStudySetResult, primaryDocumentSetResult);
		}
		
		SortedSet<Study> primaryStudies = primaryStudySetResult.getArtifacts();
		SortedSet<Study> museStudies = museStudySetResult.getArtifacts();
		
		if(primaryStudies != null)
		{
			//updatedStudies = new TreeSet<Study>();
			for(Study study : museStudies)
			{
				primaryStudies.add(study);
			}
		}
		StudySetResult updateStudySetResult = StudySetResult.createFullResult(primaryStudies);
		return ArtifactResults.create(updateStudySetResult, primaryDocumentSetResult);
	}
