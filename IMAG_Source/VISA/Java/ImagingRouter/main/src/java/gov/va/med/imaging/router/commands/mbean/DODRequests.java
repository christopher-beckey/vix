...
implements DODRequestsMBean
{
	...
	private static List<DODRequests> dodRequestsList = new ArrayList<DODRequests>();
	
	public synchronized static DODRequests getRoiCommandsStatistics()
	{
		DODRequests dodRequests = getDodRequests();
		registerMBeanServer();
		return dodRequests;
	}
	
	private static DODRequests getDodRequests()
	{
		if (dodRequestsList.size() == 0)
		{
			DODRequests dodRequests = new DODRequests(); 
			dodRequestsList.add(dodRequests);
			return dodRequests;
		}
		else
		{
			return dodRequestsList.get(0);
		}
	}
	...
	private static synchronized void registerMBeanServer()
	{
		if(dodRequestsMBeanName == null)
		{
			...
			try
			{
				...
				mBeanServer.registerMBean(getDodRequests(), dodRequestsMBeanName);
			}
			catch(Exception ex)
			{
