...
extends AbstractRetryableCommandImpl<java.lang.Void>
{
	...
	{
        getLogger().info("Asynchronous Command [{}] - processing.", this.getClass().getSimpleName());
		
		TransactionContext xactionContext = TransactionContextFactory.get();
        getLogger().info("RouterImpl.logImageAccessEventRetryable ({}) for image [{}] by [{}].", getEvent().getEventType().toString(), getEvent().getImageIen(), xactionContext.getTransactionId());
