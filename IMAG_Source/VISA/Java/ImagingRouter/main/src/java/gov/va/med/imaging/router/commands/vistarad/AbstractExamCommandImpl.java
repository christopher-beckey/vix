...
//			}
	...
	protected void cachePatientEnterpriseExams(PatientEnterpriseExams patientEnterpriseExams)
	{
		if(getCommandContext().isCachingEnabled()) 
		{
            getLogger().info("Caching [{}] exam sites.", patientEnterpriseExams.getExamSites().size());
	...
	throws MethodException, ConnectionException
	{
			...
			if(!examSite.isExamsFullyLoaded())
			{
				...
				if(!routingToken.getRepositoryUniqueId().equals(getCommandContext().getRouter().getAppConfiguration().getLocalSiteNumber()))
				{
					...
					{				
						...
						if(!fullyLoadExams)  
						{
                            getLogger().info("Exams not fully loaded, async requesting exams for patient '{}' from site '{}'.", patientIcn, routingToken);
					...
					{
                        getLogger().info("patient '{}' has 0 exams from site '{}', not getting fully loaded exams.", patientIcn, routingToken);
					}
				}
			}
		}
		...
		{
            getLogger().info("Got examSite for site '{}' and patient '{}' in cache, returning.", routingToken, patientIcn);
	...
	{
		ExamSite examSite = null;
        getLogger().info("Searching cache for exam site from site '{}' for patient '{}'.", routingToken, patientIcn);
		if(getCommandContext().isCachingEnabled()) {
			try 
			{
	...
	throws MethodException, MethodConnectionException
	{
		...
		if(isSiteDod && CommandConfiguration.getCommandConfiguration().isEnsurePatientSeenAtDoD())
		{
			...
			try
			{
                getLogger().info("Ensuring patient '{}' has been seen at the DoD by looking up treating sites.", patientIcn);
			...
			catch(PatientNotFoundException pnfX)
			{
                ...
                getLogger().warn("Patient '{}' not found, indicates patient not really seen in the DoD, returning empty result set.", patientIcn);
        ...
        getLogger().info("Got {} patient '{}' exams.", (exams == null || exams.size() == 0) ? "no" : "" + exams.size(), patientIcn);
	...
	private void cacheExamIfPossible(RoutingToken routingToken, Exam exam)
	{
		...
		{			
			if(exam != null)
			{
				...
				if(examSite != null)
				{
                    getLogger().info("Found exam site '{}' in cache, updated exam in cache.", examSite.getRoutingToken());
					try
					{
	...
	throws MethodException, ConnectionException
	{
		if(getCommandContext().isCachingEnabled()) 
		{
			...
			try
			{
                getLogger().info("Caching Exam Site [{}] for patient '{}'.", routingToken.getRepositoryUniqueId(), patientIcn);
				getCommandContext().getIntraEnterpriseCacheCache().createExamSite(
					routingToken, 
					patientIcn, 
					examSite);
	...
	throws MethodException, ConnectionException
	{
        getLogger().info("Retrieving exam images for exam '{}' from data source.", studyUrn.toString());
