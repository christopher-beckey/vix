...
extends AbstractImagingCommandImpl<ArtifactResults> {
	...
	private StudySetResult asynchronousStudySetResult;
	private DocumentSetResult asynchronousDocumentSetResult;
	...
	throws MethodException, ConnectionException {
		...
		if(callingArtifactSpi || vaRequest) {
			...
			getLogger().debug("ArtfactResults call entered with callingArtifactSpi [" + callingArtifactSpi + "] and vaRequest [" + vaRequest + "]");

			ArtifactResults museResults = null;
			if(filter.isIncludeMuseOrders()) {
				if((filter.getStudyId() == null) || (filter.getStudyId() instanceof MuseStudyURN)) {
					if(getLocalSiteId().equals(getRoutingToken().getRepositoryUniqueId())) {
						museResults = getMuseResult();
						if(filter.getStudyId() != null) {
							logger.debug((museResults == null) ? "ArtifactResults are null from Muse local site."
										:"Muse Results returned from Muse DataSource router command for [" + filter.getStudyId().toString() + "]");
							return museResults;
						}
					}
				}
			...
			ArtifactResults result = null;
			result = getArtifactResultsWithPatientArtifactSpi();
			
			if(result == null) {
				logger.debug("ArtifactResults are null from site.");
				// not sure what to do here, throwing an exception
				throw new MethodException("Got null ArtifactResults from site [" + getRoutingToken().toRoutingTokenString() + "]");
			}
			
			logger.debug("ArtifactResults contains data.");
			
			if(result.getStudySetResult() != null && result.getStudySetResult().getArtifacts() != null) {
				logger.debug("Local Site Id: " + getLocalSiteId());
				logger.debug("Repository Unique Id: " + getRoutingToken().getRepositoryUniqueId());
				logger.debug("Home Community Id: " + getRoutingToken().getHomeCommunityId());
				
				if(museResults != null) {
					result = CommonStudyFunctions.mergeWithMuseStudySetResult(result, museResults);
					logger.debug("Results returned from Muse DataSource router command.");
				}
				// need to update the study data with the consolidated site values, then create a new artifact Result object holder					
				//WFP-do I filter this?
			...
			if(ensurePatientSeenAtDoD) {
				...
				try {
					logger.info("Ensuring patient [" + getPatientIdentifier() + "] has been seen at the DoD by looking up treating sites.");
					InternalContext.getRouter().getTreatingSitesFromDataSource(getRoutingToken(), getPatientIdentifier(), true);
					...
					logger.warn("Patient [" + getPatientIdentifier() + "] not found, indicates patient not really seen in the DoD, returning empty result set.");
			...
			if(isIncludeDocuments()) {
				...
				{
					logger.error("Error creating BHIE Documents Routing Token, " + rtfX.getMessage(), rtfX);
				}
			}
			...
			if(isIncludeRadiology()) {
				try	{
					...
					logger.error("Error creating BHIE Radiology Routing Token, " + rtfX.getMessage(), rtfX);
				}				
			} 
			...
			StringBuilder sb = new StringBuilder(childCommandsCount);
			sb.append(" Asynchronous Commands: ");
			sb.append(childGetSuccessCount);
			sb.append(" Completed OK, ");
			sb.append(childGetErrorCount);
			sb.append(" Failed, ");
			sb.append(orphaned);
			sb.append(" Orphaned.");
	...
	protected ArtifactResults getMuseResult() throws MethodException, ConnectionException {
		
		getLogger().info("Start getting MUSE data !!!");
		ArtifactResults museResults = null;
		
		try {
			// Moved inside "try" to get "Patient not found" exception even tho it could happen before getting here
			//get patient $$n
			Patient patient = ImagingContext.getRouter().getPatientInformation(getRoutingToken(), getPatientIdentifier());
			filter.setMusePatientId(patient.getSsn());

			museResults = ImagingContext.getRouter().getStudyOnlyPatientArtifactResultsFromSiteMuse(
					getRoutingToken(), getPatientIdentifier(), getFilter(), 
					isIncludeRadiology(), isIncludeDocuments());
		} catch(Exception ex) {
			logger.info("ArtifactResults --> Couldn't get MUSE data for given patient Id [" + getPatientIdentifier() + "]. Possible reason: a) Unknown MUSE server b) Unresolvable site c) patient not found");			
		}

		logger.debug("Returned from Muse Study Only Patient Artifact Results Data Source command.");
	...
	private long getAsynchronousCommandWaitTimeout() {
		...
		long timeout = CommandConfiguration.getCommandConfiguration().getAsynchronousCommandWaiterTimeoutMs();
	...
	implements AsynchronousCommandResultListener<StudySetResult> {
		...
		@Override
		public void commandComplete(AsynchronousCommandResult<StudySetResult> result) {
			
			if(result.isSuccess()) {
				parentCommand.asynchronousStudySetResult = result.getResult();
				parentCommand.childGetSuccessCount++;
			} else if(result.isError()) {
				logger.info("child command results in error, adding no results.");
	...
	implements AsynchronousCommandResultListener<DocumentSetResult> {
		...
		DocumentSetResultCommandChildListener(AbstractArtifactResultsBySiteNumberCommandImpl parentCommand,	RoutingToken routingToken) {
			this.parentCommand = parentCommand;
			this.childRoutingToken = routingToken;
		}

		@Override
		public void commandComplete(AsynchronousCommandResult<DocumentSetResult> result) {
			if(result.isSuccess()) {
