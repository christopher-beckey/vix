...
extends AbstractImageCommandImpl<Void>
{
	...
	throws MethodException
	{
		...
		if(! isInstanceInCache(imageUrn, getRequestedFormatQuality() ))
		{
			...
			// it
            getLogger().info("Did not find image '{}' in cache, will attempt to get from datasource.", getImageUrn().toString());
			...
			catch(InstanceInaccessibleException iaX)
			{
                ...
                getLogger().info("InstanceInaccessibleException for image [{}], prefetch is completing on the assumption tha the other thread will cache the image", imageUrn.toString());
			}
			catch(SimultaneousWriteException swX)
			{
                getLogger().info("SimultaneousWriteException for image [{}], prefetch is completing on the assumption tha the other thread will cache the image", imageUrn.toString());
			}
		...
		else
		{
            ...
            getLogger().info("Found image '{}' in cache, prefetch complete.", getImageUrn().toString());
		}
		...
		if(responseChecksum != null && cacheChecksum != null)
		{
			try
			{
				...
				if (noResponseChecksum) 
				{
					...
					{
                        getLogger().info("Checksum for inStream '{}' IS Not Equal to data source checksum.", imageUrn);
