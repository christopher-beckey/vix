...
extends AbstractExamImageCommandImpl<Long>
{
	...
	throws MethodException, ConnectionException 
	{
		...
		if( cacheThisInstance ) 
		{
			...
			try
			{
				if(isResponseAllowedFromCache())
				{
					if(isCacheOnly())
					{
						...
						{
                            getLogger().info("Image '{}' is already cached, not returning any data.", imageUrn.toString());
							return new Long(0);
						}
					}
					else
					{	
						StreamImageFromCacheResponse response = 
							CommonImageCacheFunctions.streamImageFromCache(
									...
									getImageFormatQualityList(), outStream, 
									getImageMetadataNotification());
					...
					{					
							...
							(dataSourceImageQuality != null))
						{
							...
							int bytes = CommonImageCacheFunctions.streamImageFromCache(getCommandContext(), 
									imageUrn, dataSourceImageFormat, dataSourceImageQuality, outStream, 
									getImageMetadataNotification());
							if(bytes > 0)
							{
						...
						{				
							response = CommonImageCacheFunctions.streamImageFromCache(getCommandContext(),
									imageUrn, getImageFormatQualityList(), outStream, 
									getImageMetadataNotification());
						}
			...
			try
			{
				if(isCacheOnly())
				{
                    getLogger().info("Image '{}' is now cached, not returning any data.", imageUrn.toString());
				...
				else
				{
						...
						(dataSourceImageQuality != null))
					{
						...
						int bytes = CommonImageCacheFunctions.streamImageFromCache(getCommandContext(), 
								imageUrn, dataSourceImageFormat, dataSourceImageQuality, outStream, 
								getImageMetadataNotification(), updateImageLocation);
						if(bytes > 0)
						{
							getLogger().debug("Found instance in cache using quality and format from data source response.");
	...
	ImageNotFoundException, ImageNearLineException
	{
		...
		if(isCacheOnly())
		{
            getLogger().info("Image '{}' caching disabled, will not get image from DS without cache", imageUrn.toString());
			return new Long(0);
		}
		else
		{
