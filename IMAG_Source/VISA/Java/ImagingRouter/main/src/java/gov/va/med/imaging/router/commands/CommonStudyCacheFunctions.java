...
public class CommonStudyCacheFunctions
{
			...
			if(study.getStudyLoadLevel().isFullyLoaded())
			{
				for(Series series : study)
				{
					...
					if(oldImage != null)
					{
                        ...
                        logger.info("Image '{}' updated in cache", imageUrn.toString());
						break;
					}
				}
                logger.debug("Did not find image '{}' in cache, cannot update image entry", imageUrn.toString());
			}
			else
			{
                logger.debug("Study '{}' is not fully loaded, will not update image entry", studyUrn.toString());
			}
		}		
		else
		{
            logger.debug("Cannot find study '{}' in cache.", studyUrn.toString());
		}
	}
	...
	public static Study getStudyFromCache(ImagingCommandContext commandContext, GlobalArtifactIdentifier gaid)
	{
		...
		if(commandContext.isCachingEnabled()) {
			...
			{
                getLogger().warn("Unable to get study[{}] from cache, exception message follows.", gaid.toString(), cX);
			}
			catch (OIDFormatException oidX)
			{
                getLogger().warn("Unable to get study[{}] from cache, exception message follows.", gaid.toString(), oidX);
			}
		}
		return study;
	}
			...
			String siteNumber, Collection<Study> studyList)
	{
		...
		if(commandContext.isCachingEnabled()) 
		{
            getLogger().info("Caching [{}] studies.", studyList.size());
			...
			{		
				...
				{
                    getLogger().debug("Not caching study '{}' because it is not fully loaded", study.getStudyIen());
