...
//extends AbstractCommandImpl<ImageMetadata>
{
	...
	throws MethodException, ConnectionException 
	{
		try
		{
			...
			{
                getLogger().warn("InstanceInaccessibleException caught getting image [{}], operation will be retried.", imageUrn.toString());
				return streamImage((OutputStream)null, true, isAttemptWriteToCache());
			}
			catch(SimultaneousWriteException swX)
			{
                getLogger().warn("SimultaneousWriteException caught getting image [{}], operation will be retried.", imageUrn.toString());
				return streamImage((OutputStream)null, true, isAttemptWriteToCache());
			}
			catch(CacheException cX)
			{
                getLogger().warn("Generic CacheException caught getting image [{}], operation will be retried.", imageUrn.toString());
				return streamImage((OutputStream)null, false, false);
			}
			catch(CompositeIOException cioX)
			{
                getLogger().warn("CompositeIOException caught getting image [{}], operation will be retried.", imageUrn.toString());
				return streamImage((OutputStream)null, false, false);
			}
		}
	       ...
	       MethodException, ConnectionException, InstanceInaccessibleException, IOException, CacheException
	{
		...
		if(result == null)
		{
            getLogger().info("Image [{}] does not exist in cache, getting metadata from data source.", imageUrn.toString());
	...
	throws IOException 
	{
		...
		if(outStream == null)
		{
            getLogger().info("Image '{}' is already cached, not returning image data.", getImageUrn().toString());
			CommonImageCacheFunctions.populateImageMetadata(
				getCommandContext(), getImageUrn(), getImageFormatQualityList(), 
				notification
			);
		}
		else
		{
			StreamImageFromCacheResponse response = CommonImageCacheFunctions.streamImageFromCache(
				getCommandContext(), getImageUrn(), getImageFormatQualityList(), outStream, 
				notification);
			
			if((response != null) && (response.getBytesReturnedFromDataSource() > 0))
			{
                getLogger().info("Image '{}' found in the cache and streamed to the destination.", getImageUrn().toString());
			}
		}
	...
	private void scheduleRequestOfNearlineImage()
	{
		try
		{
            ...
            getLogger().info("Exam Image '{}' returned nearline, requesting image again in 1 minute", getImageUrn().toString());
