...
extends AbstractImagingCommandImpl<R>
{
	...
	throws MethodException
	{
		...
		{
            getLogger().info("Site number [{}] is excluded in the StudyFilter, not loading study list from this site", routingToken.getRepositoryUniqueId());
		...
		try
		{
            ...
            getLogger().info("Got {} patient '{}' document groups.", documentSetResult == null ? "null" : "" + documentSetResult.getArtifactSize(), filter.getPatientId());
	...
	private void cacheDocumentSetList(ResolvedSite resolvedSite, Collection<DocumentSet> documentSetList)
	{
		...
		{
            getLogger().info("Caching [{}] document sets.", documentSetList.size());
	...
	throws MethodException
	{
		...
		try
		{
			...
			if(documentSet == null)
			{
                ...
                getLogger().info("Found document set [{}] from data source", documentSetUrn.toString());
				transactionContext.setItemCached(Boolean.FALSE);
			}
			else
			{
                getLogger().info("Found document set [{}] in cache", documentSetUrn.toString());
