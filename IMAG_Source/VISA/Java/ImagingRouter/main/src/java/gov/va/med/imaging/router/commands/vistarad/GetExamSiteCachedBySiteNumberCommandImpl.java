...
extends AbstractExamCommandImpl<List<ExamSiteCachedStatus>> 
{
	...
	{
        getLogger().info("Finding cached exam site status for '{}' site numbers for patient '{}'.", getRoutingTokens().length, getPatientIcn());
		...
		{
            getLogger().warn("Error creating routing token from local site number '{}' in GetExamSiteCachedBySiteNumberCommandImpl, {}", localSiteNumber, rtfX.getMessage());
		}
