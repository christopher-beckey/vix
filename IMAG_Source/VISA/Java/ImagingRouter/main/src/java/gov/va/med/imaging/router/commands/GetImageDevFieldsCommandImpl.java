...
extends AbstractCommandImpl<String>
{
	...
	throws MethodException
	{
		...
		{			
			...
			{
                getLogger().error("Exception [{}] getting site number with redirect handling, using default site resolution.", t.getMessage(), t);
			...
			for(URL artifactUrl : metadataUrls )
			{
				...
				{
                    getLogger().error("Failed to contact site'{}' using '{}'.\nException details follow.", siteNumber, studyGraphUrl.toExternalForm(), uoX);
					lastException = uoX;
				}
				catch(ConnectionException cX)
				{
                    getLogger().error("Failed to contact site'{}' using '{}'.\nException details follow.", siteNumber, studyGraphUrl.toExternalForm(), cX);
					lastException = cX;
				}
				catch(MethodException mX)
				{
                    getLogger().error("Failed to contact site'{}' using '{}'.\nException details follow.", siteNumber, studyGraphUrl.toExternalForm(), mX);
					lastException = mX;
				} 
			}
		}
        ...
        getLogger().error("Unsuccessfully tried all configured protocols for site '{}'.\nPlease check that the protocol handlers are properly installed and that the \nprotocol preferences for the site specify valid protocols.", siteNumber);
