...
extends AbstractCommandImpl<Long>
{
	...
	throws MethodException, ConnectionException
	{
		...
		finally
		{
			...
			try
			{
				...
				// Store document info into Sql Location
				storeCacheDocumentLocation((ImagingCommandContext) documentRetrieveResult.getCommandContext());
			}
			catch(IOException ioX)
			{
	...
	private void storeCacheDocumentLocation(ImagingCommandContext commandContext) 
	{
        getLogger().debug("Getting Document Cache Instance for '{}'", getDocumentIdentifier().toString());
		ImmutableInstance documentInstance = null;
		...
		try
		{
			...
			{
				cache = commandContext.getIntraEnterpriseCacheCache();
			}
			else
			{
				cache = commandContext.getExtraEnterpriseCache();
			}
			
			documentInstance = cache.getDocumentContent(
		...
		{
			getLogger().error("Exception occured in getting document cache instance in storeCacheDocumentLocation", e);
			return;
		}
	}
