...
extends AbstractExamImageCommandImpl<Integer>
{
	...
	throws MethodException, ConnectionException 
	{
		...
		{
            getLogger().info("Image Text file '{}' caching enabled", imageUrn.toString());
			...
			try
			{
				if(isCacheOnly())
				{
					...
					{
                        getLogger().info("Image text file '{}' is already cached, not returning any data.", imageUrn.toString());
						return 0;
					}
				}
				else
				{
					StreamImageFromCacheResponse response = 
						CommonImageCacheFunctions.streamImageFromCache(getCommandContext(),
								imageUrn, qualityList, outStream, 
			...
			if(cacheOutStream != null)
			{
				try
				{
					...
					if(isCacheOnly())
					{
                        getLogger().info("Image text file '{}' is now cached, not returning any data.", imageUrn.toString());
						return bytesReturned;
					}
		...
		else
		{
			...
			{
                getLogger().info("Image text file '{}' caching disabled, getting image from source.", imageUrn.toString());
