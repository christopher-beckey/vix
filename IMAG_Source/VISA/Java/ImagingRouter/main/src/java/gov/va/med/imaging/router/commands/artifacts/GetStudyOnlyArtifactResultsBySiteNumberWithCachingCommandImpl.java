...
extends GetStudyOnlyArtifactResultsBySiteNumberCommandImpl
{
	...
	{		
		...
		if(isCanGetFromCache())
		{
			...
			if(result != null)
			{
                getLogger().info("Retrieved cached DocumentSetResult for patient '{}' from site '{}'.", getPatientIdentifier(), getRoutingToken().toRoutingTokenString());
				transactionContext.setItemCached(true);
				return ArtifactResults.createDocumentSetResult(result);
			}
			else
			{
                getLogger().debug("Did not get DocumentSetResult from cache for patient '{}' from site '{}'.", getPatientIdentifier(), getRoutingToken().toRoutingTokenString());
