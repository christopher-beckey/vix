...
extends AbstractImageCommandImpl<DocumentRetrieveResult>
{
	...
	throws MethodException
	{
        ...
        getLogger().info("Getting [homeCommunityID={}] [repositoryID={}] [documentID={}].", homeCommunityId, getDocumentIdentifier().getRepositoryUniqueId(), getDocumentIdentifier().getDocumentUniqueId());
		...
		{
            getLogger().info("Document '{}' caching enabled.", getDocumentIdentifier().toString());
			try
			{
				...
				{
                    getLogger().info("Document '{}' found in cache, streaming from there.", getDocumentIdentifier().toString());
				...
				//Since it's not in the local cache, try to find the cache from Remote Location via SqlImageLocation
				else
				{
					// the document was not found in the cache, stream from the data source while writing to
					// the cache simultaneously

                    getLogger().info("Document '{}' NOT found in cache, getting from datasource.", getDocumentIdentifier().toString());
					...
					res = CachingDocumentRetrieveResult.create(res, documentWriteChannel, getCommandContext());
		...
		else
		{
            ...
            getLogger().info("Document '{}' caching disabled, streaming from data source.", getDocumentIdentifier());
			transactionContext.setItemCached(Boolean.FALSE);
			res = streamFromDataSource();
		}
		return res;
	...
	{
        getLogger().info("Document '{}' getting document from source.", getDocumentIdentifier().toString());
