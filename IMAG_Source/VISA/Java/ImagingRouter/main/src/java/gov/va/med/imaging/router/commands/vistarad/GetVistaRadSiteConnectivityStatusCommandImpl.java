...
extends AbstractDataSourceExceptionHandler<SiteConnectivityStatus> 
{
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("isVistaRadSiteAvailable - Transaction ID [{}], For site '{}'.", context.getTransactionId(), getSiteNumber());
	    ...
	    {
            getLogger().error("Unable to find site with site number '{}', verify this is a valid VA site number. Cannot retrieve data from this site.", getSiteNumber());
		...
		{
            getLogger().error("The site '{}' has no available interface URLs.\nPlease check that the protocol handlers are properly installed and that the \nprotocol preferences for the site specify valid protocols.", getSiteNumber());
			return SiteConnectivityStatus.VIX_UNAVAILABLE;
		}
		...
		for(URL url : resolvedUrls )
		{
			...
			{
                getLogger().error("Failed to contact site'{}' using '{}'.\nException details follow.", getSiteNumber(), url.toExternalForm(), cX);
				lastException = cX;
			}
		}
		if(lastException != null)
		{
			...
			{
                getLogger().info("Unable to contact data source for site '{}' using ViX, returning Datasource Unavailable", getSiteNumber());
