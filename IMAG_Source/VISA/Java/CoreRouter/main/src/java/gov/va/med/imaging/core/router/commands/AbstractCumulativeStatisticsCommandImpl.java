...
extends AbstractCommandImpl<CumulativeCommandStatistics<R>>
{
	...
	throws MethodException, ConnectionException
	{
		...
		if( RoutingToken.ROUTING_WILDCARD.equals(localRoutingToken.getRepositoryUniqueId()) )
		{
			...
			{
                getLogger().warn("Error determining availabality of gateway to '{}' because that is not a valid OID.  Assuming no gateway is available and continuing.", getRoutingToken().getHomeCommunityId());
			}
		}
		else
		{
        ...
        getLogger().info("Calling child async commands for '{}' repositories.", filteredRoutingTokens.size());
        ...
        getLogger().debug("{} Asynchronous Commands: {} Completed OK, {} Failed, {} Orphaned.", childCommandsCount, cumulativeCommandStatistics.getChildGetSuccessCount(), cumulativeCommandStatistics.getChildGetErrorCount(), orphaned);
		if(orphaned > 0)
		{
            getLogger().warn("Asynchronous commands are complete, {}' command(s) were orphaned, likely means they took longer than '{}' ms", orphaned, timeout);
        ...
        getLogger().info("Returning '{}' cumulative results.", cumulativeCommandStatistics.getCumulativeResults() == null ? 0 : cumulativeCommandStatistics.getCumulativeResults().size());
	...
	throws MethodException, ConnectionException
	{
			...
			for(ResolvedArtifactSource artifactSource : artifactSourceList)
			{
				if(isRequireVixAtRemoteSites())
				{
					if(artifactSource.getArtifactSource() instanceof Site)
					{
						...
						{
                            getLogger().debug("Site '{}' has a VIX, including in result.", site.getSiteNumber());
							result.add( artifactSource.getArtifactSource().createRoutingToken() );
						}
						else
						{
                            getLogger().info("Site '{}' does not have a VIX, not including in result.", site.getSiteNumber());
						}
					}	
					else
					{
                        getLogger().info("ArtifactSource '{}' is not a Site, cannot determine if source has a VIX, not including in result.", artifactSource.getArtifactSource().toString());
					}
				}
				else
				{
        ...
        getLogger().info("Returning '{}' repositories for patient", result.size());
		return result;
	}
	...
	implements AsynchronousCommandResultListener<S>
	{
		...
		public void commandComplete(AsynchronousCommandResult<S> result)
		{
			...
			if(result.isSuccess())
			{
				...
				//	" results.");
                getLogger().info("Child command adding {} results from site '{}' for transaction Id [{}].", result.getResult() == null ? "null" : "not null", routingToken.toRoutingTokenString(), transactionId);
			...
			if(result.isError())
			{
                getLogger().info("child command results in error, adding no results from site [{}] for transaction Id [{}].", routingToken.toRoutingTokenString(), transactionId);
