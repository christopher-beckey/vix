...
{
	private final static int asynchCommandCompletionNotificationServiceThreads = 25;
	...
	public AsynchronousRouterImpl(RouterImpl router)
    {
	    super();
	    this.router = router;
    ...
    public void doAsynchronously(Command<?> command)
    {
    	...
    	{
            logger.info("AsynchronousRouterImpl.doAsynchronously() --> Submitting asynchronous command [{}] for asynchronous processing using [{}]", command.toString(), command.getClass().getSimpleName());
            ...
            logger.error("AsynchronousRouterImpl.doAsynchronously() --> Error submitting asynchronous command [{}] for asynchronous processing.  Command class is incompatible with the asynchronous router implementation.", command.toString());
    }
        ...
        public void run()
      	{
            ...
            while(running)
            {
				...
				try
	            {
	        		...
	        		{
                        logger.warn("AsynchronousRouterImpl.run() --> Unable to push transaction context (make it the current context).  Completion listeners cannot be safely notified: {}", itcmX.getMessage());
	        		}
	                ...
	                {
                        logger.info("AsynchronousRouterImpl.run() --> Asynchronous command [{}] completed with retry requested.", result.getCommand().toString());
                        ...
                        logger.info("AsynchronousRouterImpl.run() --> Asynchronous command [{}] retrying.", command.toString());
	        			doAsynchronously(command);
	                }
	                else if(transactionContextSet)
	                {
                        logger.info("AsynchronousRouterImpl.run() --> Asynchronous command [{}] completed.", result.getCommand().toString());
	                    ...
	                    if(command.isPeriodic())
	                    {
	                    	...
	                    	if(result.getThrowable() != null)
	                    	{
	                    		...
	                    		if(fatalExceptionClasses != null)
	                    		{
	                    			...
	                    			{	                    				
	                    				...
	                    				{
	                    					fatalException = true;
                                            logger.warn("AsynchronousRouterImpl.run() --> Periodic command [{}] completed with a fatal exception of type [{}], will not be rescheduled.", result.getCommand().toString(), result.getThrowable().getClass().getName());
	                    	...
	                    	{	    
	                    		...
	                    		{
                                    logger.info("AsynchronousRouterImpl.run() --> Periodic command [{}] completed but has been set to terminate - will not be added back to the asynch queue.", result.getCommand().toString());
	                    		}
	                    		else
	                    		{
                                    logger.info("AsynchronousRouterImpl.run() --> Periodic command [{}] completed and is being added back to the asynch queue.", result.getCommand().toString());
	            ...
	            {
					logger.warn("AsynchronousRouterImpl.run() --> CommandCompletionServiceCompletion interrupted waiting for results, ignoring...");
	            } 
				catch(Throwable t)
				{
                    logger.warn("AsynchronousRouterImpl.run() --> CommandCompletionServiceCompletion received exception when calling AsynchronousCommandResultListener: {}", t.getMessage());
				}
				finally
				{
