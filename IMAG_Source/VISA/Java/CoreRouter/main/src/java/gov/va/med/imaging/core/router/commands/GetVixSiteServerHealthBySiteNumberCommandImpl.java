...
extends AbstractCommandImpl<VixSiteServerHealth> 
{
	...
	private static final Logger logger = Logger.getLogger(GetVixSiteServerHealthBySiteNumberCommandImpl.class);
	...
	{
        logger.info("GetVixSiteServerHealthBySiteNumberCommandImpl.callSynchronouslyInTransactionContext() --> Retrieving VIX Server health for site [{}], forceRefresh [{}]", this.routingToken.getRepositoryUniqueId(), this.forceRefresh);
		...
		{
			String msg = "GetVixSiteServerHealthBySiteNumberCommandImpl.callSynchronouslyInTransactionContext() --> Site [" + this.routingToken.getRepositoryUniqueId() + "] does not have a VIX. Cannot get VIX health from this site.";
			logger.error(msg);
	...
	{
		return "Rounting token: " + this.routingToken.toString();
	}

	public VixServerHealthSource[] getVixServerHealthSources()
	{
		return this.vixServerHealthSources;
	}
}
