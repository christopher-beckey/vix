...
public class ProcessAsyncStorageRequestCommandImpl extends
	...
	private Logger logger = Logger.getLogger(ProcessAsyncStorageRequestCommandImpl.class);
	...
	public Boolean callSynchronouslyInTransactionContext()
        ...
        logger.info("{} unsatisfied maps detected for artifact {}", unsatisfiedMaps.size(), artifact.getId());
	...
	private ArrayList<ArtifactInstance> writeInstances(
		...
		for (Provider provider : targetProviders) {
			if (canWriteToProvider(provider)) {
				try ( InputStream inStream = currentInstance.getProvider().getArtifactStream(currentInstance) ){
					...
					results = provider.writeArtifactStream(null,
							inStream,
							// Fortify change: moved into try statement
					...
					throw new ConnectionException(storageDeviceinfo + "failed. "+ ioe);
				}
			}
		}
		return newInstances;
