...
extends AbstractCommandImpl<List<Patient>> 
{
	...
	throws MethodException, ConnectionException
	{
        ...
        getLogger().info("findPatients - Transaction ID [{}] from site [{}] for search [{}].", transactionContext.getTransactionId(), getRoutingToken().toString(), getPatientName());
		
		Set<Patient> patients = InternalContext.getRouter().getPatientsByName(
			getRoutingToken(), 
			getPatientName());

		String resultLog = ((patients == null || patients.size() == 0) ? "no" : "" + patients.size());
        getLogger().info("Got {} patients for search '{}'.", resultLog, getPatientName());
