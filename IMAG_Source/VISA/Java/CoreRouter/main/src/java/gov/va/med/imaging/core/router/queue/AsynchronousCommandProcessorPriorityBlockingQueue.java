...
implements BlockingQueue<AbstractCommandImpl<?>>
{
	...
	implements Comparator<AbstractCommandImpl<?>>
	{
        ...
        public int compare(AbstractCommandImpl<?> element1, AbstractCommandImpl<?> element2)
        {
			...
			{
                logger.debug("{} commencement date is before {} commencement date", element1.toString(), element2.toString());
				...
				else
				{
                    ...
                    logger.debug("{} estimated completion date is {}", element1.toString(), element1EstimatedCompletionDate);
			...
			else if(processingCommencementDateDelta > 0)
			{
				...
				else
				{
                    ...
                    logger.debug("{} estimated completion date is {}", element2.toString(), element2EstimatedCompletionDate);
			...
			{
                logger.debug("{} commencement date is equal to {} commencement date", element1.toString(), element2.toString());
            ...
            logger.debug("{} is {} relative to {}", element1.toString(), result > 0 ? "lower priority" : result < 0 ? "higher priority" : "equal priority", element2.toString());
					...
					try
                    {
						synchronized(this)
						{
							...
							else
							{
                                ...
                                logger.info("Transfer thread waiting ({}) for existing scheduled queue elements", timeout);
