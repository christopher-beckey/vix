...
public class AsynchronousCommandExecutor
{
	...
	private final Logger logger = Logger.getLogger(AsynchronousCommandExecutor.class);
        ...
        public void run()
        {
			while(! isShuttingDown() && !isSepuku())
			{
				try
	            {
					...
					try
					{
						...
						if(executingCommand.isPeriodic())
						{
							if(executingCommand.isPeriodicProcessingTerminated())
							{
                                ...
                                logger.warn("AsynchronousCommandExecutorThread.run() --> Periodic Command [{}] has been terminated and will NOT execute.", executingCommand.getClass().getName());
							}
							else
							{
					...
					{
						StringBuilder sb = new StringBuilder("AsynchronousCommandExecutorThread.run() --> ");
						
						sb.append(t.getClass().getName());
						sb.append(" [");
						sb.append(t.getMessage());
						sb.append("] ");
						sb.append(t.getCause() != null ? (" caused by [" + t.getCause().getClass().getName() + "]") : "");
				...
				{
					if(! isSepuku())
						logger.warn("AsynchronousCommandExecutorThread.run() --> Unexpected InterruptedException, ignoring ....");
				}
