...
public class ProcessAsyncStorageQueueCommandImpl extends
	...
	private Logger logger = Logger.getLogger(ProcessAsyncStorageQueueCommandImpl.class);
	...
	public Boolean callSynchronouslyInTransactionContext()
		...
		logger.info("ProcessAsyncStorageQueueCommandImpl.callSynchronouslyInTransactionContext() --> Checking for async storage requests...");
		
		TransactionContext transactionContext = TransactionContextFactory.get();
		transactionContext.setServicedSource(getLocalSiteId());
		InternalRouter internalRouter = InternalContext.getRouter();
		StorageBusinessRouter storageBusinesssRouter = StorageContext.getBusinessRouter();
		...
		DurableQueueMessage queueMessage = internalRouter.dequeueDurableQueueMessage(getRoutingToken(), getAsyncQueue().getId(), Integer.toString(place.getId()));
		AsyncStorageRequest request = null;
		...
		while (queueMessage != null) {
			...
			try {
				request = AsyncStorageRequest.deserializeUsingXStream(queueMessage.getMessage());
			} catch (Exception e) {
                logger.error("ProcessAsyncStorageQueueCommandImpl.callSynchronouslyInTransactionContext() --> Error parsing async storage queue message: {}", queueMessage.getMessage());
			...
			if (request != null) {
				try {
                    ...
                    logger.error("ProcessAsyncStorageQueueCommandImpl.callSynchronouslyInTransactionContext() --> Error processing async storage request: {}", e.getMessage());
			...
			queueMessage = internalRouter.dequeueDurableQueueMessage(getRoutingToken(), getAsyncQueue().getId(), Integer.toString(place.getId()));
		}
		return true;
	}

	private void requeue(DurableQueueMessage queueMessage, AsyncStorageRequest request, String[] eMailTos, String threadId) 
	throws MethodException, ConnectionException {
		...
		if (request.getNumAttempts() < getAsyncQueue().getNumRetries()) {
			logger.info("ProcessAsyncStorageQueueCommandImpl.requeue() --> Requeueing async storage request for future processing");
			...
			logger.info("ProcessAsyncStorageQueueCommandImpl.requeue() --> The maximum number of attempts for this async"
		...
		InternalContext.getRouter().enqueueDurableQueueMessage(getRoutingToken(), queueMessage);
	}
