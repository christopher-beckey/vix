...
extends AbstractCommandImpl<java.lang.Void>
{
	...
	private static final Logger logger = Logger.getLogger(DeleteTransactionLogEntryCommandImpl.class);
	
	private final Integer maxDaysAllowed;
	
	/**
    * @param Integer   	number of days of storage allowed before TransactionLog records purged 
	 */
	...
	{
        logger.info("DeleteTransactionLogEntryCommandImpl.callSynchronouslyInTransactionContext() --> Deleting anything over [{}] days-old", maxDaysAllowed);
		...
		{
            logger.warn("DeleteTransactionLogEntryCommandImpl.callSynchronouslyInTransactionContext() --> Encountered ConnectionException: {}", cX.getMessage());
   	...
   	// MethodExceptions are always logged regardless of whether we fail
		...
		{
            logger.info("DeleteTransactionLogEntryCommandImpl.callSynchronouslyInTransactionContext() --> Failed deleting anything over [{}] days-old: {}", maxDaysAllowed, mX.getMessage());
    ...
    {
		return (this.maxDaysAllowed == null ? "<null maxDaysAllowed>" : this.maxDaysAllowed.toString());
    }
}
