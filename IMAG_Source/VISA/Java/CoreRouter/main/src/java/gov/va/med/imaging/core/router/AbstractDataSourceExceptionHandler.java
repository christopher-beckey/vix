...
implements DataSourceExceptionHandler 
{
	...
	private static final Logger LOGGER = Logger.getLogger(AbstractDataSourceExceptionHandler.class);
	...
	public boolean handleException(Exception ex) 
	{
		...
		{
            LOGGER.warn("AbstractDataSourceExceptionHandler.handleException() --> Handling exception [{}]", ex.getClass().getSimpleName());
			...
			{
                LOGGER.warn("AbstractDataSourceExceptionHandler.handleException() --> Error generating new token from expierd token: {}", mX.getMessage());
				return false;
			}
		}
		return false;
	...
	{
		return (ex.getClass() == SecurityCredentialsExpiredException.class);
	}
    ...
    throws MethodException
    {
    	...
    	{
    		String msg = "AbstractDataSourceExceptionHandler.generateNewSecurityToken() --> No security application defined to generate a new token, cannot create a token with security application.";
    		LOGGER.error(msg);
    	...
    	if(site == null)
    	{
    		String msg = "AbstractDataSourceExceptionHandler.generateNewSecurityToken() --> No local Site configured to generate a new token, cannot create a new token.";
    		LOGGER.error(msg);
    		throw new MethodException(msg);
    	}

        LOGGER.info("AbstractDataSourceExceptionHandler.generateNewSecurityToken() --> Retrieving new security token from site [{}] for application [{}]", site.getSite().getSiteNumber(), securityTokenApplicatioName);
    	...
    	for(URL url : resolvedUrls )
		{
    		try
    		{
                ...
                LOGGER.debug("AbstractDataSourceExceptionHandler.generateNewSecurityToken() --> Updated security token with new value [{}]", token);
    			return;
    		}
    		catch(ConnectionException cX)
    		{
                LOGGER.error("AbstractDataSourceExceptionHandler.generateNewSecurityToken() --> Failed to contact site [{}] with URL [{}] to retrieve token.", site.getSite().getSiteNumber(), url.toExternalForm());
    			lastException = cX;
    		}
		}
    	
    	LOGGER.warn("AbstractDataSourceExceptionHandler.generateNewSecurityToken() --> Was not able to find a provider to generate a new token. Cannot continue");
