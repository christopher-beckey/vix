...
public abstract class StorageCIFSProvider extends Provider
{
	...
	{ 
		...
		try 
		{
            ...
            logger.info("StorageCIFSProvider.writeArtifactStream() --> Successfully wrote to file [{}]", fullFileSpec);
		} 
		catch (Exception e)
		{
            logger.warn("StorageCIFSProvider.writeArtifactStream() --> Failed to write to file [{}]: {}", fullFileSpec, e.getMessage());
		   	writeException = e;
		...
		finally
		{
			if(checkedOutputStream != null)
			{
				...
				{
                    logger.warn("StorageCIFSProvider.writeArtifactStream() --> Failed to delete incomplete file [{}]", fullFileSpec);
				}
			}
		}
		...
		else
		{
			createStorageTransaction(StorageTransaction.WRITE_TRANSACTION,
		        	...
		        	// OLD: writeException.getMessage(),
					writeException != null ? writeException.getMessage() : "No exception message available.",
					artifact);
