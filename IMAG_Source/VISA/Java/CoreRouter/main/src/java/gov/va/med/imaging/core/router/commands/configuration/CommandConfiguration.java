...
extends AbstractBaseFacadeConfiguration
{
	...
	private int maxMemCacheStudies = 5;
	private int simpleStudyCacheReadDelayMillis = 100;
	...
	public static void main(String [] args)
	{
		...
		boolean ensurePatientSeenAtDoD = false;
		int maxSic = 5;
		int delayMillis = 100;
		if(args.length == 1)
		{
		...
		else if(args.length == 4)
		{
			...
			callArtifactsSpi = Boolean.parseBoolean(args[2]);
			ensurePatientSeenAtDoD = Boolean.parseBoolean(args[3]);
			maxSic = Integer.parseInt(args[4]);
			...
			ensurePatientSeenAtDoD = Boolean.parseBoolean(args[3]);
			maxSic = Integer.parseInt(args[4]);
			delayMillis = Integer.parseInt(args[5]);
		}
		...
		config.setMaxMemCacheStudies(maxSic);
		config.setSimpleStudyCacheReadDelayMillis(delayMillis);
		config.storeConfiguration();
	}

	public int getMaxMemCacheStudies() {
		return maxMemCacheStudies;
	}

	public void setMaxMemCacheStudies(int maxMemCacheStudies) {
		this.maxMemCacheStudies = maxMemCacheStudies;
	}

	public int getSimpleStudyCacheReadDelayMillis() {
		return simpleStudyCacheReadDelayMillis;
	}

	public void setSimpleStudyCacheReadDelayMillis(int simpleStudyCacheReadDelayMillis) {
		this.simpleStudyCacheReadDelayMillis = simpleStudyCacheReadDelayMillis;
	}
}
