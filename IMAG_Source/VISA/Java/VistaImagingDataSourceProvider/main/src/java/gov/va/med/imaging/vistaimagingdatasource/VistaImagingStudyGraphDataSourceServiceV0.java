...
implements StudyGraphDataSourceSpi
{
	...
	throws UnsupportedOperationException, MethodException, ConnectionException
	{
        ...
        logger.info("getPatientStudies({}, {}, Study load level {}) TransactionContext ({}).", patientIdentifier, filter == null ? "<null>" : "" + filter.toString(), studyLoadLevel.toString(), TransactionContextFactory.get().getDisplayIdentity());
		...
		try
		{
			...
			catch(PatientNotFoundException pnfX)
			{
                ...
                logger.warn("Patient [{}] was not found in the VistA database, returning empty study set", patientIdentifier, pnfX);
	        ...
	        // Fortify change: check for null first; second round: there are more null checks based on group.size()
	        // so get the size once here
	        int groupSize = groups != null ? groups.size() : 0;

            logger.info("Found [{}] group(s) for patient [{}]", groupSize, patientIdentifier);
			...
			if(!studyLoadLevel.isIncludeImages())
			{
				...
				if(studyLoadLevel.isIncludeReport())
				{
                    ...
                    logger.info("Completed getPatientStudies(), returning '{}' studies.", groups.size());
					return StudySetResult.createFullResult(groups);
				}
				else
				{
                    logger.info("Completed getPatientStudies(), no report or images. Returning '{}' studies.", groups.size());
					return StudySetResult.createFullResult(groups);
				}
			}			
