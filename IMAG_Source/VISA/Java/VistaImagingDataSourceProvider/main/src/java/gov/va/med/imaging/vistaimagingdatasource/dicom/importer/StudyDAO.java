...
public class StudyDAO extends BaseImporterDAO<Study>
{
	...
	throws MethodException
	{
		...
		if (lines.length == 1)
		{
			...
			{
				// No data found
                logger.error("No previous reconcilation data found for study with study UID = {}, series UID = {}, and SOP instance UID = {}", studyUid, seriesUid, instanceUid);
				
			}
			else
			{
				// Error occurred. Log the error
                logger.error("An error occurred while retrieving previous reconcilation data for study UID = {}, series UID = {}, and SOP instance UID = {}. Error message: {}", studyUid, seriesUid, instanceUid, fields[1]);
			}
		}
		else
		{
