...
extends AbstractBaseVistaImagingVistaRadService
{
	...
	{		
        ...
        getLogger().info("postExamAccessEvent({}) TransactionContext ({}).", inputParameter, TransactionContextFactory.get().getDisplayIdentity());
		...
		try
		{
			...
			for(int i = 0; i < pieces.length; i++)
			{
				if(i == 2)
				{
					...
					{
                        getLogger().warn("Error converting string '{}' into ImageURN, cannot continue", pieces[i], urnfX);
						return false;
					}
				}
				else
