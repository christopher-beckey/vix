...
public class VistaImagingVistaRadCommonUtilities 
{
    ...
    {	
	    ...
	    if(!session.isVistaRadLoggedIn())
	    {
	    	try
	    	{
                logger.info("Session ({}) not logged in for VistARad, logging in", session.getSessionIndex());
	...
	{	
        ...
        logger.info("magJCacheLocation cacheLocationId({}) TransactionContext ({}).", cacheLocationId, TransactionContextFactory.get().getDisplayIdentity());
	...
	throws VistaMethodException, InvalidVistaCredentialsException, IOException, InvalidVistaVistaRadVersionException 
	{
        logger.info("magJUser({}) TransactionContext ({}).", requiredVersion, TransactionContextFactory.get().getDisplayIdentity());
...
*END
		...
		else
		{
			...
			if(result.startsWith("2"))
			{
                ...
                logger.warn("Version [{}] is test version, allowed to continue. Response from VistA '{}'", requiredVersion, pieces[0]);
			}
	...
	throws VistaMethodException, ConnectionException, MethodException 
	{
        ...
        logger.info("getMagJRequisitionReport({}) TransactionContext ({}).", examId, TransactionContextFactory.get().getDisplayIdentity());
	...
	{
        logger.info("getPatientICN({}) TransactionContext ({}).", patientDFN, TransactionContextFactory.get().getDisplayIdentity());
    	...
    	try
        {
	    	...
	    	if( rtn.startsWith("-1") ) 
	    	{
                logger.warn("DFN [{}] not found in database, response [{}]", patientDFN, rtn);
