...
public class OrderDAO extends BaseImporterDAO<Order>
{
	...
	protected Logger radiologyRPCLogger = Logger.getLogger(OrderDAO.class);
	...
	private boolean isValidStatusForImport(String examStatus, String creditMethod) 
	{
		...
		examStatus = examStatus.trim().toUpperCase(Locale.ENGLISH);
		
		// Determine whether or not this is a no-credit exam
		String normalizedCreditMethod = creditMethod.toUpperCase(Locale.ENGLISH).replace(" ", "");
...
//				10- ClinicalProcedure
	...
	private HashMap<String, String> getClinicalHistoryData(Study study) 
	{
		...
		hm.put("2", "CLINHIST^2^        Patient Name: " + study.getPatient().getPatientName());
 		hm.put("3", "CLINHIST^3^          Patient ID: " + StringUtil.cleanString(study.getPatient().getSsn()));
	...
	public Order setOrderExamComplete(Reconciliation reconciliation, String technicianDuz, String placeId) throws MethodException, ConnectionException
	{
		...
		if (statusChangeDetails != null)
		{
			...
			// Get the manually entered text
			manuallyEnteredText = statusChangeDetails.getManuallyEnteredText();

			// Get the report text
			reportText = statusChangeDetails.getReportText();
			
			// Get the impression
			impression = statusChangeDetails.getImpression();
		...
		if (manuallyEnteredText.equals("1")) {
			vm.addParameter(VistaQuery.LITERAL, manuallyEnteredText);
			
			// Add the list of lines of ReportText
			HashMap <String, String> hmRpt = new HashMap <String, String>();
			String[] rptTxt = reportText.split(System.lineSeparator());
			
			if ((rptTxt != null) && (rptTxt.length > 0))
			{
				for (int i=0; i<rptTxt.length; i++)
				{
					hmRpt.put(Integer.toString(i), rptTxt[i]);
				}
			}		
			vm.addParameter(VistaQuery.LIST, hmRpt);

			// Add the list of lines of Impression
			HashMap <String, String> hmImprs = new HashMap <String, String>();
			String[] imprs = impression.split(System.lineSeparator());
			
			if ((imprs != null) && (imprs.length > 0))
			{
				for (int i=0; i<imprs.length; i++)
				{
					hmImprs.put(Integer.toString(i), imprs[i]);
				}
			}		
			vm.addParameter(VistaQuery.LIST, hmImprs);
		}
