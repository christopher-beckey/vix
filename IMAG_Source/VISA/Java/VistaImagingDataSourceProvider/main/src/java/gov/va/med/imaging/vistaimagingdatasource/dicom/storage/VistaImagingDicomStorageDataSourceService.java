...
implements DicomStorageDataSourceSpi, VistaSessionFactory
{
	...
	public List<InstrumentConfig> getDgwInstrumentList(String hostName) throws MethodException, ConnectionException
	{
		...
		try
		{
            ...
            logger.info("{} RPC: {}", vm.getRpcName(), StringUtils.displayEncodedChars(returnValue));
		...
		finally
		{
        	...
        	// OLD: try{session.close();} catch(Throwable t){}
        	try{ if(session != null) { session.close(); } } catch(Exception exc) {/*unrecoverable so do nothing*/}
		}
	...
	public ProcedureRef getOrCreateProcedureRef(PatientRef patientRef, ProcedureRef procedureRef) throws MethodException, ConnectionException
	{
		...
		if(logger.isDebugEnabled()){
            logger.debug("{}:Set Patient REF IEN.", this.getClass().getName());}
		procedureRef.setPatientRefIEN(patientRef.getIEN());
		if(logger.isDebugEnabled()){
            logger.debug("{}: Get Procedure REF if exist.", this.getClass().getName());}
		...
		if (persistentRef == null)
		{
			...
			if(logger.isDebugEnabled()){
                logger.debug("{}: Create Procedure REF.", this.getClass().getName());}
