...
public class ImagingSiteCredentials
{
	...
	{
        logger.info("Searching imaging site credentials from site '{}' for network location with path '{}'.", getSiteNumber(), imagePath);
		if(imagePath != null)
		{
			for(NetworkLocation networkLocation : networkLocations)
			{
				...
				{
                    logger.info("Found network location for path '{}' from site '{}'.", imagePath, getSiteNumber());
					return networkLocation;
				}
			}
		}
        logger.info("Did not find network location for path '{}' from site '{}', using default site credentials to access image share.", imagePath, getSiteNumber());
