...
implements VistaRadDataSourceSpi 
{
	...
	{
        getLogger().info("isVersionCompatible searching for version [{}], TransactionContext ({}).", getRequiredVistaImagingVersion(), TransactionContextFactory.get().getDisplayIdentity());
		VistaSession localVistaSession = null;		
		try
		{			
			localVistaSession = getVistaSession();
            getLogger().info("Got VistaSession ({}) for version [{}].", localVistaSession.getSessionIndex(), getRequiredVistaImagingVersion());
			return true;
		}
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("{}.getRelevantPriorCptCodes({}) TransactionContext ({}).", getClassSimpleName(), cptCode, TransactionContextFactory.get().getDisplayIdentity());
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("{}.getActiveExams({}) TransactionContext ({}).", getClassSimpleName(), listDescriptor, TransactionContextFactory.get().getDisplayIdentity());
		VistaSession vistaSession = null;
		try
		{
			vistaSession = getVistaSession();			
            ...
            getLogger().info("Returning active exams containing '{}' exams.", activeExams.size());
			return activeExams;
		}
	...
	private ActiveExams translateActiveExamsResponse(String response, VistaSession vistaSession)
	{
        ...
        getLogger().info("Translating '{}' lines into active exams response.", lines != null ? lines.length : 0);
		String headerLine1 = "";
		String headerLine2 = "";
		if(lines.length > 0)
		{
			...
			HashMap<String, String> patientDfnMap)
	{
		...
		{
            getLogger().debug("Found ICN '{}' in map for DFN '{}'", patientIcn, patientDfn);
		}
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("{}.getExamReport({}) TransactionContext ({}).", getClassSimpleName(), studyUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
		VistaSession vistaSession = null;
		try
		{
			vistaSession = getVistaSession();	
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("{}.getExamRequisitionReport({}) TransactionContext ({}).", getClassSimpleName(), studyUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
		VistaSession vistaSession = null;
		try
		{
			vistaSession = getVistaSession();	
    ...
    throws MethodException, ConnectionException
    {
        ...
        getLogger().info("{}.getExamsForPatient({}) fullyLoaded={}, forceImagesFromJb={} TransactionContext ({}).", getClassSimpleName(), patientICN, fullyLoadExams, forceImagesFromJb, TransactionContextFactory.get().getDisplayIdentity());
		VistaSession vistaSession = null;
		try
		{
			vistaSession = getVistaSession();
			List<Exam> exams = getShallowExamsForPatientInternal(vistaSession, patientICN, patListColumnsIndicator);
            getLogger().info("Found '{}' exams for patient [{}]", exams.size(), patientICN);
			if(fullyLoadExams)
			{
				for(Exam exam : exams)
				{
	...
	{				
        ...
        getLogger().info("loadExam({}) TransactionContext ({}).", studyUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
		String report = getExamReportInternal(vistaSession, studyUrn);
        ...
        getLogger().info("Exam '{}' loaded with '{}' images.", studyUrn.toString(), images == null ? "<null>" : "" + images.size());
	}
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("{}.getExamsForPatient({}) TransactionContext ({}).", getClassSimpleName(), studyUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
		VistaSession vistaSession = null;
		try
		{
			vistaSession = getVistaSession();
			ExamImages images = getExamImages(vistaSession, studyUrn, true);
            getLogger().info("Found '{}' images for exam URN '{}'.", images.size(), studyUrn.toString());
			return images;
		}
		catch(InvalidVistaVistaRadVersionException vvrvX)
		{
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("{}.getExam({}) TransactionContext ({}).", getClassSimpleName(), studyUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
		VistaSession vistaSession = null;
		try
		{
			vistaSession = getVistaSession();
