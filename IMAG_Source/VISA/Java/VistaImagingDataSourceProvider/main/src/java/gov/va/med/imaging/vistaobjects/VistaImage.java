...
implements Serializable, Comparable<VistaImage>
{
	...
	throws VistaParsingException
	{
		...
		String type = pieces[6];
		if(type != null && type.length() > 0){
			image.imgType = Integer.parseInt(type);
		}
		//image.imgType = Integer.parseInt(pieces[6]);
		image.procedure = pieces[7];
		String procDate = pieces[8];
		if(procDate != null && procDate.length() > 0){
			image.procedureDate = VistaTranslatorUtility.convertVistaDatetoDate(procDate);			
		}
		...
		if(pieces.length >= 31)
		{
			String capDate = pieces[22];
			if(capDate != null && capDate.length() > 0){
				image.captureDate = VistaTranslatorUtility.convertVistaDatetoDate(capDate);
			}
			String docDate = pieces[23];
			if(docDate != null && docDate.length() > 0){
				image.documentDate = VistaTranslatorUtility.convertVistaDatetoDate(docDate);
			}
			...
			String boolSensitive = pieces[28];
			if(boolSensitive != null & boolSensitive.length() > 0){
				image.sensitive = VistaTranslatorUtility.convertVistaBooleanToBoolean(boolSensitive);
			}
		...
		if(pieces.length >= 36)
		{
			...
			String noteResulted = pieces[32];
			if(noteResulted != null && noteResulted.length() > 0){
				image.associatedNoteResulted = noteResulted.trim();
			}
			...
			String description = pieces[34];
			if(description != null && description.length() > 0){
				image.imageAnnotationStatusDescription = description.trim();
			}
			String pkg = pieces[35];
			if(pkg != null && pkg.length() > 0){
				image.imagePackage = pkg.trim();
			}
		}
