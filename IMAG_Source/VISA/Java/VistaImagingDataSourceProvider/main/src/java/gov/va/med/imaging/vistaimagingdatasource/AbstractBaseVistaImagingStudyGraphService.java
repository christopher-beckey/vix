...
implements StudyGraphDataSourceSpi 
{
	...
	{
        logger.info("isPatientRestricted({}, ...) TransactionContext ({}).", patientDfn, TransactionContextFactory.get().getDisplayIdentity());
	...
	throws MethodException, ConnectionException 
	{
        ...
        logger.info("getStoredFilters({}) TransactionContext ({}).", globalRoutingToken, TransactionContextFactory.get().getTransactionId());
		
		try
		{
			vistaSession = getVistaSession();
			String duz = TransactionContextFactory.get().getDuz();
			VistaQuery vm = VistaImagingQueryFactory.createGetFilterList(duz);
			String rtn = vistaSession.call(vm);
			
			return VistaImagingTranslator.translateFilters(rtn, globalRoutingToken);	
		}
