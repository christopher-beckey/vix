...
public class WorkItemDAO extends BaseWorkListDAO<WorkItem>
{
	...
	private static String GET_PRECACHE_WORK_ITEMS = "MAGN PRECACHE BY CPT";
	private static String UPDATE_WORK_ITEM = "MAGV UPDATE WORK ITEM";
	private static String UPDATE_WORK_ITEM_SERVICE = "MAGV UPDATE WORK ITEM SERVICE";
	...
	private static String LastIENProcessed = "LastIENProcessed";
	private static String MessageLine = "Message";
	private static String Tag = "Tag";
	private static String UpdateService = "1";
	//
	// Constructor
	//
	public WorkItemDAO(){}
	...
	public VistaQuery generateCreateQuery(WorkItem workItem) 
	{
		...
		vm.addParameter(VistaQuery.LITERAL, UpdateService); //Update Service
	...
	public VistaQuery generateFindByCriteriaQuery(Object criteria)
	{
		...
		String lastIenProcessed = filter.getLastIenProcessed() != null ? filter.getLastIenProcessed() : "";
		String retrievalOrder = filter.getRetrievalOrder() != null ? filter.getRetrievalOrder() : "";
		String fromDate = filter.getFromDate() != null ? filter.getFromDate() : "";
		String toDate = filter.getToDate() != null ? filter.getToDate() : "";
		...
		vm.addParameter(VistaQuery.LITERAL, lastIenProcessed);
		vm.addParameter(VistaQuery.LITERAL, retrievalOrder);
		vm.addParameter(VistaQuery.LITERAL, fromDate);
		vm.addParameter(VistaQuery.LITERAL, toDate);
	
		return vm;
	}

	@Override
	public VistaQuery generateFindByCptQuery(String idType, String patientId, String cptCode)
	{
		VistaQuery vm = new VistaQuery(GET_PRECACHE_WORK_ITEMS);
		vm.addParameter(VistaQuery.LITERAL, idType);
		vm.addParameter(VistaQuery.LITERAL, patientId);
		vm.addParameter(VistaQuery.LITERAL, cptCode);
		
		return vm;
	}

	@Override
	public List<WorkItem> translateFindByCpt(Object criteria, String returnValue) 
	throws MethodException, RetrievalException
	{
		...
		if ((lines.length > 0) && (lines[0].equals("1")))
		{
			for (int i = 1; i<lines.length; i++)
			{
				String line = lines[i];
				if(logger.isDebugEnabled()){logger.debug("Translating line: " + line + " to workitem");}

				WorkItem workitem = new WorkItem();
				workitem.setPlaceId(StringUtils.Piece(line, StringUtils.CARET, 1));
				
				workitem.addTag("patienticn", StringUtils.Piece(line, StringUtils.CARET, 3));
				workitem.addTag("contextid", StringUtils.Piece(line, StringUtils.CARET, 4).replace(StringUtils.TILDE, StringUtils.CARET));
	...
	throws MethodException
	{
		...
		{
			lastIenProcessed = Integer.parseInt(fields[1]);
		}
		
		// We made it here, so create the list of work items
		List<WorkItem> workItems = parseWorkItems(lines, lastIenProcessed);
		
		return workItems;
	}
	private WorkItem parseSingleWorkItem(String[] lines) 
	{
		List<WorkItem> workItems = parseWorkItems(lines, 0);
	...
	private List<WorkItem> parseWorkItems(String[] lines, int lastIenProcessed) 
	{
		List<WorkItem> workItems = new ArrayList<WorkItem>();
		WorkItem workItem = null;
		...
		for (int i=1; i<lines.length; i++)
		{
			...
			if (lineType.equals(WorkItemHeader))
			{
				...
				if ((lastIenProcessed > 0) && (workItem.getId() == lastIenProcessed)) {
					workItem.setLastIenFlag("*");
				}
	...
	public String updateWorkItemService(String service,	String modality, String procedure, String newService)
	throws MethodException, ConnectionException
	{
		VistaQuery vm = new VistaQuery(UPDATE_WORK_ITEM_SERVICE);
		vm.addParameter(VistaQuery.LITERAL, service);
		vm.addParameter(VistaQuery.LITERAL, modality);
		vm.addParameter(VistaQuery.LITERAL, procedure);
		vm.addParameter(VistaQuery.LITERAL, newService);
		
		return executeRPC(vm);
	}
