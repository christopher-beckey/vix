...
public class WorkItemProcedureDAO extends BaseImporterDAO<ImporterFilter>
{
	private static String GET_WORK_ITEM_PROCEDURES = "MAGV GET WORK ITEM PROCEDURES";

	//
	// Constructor
	//
	public WorkItemProcedureDAO(){}
	public WorkItemProcedureDAO(VistaSessionFactory sessionFactory)
	{
		this.setSessionFactory(sessionFactory);
	}
	
	public List<ImporterFilter> getWorkItemProcedures() 
	throws MethodException, ConnectionException 
	{
		VistaQuery vm = generateGetWorkItemProceduresQuery();
	...
	{
		// Create the query
		VistaQuery vm = new VistaQuery(GET_WORK_ITEM_PROCEDURES);
		return vm;
	}
	...
	throws MethodException
	{
		...
		List<ImporterFilter> procedures = new ArrayList<ImporterFilter>();

		// Skip the first line (which was the record count)
		for (int i=1; i<lines.length; i++)
		{
			procedures.add(new ImporterFilter(lines[i]));
		}
