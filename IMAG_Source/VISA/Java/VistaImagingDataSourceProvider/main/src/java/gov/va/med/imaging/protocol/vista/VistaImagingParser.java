...
public class VistaImagingParser
{
	...
	public List<ParsedVistaLine> parse(String[] lines, boolean discardUnknownRootLines)
	{
		...
		for (OntologyDelimiterKey ontologyDelimiterKey : ontologyDelimiterKeys)
			if (ontologyDelimiterKey == null || ontologyDelimiterKey.getAdminkey() == null
				|| ontologyDelimiterKey.getAdminkey().length() <= 0)
		...
		for (String line : lines)
		{
			...
			//if(logger.isDebugEnabled()){logger.debug("Parsing line '" + line + "'.");}
			...
			for (ontologyKeyIndex = 0; ontologyKeyIndex < ontologyDelimiterKeys.length; ++ontologyKeyIndex)
				if (ontologyDelimiterKeys[ontologyKeyIndex].getAdminkey().equals(key))
					break;
	...
	static class OntologyDelimiterKey
	{
		 ...
		 * @param key
		 */
		OntologyDelimiterKey(String adminkey)
		{
		 ...
		 */
		OntologyDelimiterKey(String adminkey, String[] exceptionalKeys)
		{
			this.adminkey = adminkey;
