...
//	protected char separator = '~';
	...
	protected static final Logger logger = Logger.getLogger(EntityDAO.class);
	...
	public VistaQuery generateGetWorkItemProceduresQuery() throws MethodException
	{
		throw new DaoMethodNotImplementedException();
	}

	public List<T> findByCpt(String idType, String patientId, String cptCode)
	throws MethodException, ConnectionException 
	{
		VistaQuery vm = generateFindByCptQuery(idType, patientId, cptCode);
		return translateFindByCpt(null, executeRPC(vm));
	}

	public VistaQuery generateFindByCptQuery(String idType, String patientId, String cptCode) throws MethodException
	{
		throw new DaoMethodNotImplementedException();
	}
	...
	public List<T> translateFindByCpt(Object criteria, String returnValue) throws MethodException, RetrievalException
	{
		throw new DaoMethodNotImplementedException();
	}
	...
	private void logVistaQueryResults(VistaQuery vm, String results){ 
		//WFP-change code to push the truncation work into the .displayEncodedChars() method.
		if(results.length() > 320){
			logger.info(vm.getRpcName()+" RPC Results:"+StringUtils.displayEncodedChars(results).substring(0, 320)
				+ "...](truncated)");
		}
		else{
			logger.info(vm.getRpcName()+" RPC Results:"+StringUtils.displayEncodedChars(results));			
		}
		if(logger.isDebugEnabled()){logger.debug(vm.getRpcName()+" RPC Results:"+StringUtils.displayEncodedChars(results));}
	}
