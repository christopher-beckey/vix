...
//	private final static NetworkLocationCacheManager networkLocationCache = new NetworkLocationCacheManager();
	...
	throws MethodException, ConnectionException, IOException 
	{
        ...
        logger.info("getImageHISUpdates({}) TransactionContext ({}).", decodedImageIen, TransactionContextFactory.get().getDisplayIdentity());
		...
		try
		{
			...
			if(value != null)
			{
				// 11/17/17 CPT: removing SOP UID at all
				if(logger.isDebugEnabled()){
                    logger.debug("His Update contained DICOM SOP INSTANCE UID field: {} -- REMOVED from list!", value);}
				hisUpdate.remove(DICOM_SOP_INSTANCE_UID_FIELD);
//				if(logger.isDebugEnabled()){logger.debug("His Update contains DICOM SOP INSTANCE UID field");}
...
//				}
	...
	throws MethodException, ConnectionException, IOException
	{
        logger.info("getNewSopInstanceUID({}) TransactionContext ({}).", decodedImageIen, TransactionContextFactory.get().getDisplayIdentity());
