...
extends AbstractBaseVistaRadImageDataSourceService 
{
	...
	throws MethodException, ConnectionException, IOException 
	{
        ...
        getLogger().info("getImageHISUpdates({}) TransactionContext ({}).", imageIen, TransactionContextFactory.get().getDisplayIdentity());
		...
		try
		{
			...
			if(value != null)
			{
                ...
                getLogger().debug("His Update contained DICOM SOP INSTANCE UID field: {} -- REMOVED from list!", value);
...
//				}
	...
	throws MethodException, ConnectionException, IOException
	{
        getLogger().info("getNewSopInstanceUID({}) TransactionContext ({}).", decodedImageIen, TransactionContextFactory.get().getDisplayIdentity());
