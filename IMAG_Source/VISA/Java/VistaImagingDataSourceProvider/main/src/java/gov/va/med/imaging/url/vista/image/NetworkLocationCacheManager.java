...
public class NetworkLocationCacheManager  {
	...
	public void updateImagingSiteCredentials(ImagingSiteCredentials imagingSiteCredentials)
	{
		...
		if(logger.isDebugEnabled()){
            logger.debug("Putting Imaging Site Credentials for site '{}' into cache", imagingSiteCredentials.getSiteNumber());}
	...
	public void updateNetworkLocations(List<NetworkLocation> networkLocations) {
		...
		for(int i = 0; i < networkLocations.size(); i++) {
			...
			if(logger.isDebugEnabled()){logger.debug("Putting network location [" + cacheItem.getKey() + "] into cache");}
	...
	public void purgeExpiredCacheItems() {
		if(logger.isDebugEnabled()){logger.debug("Checking for old entries in the NetworkLocation cache");}
		if(cache == null)
			return;
