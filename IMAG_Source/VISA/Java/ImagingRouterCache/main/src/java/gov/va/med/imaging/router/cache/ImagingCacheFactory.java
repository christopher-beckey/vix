...
implements ObjectFactory
{
	...
	throws Exception 
	{
        ...
        logger.info("{} getObjectInstance(Reference, '{}', '{}, ...) ", getClass().getSimpleName(), name.toString(), nameCtx.toString());
        ...
        logger.info("Cache Class Name [{}]", ImagingCache.class.getName());
    	...
    	{
            logger.error("{} reference to '{}' is null.", getClass().getSimpleName(), resourceName);
    	}
    	else
    	{
        	msg = 
            ...
            logger.info("{} reference to '{}' is of type [{}].", getClass().getSimpleName(), resourceName, implementation.getClass().getName());
	...
	{
		logger.info("Creating ApplicationContext for VASourcedCache");
		try
		{
		...
		{
            logger.error("Error Creating dodSourcedCache. {}", e.getMessage());
			return null;
		}
	...
	{
		logger.info("Creating ApplicationContext for DODSourcedCache");
		try
		{
			ApplicationContext appContext = getApplicationContext();
	...
	private synchronized static ApplicationContext getApplicationContext() 
	{
		if(cacheApplicationContext == null)
		{
            ...
            logger.info("VIX Cache context {} created.", cacheApplicationContext.hashCode());
