...
implements InvocationHandler
{
	...
	// the instance logger 
	private final static Logger logger = Logger.getLogger(TransactionContextProxyInvocationHandler.class);
	...
	throws Throwable
	{
		//logger.debug("...entering invoke method.");
		try
		{
		...
		catch(NoSuchMethodException nsmX)
		{
			...
			if(principal == null)
			{
                logger.warn("Attempt to call method {} after security context has been cleared is being ignored.", method.getName());
				return null;
			}
	...
	{
		//logger.debug("...entering invokeMetaMethods method.");
		...
		if(principal == null)
		{
            logger.warn("Attempt to call method {} with no security context is being ignored.", method.getName());
			return null;
		}
		...
		msg.append("Method ["+ method.getName() + "] did not make it thru any of the IF statements.");
		msg.append("  Method Return Type: ["+method.getReturnType()+"]");
		msg.append("  Method Parameter Types: ["+ method.getParameterTypes()+"]");
	...
	throws NoSuchMethodException
	{
		//logger.debug("...entering invokeKnownMethod method.");
		if(principal != null)
		{
			...
			if(isImmutableProperty(propertyName))
			{
				...
				else if( isSetterAccessor(methodName) && isSetterArgs )
				{
					...
					else
					{
                        ...
                        logger.warn("Attempt to set transaction context property '{}', which is immutable in the current context.\nLikely culprit is [{}]'{}.{}:{}'. ", propertyName, Thread.currentThread().getName(), element.getClassName(), element.getMethodName(), element.getLineNumber());
					}
					return null;
				}
				else
				{
                    logger.warn("Attempt to set immutable transaction context property '{}' or calling get accessor with arguments is being ignored.", propertyName);
					return null;
				}
			}
			...
			else if(isRequestorProperty(propertyName))
			{
                ...
                logger.warn("Attempt to access transaction context property '{}' but either method naming or argument list is not correct.  Accessor methods must follow JavaBean standards for String properties", propertyName);
				return null;
			}
			...
			{
				StringBuilder msg = new StringBuilder();
				msg.append("Method Name: [" + methodName+"]");
				msg.append("Property Name: [" + propertyName+"]");
				msg.append("   Is immutable? [" + isImmutableProperty(propertyName)+"]");
				msg.append("   Is requestor? ["+ isRequestorProperty(propertyName)+"]");
				msg.append("   Is debug? [" + isDebugProperty(propertyName)+"]");
				logger.trace(msg.toString());
				throw new NoSuchMethodException();
			}
		}
		...
		{
            logger.warn("Attempted to call TransactionContext method '{}' and backing instance of VistaRealmPrincipal does not exist.", method.getName());
			return null;
		}
	}
	...
	private String invokePropertySetAccessor(VistaRealmPrincipal principal, String propertyName, String value)
	{
		...
		if(oldValue != null)
            logger.info("Overwriting known property '{}'; from '{}' to '{}'.", propertyName, oldValue, value);
	...
	private Object invokeImmutablePropertyGetAccessor(VistaRealmPrincipal principal, String propertyName)
	{
        ...
        logger.warn("Attempt to get unknown immutable transaction context properties '{}' is being ignored.", propertyName);
		return null;
	}
	...
	{
		//logger.debug("...entering invokeUnknownMethod method.");
		if(principal != null)
		{
			...
			if(isSetter && args != null && args.length == 1)
			{
				...
				if(currentValue != null)
				{
					if( isWarnOnPropertOverwrite() )
                        logger.warn("TransactionContext[{}]Overwriting application property '{}'; from '{}' to '{}'.\n{}", principal.hashCode(), propertyName, currentValue, args[0] == null ? "<null>" : "" + args[0].toString(), element == null ? "Unknown offending element" : "Offending element may be '" + element.getClassName() + "." + element.getMethodName() + ":" + element.getLineNumber() + "'");
				}
                logger.debug("{} is setting property name '{}' from value '{}' to '{}'", element == null ? "unknown" : "" + (element.getClassName() + ":" + element.getLineNumber()), propertyName, currentValue, args[0]);
