	...
	implements ElementVisitor<Void, Name>
	{
		...
		public Void visitType(TypeElement e, Name p)
		{
			...
			finally
			{
	        	...
	        	// OLD: try{proxySourceWriter.close();}catch(Throwable t){}
	        	try{ if(proxySourceWriter != null) { proxySourceWriter.close(); } } catch(Exception exc) {/*unrecoverable so do nothing*/}
			}
