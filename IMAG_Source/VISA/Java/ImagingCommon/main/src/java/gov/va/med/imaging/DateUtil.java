...
public class DateUtil
{
	...
	public static Date todayISOFormat(Date date, boolean startTime)
    {
		if(date == null)
			return null;
		
		DateFormat dayGranularity = new SimpleDateFormat("yyyy-MM-dd");
		DateFormat secondGranularity = new SimpleDateFormat("yyyy-MM-dd HH:mm");
		String today = dayGranularity.format(date);
		
		try
        {
	        return secondGranularity.parse(today + (startTime ? " 00:00" : " 23:59"));
        } 
		catch (ParseException e)
        {
			// short of a coding error this should never happen
	...
	public static String changeDateFormat(String dateStr, String fromFormat, String toFormat)
	{
	    ...
	    try 
	    {
	    	resultFormat = toFormatter.format(fromFormatter.parse(dateStr));
	    } 
	    catch (Exception e) 
	    {
	        e.printStackTrace();
