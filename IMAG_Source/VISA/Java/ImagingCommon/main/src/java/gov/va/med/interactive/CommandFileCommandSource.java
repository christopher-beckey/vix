...
{
	private final static Logger LOGGER = Logger.getLogger(CommandFileCommandSource.class);
	...
	{	
		try
		{
			return new CommandFileCommandSource(new URL(sourceUrl).openStream(), sourceUrl, substitutionStrings);
		}
		catch (MalformedURLException x)
		{
			...
			InputStream localInStream = CommandFileCommandSource.class.getClassLoader().getResourceAsStream(sourceUrl);
			
			if(localInStream == null)
				throw new IOException("CommandFileCommandSource.create() --> Unable to open command file [" + sourceUrl + "] as a resource.");
			
			return new CommandFileCommandSource(localInStream, sourceUrl, substitutionStrings);
		}
		catch(IOException ioX)
		{
            LOGGER.warn("{} Return null.", ioX.getMessage());
			return null;
		}
	}
	/**
	...
	public void run()
	{
		...
		for(String[] commandText : commandsText)
		{
			...
			try
			{
                ...
                LOGGER.debug("CommandFileCommandSource.run() --> Created command of type [{}]", command.getClass().getName());
			...
			{
                LOGGER.warn("CommandFileCommandSource.run() --> Unable to create a valid [{}] command: {}", commandName, x.getMessage());
			}
		}
	...
	public synchronized String[][] getCommandsText()
	{
		if(this.commandsText == null) {
			InputStream inputStream = getInStream();
			try {
				this.commandsText = readCommandText(inputStream, getSubstitutionStrings());
			} finally {
				if (inputStream != null) {
					try {
						inputStream.close();
	...
	public static String[][] readCommandText(InputStream inStream, String[] substitutionStrings)
	{
		...
		{
            LOGGER.warn("CommandFileCommandSource.run() --> Error reading command file: {}", ioX.getMessage());
		}
