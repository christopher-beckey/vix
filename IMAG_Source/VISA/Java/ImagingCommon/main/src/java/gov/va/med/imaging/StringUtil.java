...
public class StringUtil
{
    ...
    {
    	return StringUtils.cleanString(aString);
    }
    
    /**
     * QN added for Fortify fixes
     * 
     * Helper method to convert a given file path's file separator to the
     * deployed destination's file separator
     * 
     * @param String		file path to convert
     * @return String 		result (same or converted)
     *  
     */
	public static String toSystemFileSeparator(String filePath) 
	{
		String result = null;
		
		if(filePath != null) 
		{
			if(filePath.contains("\\"))  // Windows based path 
			{
				result = filePath.replace("\\", File.separator);
			} 
			else if (filePath.contains("/")) // UNIX based path
			{
				result = filePath.replace("/", File.separator);
			} 
			else 
