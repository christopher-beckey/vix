...
{
	private static final Logger LOGGER = Logger.getLogger(GlobalArtifactIdentifierFactory.class);
	...
	static
	{
		...
		registerRealization(gov.va.med.imaging.P34ImageURN.class);					// An identifier of an VA new data structure image
		...
		registerRealization(gov.va.med.imaging.P34StudyURN.class);					// An identifier of a VA new Data Structure study
		registerRealization(gov.va.med.imaging.MuseStudyURN.class);					// An identifier of a GE Muse study
		registerRealization(gov.va.med.imaging.MuseImageURN.class);					// An identifier of a GE Muse image
	...
	public static void registerRealization(Class<? extends GlobalArtifactIdentifier> realization)
	{
		try
		{
			...
			if(! Modifier.isPublic(classModifiers) )
				throw new Exception("GlobalArtifactIdentifierFactory.registerRealization() --> The class [" + realization.getName() + "] must be declared public and it is not.");
			...
			if( !GlobalArtifactIdentifier.class.isAssignableFrom(requiredCreate.getReturnType()) )
				throw new Exception("GlobalArtifactIdentifierFactory.registerRealization() --> The method [" + REQUIRED_CREATE_METHOD_NAME + "] must return type GlobalArtifactIdentifier and it does not.");
			
			int createModifiers = requiredCreate.getModifiers();
			if(! Modifier.isStatic(createModifiers) )
				throw new Exception("GlobalArtifactIdentifierFactory.registerRealization() --> The method [" + REQUIRED_CREATE_METHOD_NAME + "] is not declared static and it must be.");
			
			if(! Modifier.isPublic(createModifiers) )
				throw new Exception("GlobalArtifactIdentifierFactory.registerRealization() --> The method [" + REQUIRED_CREATE_METHOD_NAME + "] is not declared public and it must be.");
			...
			if( boolean.class != requiredApplicable.getReturnType() && Boolean.class != requiredApplicable.getReturnType() )
				throw new Exception("GlobalArtifactIdentifierFactory.registerRealization() --> The method [" + REQUIRED_APPLICABLE_METHOD_NAME + "] must return type boolean and it does not.");
			
			int requiredApplicableModifiers = requiredApplicable.getModifiers();
			if(! Modifier.isStatic(requiredApplicableModifiers) )
				throw new Exception("GlobalArtifactIdentifierFactory.registerRealization() --> The method [" + REQUIRED_APPLICABLE_METHOD_NAME + "] is not declared static and it must be.");
			
			if(! Modifier.isPublic(requiredApplicableModifiers) )
				throw new Exception("GlobalArtifactIdentifierFactory.registerRealization() --> The method [" + REQUIRED_APPLICABLE_METHOD_NAME + "] is not declared public and it must be.");
			
			registeredRealizations.add(realization);
            LOGGER.info("GlobalArtifactIdentifierFactory.registerRealization() --> Successfully registed GlobalArtifactIdentifier class [{}] with factory.", realization.getSimpleName());
		}
		catch (Throwable x)
		{
            LOGGER.warn("GlobalArtifactIdentifierFactory.registerRealization() --> Unable to register [{}] as a GlobalArtifactIdentifier: {}", realization.getName(), x.getMessage());
		}		
	}
	...
	throws GlobalArtifactIdentifierFormatException
	{
        ...
        LOGGER.debug("GlobalArtifactIdentifierFactory.create() --> Trying to create GlobalArtifactIdentifier from [{}]", stringified);
	...
	throws GlobalArtifactIdentifierFormatException
	{
		...
		if(!parserMatcher.matches())
			throw new GlobalArtifactIdentifierFormatException("GlobalArtifactIdentifierFactory.createGlobalArtifactIdentifier() --> Could not parse [" + stringified + "] cannot be parsed into a generic global artifact identifier form.");
	...
	throws GlobalArtifactIdentifierFormatException
	{
		...
		for(Class<? extends GlobalArtifactIdentifier> coerceToClass : candidateRealizations)
		{
			...
			{
				String msg = 
					"GlobalArtifactIdentifierFactory.create(1) --> Unable to create an identifier with the home community Id [" + homeCommunityId + 
					"], repository Id [" + repositoryId + "], document Id [" + documentId + "]: " + x.getMessage();
				LOGGER.error(msg);
