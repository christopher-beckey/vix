...
implements GlobalArtifactURN, GlobalArtifactIdentifier
{
	...
	throws URNFormatException
	{
		...
		if(! GlobalArtifactURNImpl.namespaceIdentifier.equals(namespaceIdentifier))
			throw new URNFormatException("GlobalArtifactURNImpl(1) --> Namespace identifier [" + namespaceIdentifier + "] is not understood by [GlobalArtifactURNImpl]");
	}
	...
	throws URNFormatException
	{
		...
		if(! GlobalArtifactURNImpl.namespaceIdentifier.equals(urnComponents.getNamespaceIdentifier()))
			throw new URNFormatException("GlobalArtifactURNImpl(2) --> Namespace identifier [" + urnComponents.getNamespaceIdentifier() + "] is not understood by [GlobalArtifactURNImpl]");
	}
	...
	throws URNFormatException
	{
		...
		if( !COMMUNITY_ID_PATTERN.matcher(homeCommunityId).matches() )
			throw new URNFormatException("GlobalArtifactURNImpl(3) --> Home community ID is not in the valid format.");
		
		if( !REPOSITORY_ID_PATTERN.matcher(repositoryUniqueId).matches() )
			throw new URNFormatException("GlobalArtifactURNImpl(3) --> Repository unique ID is not in the valid format.");
		
		if( !DOCUMENT_ID_PATTERN.matcher(documentUniqueId).matches() )
			throw new URNFormatException("GlobalArtifactURNImpl(3) --> Document unique ID is not in the valid format.");
	...
	throws URNFormatException
	{
		...
		if(! nssMatcher.matches() )
			throw new URNFormatException("GlobalArtifactURNImpl.parseNamespaceSpecificString() --> The namespace specific string [" + namespaceSpecificString + "] is not in the correct format.");
	...
	throws CloneNotSupportedException
	{
		...
		{
			String msg = "GlobalArtifactURNImpl.clone() --> Unable to clone [" + toString() + "]: " + x.getMessage();
			getLogger().error(msg);
