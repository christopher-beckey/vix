...
public class CodingScheme
{
	...
	public static void add(String name, URI canonicalURI, URI ... aliases)
	{
		...
		synchronized(knownCodingSchemes)
		{
			...
			{
                Logger.getLogger(CodingScheme.class).warn("Unable to add new CodingScheme because the name '{}' is already known.", name);
				return;
			}
			if(valueOf(canonicalURI) != null)
			{
                Logger.getLogger(CodingScheme.class).warn("Unable to add new CodingScheme because the canonical URI '{}' is already known.", canonicalURI);
				return;
			}
			if(aliases != null)
				for(URI alias : aliases)
					...
					{
                        Logger.getLogger(CodingScheme.class).warn("Unable to add new CodingScheme because the alias URI '{}' is already known.", alias);
						return;
					}
