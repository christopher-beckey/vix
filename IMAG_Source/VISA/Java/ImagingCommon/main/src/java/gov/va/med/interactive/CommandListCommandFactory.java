...
{
	private final static Logger LOGGER = Logger.getLogger(CommandListCommandFactory.class);
	
	// for commands loaded by package, this is the name of the class providing the command list
	...
	throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException
	{
		...
		{
            LOGGER.info("CommandListCommandFactory.loadCommandsFromPackageNames() --> Loading commands from package {}]", commandPackageName);
			try
			{
				...
				for(String command : commandClassNames)
				{
					...
					try
					{
						...
						CommandDescription commandDescription = new CommandDescription(commandName, commandHelp, commandShortcut, parametersDescription);
						
						knownCommands.put(commandDescription, commandClass);
					} 
					catch (ClassNotFoundException cnfX)
					{
                        LOGGER.warn("CommandListCommandFactory.loadCommandsFromPackageNames() --> Command name [{}] was not found: {}", commandName, cnfX.getMessage());
					}
					catch (ClassCastException ccX)
					{
                        LOGGER.warn("CommandListCommandFactory.loadCommandsFromPackageNames() --> Unable to cast command name [{}]: {}", commandName, ccX.getMessage());
					}
				}
			}
			catch(ClassNotFoundException cnfX)
			{
                LOGGER.warn("CommandListCommandFactory.loadCommandsFromPackageNames() --> Unable to load command list from package [{}]. Command list class does not exist.", commandPackageName);
			}
			catch (InstantiationException x)
			{
                LOGGER.warn("CommandListCommandFactory.loadCommandsFromPackageNames() --> Unable to create command list class instance for command list package [{}]", commandPackageName);
			}
		}
	}
	...
	throws InstantiationException, IllegalAccessException, ClassCastException, SecurityException, NoSuchMethodException, IllegalArgumentException, InvocationTargetException
	{
		...
		if(commandDescription != null)
		{
			...
			{
				throw new IllegalArgumentException("CommandListCommandFactory.createCommand() --> Command [" + commandText + "] has associated arguments but the command definition has no paremeters defined.");
			}
