...
{
	private static final Logger LOGGER = Logger.getLogger(ConfigurationInvocationHandler.class);
	...
	public final static String ROOT_CONTEXT = Messages.getString("Configuration.ROOT_CONTEXT_NAME"); //$NON-NLS-1$
	public final static String CONFIGURATION_STORAGE_IMPLEMENTATION = Messages.getString("Configuration.STORAGE_IMPLEMENTATION");  //$NON-NLS-1$
	public final static String CONFIGURATION_STORAGE_ROOT_URL = Messages.getString("ConfigurationSTORAGE_ROOT_URL");  //$NON-NLS-1$
	...
	private final static Map<ConfigurationPropertyName, Configuration> contextProxyMap = new HashMap<ConfigurationPropertyName, Configuration>();
		...
		if(rootContext == null)
		{
			...
			catch (Throwable t)
			{
				if(t.getCause() != null && t.getCause() instanceof NamingException)
                    LOGGER.warn("ConfigurationInvocationHandler.getRootConfiguration() --> Unable to load configuration: {}", t.getMessage());
				else
                    LOGGER.error("ConfigurationInvocationHandler.getRootConfiguration() --> Exception [{}] Unable to load configuration: {}", t.getClass().getSimpleName(), t.getMessage());
			}
		}
	...
	throws NamingException
	{
		...
		{
			throw new ConfigurationException("ConfigurationInvocationHandler.createStorageImplementation() --> The storage class [" + className + "] was not found. Unable to load or store configuration.");
		}
		catch (InstantiationException x)
		{
			throw new ConfigurationException("ConfigurationInvocationHandler.createStorageImplementation() --> The storage class [" + className + "] was found but did not allow instantiation with the default constructor.");
		}
		catch (IllegalAccessException x)
		{
			throw new ConfigurationException("ConfigurationInvocationHandler.createStorageImplementation() --> The storage class [" + className + "] was found but not accessible.");
		}
	}
	...
	throws NamingException
	{
		...
		if(Configuration.class.isAssignableFrom(propertyType) || ConfigurationContext.class.isAssignableFrom(propertyType))
			throw new NamingException("ConfigurationInvocationHandler.handleSetAccessor() --> Configuration sub-context [" + accessor.getPropertyName() + "] is explicitly disallowed."); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if(Configuration.class.isInstance(oldValue) || ConfigurationContext.class.isInstance(oldValue))
			throw new NamingException("ConfigurationInvocationHandler.handleSetAccessor() --> Over-writing configuration sub-context [" + accessor.getPropertyName() + "] is explicitly disallowed."); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if(CanonicalAccessor.SUBJECTTYPE.COLLECTION == accessor.getSubjectType())
		{
			if( oldValue != null  )
				if(oldValue instanceof ConfigurationList)
					...
					throw new NamingException("ConfigurationInvocationHandler.handleSetAccessor() --> Attempt to set a Collection member on a bound instance that is not a Collection");
			else
			{
		...
		else if(CanonicalAccessor.SUBJECTTYPE.MAP == accessor.getSubjectType())
		{
			if( oldValue != null  )
				if(oldValue instanceof ConfigurationMap)
					...
					throw new NamingException("ConfigurationInvocationHandler.handleSetAccessor() --> Attempt to set a Map member on a bound instance that is not a Map");
			else
			{
	...
	throws NamingException
	{
		...
		{
			throw new ConfigurationException("ConfigurationInvocationHandler.load() --> Attempting to build the URL of the persistent configuration failed for [" + urlString + "]");
		}
	...
	throws NamingException
	{
		...
		{
			throw new ConfigurationException("ConfigurationInvocationHandler.store() --> Attempting to build the URL of the persistent configuration failed for [" + urlString + "]");
		}
