...
public class ClassTranslator<SRC, DEST>
{
	...
	private static final Map<ConversionMapKey, Method> conversionMethodMap = new HashMap<ConversionMapKey, Method>();
	...
	public static <SRC extends Object, DEST extends Object> ClassTranslator<SRC, DEST> create(ConversionMapKey conversionMapKey)
	{
		try
		{
		...
		{
            LOGGER.warn("ClassTranslator.create() --> Return null. Unable to convert [{}] to [{}]", conversionMapKey.getSourceClass().getName(), conversionMapKey.getDestinationClass().getName());
	...
	public static <SRC extends Object, DEST extends Object> ClassTranslator<SRC, DEST> create(Class<SRC> sourceClass, Class<DEST> destinationClass)
	{
	...
	public ClassTranslator(Method conversionMethod)
	{
		...
		if( !Modifier.isPublic(this.conversionMethod.getModifiers()) )
			throw new IllegalArgumentException("ClassTranslator() --> The method [" + conversionMethod.getName() + "] must be a public method.");
		...
		if( Modifier.isStatic(this.conversionMethod.getModifiers()) && conversionMethod.getParameterTypes().length == 1 )
			...
			throw new IllegalArgumentException("ClassTranslator() --> The method [" + conversionMethod.getName() + "] must be either a static method taking one parameter or an instance method taking none.");
	}
	...
	public Object convert(Object value)
	{
		...
		{
            LOGGER.warn("ClassTranslator.convert() --> Return null. Encountered exception [{}] while trying to convert [{}]", ex.getClass().getSimpleName(), conversionMethod);
		}
