...
extends URNFactory
{
	...
	private static final Class<?>[] MUSE_STUDY_CREATE_PARAMETERS = new Class<?>[]{String.class, String.class, String.class, String.class};	
	...
	throws URNFormatException
	{
		String msgIdentifier = 
			"site ID[" + originatingSiteId + "], assignedId [" + assignedId 
				+ "], patientId [" + patientId + "], museServerId [" + museServerId +"]"; 
		
		if(! URNFactory.isRegisteredUrnClass(expectedClass) )
			throw new URNFormatException("Unable to create URN from " + msgIdentifier + 
				" because the expected class '" + expectedClass.getName() + " is not registered with the URNFactory");
		String[] vaUrnComponents = new String[]{originatingSiteId, assignedId, patientId, museServerId};
		
		return (T)URNFactory.create(expectedClass, URNFactory.FACTORY_METHOD_NAME, MUSE_STUDY_CREATE_PARAMETERS, vaUrnComponents);
	}
