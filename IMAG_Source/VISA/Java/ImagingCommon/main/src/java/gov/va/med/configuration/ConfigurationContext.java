...
implements Serializable, ContextEventListener
{
	...
	throws NamingException
	{
		...
		if(! (value instanceof Serializable) )
			throw new NamingException("ConfigurationContext.bind() --> [" + value.getClass().getName() + "] must implement Serializable");
		
		bind(new ConfigurationPropertyName(propertyName), value);
	}
	...
	throws NamingException
	{
		...
		else
		{
			...
			if(child == null)
				throw new NamingException("ConfigurationContext.bind(2) --> Path [" + childName + "] does not exist.");
			else if(! (child instanceof ConfigurationContext) )
				throw new NamingException("ConfigurationContext.bind(2) --> The path [" + childName + "] exists but not of ConfigurationContext.");
			else
			{
	...
	throws NamingException
	{
		...
		return lookup(expectedClass, new ConfigurationPropertyName(propertyName));
	}
	...
	throws NamingException
	{
		if(propertyName.size() > 1)
		{
			...
			if(cc == null)
				throw new NamingException("ConfigurationContext.lookup() --> Failed to look up path [" + propertyName.toString() + "], context [" + contextName.toString() + "] does not exist.");
		...
		if(child != null)
		{
			if(child instanceof ConfigurationContext)
			{
				...
				throw new NamingException(
					"ConfigurationContext.lookupDescendantContext() --> The bound name [" + childBoundName + 
					"] is not an instance of type [" + child.getClass().getName() + "]");
		}
		else
			return null;
	}
	...
	throws NamingException
	{
		return createConfigurationList(new ConfigurationPropertyName(name), lastElementIsProperty);
	}
	...
	public void contextEventNotification(ConfigurationContextEvent sourceEvent)
	{
		...
		{
			LOGGER.warn("ConfigurationContext.contextEventNotification() --> Internal error building Name when propogating context change event.");
		}
	}
	...
	void childCollectionEventNotification(ConfigurationContextEvent sourceEvent)
	{
		...
		{
			LOGGER.error("ConfigurationContext.childCollectionEventNotification() --> Internal error building Name when propogating child change event.");
		}
	...
	void passAlong(ConfigurationContextEvent sourceEvent)
	{
		...
		{
			LOGGER.error("ConfigurationContext.passAlong() --> Internal error building Name when propogating context change event.");
		}
	}
