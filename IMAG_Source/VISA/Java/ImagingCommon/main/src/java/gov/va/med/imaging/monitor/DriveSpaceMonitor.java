...
extends TimerTask
{
	...
	private static DriveSpaceMonitor createOSSpecificMonitor()
	{
		...
		osName = osName.trim().toLowerCase(Locale.ENGLISH);
	...
	extends DriveSpaceMonitor
	{
		...
		throws Exception
		{
			...
			String spaceCommand = freeSpaceCommand + drive + ":";
			
			Process p = Runtime.getRuntime().exec(StringUtil.cleanString(spaceCommand)); //$NON-NLS-1$
			InputStream inStream = new BufferedInputStream( p.getInputStream() );
			LineNumberReader reader = new LineNumberReader(new InputStreamReader(inStream));
			String line;
		...
		public long parseNumeric(String numeric)
		{
			...
			if(numeric.toLowerCase(Locale.ENGLISH).startsWith("0x")) //$NON-NLS-1$
