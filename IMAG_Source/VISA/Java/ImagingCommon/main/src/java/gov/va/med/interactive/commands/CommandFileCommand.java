...
{
	private static final Logger LOGGER = Logger.getLogger(CommandFileCommand.class);
	...
	{
		// Fortify: the overall logic here is really bad, especially the try/catch.  Need to re-write but leave as is for now
		...
		try
		{
			try
			{
				textSourceStream = new URL(sourceUrl).openStream();
			}
			catch (MalformedURLException x)
			{
                LOGGER.warn("CommandFileCommand.readCommandText() --> Unable to open command file [{}] as URL. Trying as source...", sourceUrl);
		...
		{
            LOGGER.warn("CommandFileCommand.readCommandText() --> Unable to open command file [{}]: {}", sourceUrl, ioX.getMessage());
		}
		...
		if(textSourceStream != null)
		{
			...
			{
                LOGGER.error("CommandFileCommand.readCommandText() --> Error reading command file [{}]: {}", sourceUrl, ioX.getMessage());
				ioX.printStackTrace();
			}
			finally
			{
				// Fortify change: added this block to hopefully fix it
				try{ if(textSourceStream != null) { textSourceStream.close(); } } catch(Exception exc) {/*unrecoverable so do nothing*/}
			}
		}
		else
            LOGGER.error("CommandFileCommand.readCommandText() --> Error opening command file [{}]", sourceUrl);
	}
}
