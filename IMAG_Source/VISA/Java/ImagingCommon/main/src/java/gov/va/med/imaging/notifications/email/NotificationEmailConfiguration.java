...
public class NotificationEmailConfiguration extends AbstractBaseFacadeConfiguration
{
    ...
    public List<InternetAddress> getRecipientsForNotificationType(NotificationTypes type)
    {
        ...
        if (recipients == null || recipients.size() == 0)
        {
            ...
            {
                LOGGER.warn("NotificationEmailConfiguration.getRecipientsForNotificationType() --> Error creating the InternetAddress for email notification's default recipient: {}", e.getMessage());
            }
        }
    ...
    public void setRecipientsForNotificationType(NotificationTypes notificationType, String commaSeparatedRecipientAddresses)
    {
    	...
    	String [] recipientAddresses = StringUtil.split(commaSeparatedRecipientAddresses, StringUtil.COMMA);
    	...
    	for (String recipientAddress : recipientAddresses)
    	{
            ...
            {
                LOGGER.warn("NotificationEmailConfiguration.setRecipientsForNotificationType() --> Error creating the InternetAddress for email notification's default recipient: {}", e.getMessage());
            }
    	}
    ...
    public AbstractBaseFacadeConfiguration loadDefaultConfiguration()
    {
        ...
        {
            LOGGER.warn("NotificationEmailConfiguration.loadDefaultConfiguration() --> Error creating URI - bad address for the SMTP server: {}", e.getMessage());
        }
        catch(AddressException e)
        {
            LOGGER.warn("NotificationEmailConfiguration.setRecipientsForNotificationType() --> Error creating an InternetAddress: {}", e.getMessage());
        }
    ...
    public synchronized static NotificationEmailConfiguration getConfiguration() 
    {
        ...
        {
            LOGGER.warn("NotificationEmailConfiguration.getConfiguration() --> Return null. Unable to get configuration: {}", clcX.getMessage());
        	return null;
        }
    }
