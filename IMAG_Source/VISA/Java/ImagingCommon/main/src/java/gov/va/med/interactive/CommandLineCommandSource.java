...
{
	private final static Logger LOGGER = Logger.getLogger(CommandLineCommandSource.class);
	...
	public void run()
	{
		...
		while(! isExitRequested())
		{
			...
			{
                LOGGER.warn("CommandLineCommandSource.run() --> CommandLineParseException: {}", x.getMessage());
			} 
			catch (IOException x)
			{
                LOGGER.warn("CommandLineCommandSource.run() --> IOException: {}", x.getMessage());
			}
		}
	}
	...
	throws CommandLineParseException
	{
		...
		for(int index=0; index < args.length; ++index)
		{
			...
			if(currentArg.startsWith("<") && currentArg.endsWith(">"))
			{
				if(activeCommandText == null)
					throw new CommandLineParseException("CommandLineCommandSource.parseCommandLine() --> Invalid command line syntax [" + currentArg + "] to associate a command with.");
			...
			else
			{
				if(activeCommandText != null)
				{
					try
					{
                        ...
                        LOGGER.debug("CommandLineCommandSource.parseCommandLine() --> Created command of type [{}]", command.getClass().getName());
						commands.add(command);
					}
					catch (Exception x)
					{
                        LOGGER.warn("CommandLineCommandSource.parseCommandLine() --> Unable to create a valid command from [{}]", activeCommandText.toString());
		...
		if(activeCommandText != null)
		{
			...
			{
                LOGGER.warn("CommandLineCommandSource.parseCommandLine() --> Encountered exception [{}] while creating command from {}]: {}", x.getClass().getSimpleName(), activeCommandText.toString(), x.getMessage());
				allCommandsValid = false;
			}
		}
