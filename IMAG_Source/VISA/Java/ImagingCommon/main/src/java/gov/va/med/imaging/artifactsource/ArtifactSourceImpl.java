...
{
	private final static long serialVersionUID = 6487342321516780259L;
	private final static Logger LOGGER = Logger.getLogger(ArtifactSourceImpl.class);
	...
	public ArtifactSourceImpl(ArtifactSourceMemento memento)
	{
		...
		{
			throw new IllegalArgumentException("ArtifactSourceImpl() --> The home community ID [" + memento.getHomeCommunityId() + "] is not a valid OID.");
		}
		...
		for(String url : memento.getMetadataUrls())
			...
			{
                LOGGER.warn("ArtifactSourceImpl() --> The metadata URL [{}] is not a valid URL and is being ignored: ", url);
			}
		...
		for(String url : memento.getArtifactUrls())
			...
			{
                LOGGER.error("\"ArtifactSourceImpl() --> The artifact URL [{}] is not a valid URL and is being ignored.", url);
			}
	}
	...
	public RoutingToken createRoutingToken()
	{
		...
		{
            LOGGER.error("ArtifactSourceImpl.createRoutingToken() --> Return null. Unable to create routing token from artifact source [{}]", this.toString());
			return null;
		}
	}
	...
	extends HashSet<URL>
	{
		...
		public boolean add(URL url)
		{
			...
			if(existing != null)
			{
                LOGGER.warn("URLUniqueProtocolSet.add() --> Existing URL [{}] is being replaced by [{}] because the protocols are the same.", existing.toString(), url.toString());
				remove(existing);
			}
