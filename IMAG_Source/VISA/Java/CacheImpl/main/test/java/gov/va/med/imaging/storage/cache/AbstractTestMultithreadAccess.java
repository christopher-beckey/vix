...
//	}
	...
	throws IOException, InterruptedException
	{
		...
		String testError = "ErrorTestKey";
		
		try
		{
			// create an instance and close it with an error, which should
			// delete the instance
			createAndWriteInstance(regionName, groups, testError, sampleData, true);
			Cache cache = getCache();
			assertNotNull("Cache instance is null after writing an instance, WTF?!!!", cache);
			
			Instance instance = cache.getInstance(regionName, groups, testError);
	...
	throws InterruptedException
	{
		for(Future<WorkerResult> result:results)
		{
			try
			{
                ...
                getLogger().info("{} - worker result is {}", getName(), workerResult.toString());
			} 
			catch (ExecutionException x)
			{
                getLogger().info("{} - worker exception is {}", getName(), x.getCause(), x.getCause());
			}
		}
        getLogger().info("{} completed waiting for worker threads, all results should be available", getName());
	}
