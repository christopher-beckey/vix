...
extends AbstractCacheTest
{
	...
	throws CacheException, IOException
	{
		...
		if(tempInstance != null)
		{
			...
			finally
			{
				try
				{
                    logger.debug("Wrote {} bytes with checksum {}, to instance '{}' {}.", bytesWritten, writeChannel.getChecksum() == null ? "<not caclculate>" : "" + writeChannel.getChecksum().getValue(), key, errorClose ? "(error close)" : "(normal close)");
	...
	throws CacheException, IOException
	{
		...
		finally
		{
			if(readChannel != null && readChannel.getChecksum() != null)
                logger.debug("Read {} bytes with checksum {}, to instance '{}' .", bytesRead, readChannel.getChecksum().getValue(), key);
			else
                logger.debug("Read {} bytes with checksum <not calculated>, to instance '{}' .", bytesRead, key);
	...
	throws CacheException, IOException
	{
		...
		{
            logger.debug("Read {} bytes with checksum {}, to instance '{}' {}.", bytesRead, inChannel.getChecksum() == null ? "<not caclculate>" : "" + inChannel.getChecksum().getValue(), key, errorClose ? "(error close)" : "(normal close)");
