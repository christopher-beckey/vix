...
{
	private static final Logger LOGGER = Logger.getLogger(FileSystemGroup.class);
	
	private File groupDirectory;
	private FileSystemInstanceSet childInstances;
	private FileSystemGroupSet childGroups;
	

	/**
	 * 
	 * @param File								group directory
	 * @param InstanceByteChannelFactory		instance factory channel
	 * @param int								seconds read waits for write completion
	 * @param boolean							set modification time on read
	 * @return FileSystemGroup					result
	...
	throws CacheException
	{
		...
		{
            LOGGER.debug("FileSystemGroup.getOrCreate() --> Creating persistence for group [{}]", groupDirectory.getAbsolutePath());
	...
	throws CacheException
	{
        LOGGER.info("Group '{}' is removing itself...", this.getName());
		...
		while( !groupDirectory.delete() && System.currentTimeMillis() < deleteTimeout )
			...
			catch (InterruptedException e) {LOGGER.error("FileSystemGroup.delete() --> Interrupted while waiting for file deletion."); break;}
		if(groupDirectory.exists())
			throw new PersistenceIOException("FileSystemGroup.delete() --> Failed to delete group directory [" + groupDirectory.getPath() + "]");

        LOGGER.info("FileSystemGroup.delete() --> Group [{}] is removed.", this.getName());
	}
