...
implements Group, Comparable<PersistentGroup>
{
	...
	throws CacheException
	{
		...
		for(Iterator<Group> iter=getPersistentGroupSet().hardReferenceIterator(); iter.hasNext(); )
		{
			...
			{
                log.info("Concurrent modification exception occurred while evicting{}.  Some groups may not have been evicted but will be on subsequent sweeps.  Don't worry about it.", childGroup.getName());
			}
		}
