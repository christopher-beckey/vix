...
implements InstanceByteChannelFactory<File>, DynamicMBean, CacheLifecycleListener
{
	...
	private  final static Logger LOGGER = Logger.getLogger(FileSystemByteChannelFactory.class);
	...
	throws PersistenceIOException, CacheException
	{
		try
		{
			...
			{
                LOGGER.warn("FileSystemByteChannelFactory.getInstanceReadableByteChannel() --> Unable to create checksum instance of class [{}]", getChecksumClassname());
				checksum = null;
			}
		...
		{
            LOGGER.error("FileSystemByteChannelFactory.getInstanceReadableByteChannel() --> Encountered IO exception: {}", ioX.getMessage());
			throw new PersistenceIOException("FileSystemByteChannelFactory.getInstanceReadableByteChannel() --> Encountered IO exception", ioX);
		}
	}
	...
	throws PersistenceIOException, CacheException
	{
		try
		{
			...
			{
                LOGGER.warn("FileSystemByteChannelFactory.getInstanceWritableByteChannel() --> Unable to create checksum instance of class [{}]", getChecksumClassname());
				checksum = null;
			}
		...
		{
            LOGGER.error("FileSystemByteChannelFactory.getInstanceWritableByteChannel() --> Encountered IO exception: {}", ioX.getMessage());
			throw new PersistenceIOException("FileSystemByteChannelFactory.getInstanceWritableByteChannel() --> Encountered IO exception", ioX);
		}
	}
