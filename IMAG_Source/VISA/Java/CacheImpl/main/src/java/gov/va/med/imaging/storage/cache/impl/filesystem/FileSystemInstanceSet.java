...
extends PersistentInstanceSet
{
	...
	throws CacheException
	{
        getLogger().info("ENTERING internalSynchronizeChildren(), {} child references.", this.size());
		// prune unused references
		pruneNullSoftReferences();
        getLogger().info("WITHIN internalSynchronizeChildren(), {} child references.", this.size());

		// get a list of all the child files
		for( File childFile : getChildFiles() )
		{
			// Fortify change: added codes to normalize 'instanceName' for line 102
			String instanceName = FilenameUtils.normalize(childFile.getName());		// the file name and the Instance name are the same
        ...
        getLogger().info("EXITING internalSynchronizeChildren(), {} child references.", this.size());
	}
