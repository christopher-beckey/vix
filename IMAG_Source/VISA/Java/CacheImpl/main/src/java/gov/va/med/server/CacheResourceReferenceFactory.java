...
implements ObjectFactory 
{
	...
	throws Exception 
	{
		...
		{
            LOGGER.warn("CacheResourceReferenceFactory.getObjectInstance() --> Unable to validate returned value is expected type [{}]. Continuing...", referenceClassName);
		}
		...
		{
            LOGGER.info("CacheResourceReferenceFactory.getObjectInstance() --> Returning reference [{}] to cache manager.", getClass().getSimpleName());
			
			if(referenceClass != null && !referenceClass.isInstance(cacheManager) )
			{
                LOGGER.error("CacheResourceReferenceFactory.getObjectInstance() --> Returned value [{}] is NOT expected type. Return null.", referenceClassName);
		...
		{
            LOGGER.info("CacheResourceReferenceFactory.getObjectInstance() --> Returning reference [{}] to cache instance.", getClass().getSimpleName());
