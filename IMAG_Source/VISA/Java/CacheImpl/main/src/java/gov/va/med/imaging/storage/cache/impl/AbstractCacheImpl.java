...
implements Cache
{
	private final static Logger LOGGER = Logger.getLogger(AbstractCacheImpl.class);
	
	private String name;			// the name of the cache, mostly used for management naming
	...
	throws CacheInitializationException
	{
		...
		if(memento == null)
		{
			LOGGER.info("AbstractCacheImpl(CacheMemento) --> Given 'memento' is null. Cache can not be initialized.");
			throw new CacheInitializationException("AbstractCacheImpl() --> Given 'memento' is null. Cache can NOT be initialized.");
		}
		
		this.name = memento.getName();
        LOGGER.info("AbstractCacheImpl(CacheMemento) --> Constructing cache from memento [{}], setting cache URI to [{}]", this.name, memento.getLocationUri());
		
		try
		{
			setLocationUri( new URI(memento.getLocationUri()) );
		} 
		catch (URISyntaxException x)
		{
			String message = "AbstractCacheImpl(CacheMemento) --> URI [" + memento.getLocationUri() + "] is in an invalid format. Cache will NOT be initialized.";
			LOGGER.error(message);
			throw new CacheInitializationException(message, x);
		}
		...
		{
			String message = "AbstractCacheImpl(CacheMemento) --> Unable to initialize the eviction timer";
			LOGGER.error(message);
			throw new CacheInitializationException(message, x);
		} 
		catch (InvalidSweepSpecification x)
		{
			String message = "AbstractCacheImpl(CacheMemento) --> Unable to initialize the eviction timer, invalid sweep specification";
			LOGGER.error(message);
			throw new CacheInitializationException(message, x);
		}
	}
	...
	throws CacheInitializationException
	{
		...
		{
			String message = "AbstractCacheImpl() --> Unable to initialize the eviction timer";
			LOGGER.error(message);
			throw new CacheInitializationException(message, x);
		} 
		catch (InvalidSweepSpecification x)
		{
			String message = "AbstractCacheImpl() --> Unable to initialize the eviction timer, invalid sweep specification";
			LOGGER.error(message);
			throw new CacheInitializationException(message, x);
		}
		...
		{
            LOGGER.warn("AbstractCacheImpl() --> Encountered cache state exception: {}", x.getMessage());	// won't happen unless the cache code defaults to initialized
		}
	}
	...
	{
		return this.locationUriScheme;
	}
	
	@Override
	public String getLocationPath()
	{
		return this.locationUriPath;
	}
	...
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.storage.cache.Cache#isStudyLocationExist()
	 */
	@Override
	public Boolean isImageFilesCached(String studyid, String imageid, String siteid, String PatientIdentifier, String regionName) 
	{
		return true;
	}
	//================================================================================================
	...
	throws CacheException
	{
		if(isInitialized())
		{
			LOGGER.error("AbstractCacheImpl.addRegion() --> Illegal attempt to add a region on an initialized FileSystemCache instance");
			throw new CacheStateException("AbstractCacheImpl.addRegion() --> Illegal attempt to add a region on an initialized FileSystemCache instance");
		}
		...
		{
            LOGGER.error("AbstractCacheImpl.addRegion() --> Attempt to add duplicate region name [{}] is denied.", region.getName());
			throw new InvalidRegionNameException("AbstractCacheImpl.addRegion() --> Attempt to add duplicate region name [" + region.getName() + "] is denied.");
		}
	...
	throws CacheStateException
	{
		if(isInitialized())
		{
			LOGGER.error("AbstractCacheImpl.clearRegions() --> Illegal attempt to clear regions on an initialized FileSystemCache instance");
			throw new CacheStateException("AbstractCacheImpl.clearRegions() --> IIllegal attempt to clear regions on an initialized FileSystemCache instance");
		}		
		getRegions().clear();
	...
	throws CacheException
	{
		...
		if(region == null)
		{
			String msg = "AbstractCacheImpl.setRegionEvictionStrategy() --> No Region matched region name [" + regionName + "]";
			LOGGER.error(msg);
			throw new InvalidRegionNameException(msg);
		}		
	...
	throws CacheException
	{
		...
		if (region == null)
		{
			...
			String msg = "AbstractCacheImpl.getOrCreateInstance() --> No Region matched region name [" + regionName + "]";
			LOGGER.error(msg);
			throw new RegionDoesNotExistException(msg);
		}
		try
		{
		...
		{
			incrementGetOrCreateOperationError();
            LOGGER.error("AbstractCacheImpl.getOrCreateInstance() --> Encountered cache exception: {}", e.getMessage());
			throw e;
		}
	}
	...
	throws CacheException
	{
		...
		if (region == null)
		{
			...
			String msg = "AbstractCacheImpl.getInstance() --> No Region matched region name [" + regionName + "]";
			LOGGER.error(msg);
			throw new RegionDoesNotExistException(msg);
		}
		...
		{
			incrementGetOperationError();
            LOGGER.error("AbstractCacheImpl.getInstance() --> Encountered cache exception: {}", e.getMessage());
			throw e;
		}
	}
	...
	throws CacheException
	{
		...
		if (region == null)
		{
			...
			String msg = "AbstractCacheImpl.deleteInstance() --> No Region matched region name [" + regionName + "]";
			LOGGER.error(msg);
			throw new RegionDoesNotExistException(msg);
		}
		...
		{
			incrementDeleteOperationError();
            LOGGER.error("AbstractCacheImpl.deleteInstance() --> Encountered cache exception: {}", e.getMessage());
			throw e;
		}
	}
	...
	throws CacheException 
	{
		...
		{
			incrementGetOrCreateOperationError();
			String msg = "AbstractCacheImpl.getOrCreateGroup() --> --> No Region matched region name [" + regionName + "]";
			LOGGER.error(msg);
			throw new RegionDoesNotExistException(msg);
		}
		try
		...
		{
			incrementGetOrCreateOperationError();
            LOGGER.error("AbstractCacheImpl.getOrCreateGroup() --> Encountered cache exception: {}", e.getMessage());
			throw e;
		}
	}
	...
	throws CacheException 
	{
		...
		{
			incrementGetOperationError();
			String msg = "AbstractCacheImpl.getGroup() --> No Region matched region name [" + regionName + "]";
			LOGGER.error(msg);
			throw new RegionDoesNotExistException(msg);
		}
		...
		try
		{
			...
			{
				incrementGetOperationInstanceNotFound();
				LOGGER.debug("AbstractCacheImpl.getGroup() --> Group not found in Region.");
			}
			else
			{
				incrementGetOperationSuccessful();
                LOGGER.debug("AbstractCacheImpl.getGroup() --> Got Group [ {}] in Region.", group.toString()); // Potential NPE
			}
	...
	throws CacheException
	{
		...
		{
			incrementDeleteOperationError();
			String msg = "AbstractCacheImpl.deleteGroup() --> No Region matched region name [" + regionName + "]";
			LOGGER.error(msg);
			throw new RegionDoesNotExistException(msg);
		}
		...
		{
			incrementDeleteOperationError();
            LOGGER.error("AbstractCacheImpl.deleteGroup() --> Encountered cache exception: {}", e.getMessage());
			throw e;
		}
	}
	...
	public void cacheLifecycleEvent(CacheLifecycleEvent event)
	{
		if(!isInitialized())
		{
			LOGGER.warn("AbstractCacheImpl.cacheLifecycleEvent() --> START message received when cache is not initialized. \nCache must be manually started after initialization is complete.");
			return;
		}
	...
	private void notifyByteChannelFactoryCacheLifecycleEvent(CacheLifecycleEvent event)
	{
		...
		{
            LOGGER.warn("AbstractCacheImpl.notifyByteChannelFactoryCacheLifecycleEvent() --> Unable to start ByteChannelFactory for cache [{}]", getName());
		}
	}
	...
	private void notifyRegionsCacheLifecycleEvent(CacheLifecycleEvent event)
	{
		for(Region region: getRegions())
		{
			...
			{
                LOGGER.warn("AbstractCacheImpl.notifyRegionsCacheLifecycleEvent() --> Unable to start Region for cache [{}]", getName());
			}
		}
	}
	...
	throws CacheStateException
	{
		if(isInitialized())
		{
			String msg =  "AbstractCacheImpl.setInstanceByteChannelFactory() --> Illegal attempt to change the InstanceByteChannelFactory after cache initialization."; 
			LOGGER.warn(msg);
			throw new CacheStateException(msg);
		}
	...
	throws CacheStateException
	{
		if(isInitialized())
		{
			String msg =  "AbstractCacheImpl.addEvictionStrategy() --> Illegal attempt to add an eviction strategy on an initialized FileSystemCache instance."; 
			LOGGER.warn(msg);
			throw new CacheStateException(msg);
		}
	...
	throws CacheStateException
	{
		if(isInitialized())
		{
			String msg =  "AbstractCacheImpl.clearEvictionStrategies() --> Illegal attempt to clear eviction strategies on an initialized FileSystemCache instance."; 
			LOGGER.warn(msg);
			throw new CacheStateException(msg);
		}
	...
	public synchronized MBeanInfo getMBeanInfo() 
	{
		if(mBeanInfo == null)
			...
			{
                LOGGER.warn("AbstractCacheImpl.getMBeanInfo() --> Error creating MBeanInfo, management and monitoring of cache will not be available: {}", x.getMessage());
			}
	...
	throws AttributeNotFoundException, MBeanException, ReflectionException
	{
		if(attribute == null)
		{
			LOGGER.error("AbstractCacheImpl.getAttribute() --> Given attribute name is null.");
			throw new AttributeNotFoundException("AbstractCacheImpl.getAttribute() --> Given attribute name is null.");
		}
	...
	throws AttributeNotFoundException, InvalidAttributeValueException, MBeanException, ReflectionException
	{
		if(attribute == null)
		{
			LOGGER.error("AbstractCacheImpl.setAttribute() --> Given attribute name is null.");
			throw new AttributeNotFoundException("AbstractCacheImpl.getAttribute() --> Given attribute name is null.");
		}
