...
implements Region, PersistentRegionMBean
{
		...
		try
		{
			...
			for(int retries=Region.SIZE_CALCULATION_RETRIES; retries > 0; retries--)
			{
				try
				{
                    ...
                    log.info("Successfully got Region '{}' child groups for used space calculation.", getName());
					break;
				}
				catch (ConcurrentModificationException cmX)
				{
                    log.warn("ConcurrentModificationException while getting Region '{}' child groups, retrying ....", getName());
				}
			}
	...
	public void start()
	{
        log.info("{} instance '{}' starting.", this.getClass().getName(), this.getName());
	}

	public void stop()
	{
        log.info("{} instance '{}' stopping.", this.getClass().getName(), this.getName());
	}
	...
	throws CacheException
	{
		...
		if( isInitialized() )
		{
			...
			for(Iterator<Group> iter=getPersistentGroupSet().hardReferenceIterator(); iter.hasNext(); )
			{
				...
				{
                    log.warn("Concurrent modification exception occurred while evicting '{}'.  Some groups may not have been evicted but will be on subsequent sweeps.  Don't worry about it.", childGroup.getName());
				}
			}
			...
			if(deadGroups != null)
			{
				...
				{
                    log.info("Group '{} queued for eviction is being deleted.", deadGroup.getName());
					try
					{
					...
					{
                        log.info("Unable to evict entirety of group '{}', at least one instance is still open.  The cache will clean itself up when the byte channel factory closes the instance.", deadGroup.getName());
					}
				}
			}
		}
