...
import gov.va.med.imaging.federation.configuration.FederationWebAppConfiguration;
 ...
 *
 */
public abstract class AbstractFederationExamImageServlet
extends AbstractFederationServlet
{
	...
	{	
		...
		acceptableReferenceResponseTypes.addAll(
			FederationWebAppConfiguration.getFederationWebAppConfiguration().getFederationExamImageReferenceAcceptableFormatList());
				
		acceptableDiagnosticResponseTypes = new ArrayList<ImageFormat>();
		acceptableDiagnosticResponseTypes.addAll(
			FederationWebAppConfiguration.getFederationWebAppConfiguration().getFederationExamImageDiagnosticAcceptableFormatList());		
	}
	...
	throws ServletException, IOException 
	{
		...
		try
		{
            ...
            getLogger().info("AbstractFederationExamImageServlet.doGet() --> Accept list from client [{}]", contentTypeList);
			...
			streamImageIntoZipStream(imageUrn, imageQuality, acceptableResponseContent, response);
		}
		catch(HttpHeaderParseException httphpX)
		{
			String msg = "AbstractFederationExamImageServlet.doGet() --> Error parsing Federation HTTP header information: " + httphpX.getMessage();	
			getLogger().warn(msg);
		...
		{
			String msg = "AbstractFederationExamImageServlet.doGet() --> Request is not a valid Federation query compliance request: " + wqcX.getMessage();
			getLogger().warn(msg);
			transactionContext.setErrorMessage(msg);
		...
		{
			String msg = "AbstractFederationExamImageServlet.doGet() --> Routing token formatting error when sending image content: " + rtfX.getMessage();
			getLogger().warn(msg);
			transactionContext.setErrorMessage(msg);
	...
	{
		return getFederationRouter().getExamInstanceByImageUrn(imageUrn, 
				metadataCallback, outStream, requestedFormatQuality);
	}
	...
	throws MethodException, ConnectionException
	{
		return getFederationRouter().getExamTextFileByImageUrnAsChild(imageUrn, metadataNotification, outStream);
	}
