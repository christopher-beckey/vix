...
{	
	...
	public String toXml()
	{
		...
		{
			sb.append("<Property name=\"" + encodeString(key) + "\" value=\"" + vixServerHealthProperties.get(key) + "\" />");
		}
		sb.append("</VixServerHealth>");
		return sb.toString();
	}	
	...
	private void insertTransactionLogValues()
	{
		try
		{
			String vixConfigDir = System.getenv("vixtxdb");
			File transactionLogsDir = FileUtilities.getFile(vixConfigDir);
	...
	private void insertVixCacheValues()
	{
		try
		{
			...
			File vCache = FileUtilities.getFile(vixCacheDir);
			...
			long tomcatCacheDirSize = FileUtils.sizeOfDirectory(FileUtilities.getFile(vixCacheDir));
	...
	private void insertTomcatLogsValues()
	{
		try
		{
			...
			if((tomcatDirectory != null) && (tomcatDirectory.length() > 0))
			{
				...
				File tomcatDrive = FileUtilities.getFile(tomcatDirectory);
				...
				// Fortify change: make sure the file separator is OK and normalized below
				String tomcatLogsDir = StringUtil.toSystemFileSeparator(tomcatDirectory + "\\logs");
				
				addProperty(VixServerHealthProperties.VIX_SERVER_HEALTH_VIX_TOMCAT_LOGS_DIR, tomcatLogsDir);
				
				long tomcatLogsDirSize = FileUtils.sizeOfDirectory(new File(FilenameUtils.normalize(tomcatLogsDir)));
	...
	private void insertEnvironmentVariableValues()
	{
		...
		String hostname = NetUtilities.getUnsafeLocalHostName("UNKNOWN");
	...
	private void getJmxAttributes(MBeanServer server, JMXHealthConfigurationEntry entry)
	{
		...
		{		
			Set<ObjectInstance> instances = server.queryMBeans(objectName, objectName);
            logger.debug("Found '{}' MBean thread instances.", instances.size());
			for(ObjectInstance instance : instances)
			{
