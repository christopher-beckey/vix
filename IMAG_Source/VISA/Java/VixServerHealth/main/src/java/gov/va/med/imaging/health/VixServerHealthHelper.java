...
public class VixServerHealthHelper 
{
			...
			VixServerHealthSource [] vixServerHealthSources)
	{
		...
		countdownLatch = null;
        logger.info("Completed getting health for '{}' sites, got '{}' results. '{}' did not complete in time", sites.size(), result.size(), remaining);
		return result;
	}
	...
	{
        logger.info("Getting VIX Site health from site '{}', forceRefresh={}", site.getSiteNumber(), forceRefresh);
		...
		if(!forceRefresh)
		{
			synchronized(vixSiteServerHealthCache)
			{
				...
				if(health != null)
				{
                    logger.debug("Found health for site '{}' in cache.", site.getSiteNumber());
					if(health.expired())
					{
                        logger.debug("Health for site '{}' from cache expired, will request new version.", site.getSiteNumber());
	...
	{		
        ...
        logger.info("Requesting VIX Server health from site '{}' using URL '{}'.", site.getSiteNumber(), url);
		...
		{
            logger.info("Got HTTP resonse code '{}', parsing result.", responseCode);
			try
			{
