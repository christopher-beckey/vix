...
public class SmbCredentials 
{
	...
	{
		String domainUser = storageCredentials.getUsername();
		domainUser = domainUser.replace('\\', '/');
		String[] storageCredUser = domainUser.split("/");

		String domain = ""; 
		String username = ""; 

		if (storageCredUser.length > 3) // smb://server:port/username
		{
			domain = storageCredUser[2]; 
			username = storageCredUser[3]; 
		}
		else
		{
			username = storageCredentials.getUsername();
			username = username.replace('\\', '/');
			
			String usernameParts[] = StringUtil.split(username, StringUtil.SLASH);
