...
public class SmbConnectionInformationManager
{
	...
	public int getSuccessfulPort(String server, int defaultPort)
	{
		...
		if(item == null)
		{
            logger.debug("Did not find successful port for server '{}', returning default port '{}'.", server, defaultPort);
			return defaultPort;
		}
        logger.debug("Found successful port for server '{}', returning port '{}'.", server, item.getSuccessfulPort());
	...
	public void updateSuccessfulPort(String server, int port)
	{
		...
		if(server == null)
			return;
        logger.debug("Updating the known succsesful port for server '{}' to '{}'.", server, port);
