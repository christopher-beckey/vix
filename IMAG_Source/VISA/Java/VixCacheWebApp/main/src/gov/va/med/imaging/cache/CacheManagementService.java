...
import gov.va.med.imaging.cache.AbstractGroupParent;
import gov.va.med.imaging.cache.AbstractNamedVO;
...
import gov.va.med.imaging.cache.CacheSemantics;
...
{
	private static final Logger logger = Logger.getLogger(CacheManagementService.class);
	...
	{
        logger.debug("ENTERING getCacheManager(), CacheManager instance is {}", cacheManager == null ? "<null>" : "" + cacheManager.toString());
	...
	throws IllegalArgumentException 
	{
		...
		try 
		{
            ...
            logger.debug("EXITING getCacheRegionMetadata({}) SUCCESS -> {}", regionPath.toString(), result.toString());
			return result;
		} 
		catch (Exception x) 
		{
			x.printStackTrace();
            logger.debug("EXITING getCacheRegionMetadata({}) EXCEPTION {}", regionPath.toString(), x.toString());
			return null;
		}
	}
	...
	throws CacheException
	{
			...
			groupIterator.hasNext(); )
		{
			...
			if(childGroup == null)
			{
                logger.warn("Ignoring NULL in group '{}' children.", group.getName());
				continue;
			}
            logger.debug("{}, depth is {}", childGroup.getName(), childrenDepth == null ? "null" : "" + childrenDepth.toString());
