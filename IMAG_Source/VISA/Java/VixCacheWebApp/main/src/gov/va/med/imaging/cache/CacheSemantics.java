...
public class CacheSemantics 
{
	...
	private static Logger logger = Logger.getLogger(CacheSemantics.class);
	
	static
	{
		rbSemantics = ResourceBundle.getBundle("gov.va.med.imaging.cache.CacheSemantics");
		...
		for(Enumeration<String> keyEnum = rbSemantics.getKeys(); keyEnum.hasMoreElements(); )
		{
			...
			{
                logger.error("The CacheSemantics properties file contains an invalid key '{}'.  Keys MUST be in the form <cache name>.<region name>.groups -or- <cache name>.<region name>.instance .", key);
				continue;
			}
