...
implements DataSourceProvider
{
	...
	public synchronized void setProviderClassLoader(ClassLoader classLoader)
	{
		if (providerServiceLoader != null)
			throw new IllegalStateException("Provider.setProviderClassLoader() --> The provider class loader must be set before any calls to acquire service implementations.");
	...
	throws ConnectionException
	{
		...
		if(localDataSourceFactory == null)
			throw new ConnectionException("Provider.createLocalDataSource(1) --> Failed to create a local data source factory producing instances of [" + spiType.getSimpleName() + "]");
	...
	throws ConnectionException
	{
		LocalServiceProviderFactory<S> localDataSourceFactory = 
			this.providerServiceFactories.getOrCreateLocalServiceProviderFactory(spiType);
		if(localDataSourceFactory == null)
			throw new ConnectionException("Provider.createLocalDataSource(2) --> Failed to create a data source factory producing instances of [" + spiType.getSimpleName() + "]");
	...
	throws ConnectionException
	{
		LocalServiceProviderFactory<S> localDataSourceFactory = 
			this.providerServiceFactories.getOrCreateLocalServiceProviderFactory(spiType);
		if(localDataSourceFactory == null)
			throw new ConnectionException("Provider.createLocalDataSources() --> Failed to create a data source factory producing instances of '" + spiType.getSimpleName() + "'.");
	...
	throws ConnectionException
	{
		...
		if(versionableDataSourceFactory == null)
			throw new ConnectionException("Provider.createVersionableDataSource() --> Failed to create a data source factory producing instances of [" + spiType.getSimpleName() + "]");
	...
	@Override
	public DicomServicesDataSourceSpi createDicomServicesDataSource(
			ResolvedArtifactSource resolvedArtifactSource,
			String protocol, 
			DataSourceExceptionHandler... dataSourceExceptionHandlers) 
					throws ConnectionException {
		return createVersionableDataSource(DicomServicesDataSourceSpi.class, resolvedArtifactSource, protocol, dataSourceExceptionHandlers);
	}

	@Override
	public ViewerServicesDataSourceSpi createViewerServicesDataSource(
			ResolvedArtifactSource resolvedArtifactSource,
			String protocol, 
			DataSourceExceptionHandler... dataSourceExceptionHandlers) 
					throws ConnectionException {
		return createVersionableDataSource(ViewerServicesDataSourceSpi.class, resolvedArtifactSource, protocol, dataSourceExceptionHandlers);
	}
	...
	public ProviderService findProviderServiceWithLatestProtocolVersion(
		...
		URL url)
	{
        logger.info("Provider.findProviderServiceWithLatestProtocolVersion() --> Searching installed providers for service [{}, {}]", spiType.getSimpleName(), url.toExternalForm());
		...
		String protocol)
	{
        ...
        logger.info("Provider.findProviderServices() --> Searching installed providers for services [{}, {}]", spiType.getSimpleName(), protocol);
	...
	public SortedSet<ProviderService> findProviderLocalServices(Class<? extends DataSourceSpi> spiType)
	{
        ...
        logger.info("Provider.findProviderLocalServices() --> Searching local providers for services [{}]", spiType.getSimpleName());
		...
		String requestedProtocol)
	{
		...
		if (providerServices == null) // if no services are defined by the
            ...
            logger.warn("Provider.getServices() --> Provider [{}], version [{}] defines no services.", this.getName(), this.getVersion());
		else
	...
	public SortedSet<ProviderService> getLocalServices(Class<? extends DataSourceSpi> spiType)
	{
		...
		if (providerServices == null || providerServices.size() == 0)
            logger.warn("Provider.getServices() --> Provider {}], vesion [{}] defines no local services.", this.getName(), this.getVersion());
		else
			for (ProviderService service : providerServices)
