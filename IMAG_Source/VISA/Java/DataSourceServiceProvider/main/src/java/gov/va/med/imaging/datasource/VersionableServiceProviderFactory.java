...
extends ServiceProviderFactory<T>
{
	...
	throws ConnectionException
	{
		if(resolvedArtifactSource == null || protocol == null)
			throw new IllegalArgumentException("VersionableServiceProviderFactory.createDataSource() --> Both the ResolvedArtifactSource and the protocol must be provided to create a data source.");
		...
		{
            Provider.logger.info("VersionableServiceProviderFactory.createDataSource() --> Found cached Service for product [{}] provider using protocol [{}] accessing [{}]", getProductTypeName(), protocol, resolvedArtifactSource == null ? "no ResolvedArtifactSource" : resolvedArtifactSource.toString());
			
			T dataSource = createDataSourceInstance(resolvedArtifactSource, protocol, cachedService);

            Provider.logger.info("VersionableServiceProviderFactory.createDataSource() --> Created DataSource provider accessing [{}]", resolvedArtifactSource == null ? "no ResolvedArtifactSource" : resolvedArtifactSource.toString());

			return dataSource;
		}
		else
		{
            Provider.logger.info("VersionableServiceProviderFactory.createDataSource() --> Creating Service for product [{}] provider using protocol [{}] accessing [{}]", getProductTypeName(), protocol, resolvedArtifactSource == null ? "no ResolvedArtifactSource" : resolvedArtifactSource.toString());
			
			SortedSet<ProviderService> services = getParentProvider().findProviderServices(this.getSpiType(), protocol);

            Provider.logger.info("VersionableServiceProviderFactory.createDataSource() --> Product [{}] has [{}] number of potential service implementation(s).", getProductTypeName(), services == null ? "zero" : services.size());
			
			if (services == null)
			{
                Provider.logger.info("VersionableServiceProviderFactory.createDataSource() --> Applicable service implementations of type [{}] for [{}] are NOT available.", getProductTypeName(), resolvedArtifactSource.toString());
			...
			for (ProviderService service : services)
			{
				...
				if (dataSource != null)
				{
                    ...
                    Provider.logger.warn("VersionableServiceProviderFactory.createDataSource() --> Failed to create [{}] provider accessing [{}]", getProductTypeName(), resolvedArtifactSource == null ? "no URL" : resolvedArtifactSource.toString());
			}

            Provider.logger.info("VersionableServiceProviderFactory.createDataSource() --> Applicable service implementations of type [{}] for [{}] are available but versions are incompatible.", getProductTypeName(), resolvedArtifactSource.toString());
	...
	throws ConnectionException
	{
		...
		if(implementingClass == null)
			throw new IllegalArgumentException("VersionableServiceProviderFactory.createDataSourceInstance() --> The implementing class in [" + service.toString() + "] is null and must not be.");
		
		// a message fragment used in many logging messages
		String message = 
			" VersionableServiceProviderFactory.createDataSourceInstance() --> Creating instance of [" + (implementingClass == null ? "null" : implementingClass.getName()) + "].\n" +
		...
		try
		{
			...
			try
			{
				...
				if (dataSource == null) {
					Provider.logger.error("createInstanceUsingStaticCreateMethod(1) returned null. This is a coding error and MUST be addressed.");
				} else {
					Provider.logger.debug("Successfully created instance of [{}] using createInstanceUsingStaticCreateMethod(1).", dataSource.getClass().getName());
				}
			}
			catch (NoSuchMethodException nsmX1)
			{
				Provider.logger.debug("VersionableServiceProviderFactory.createDataSourceInstance() --> Unable to create data source provider using createInstanceUsingStaticCreateMethod(1), continuing..." );
				try
				{
					dataSource = createInstanceUsingStaticCreateMethod(
						implementingClass, 
					...
					if (dataSource == null) {
						Provider.logger.error("createInstanceUsingStaticCreateMethod(2) returned null. This is a coding error and MUST be addressed.");
					} else {
						Provider.logger.debug("Successfully created instance of [{}] using createInstanceUsingStaticCreateMethod(2).", dataSource.getClass().getName());
					}
				}
				catch(NoSuchMethodException nsmX2)
				{
					Provider.logger.debug("VersionableServiceProviderFactory.createDataSourceInstance() --> Unable to create data source provider using createInstanceUsingStaticCreateMethod(2), continuing..." );
					dataSource = createInstanceUsingConstructor(
						implementingClass, 
						service, 
						REQUIRED_CONSTRUCTOR_TYPES, 
						new Object[]{ resolvedArtifactSource, protocol });
                    Provider.logger.debug("Successfully created instance of [{}] using createInstanceUsingConstructor().", dataSource.getClass().getName());
				}
			}
		}
