 ...
 */
package gov.va.med.imaging.mix.storage;
 ...
 *
 */
public class MixStorageUtility 
extends AbstractBufferedImageStorageFacade
{
	private final MixProxy imagingProxy;
	
	public MixStorageUtility(MixProxy imagingProxy)
	{
	...
	{	
		...
		if(sizedStream == null)
			throw new ImageNotFoundException("MIXClient: No input stream returned from MIX source [" + imagingProxy.getImageProxyService() + "] for image [" + imageIdentifier + "].");
		...
		if(imageInputStream.isEmptyStream())
			throw new ImageNotFoundException("MIXClient Detected 0 length input stream response from MIX resorce for image '" + imageIdentifier + "'.");
