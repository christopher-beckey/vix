...
extends AbstractFederationImageDataSourceService
{
	...
	public boolean isVersionCompatible() 
	{
		...
		try
		{
            ...
            getLogger().debug("Found FederationProxyServices, looking for '{}' service type at site [{}].", serviceType, getSite().getSiteNumber());
			getFederationProxyServices().getProxyService(serviceType);
            getLogger().debug("Found service type '{}' at site [{}], returning true for version compatible.", serviceType, getSite().getSiteNumber());
			return true;
		}
		catch(gov.va.med.imaging.proxy.exceptions.ProxyServiceNotFoundException psnfX)
		{
            getLogger().warn("Cannot find proxy service type '{}' at site [{}]", serviceType, getSite().getSiteNumber());
			return false;
		}
	}
	...
	{
        getLogger().info("getImageInformation({}), TransactionContext ({}).", imagingUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
	...
	throws MethodException, ConnectionException, ImageNotFoundException 
	{
        getLogger().info("getImageDevFields({}), TransactionContext ({}).", imagingUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
	...
	throws MethodException, ConnectionException, ImageNotFoundException 
	{
        getLogger().info("getImageSystemGlobalNode({}), TransactionContext ({}).", imagingUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
