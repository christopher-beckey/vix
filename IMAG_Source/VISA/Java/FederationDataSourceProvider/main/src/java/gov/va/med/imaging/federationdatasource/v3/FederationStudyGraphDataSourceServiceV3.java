 ...
 */
package gov.va.med.imaging.federationdatasource.v3;
...
import gov.va.med.imaging.federationdatasource.AbstractFederationStudyGraphDataSourceService;
...
extends AbstractFederationStudyGraphDataSourceService 
{
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationStudyGraphDataSourceServiceV3.getProxy() --> Did not receive any applicable services from IDS service for site number [" + getSite().getSiteNumber() + "]");
	...
	{
        getLogger().info("FederationStudyGraphDataSourceServiceV3.getPatientStudies() --> For patient Id [{}], StudyLoadLevel [{}], transaction identity [{}]", patientIdentifier.toString(), studyLoadLevel, TransactionContextFactory.get().getDisplayIdentity());
		
		if(patientIdentifier.getPatientIdentifierType().isLocal())
			throw new MethodException("FederationStudyGraphDataSourceServiceV3.getPatientStudies() --> Cannot use local patient identifier to retrieve remote patient information");
		...
		{
			String msg = "FederationStudyGraphDataSourceServiceV3.getPatientStudies() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
		...
		if(filter != null)
		{
			...
			{
                getLogger().info("FederationStudyGraphDataSourceServiceV3.getPatientStudies() --> Filtering study by study Id [{}]", filter.getStudyId());
			}
		}
		
		SortedSet<Study> studies = getProxy().getStudies(patientIdentifier.getValue(), filter, getSite().getSiteNumber(), studyLoadLevel);
        getLogger().info("FederationStudyGraphDataSourceServiceV3.getPatientStudies() --> Got [{}] study(ies) from site number [{}]", studies == null ? 0 : studies.size(), getSite().getSiteNumber());
