...
implements ImageDataSourceSpi 
{
	...
	throws MethodException, ConnectionException 
	{
        ...
        logger.info("getImage({}) from GlobalArtifactIdentifier TransactionContext ({}).", imageId, TransactionContextFactory.get().getDisplayIdentity());
	...
	throws MethodException, ConnectionException, ImageNotFoundException, ImageNearLineException 
	{
        ...
        logger.info("getImageTXTFile({}) from Image URN TransactionContext ({}).", imageId, TransactionContextFactory.get().getDisplayIdentity());
