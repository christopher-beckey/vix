...
{	
	...
	throws MethodException, ConnectionException
	{
        ...
        getLogger().info("getHealthSummaryTypes, Transaction [{}] initiated to '{}'.", transactionContext.getTransactionId(), routingToken.toRoutingTokenString());
        ...
        getLogger().info("getHealthSummaryTypes, Transaction [{}] returned [{}] health summaries.", transactionContext.getTransactionId(), healthSummary == null ? "null" : "not null");
		List<HealthSummaryType> result = FederationRestTranslator.translate(healthSummary);
        getLogger().info("getHealthSummaryTypes, Transaction [{}] returned response of [{}] health summaries business object.", transactionContext.getTransactionId(), result == null ? "null" : "not null");
		return result;	
	}
	...
	{
		TransactionContext transactionContext = TransactionContextFactory.get();
        getLogger().info("getHealthSummary, Transaction [{}] initiated for '{}' for patient '{}'.", transactionContext.getTransactionId(), healthSummaryUrn.toString(), patientIcn);
        ...
        getLogger().info("getHealthSummary, Transaction [{}] returned [{}] patient health summary.", transactionContext.getTransactionId(), healthSummary == null ? "null" : "not null");
		String result = RestCoreTranslator.translate(healthSummary);;
        getLogger().info("getHealthSummary, Transaction [{}] returned response of [{}] patient health summary business object.", transactionContext.getTransactionId(), result == null ? "null" : "not null");
		return result;	
	}
}
