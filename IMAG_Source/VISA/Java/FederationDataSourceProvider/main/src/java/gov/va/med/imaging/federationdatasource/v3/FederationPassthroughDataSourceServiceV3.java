...
import gov.va.med.imaging.federationdatasource.AbstractFederationPassthroughDataSourceService;
...
extends AbstractFederationPassthroughDataSourceService 
{
	...
	{
        getLogger().info("FederationPassthroughDataSourceServiceV3.executePassthroughMethod() --> For given method [{}], transaction Id [{}]", method, TransactionContextFactory.get().getTransactionId());
		try 
		{
		...
		{
			String msg = "FederationPassthroughDataSourceServiceV3.executePassthroughMethod() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationPassthroughDataSourceServiceV3.getProxy() --> Did not receive any applicable services from IDS service for site number [" + getSite().getSiteNumber() + "]");
