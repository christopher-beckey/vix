...
implements VistaRadDataSourceSpi
{
	...
	public boolean isVersionCompatible() 
	{
		...
		try
		{
            getLogger().debug("Found FederationProxyServices, looking for '{}' service type at site [{}].", ProxyServiceType.vistaRadMetadata, getSite().getSiteNumber());
			getFederationProxyServices().getProxyService(ProxyServiceType.vistaRadMetadata);
            getLogger().debug("Found service type '{}' at site [{}], returning true for version compatible.", ProxyServiceType.vistaRadMetadata, getSite().getSiteNumber());
			return true;
		}
		catch(ProxyServiceNotFoundException psnfX)
		{
            getLogger().warn("Cannot find proxy service type '{}' at site [{}]", ProxyServiceType.vistaRadMetadata, getSite().getSiteNumber());
			return false;
		}
	}
