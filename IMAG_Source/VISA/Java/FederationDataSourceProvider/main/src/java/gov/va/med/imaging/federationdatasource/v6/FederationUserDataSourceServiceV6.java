...
extends FederationUserDataSourceServiceV5
{
	...
	private FederationRestUserProxyV6 proxy;
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationUserDataSourceServiceV6.getProxy() --> Did not receive any applicable services from IDS service for site [" + getSite().getSiteNumber() + "]");
			proxy = new FederationRestUserProxyV6(proxyServices, getFederationConfiguration());
		}
		return proxy;
	}
	...
	{
        getLogger().info("FederationUserDataSourceServiceV6.verifyElectronicSignature() --> Transaction identity [{}]", TransactionContextFactory.get().getDisplayIdentity());
		try 
		{
		...
		{
			String msg = "FederationUserDataSourceServiceV6.verifyElectronicSignature() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
        ...
        getLogger().info("FederationUserDataSourceServiceV6.verifyElectronicSignature() --> Got [{}] electronic signature result for site number [{}]", result == null ? "null" : result.isSuccess(), getSite().getSiteNumber());
		return result;
	}
}
