...
extends AbstractFederationExternalPackageDataSourceService 
{
	...
	{
        getLogger().info("FederationExternalPackageDataSourceServiceV3.getStudiesFromCprsIdentifier() --> For Cprs Identifier [{}], transaction Id [{}]", cprsIdentifier, TransactionContextFactory.get().getTransactionId());
		try 
		{
		...
		{
			String msg = "FederationExternalPackageDataSourceServiceV3.getStudiesFromCprsIdentifier() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
