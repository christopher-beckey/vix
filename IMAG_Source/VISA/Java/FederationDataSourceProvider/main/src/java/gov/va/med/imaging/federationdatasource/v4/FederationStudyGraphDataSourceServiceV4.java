...
extends AbstractFederationStudyGraphDataSourceService 
{
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationStudyGraphDataSourceServiceV4.getProxy() --> Did not receive any applicable services from IDS service for site [" + getSite().getSiteNumber() + "]");
	...
	throws MethodException, ConnectionException 
	{
		...
		StudySetResult result = getProxy().getStudies(patientIdentifier.getValue(), filter, globalRoutingToken, studyLoadLevel);
		SortedSet<Study> studies = result.getArtifacts();
        getLogger().info("FederationStudyGraphDataSourceServiceV4.getPatientStudies() --> Got [{}] studies from site [{}]", studies == null ? 0 : studies.size(), getSite().getSiteNumber());
		return result;		
	}
	...
	throws SecurityException 
	{
		...
		try
		{
            getLogger().debug("FederationStudyGraphDataSourceServiceV4.isVersionCompatible() --> Found FederationProxyServices, looking for [{}] service type at sitenumber [{}]", serviceType, siteNumber);
		...
		{
            getLogger().warn("FederationStudyGraphDataSourceServiceV4.isVersionCompatible() --> Cannot find proxy service type [{}] at site number [{}]", serviceType, siteNumber);
			return false;
		}
	}
