...
extends FederationPatientDataSourceServiceV5 
{
	...
	private FederationRestPatientProxyV6 proxy;
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationPatientDataSourceServiceV6.getProxy() --> Did not receive any applicable services from IDS service for site [" + getSite().getSiteNumber() + "]");
			proxy = new FederationRestPatientProxyV6(proxyServices,	getFederationConfiguration());
		}
		return proxy;
	}
...
*/
	...
	{
        getLogger().info("FederationPatientDataSourceServiceV6.getHealthSummaryTypes() --> from site [{}], transaction Id [{}]", globalRoutingToken.toString(), TransactionContextFactory.get().getTransactionId());
		try 
		{
		...
		{
			String msg = "FederationPatientDataSourceServiceV6.getHealthSummaryTypes() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
	...
	throws MethodException, ConnectionException
	{
        getLogger().info("FederationPatientDataSourceServiceV6.getHealthSummary() --> For patient Id [{}, {}], transaction Id [{}]", patientIdentifier, healthSummaryUrn.toString(), TransactionContextFactory.get().getTransactionId());
		try 
		{
			federationConnection.connect();
		}
		...
		{
			String msg = "FederationPatientDataSourceServiceV6.getHealthSummary() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, ioX);
		}
		if(patientIdentifier.getPatientIdentifierType().isLocal())
			throw new MethodException("FederationPatientDataSourceServiceV6.getHealthSummary() --> Cannot use local patient identifier to retrieve remote patient information");
