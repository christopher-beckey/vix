...
extends AbstractFederationPatientDataSourceService 
{
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationPatientDataSourceServiceV3.getProxy() --> Did not receive any applicable services from IDS service for site number [" + getSite().getSiteNumber() + "]");
	...
	throws MethodException, ConnectionException 
	{
        getLogger().info("FederationPatientDataSourceServiceV3.getPatientSensitivityLevel() --> For patient Id [{}], transaction Id [{}]", patientIdentifier, TransactionContextFactory.get().getTransactionId());
		...
		{
			String msg = "FederationPatientDataSourceServiceV3.getPatientSensitivityLevel() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
	...
	throws MethodException, ConnectionException 
	{
        getLogger().info("FederationPatientDataSourceServiceV3.getTreatingSites() --> For patient Id [{}], transaction Id ({}]", patientIdentifier, TransactionContextFactory.get().getTransactionId());
		if(patientIdentifier.getPatientIdentifierType().isLocal())
			throw new MethodException("FederationPatientDataSourceServiceV3.getTreatingSites() --> Cannot use local patient identifier to retrieve remote patient information");
		try 
		{
			federationConnection.connect();
		}
		...
		{
			String msg = "FederationPatientDataSourceServiceV3.getTreatingSites() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, ioX);
		}
