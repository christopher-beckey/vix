...
implements PatientArtifactDataSourceSpi
{
	...
	{
        LOGGER.info("FederationPatientArtifactDataSourceServiceV4.getPatientArtifacts() --> For patient Id [{}], StudyLoadLevel [{}], transaction identity {}]", patientIdentifier, studyLoadLevel, TransactionContextFactory.get().getDisplayIdentity());
		...
		{
			String msg = "FederationPatientArtifactDataSourceServiceV4.getPatientArtifacts() --> Failed to connect: " + ioX.getMessage();
			LOGGER.error(msg);
		...
		ArtifactResults result = getProxy().getPatientArtifacts(patientIdentifier.getValue(), studyFilter, globalRoutingToken, 
	...
	public boolean isVersionCompatible() throws SecurityException
	{
		...
		try
		{
            getLogger().debug("FederationPatientArtifactDataSourceServiceV4.isVersionCompatible() --> Found FederationProxyServices, looking for [{}] service type at site number [{}]", serviceType, siteNumber);
			getFederationProxyServices().getProxyService(serviceType);
            getLogger().debug("FederationPatientArtifactDataSourceServiceV4.isVersionCompatible() --> Found service type [{}] at site number [{}], returning true for version compatible.", serviceType, siteNumber);
			return true;
		}
		catch(gov.va.med.imaging.proxy.exceptions.ProxyServiceNotFoundException psnfX)
		{
            getLogger().warn("FederationPatientArtifactDataSourceServiceV4.isVersionCompatible() --> Cannot find proxy service type [{}] at site number [{}]", serviceType, getSite().getSiteNumber());
			return false;
		}
	}
