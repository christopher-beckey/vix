...
implements IFederationProxy 
{
	...
	private final static Logger LOGGER = Logger.getLogger(AbstractFederationProxy.class);
	...
	throws ImageNotFoundException, ConnectionException, MethodException 
	{
		...
		{
			String msg = "AbstractFederationProxy.getPatientIdentifierImage() --> Error finding proxy service for photo Id: " + psnfX.getMessage();
			LOGGER.error(msg);
        ...
        LOGGER.info("AbstractFederationProxy.getPatientIdentifierImage() --> Transaction Id [{}] initiated, using URL [{}]", transactionIdentifier, imageUrlString);
		...
		try
		{
			...
			client.getHttpConnectionManager().getParams().setConnectionTimeout(60000); // DKB - 30 sec to get a connection.  QN changed to 1 min.
            client.getHttpConnectionManager().getParams().setSoTimeout(300000); // 120 sec to wait for data. QN changed to 5 mins.
			...
			addSecurityContextToHeader(client, getMethod, true, ProxyServiceType.photo);
			...
			if(txtFileLengthHeader != null)
			{
				...
				{
                    LOGGER.warn("AbstractFederationProxy.getPatientIdentifierImage() --> Error parsing TXT file length header value [{}]: {}", imgFileLengthHeader.getValue(), nfX.getMessage());
				}
			}
			else
				txtLength = 0;
			...
			if(responseQualityHeader != null)
			{
				...
				{
                    LOGGER.warn("AbstractFederationProxy.getPatientIdentifierImage() --> Error parsing quality header value [{}]: {}", responseQualityHeader.getValue(), nfX.getMessage());
				}
			}
			
			Header responseMachineNameHeader = getMethod.getResponseHeader(TransactionContextHttpHeaders.httpHeaderMachineName);
			
			if(responseMachineNameHeader != null)
			{
				String machineName = responseMachineNameHeader.getValue();
                LOGGER.info("AbstractFederationProxy.getPatientIdentifierImage() --> Received response from machine name [{}]", machineName);
				transactionContext.setDataSourceResponseServer(machineName);
			}
		} 
		catch (HttpException e)
		{
			responseMessage = "AbstractFederationProxy.getPatientIdentifierImage() --> HTTPException on execute GET method, got response code [" + result + "]: " + e.getMessage();
			LOGGER.error(responseMessage);
			throw new ConnectionException(responseMessage, e);
		} 
		catch (IOException e)
		{
			responseMessage = "AbstractFederationProxy.getPatientIdentifierImage() --> IOException on execute GET method, got response code [" + result + "]: " + e.getMessage();
			LOGGER.error(responseMessage);
			throw new ConnectionException(responseMessage, e);
		}
		...
		if (result >= HttpStatus.SC_CONTINUE && result < HttpStatus.SC_OK)
		{
			...
			if(message == null)
				message = "AbstractFederationProxy.getPatientIdentifierImage() --> Server response was [" + result + "]. Responses other than 200 (OK) are considered errors.";

            LOGGER.warn("Transaction [{}] response code = [{}]. The message is [{}]", transactionIdentifier, result, message);
		...
		else if(result == HttpStatus.SC_OK)
		{
			try
			{
				...
				{
					responseMessage = "AbstractFederationProxy.getPatientIdentifierImage() --> Exception on get response header/value";
					LOGGER.error(responseMessage ,e);
				}

                LOGGER.info("AbstractFederationProxy.getPatientIdentifierImage() --> Transaction [{}] returned SUCCESS. Content Length [{}]", transactionIdentifier, bytesRead);
			...
			{
                LOGGER.error("AbstractFederationProxy.getPatientIdentifierImage() --> Error: {}", e.getMessage());
			}
		}
		...
		{
			String message = "AbstractFederationProxy.getPatientIdentifierImage() --> Server response was [" + result + "]. Responses other than 200 (OK) are considered errors.";
			...
			{
				throw new ImageNotFoundException("AbstractFederationProxy.getPatientIdentifierImage() --> Cannot find photo for patient Id [" + patientIcn + "], received 409 result");
			}		
		...
		else if (result >= HttpStatus.SC_INTERNAL_SERVER_ERROR)
		{
			...
			{
                LOGGER.warn("AbstractFederationProxy.getPatientIdentifierImage() --> Transaction [{}] returned error with null input stream indicating image did not exist, throwing ImageNotFoundException.", transactionIdentifier);
				...
				throw new ImageNotFoundException("AbstractFederationProxy.getPatientIdentifierImage() --> Response returned error with null input stream indicating image did not exist, throwing ImageNotFoundException.");
			}
			
			if(result == HttpStatus.SC_BAD_GATEWAY)
			{
				String msg = "AbstractFederationProxy.getPatientIdentifierImage() --> ConnectionException transaction [" + transactionIdentifier + "], response code = [" + result + "] message is [" + message + "]";
				LOGGER.error(msg);	
				throw new ConnectionException(msg);
			}
			else
			{	String msg = "AbstractFederationProxy.getPatientIdentifierImage() --> MethodException transaction [" + transactionIdentifier + "], response code = [" + result + "] message is [" + message + "]";	
				LOGGER.error(msg);			
