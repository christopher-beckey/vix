...
extends FederationPatientDataSourceServiceV4 
{
	...
	private FederationRestPatientProxyV5 proxy;
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationPatientDataSourceServiceV5.getProxy() --> Did not receive any applicable services from IDS service for site [" + getSite().getSiteNumber() + "]");
			proxy = new FederationRestPatientProxyV5(proxyServices, getFederationConfiguration());
		}
		return proxy;
	}
	...
	{
        getLogger().info("FederationPatientDataSourceServiceV5.getPatientInformation() --> For patient Id [{}], transaction Id [{}]", patientIdentifier, TransactionContextFactory.get().getTransactionId());
		try 
		{
		...
		{
			String msg = "FederationPatientDataSourceServiceV5.getPatientInformation() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, ioX);
		}
		if(patientIdentifier.getPatientIdentifierType().isLocal())
			throw new MethodException("FederationPatientDataSourceServiceV5.getPatientInformation() --> Cannot use local patient identifier to retrieve remote patient information");
	...
	throws MethodException, ConnectionException
	{
        getLogger().info("FederationPatientDataSourceServiceV5.getPatientMeansTest() --> For patient Id [{}], transaction Id [{}]", patientIdentifier, TransactionContextFactory.get().getTransactionId());
		try 
		{
			federationConnection.connect();
		}
		...
		if(patientIdentifier.getPatientIdentifierType().isLocal())
			throw new MethodException("FederationPatientDataSourceServiceV5.getPatientMeansTest() --> Cannot use local patient identifier to retrieve remote patient information");
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("FederationPatientDataSourceServiceV5.findPatients() --> Searching for patient [{}], transaction Id [{}]", searchName, TransactionContextFactory.get().getTransactionId());
		try 
		{
			federationConnection.connect();
		}
		...
		{
			String msg = "FederationPatientDataSourceServiceV5.findPatients() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, ioX);
		}
