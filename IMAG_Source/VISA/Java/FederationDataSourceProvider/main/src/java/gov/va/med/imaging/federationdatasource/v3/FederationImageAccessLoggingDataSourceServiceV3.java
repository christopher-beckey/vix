...
extends AbstractFederationImageAccessLoggingDataSourceService 
{
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationImageAccessLoggingDataSourceServiceV3.getProxy() --> Did not receive any applicable services from IDS service for site number [" + getSite().getSiteNumber() + "]");
	...
	{
        getLogger().info("FederationImageAccessLoggingDataSourceServiceV3.LogImageAccessEvent() --> Image IEN [{}), transaction identity [{}]", logEvent.getImageIen(), TransactionContextFactory.get().getDisplayIdentity());
		if(logEvent == null)
			return;
		try 
		{
		...
		{
			String msg = "FederationImageAccessLoggingDataSourceServiceV3.LogImageAccessEvent() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, ioX);
		}		
		boolean result = getProxy().logImageAccessEvent(logEvent);
        getLogger().info("FederationImageAccessLoggingDataSourceServiceV3.LogImageAccessEvent() --> Completed. Result [{}]", result ? "ok" : "error");
	}
