...
extends AbstractFederationImageAccessLoggingDataSourceService 
{
	...
	{
        getLogger().info("FederationImageAccessLoggingDataSourceServiceV3.LogImageAccessEvent() --> For IEN [{}], transaction Id [{}]", logEvent.getImageIen(), TransactionContextFactory.get().getDisplayIdentity());
		if(logEvent == null)
			return;
		try 
		{
		...
		{
            getLogger().error("FederationImageAccessLoggingDataSourceServiceV3.LogImageAccessEvent() --> Error logging image access event: {}", ioX.getMessage());
			throw new FederationConnectionException(ioX);
		}		
		boolean result = getProxy().logImageAccessEvent(logEvent);
        getLogger().info("FederationImageAccessLoggingDataSourceServiceV3.LogImageAccessEvent() --> Completed with result [{}]", result ? "ok" : "error");
	}
