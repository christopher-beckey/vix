...
{
	private final static Logger LOGGER = Logger.getLogger(FederationRestPatientArtifactProxyV4.class);
	...
	throws InsufficientPatientSensitivityException, MethodException, ConnectionException
	{
        ...
        LOGGER.info("FederationRestPatientArtifactProxyV4.getPatientArtifacts() --> Transaction Id [{}] initiated, patient '{}' to '{}'.", transactionId, patientIcn, routingToken.toRoutingTokenString());
		...
		{
			LOGGER.error("FederationRestPatientArtifactProxyV4.getPatientArtifacts() --> Got null FederationArtifactResultsType from Federation data source");			
			return null;
		}
        getLogger().info("FederationRestPatientArtifactProxyV4.getPatientArtifacts --> Transaction Id [{}] returned [{}] FederationArtifactResultsType.", transactionContext.getTransactionId(), artifactResults == null ? "null" : artifactResults.getArtifactSize());
		...
		{
			String msg = "FederationRestPatientArtifactProxyV4.getPatientArtifacts() --> Error translating artifact results into business objects: " + tX.getMessage();
			LOGGER.error(msg);
			throw new MethodException(msg, tX);			
		}
        LOGGER.info("FederationRestPatientArtifactProxyV4.getPatientArtifacts --> Transaction [{}] returned response of [{}] ArtifactResults business object.", transactionContext.getTransactionId(), result == null ? "null" : result.getArtifactSize());
		return result;		
	}
