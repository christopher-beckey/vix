...
implements VistaRadImageDataSourceSpi
{
	...
	public boolean isVersionCompatible()
	{
		...
		try
		{
            getLogger().debug("Found FederationProxyServices, looking for '{}' service type at site [{}].", getVersionCompatibleProxyServiceType(), getSite().getSiteNumber());
			getFederationProxyServices().getProxyService(getVersionCompatibleProxyServiceType());
            getLogger().debug("Found service type '{}' at site [{}], returning true for version compatible.", getVersionCompatibleProxyServiceType(), getSite().getSiteNumber());
			return true;
		}
		catch(ProxyServiceNotFoundException psnfX)
		{
            getLogger().warn("Cannot find proxy service type '{}' at site [{}]", getVersionCompatibleProxyServiceType(), getSite().getSiteNumber());
			return false;
		}
	}
