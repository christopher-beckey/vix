...
implements DocumentSetDataSourceSpi 
{
	...
	throws MethodException, ConnectionException 
	{
		...
		StudySetResult studySet = this.getPatientStudies(globalRoutingToken, patientIdentifier,	filter, StudyLoadLevel.STUDY_AND_IMAGES);
		
		if(studySet == null)
		{
			getLogger().error("FederationDocumentSetDataSourceServiceV3.getPatientDocumentSets() --> Got null StudySetResult. Return null.");
			return null;
		}
		...
		{
			String msg = "FederationDocumentSetDataSourceServiceV3.getPatientDocumentSets() --> Failed to transform studies into document sets: " + x.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, x);
		}
	}
}
