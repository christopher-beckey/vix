...
extends AbstractFederationExternalPackageDataSourceService 
{
	...
	{
        getLogger().info("FederationExternalPackageDataSourceServiceV2.getStudyFromCprsIdentifier() --> for Cprs Identifier [{}] transaction Id [{}]", cprsIdentifier, TransactionContextFactory.get().getTransactionId());
		try 
		{
		...
		{
            getLogger().error("FederationExternalPackageDataSourceServiceV2.getStudyFromCprsIdentifier() --> Error getting study from CPRS identifier: {}", ioX.getMessage());
