...
extends AbstractFederationImageAccessLoggingDataSourceService 
{
	...
	{
        getLogger().info("FederationImageAccessLoggingDataSourceServiceV4.LogImageAccessEvent() --> For IEN [{}], transaction Id [{}]", logEvent.getImageIen(), TransactionContextFactory.get().getDisplayIdentity());
		if(logEvent != null)
		{
			try 
			{
			...
			{
                getLogger().error("FederationImageAccessLoggingDataSourceServiceV4.LogImageAccessEvent() --> Error logging image access event: {}", ioX.getMessage());
				throw new FederationConnectionException(ioX);
			}		
			boolean result = getProxy().logImageAccessEvent(logEvent);
            getLogger().info("FederationImageAccessLoggingDataSourceServiceV4.LogImageAccessEvent() --> Completed with result [{}]", result ? "ok" : "error");
		}
	}
	...
	public boolean isVersionCompatible() 
	{
		...
		{
            getLogger().warn("Cannot find proxy service type [{}] at site number [{}]", serviceType, siteNumber);
			return false;
		}
	}
