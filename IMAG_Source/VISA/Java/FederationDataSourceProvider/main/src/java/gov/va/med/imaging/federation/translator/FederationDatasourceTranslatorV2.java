...
extends AbstractFederationDatasourceTranslator
{
	...
	throws InsufficientPatientSensitivityException
	{
		...
		{
			getLogger().info("FederationDatasourceTranslatorV2.transformStudies() --> Study result contains error message, converting into InsufficientPatientSensitivityException");
        ...
        getLogger().info("FederationDatasourceTranslatorV2.transformStudies() --> Transaction Id [{}] returned {}] studies", transactionContext.getTransactionId(), studiesType.getStudy() == null ? 0 : studiesType.getStudy().length);
			...
			StudyLoadLevel studyLoadLevel)
	{
		...
		{
            getLogger().warn("FederationDatasourceTranslatorV2.transformStudy() --> Unable to create a Study from the given key elements. Can't proceed: {}", x.getMessage());
			// Fortify change: could be here but not sure if Fortify realizes the 'study' can't be null. Do the below instead. 
			//return null;
		}
		...
		if(study == null) 
		{
			getLogger().info("FederationDatasourceTranslatorV2.transformStudy() --> after creation, 'study' is still null. Can't proceed. Return null.");
			return null;
		}
	...
	private Image transformImage(gov.va.med.imaging.federation.webservices.types.v2.FederationInstanceType instanceType)
	{
		...
		{
			getLogger().error("FederationDatasourceTranslatorV2.transformImage() --> Unable to create an Image instance from the given key elements. Return null.");
			return null;
		}
	...
	public SortedSet<Patient> transformPatients(gov.va.med.imaging.federation.webservices.types.v2.PatientType [] patients)
	{
		...
		for(int i = 0; i < patients.length; i++)
		{
			...
			{
                getLogger().warn("FederationDatasourceTranslatorV2.transformPatients() --> Return null. Encountered ParseException while transforming patients: {}", pX.getMessage());
			}
		}		
		return result;
	}
