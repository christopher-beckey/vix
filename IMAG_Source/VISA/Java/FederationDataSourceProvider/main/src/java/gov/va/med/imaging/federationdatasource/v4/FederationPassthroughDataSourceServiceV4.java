...
extends AbstractFederationPassthroughDataSourceService 
{
	...
	private FederationRestPassthroughProxyV4 proxy;	
	...
	{
        getLogger().info("FederationPassthroughDataSourceServiceV4.executePassthroughMethod() --> For given method [{}], transaction Id [{}]", method, TransactionContextFactory.get().getTransactionId());
		try 
		{
		...
		{
			String msg = "FederationPassthroughDataSourceServiceV4.executePassthroughMethod() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
	...
	public boolean isVersionCompatible() 
	{
		...
		{
            getLogger().warn("FederationPassthroughDataSourceServiceV4.isVersionCompatible() --> Cannot find proxy service type [{}] at site number [{}]", serviceType, siteNumber);
			return false;
		}
	}
