...
extends AbstractFederationDatasourceTranslator
{
	...
	public Study transformStudy(gov.va.med.imaging.federation.webservices.types.FederationStudyType studyType)
	{
		...
		{
			getLogger().error("Unable to create a Study from the given key elements.  Can't proceed.", x);
		...
		if(study == null) 
		{
			LOGGER.info("FederationDatasourceTranslator.transformStudy() --> after creation, 'study' is still null. Can't proceed. Return null.");
			return null;
		}
	...
	private Image transformImage(gov.va.med.imaging.federation.webservices.types.FederationInstanceType instanceType)
	{
		...
		{
			LOGGER.warn("FederationDatasourceTranslator.transformImage() --> Unable to create an Image instance from the given key elements. Return null.");
			return null;
		}
	...
	public SortedSet<Patient> transformPatients(gov.va.med.imaging.federation.webservices.types.PatientType [] patients)
	{
		...
		for(int i = 0; i < patients.length; i++)
		{
			...
			{
				LOGGER.warn("FederationDatasourceTranslator.transformPatients() --> Encoutnered ParseException while transforming patients. Return null.");
			}
		}		
		return result;
	}
