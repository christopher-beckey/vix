...
extends AbstractFederationRestProxy 
{
	...
	throws InsufficientPatientSensitivityException, MethodException, ConnectionException
	{
        ...
        getLogger().info("FederationRestStudyProxyV4.getStudies() --> Transaction Id [{}] initiated, patient ICN [{}] to routing token [{}]", transactionId, patientIcn, routingToken.toRoutingTokenString());
		...
		if(studyResult == null)
		{
			getLogger().error("FederationRestStudyProxyV4.getStudies() --> Got null FederationStudyResultType from Federation data source");			
			return null;
		}
        getLogger().info("FederationRestStudyProxyV4.getStudies() --> Transaction Id [{}] returned [{}] FederationStudyResultType.", transactionId, studyResult == null ? "null" : "not null");
		...
		{
			String msg = "FederationRestStudyProxyV4.getStudies() --> Error translating studies into business objects: " + tX.getMessage();
			getLogger().error(msg);
			throw new MethodException(msg, tX);			
		}
        getLogger().info("FederationRestStudyProxyV4.getStudies() --> Transaction Id [{}] returned response of [{}] StudySetResult business object.", transactionId, result == null ? "null" : "not null");
		return result;		
	}
