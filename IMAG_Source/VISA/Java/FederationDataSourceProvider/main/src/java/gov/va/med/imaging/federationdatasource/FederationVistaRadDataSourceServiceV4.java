...
extends AbstractFederationVistaRadDataSourceService 
{
	...
	{
        getLogger().info("FederationVistaRadDataSourceServiceV4.getExam() --> For Study URN [{}], transaction identity [{}]", studyUrn.toString(), TransactionContextFactory.get().getDisplayIdentity());
		try 
		{
	...
	throws MethodException, ConnectionException 
	{
        getLogger().info("FederationVistaRadDataSourceServiceV4.getActiveExams() --> For list descriptor [{}], transaction identity [{}]", listDescriptor, TransactionContextFactory.get().getDisplayIdentity());
		try 
		{
			federationConnection.connect();			
		}
	...
	throws ConnectionException
	{
		if(proxy == null)
		{
			...
			if(proxyServices == null)
				throw new ConnectionException("FederationVistaRadDataSourceServiceV4.getProxy() --> Did not receive any applicable services from IDS service for site number [" + getSite().getSiteNumber() + "]");
	...
	public boolean isVersionCompatible() 
	{
		...
		{
			getLogger().debug("FederationVistaRadDataSourceServiceV4.isVersionCompatible() --> No FederationProxyServices.  Return false.");
			return false;
		}
		...
		{
            getLogger().warn("FederationVistaRadDataSourceServiceV4.isVersionCompatible() --> Return false. Cannot find proxy service type [{}] at site number [{}]", serviceType, siteNumber);
			return false;
		}
	}
	...
	throws MethodException, ConnectionException 
	{
        getLogger().info("FederationVistaRadDataSourceServiceV4.getExamsForPatient() --> For patient ICN [{}], fullyLoaded [{}], Ttransaction identity [{}]", patientICN, fullyLoadExams, TransactionContextFactory.get().getDisplayIdentity());
		try 
		{
			federationConnection.connect();			
		}
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("FederationVistaRadDataSourceServiceV4.postExamAccessEvent() --> Got [{}] result from site number [{}]", result, getSite().getSiteNumber());
		return result;	
	}
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("FederationVistaRadDataSourceServiceV4.getRelevantPriorCptCodes() --> Got [{}] relevant CPT codes from site number [{}]", result == null ? 0 : result.length, getSite().getSiteNumber());
		return result;	
	}
	...
	throws MethodException, ConnectionException 
	{
		...
		{
			String msg = "FederationVistaRadDataSourceServiceV4.getNextPatientRegistration() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, ioX);
		}
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("FederationVistaRadDataSourceServiceV4.getExamImagesForExam() --> Got [{}] exam images from site [{}]", images == null ? 0 : images.size(), getSite().getSiteNumber());
		return images;		
	}
}
