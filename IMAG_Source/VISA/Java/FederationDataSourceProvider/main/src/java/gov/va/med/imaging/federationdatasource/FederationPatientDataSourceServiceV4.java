...
extends AbstractFederationPatientDataSourceService 
{
	...
	public boolean isVersionCompatible()
	{
		...
		{
			getLogger().debug("FederationPatientDataSourceServiceV4.isVersionCompatible() --> No FederationProxyServices.  Return false.");
			return false;
		}
		...
		try
		{
            ...
            getLogger().debug("FederationPatientDataSourceServiceV4.isVersionCompatible() --> Found service type [{}] at site number [{}], returning true for version compatible.", serviceType, siteNumber);
			return true;
		}
		catch(gov.va.med.imaging.proxy.exceptions.ProxyServiceNotFoundException psnfX)
		{
            getLogger().warn("FederationPatientDataSourceServiceV4.isVersionCompatible() --> Return false. Cannot find proxy service type [{}] at site number [{}]", serviceType, siteNumber);
			return false;
		}
	}
