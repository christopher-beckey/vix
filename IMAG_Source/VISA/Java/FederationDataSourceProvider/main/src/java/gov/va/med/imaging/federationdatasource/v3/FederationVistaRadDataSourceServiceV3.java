...
{	
	...
	{
        getLogger().info("FederationVistaRadDataSourceServiceV3.getActiveExams() --> For list descriptor [{}], transaction Id [{}]", listDescriptor, TransactionContextFactory.get().getDisplayIdentity());
		try 
		{
	...
	throws MethodException, ConnectionException 
	{
        ...
        getLogger().info("FederationVistaRadDataSourceServiceV3.getExamImagesForExam() --> Ggot [{}] exam images from site number [{}]", images == null ? 0 : images.size(), getSite().getSiteNumber());
		return images;		
	}
	...
	throws MethodException, ConnectionException 
	{
        getLogger().info("FederationVistaRadDataSourceServiceV3.getRelevantPriorCptCodes() --> Forfor CPT Code [{}], transaction identity [{}]", cptCode, TransactionContextFactory.get().getDisplayIdentity());
	...
	throws MethodException, ConnectionException 
	{
		...
		{
			String msg = "FederationVistaRadDataSourceServiceV3.postExamAccessEvent() --> Failed to connect: " + ioX.getMessage();
			getLogger().error(msg);
			throw new FederationConnectionException(msg, ioX);
		}
