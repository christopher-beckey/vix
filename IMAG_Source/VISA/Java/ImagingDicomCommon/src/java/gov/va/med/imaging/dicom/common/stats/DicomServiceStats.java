...
public class DicomServiceStats implements DicomServiceStatsMBean {
	...
	public void incrementInboundAssociationAcceptCount(String aet, String ipAddr){
		try {
			InboundDicomAssociationStatistics assoc = getFromInboundAssociationList(aet.toUpperCase(Locale.ENGLISH), ipAddr);
			assoc.incrementAcceptedAssociationsCount();
		} catch (Exception X) {
            logger.warn("{} failed to update Statistics.", this.getClass().getName());
		}
	}
	...
	public void incrementInboundDimseMessageAcceptCount(String aet, String dimseService){
		try {
			InboundDicomDimseStatistics dimse = getFromDimseList(aet.toUpperCase(Locale.ENGLISH), dimseService);
	...
	public void incrementInboundObjectProcessedCount(String aet, String manufacturer, String model, String sopClassUID){
		try {
			InboundDicomObjectStatistics storage  = getFromStorageSCPList(aet.toUpperCase(Locale.ENGLISH));
