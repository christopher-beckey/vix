...
extends AbstractApplicationContextTagSupport
{
	...
	public static <T extends Tag> T findAncestorWithClazz(Tag tag, Class<T> clazz)
	{
	...
	private Class<T> getBusinessObjectType()
	{
		...
		else
		{
			...
			{
                getLogger().error("PANIC: business object to be displayed is not what is expected.  Expected type is Class<T>, type is actually '{}'.Following NullPointerExceptions may be tracable to this error.", typeMsg, ccX);
				return null;
			}
			catch (Throwable t)
			{
                getLogger().error("Error getting business object to be displayed.  Expected type is Class<T>, type is actually '{}'.A following NullPointerException may be tracable to this error.", typeMsg, t);
				return null;
			}
		}
	}
