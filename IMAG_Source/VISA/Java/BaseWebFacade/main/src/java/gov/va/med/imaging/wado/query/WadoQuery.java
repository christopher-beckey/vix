...
//	public final static String defaultImageQualityValue = "90"; // force compression, set DIAG cannonical
	...
	private static transient Logger logger = Logger.getLogger(WadoQuery.class);
	/**
	 * 
	 * @param request
	 * @return
	...
	throws WadoQueryComplianceException, HttpHeaderParseException 
	{
		...
		if(lowerCasedMap.containsKey(instanceUrnQueryKey))
		{
			...
			try
			{
				...
				//Viewer decodes encoded uri during image request which is causing request error for DoD document.
				...
				if(logger.isDebugEnabled()){
                    logger.debug("documentId before transform: {}", imageUrnAsString);}
				if (imageUrnAsString.startsWith("urn:paid:"))
				{
					imageUrnAsString = replaceColonWithSquareBracket(imageUrnAsString);
					if(logger.isDebugEnabled()){
                        logger.debug("documentId after transform: {}", imageUrnAsString);}
				}
			...
			{
				if(logger.isDebugEnabled()){
                    logger.debug("wadoquery exception: {}", e.getMessage());}
	...
	private String replaceColonWithSquareBracket(String imageUrnAsString) 
	{
		if (imageUrnAsString.endsWith("]") || imageUrnAsString.endsWith("%5d"))
		{
			return imageUrnAsString;
		}

		String[] imageUrnParse = imageUrnAsString.split(":");
		if (imageUrnParse.length > 2)
		{
			int lastIndex = imageUrnAsString.lastIndexOf(":");
			String imageUrnWithoutPatient = imageUrnAsString.substring(0, lastIndex);
			String patientId = imageUrnAsString.substring(lastIndex+1, imageUrnAsString.length());
			imageUrnAsString = imageUrnWithoutPatient + "[" + patientId + "]";
			return imageUrnAsString;
		}				

		imageUrnParse = imageUrnAsString.split("%3a");
		if (imageUrnParse.length > 2)
		{
			int lastIndex = imageUrnAsString.lastIndexOf("%3a");
			String imageUrnWithoutPatient = imageUrnAsString.substring(0, lastIndex);
			String patientId = imageUrnAsString.substring(lastIndex+3, imageUrnAsString.length());
			imageUrnAsString = imageUrnWithoutPatient + "%5b" + patientId + "%5d";
			return imageUrnAsString;
		}
