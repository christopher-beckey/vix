...
extends HttpServlet
{
	...
	throws ServletException, IOException
	{
		...
		if(loggingLevel != null)
		{
			...
			out.println("  <HEAD><TITLE>A Servlet</TITLE></HEAD>");
			out.println("  <BODY>");
			out.print("    Logging Level set to" + Encode.forHtmlContent(loggingLevel));
			out.println("  </BODY>");
			out.println("</HTML>");
		}
		else
...
//		PropertyConfigurator.configure(configuration);
		...
		ConfigurationSource source = new ConfigurationSource(propertiesStream);
...
//		);
		...
		Configurator.initialize(null, getServletContext().getRealPath("/") + "/" + level + ".lcf");
	}
