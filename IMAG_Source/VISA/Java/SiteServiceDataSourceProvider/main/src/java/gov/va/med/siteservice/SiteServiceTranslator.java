...
//				}
	...
	/**
	 * Translate Nodes to the Site model and add VISTA protocol to the protocol list(if not present) under the Site.
	 * @param siteNodes
	 * @return List<Site>
	 */
	public List<Site> translateSiteNodes(NodeList siteNodes)
	{
		...
		Site site;
		String siteNumber = "", siteName, siteAbbr, vistaServer, acceleratorServer, regionId, protocol, modality;
		...
		for (int siteIndex = 0; siteIndex < siteNodes.getLength(); siteIndex++){
			try{
				...
				for (int dataSourceIndex=0;dataSourceIndex<dataSourceNodes.getLength();dataSourceIndex++){
					...
					modality = "";
					if (dataSourceElement.hasAttribute("modality")) {
						modality = dataSourceElement.getAttribute("modality");
					}
					if (protocol.equals("VIX")){
						...
						siteConnections.put(SiteConnection.siteConnectionVix, 
								new SiteConnection(SiteConnection.siteConnectionVix, acceleratorServer, acceleratorPort, modality));
					}
					...
					else
					{
						...
						siteConnections.put(protocol, 
								new SiteConnection(protocol, server, port, modality));
					}
					
					// always add an entry for VistA
					siteConnections.put(SiteConnection.siteConnectionVista, 
							new SiteConnection(SiteConnection.siteConnectionVista, vistaServer, vistaPort, modality));
				}
	...
	/**
	 * Translate Nodes to the Site list.
	 * @param siteNodes
	 * @return List<Site>
	 */
	public List<Site> translateSiteNodesForSiteService(NodeList siteNodes)
	{
		List<Site> vaSites = new ArrayList<Site>(siteNodes.getLength());
		NodeList dataSourceNodes;
		Element siteElement, dataSourceElement;
		...
		for (int siteIndex = 0; siteIndex < siteNodes.getLength(); siteIndex++) {
			try {
				...
				vistaServer = "";
				acceleratorServer = "";
				acceleratorPort = 0; // default the VIX port to 0, if not in
										// site service, not using VIX
				dataSourceNodes = siteElement.getElementsByTagName("DataSource");
				for (int dataSourceIndex = 0; dataSourceIndex < dataSourceNodes.getLength(); dataSourceIndex++) {
					acceleratorServer = "";
					acceleratorPort = 0;
					...
					dataSourceElement = (Element) dataSourceNodes.item(dataSourceIndex);
					protocol = dataSourceElement.getAttribute("protocol");
					if (dataSourceElement.hasAttribute("source")) {
						acceleratorServer = dataSourceElement.getAttribute("source");
					}
					if (dataSourceElement.hasAttribute("port")) {
						acceleratorPort = Integer.parseInt(dataSourceElement.getAttribute("port"));
					...
					siteConnections.put(protocol,
							new SiteConnection(protocol, acceleratorServer, acceleratorPort, modality));
				}
				site = new SiteImpl(siteNumber, siteName, siteAbbr, vistaServer, vistaPort, acceleratorServer,
						acceleratorPort, regionId, sitePatientLookupable, siteUserAuthenticatable, siteConnections);
				vaSites.add(site);
                logger.info("SiteService added site ({})", site.toString());
			} catch (MalformedURLException mux) {
                logger.error("SiteService failed to add site ({}) due to {}", siteNumber, mux.getMessage());
			}
		}
		return vaSites;
	}
