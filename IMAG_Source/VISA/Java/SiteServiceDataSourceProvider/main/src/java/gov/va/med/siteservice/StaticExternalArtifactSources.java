...
implements ExternalArtifactSources, Serializable, Iterable<RoutingToken>
{
	...
	public void add(RoutingToken routingToken, ArtifactSourceImpl artifactSource)
	{
		...
		{
            getLogger().error("routingToken is '{}', indirectionRoutingToken is '{}'.", routingToken == null ? "<null>" : "" + routingToken.toString(), artifactSource == null ? "<null>" : "" + artifactSource.toString());
			return;
		}
		...
		if(oldValue != null)
            getLogger().warn("Replaced '{}' with '{}'.", oldValue, newValue);
	}
	...
	public void addIndirection(RoutingToken routingToken, RoutingTokenImpl indirectionRoutingToken)
	{
		...
		{
            getLogger().error("routingToken is '{}', indirectionRoutingToken is '{}'.", routingToken == null ? "<null>" : "" + routingToken.toString(), indirectionRoutingToken == null ? "<null>" : "" + indirectionRoutingToken.toString());
			return;
		}
		RoutingTokenImpl routingTokenImpl = RoutingTokenImpl.create(routingToken);
        getLogger().trace("Adding routing token '{}'->'{}'.", routingTokenImpl == null ? "<null>" : "" + routingTokenImpl.toString(), indirectionRoutingToken == null ? "<null>" : "" + indirectionRoutingToken.toString());
	...
	public ArtifactSourceLookupResult get(RoutingToken routingToken, boolean requireEquivalence)
	{
		...
		{
            getLogger().debug("routingToken homecommunityId: {}", routingToken.getHomeCommunityId());
            getLogger().debug("routingToken repositoryId: {}", routingToken.getRepositoryUniqueId());
		}
		...
		for(Entry<RoutingTokenImpl, ArtifactSourceLookupResult> entry : entries)
		{
			...
			if (mapKey != null)
			{
                getLogger().debug("entry mapkey homecommunityId: {}", mapKey.getHomeCommunityId());
                getLogger().debug("entry mapkey repositoryId: {}", mapKey.getRepositoryUniqueId());
			}
