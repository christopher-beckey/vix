...
implements SiteResolutionDataSourceSpi
{
	...
	public static List<SiteServiceStatistics> siteServiceStatisticsList = new ArrayList<SiteServiceStatistics>();
		...
		Site site) 
	{
		...
		for(String protocol : protocolPreferences)
		{
			...
			if(psc != null)
			{
                ...
                logger.debug("Site '{}', protocol '{}' => {}", site.getSiteNumber(), protocol, artifactUrl == null ? "null" : "" + artifactUrl.toString());
				if(artifactUrl != null)
				{
					...
					try
					{
						metadataUrls.add( new URL( StringUtil.cleanString(protocol), StringUtil.cleanString(metadataServer), metadataPort, StringUtil.cleanString(metadataFile) ) );
						artifactUrls.add( new URL( StringUtil.cleanString(protocol), StringUtil.cleanString(artifactServer), artifactPort, StringUtil.cleanString(artifactFile) ) );
		            } 
					catch (MalformedURLException e)
		            {
						...
						logger.error(StringUtil.cleanString(msg));
						// create a new MalformedURLException with our message
						urlExceptions.put(StringUtil.cleanString(protocol), new MalformedURLException(StringUtil.cleanString(msg)));
		            }
				}
			}
			else
	...
	private static synchronized void registerMBeanServer(SiteService siteService)
	{
		if(siteServiceManagerMBeanName == null)
		{
			...
			{				
	            // add statistics
				siteServiceStatisticsList.add(new SiteServiceStatistics(siteService));
				SiteServiceStatistics siteServiceMbean = siteServiceStatisticsList.get(0);
	...
	throws UnsupportedOperationException, MethodException, ConnectionException
	{
		...
		{
            logger.info("{}=> indirected to =>{}", routingToken.toRoutingTokenString(), externalResolutionResult.getRoutingToken().toRoutingTokenString());
		...
		else
		{
			...
			if(resolvedArtifactSource != null)
                logger.info("{}=> resolved as external artifact source to =>{}", routingToken.toString(), resolvedArtifactSource.toString());
			...
			{
                logger.debug("resolvesite: {}", routingToken.getRepositoryUniqueId());
				for (Site site: siteService)
				{
                    logger.debug("siteservice {} - {} - {}", site.getSiteAbbr(), site.getSiteName(), site.getSiteNumber());
				}
				resolvedArtifactSource = resolveSite(routingToken.getRepositoryUniqueId());
                logger.debug("{}=> resolved as site to =>{}", routingToken.toString(), resolvedArtifactSource == null ? "null" : "" + resolvedArtifactSource.toString());
			}
	...
	throws UnsupportedOperationException
	{
		...
		{
            logger.info("Site '{}' is disabled, cannot resolve site", site.getSiteNumber());
			return ResolvedSiteImpl.create(
					site, 
					protocolPreferenceFactory.isSiteLocal(site.getSiteNumber()), 
			    	protocolPreferenceFactory.isSiteAlien(site.getSiteNumber()),
	...
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#addRegion
	 */
	public Boolean addRegion(String regionName, String regionId)
	throws MethodException, ConnectionException{
		return siteService.addRegionToVhaSitesXml(regionName, regionId);
	}
	
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#addSite
	 */
	public Boolean addSite(String regionName, String regionId, Site site)
			throws MethodException, ConnectionException{
		return siteService.addSiteToVhaSitesXml(regionName, regionId, site);
	}
	
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#updateRegion
	 */
	public Boolean updateRegion(String oldRegionName,String oldRegionId, String newRegionName, String newRegionId)
			throws MethodException, ConnectionException{
		return siteService.updateRegionToVhaSitesXml(oldRegionName, oldRegionId, newRegionName, newRegionId);
	}
	
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#deleteRegion
	 */
	public Boolean deleteRegion(String regionId)
			throws MethodException, ConnectionException{
		return siteService.deleteRegionFromVhaSitesXml(regionId);
	}
	
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#deleteSite
	 */
	public Boolean deleteSite(String regionId, String regionName, String siteId, String siteName)
			throws MethodException, ConnectionException{
		return siteService.deleteSiteFromVhaSitesXml(regionId, regionName, siteId, siteName);
	}
	
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#updateSite
	 */
	public Boolean updateSite(String regionName, String regionID, String prevSiteName,
			String prevSiteId, Site site) throws MethodException, ConnectionException{
		return siteService.updateSiteToVhaSitesXml(regionName, regionID, prevSiteName, prevSiteId, site);
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see	 gov.va.med.imaging.datasource.SiteResolutionDataSource#addProtocol
	 */
	public Boolean addProtocol(String regionName, String regionID, String siteName, String siteId,
		...
		return siteService.addProcToVhaSitesXml(regionName, regionID, siteName, siteId, siteConnection);
	}
	
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#updateProtocol
	 */
	public Boolean updateProtocol(String regionName, String regionID, String siteName, String siteId,
			String prevProtocol, int prevPort, SiteConnection protocol) throws MethodException, ConnectionException {
		return siteService.updateProcToVhaSitesXml(regionName, regionID, siteName, siteId, prevProtocol, prevPort,
				protocol);
	}
	
	/* (non-Javadoc)
	 * @see gov.va.med.imaging.datasource.SiteResolutionDataSource#deleteProtocol
	 */
	public Boolean deleteProtocol(String regionName, String regionId, String siteName, String siteId,
			String protocolName) throws MethodException, ConnectionException {
		return siteService.deleteProcToVhaSiteXml(regionName, regionId, siteName, siteId, protocolName);
	}
	...
	throws MethodException, ConnectionException 
	{
		return siteService.getAllRegionsForSiteService();
	}
