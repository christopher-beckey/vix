...
public class VistaCommonTranslator 
{
	...
	throws VistaMethodException
	{
        ...
        logger.info("Patient Sensitive level for patient (DFN): '{}' is '{}'", patientDfn, code);
	...
	public static Date convertVistaDatetoDate(String vistaDate)
	{
		...
		{
            logger.error("Error parsing date [{}] from VistA", vistaDate, pX);
		}
		return null;
	}
...
//	}
	...
	public static List<String> convertSiteStringToSiteStringList(String siteString) 
	{
        ...
        logger.info("Converted site list '{}' to '{}'.", initialSiteList.toString(), convertedSiteList.toString());
			...
			boolean discardUnknownRootLines)
    {
        ...
        logger.info("Parsing {} lines using {} keys.", lines.length, ontologyDelimiterKeys.length);
			...
			else if(ontologyKeyIndex == -1)
			{
				...
				else
				{
					if(discardUnknownRootLines)
                        logger.warn("There is no current line to add unknown line '{}' to, discarding.", line);
					else
	...
	static class ParsedVistaLine
	{
        ...
        public boolean equals(Object obj)
        {
        	if(obj instanceof ParsedVistaLine)
        	{
        		...
        		{
                    logger.info("this.key '{}' is not equal to that.key '{}'.", this.getKey(), that.getKey());
        			return false;
        		}
        		
        		if(this.values.size() != that.values.size())
        		{
                    logger.info("this.values.size '{}' is not equal to that.values.size '{}'.", this.values.size(), that.values.size());
        			return false;
        		}
        		...
        		for(; thisValueIterator.hasNext() && thatValueIterator.hasNext(); )
        		{
        			...
        			{
                        logger.info("this({}).value '{}' is not equal to that({}).value '{}'.", this.getKey(), thisValue, that.getKey(), thatValue);
        				return false;
        			}
        		}
        		...
        		{
                    logger.info("this.properties.size '{}' is not equal to that.properties.size '{}'.", this.properties.size(), that.properties.size());
        			return false;
        		}
        		...
        		{
                    logger.info("this.children.size '{}' is not equal to that.children.size '{}'.", this.children.size(), that.children.size());
        			return false;
        		}
	...
	throws VistaMethodException
	{
		...
		int code = Integer.parseInt(lines[0].trim());
        logger.debug("Means test for patient (DFN): '{}' is '{}'", patientDfn, code);
		StringBuilder sb = new StringBuilder();
		for(int i = 1; i < lines.length; i++)
		{
			sb.append(lines[i]);
	...
	public static Date convertMDateToDate(String mDate)
	{
		// TH 7/23/2017: add handling for mdatelength()=12,mdatelength() > 14,mdatelength() < 7 
		try
		{
			// 3140210.1349
			// 3140219.144804
			...
			else if(mDate.length() == 9) // 3140221.1 --> 1 am
			{
				mDate += "0";
				dateFormat = "yyyyMMdd.HH";
			}
			else if(mDate.length() == 10) // 3140221.13
			{
				dateFormat = "yyyyMMdd.HH";
			}
			else if(mDate.length() == 11) // 3140221.131 -> 1:10 pm
			{
				mDate += "0";
				dateFormat = "yyyyMMdd.HHmm";
			}
			else if(mDate.length() == 12) // 3140221.1313 -> 1:13 
			{
				dateFormat = "yyyyMMdd.HHmm";
			}
			else if(mDate.length() == 13)// 3140221.13101 -> 1:10:10 pm
			{
				mDate += "0";
				dateFormat = "yyyyMMdd.HHmmss";
			}
			else if(mDate.length() == 14) // includes seconds
			{
				dateFormat = "yyyyMMdd.HHmmss";
			}
			...
			String first2DigitsOfYear = "19";
			if(mDate.startsWith("3"))
				first2DigitsOfYear = "20";
			mDate = first2DigitsOfYear + mDate.substring(1);
			return format.parse(mDate);
			
		}
		catch(ParseException pX)
		{
            logger.error("Error parsing mDate [{}], {}", mDate, pX.getMessage());
			return null;
		}
