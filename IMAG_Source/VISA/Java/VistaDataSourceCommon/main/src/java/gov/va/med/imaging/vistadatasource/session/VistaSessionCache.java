...
class VistaSessionCache
{
				...
				try
                {
	                ...
	                synchronized (cache)
                    {
		                for( Iterator<CacheEntry> cacheIterator = cache.iterator(); cacheIterator.hasNext(); )
		                {
		                	...
		                	{
                                Logger.getLogger(VistaConnection.class).info("VistaSession instance '{}' has been connected and unused for {} seconds, max connection time is {} seconds.  The session, and its associated connection, are being closed\n  This information WILL NOT BE REPEATED.", vistaSession.getSessionIndex(), (now - vistaSession.getLastUsedTime()) / 1000, getMaximumIdleTime() / 1000);
		                		...
		                		&& (vistaSession.getLastInternalTime() < minLastBrokerTimeoutTime))
		                	{
		                		...
		                		{
                                    logger.info("VistaSession ({}) instance has been idle for {} seconds, beyond broker keep alive time of {} seconds, refreshing connection", vistaSession.getSessionIndex(), (now - vistaSession.getLastInternalTime()) / 1000, brokerTimeoutTime);
	...
	synchronized void put(VistaSession vistaSession)
	{
        ...
        logger.info("Added VistaSession ({}) to VistaSessionCache.", vistaSession.getSessionIndex());
	}
	...
	synchronized VistaSession get(URL url, String securityContext, String version)
	{
		...
		synchronized(cache)
		{
			for(Iterator<CacheEntry> cacheEntryIter = cache.iterator(); cacheEntryIter.hasNext(); )
			{
				...
				{
                    logger.info("Removing VistaSession ({}) from VistaSessionCache for use.", indexCacheEntry.getValue().getSessionIndex());
