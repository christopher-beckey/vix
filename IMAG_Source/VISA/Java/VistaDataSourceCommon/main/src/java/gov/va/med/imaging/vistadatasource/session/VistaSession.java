...
import gov.va.med.imaging.protocol.vista.exceptions.*;
...
public class VistaSession
{
	...
	throws IOException, ConnectionException, MethodException, SecurityCredentialsExpiredException
	{
        ...
        logger.info("getOrCreate({}) with security Context ({}) and version ({}).", url.toExternalForm(), securityContextToSet, version);
		VistaSession session = VistaSessionCache.getSingleton().get(url, securityContextToSet, version);
        logger.info("getOrCreate({}) {} session in cache", url.toExternalForm(), session == null ? "did not find" : "found");
		...
		if(session == null)
		{
			...
			if(session != null)
			{
                logger.info("getOrCreate({}) connecting new VistaSession.", url.toExternalForm());
				connectAndLogin(url, session);			
			}
		}

        logger.info("getOrCreate({}) {} VistaSession.", url.toExternalForm(), session == null ? "did not instantiate" : "instantiated");
	...
	throws IOException, VistaMethodException, InvalidVistaCredentialsException
	{
		...
		try
		{
            ...
            logger.info("getWelcomeMessage({}) connecting VistaConnection.", url.toExternalForm());
			long startTime = System.currentTimeMillis();
			session.connect();
            logger.info("getWelcomeMessage({}) connected to Vista in '{}' ms, now logging in...", url.toExternalForm(), System.currentTimeMillis() - startTime);
			try
	        {
	...
	throws InvalidUserCredentialsException, CredentialsExpiredException, MethodException, IOException, ConnectionException
	{
		...
		try
		{
            ...
            logger.info("authenticateUser({}) connecting VistaConnection.", url.toExternalForm());
			long startTime = System.currentTimeMillis();
			session.connect();
            logger.info("authenticateUser({}) connected to Vista in '{}' ms, now logging in...", url.toExternalForm(), System.currentTimeMillis() - startTime);
			try
	        {
				// another sacrifice to Vista, logging in too quickly after connecting sometimes causes problems
	            Thread.sleep(100);
	...
	{
        logger.info("connectAndLogin({}) connecting VistaConnection.", url.toExternalForm());
		long startTime = System.currentTimeMillis();
		session.connect();
        logger.info("connectAndLogin({}) connected to Vista in '{}' ms, now logging in...", url.toExternalForm(), System.currentTimeMillis() - startTime);
		try
        {
			// another sacrifice to Vista, logging in too quickly after connecting sometimes causes problems
            Thread.sleep(100);
        ...
        logger.info("connectAndLogin({}) VistaConnection connected, logging in.", url.toExternalForm());
		session.login();
        logger.info("connectAndLogin({}) VistaConnection connected, logged in.", url.toExternalForm());
	}
	...
	private VistaSession(URL url, Site site, String securityContext, String version)
	{
		...
		setSessionIndex();
        logger.info("Created VistaSession ({}) with security context ({}) and version ({})", getSessionIndex(), securityContext, version);
	}
	...
	public void close()
	{
		...
		{
            logger.warn("VistaSession ({}) had a failed call, disconnecting immediately", getSessionIndex());
			disconnect(true);
		}
		else
	...
	throws ConnectionException, MethodException, SecurityCredentialsExpiredException
	{
		...
		String appName = null;
		if ((transactionContext.getSecurityTokenApplicationName() != null) && (!transactionContext.getSecurityTokenApplicationName().isEmpty()))
		{
			appName = transactionContext.getSecurityTokenApplicationName();
		}
        ...
        logger.info("Determining local/remote login, Connected site number is '{}', authenticated security realm is '{}{}', and authenticated by VistA is '{}'.", connectedSiteNumber, authenticatedSecurityRealm, (appName != null) ? "', client application is '" + appName : "", authenticatedByVista);
		
		if((connectedSiteNumber.equals(authenticatedSecurityRealm)) &&
			(authenticatedByVista) && !isBseTokenOnly())				
		{
            logger.info("Attempting local login for user '{}' on site '{}', authenticated to realm '{}'.", transactionContext.getName(), connectedSiteNumber, authenticatedSecurityRealm);
			try
            {
	            localLogin();
			...
			catch( Exception x )
            {
                ...
                logger.warn("Unable to login using local credentials for user '{}', default to remote login.", transactionContext.getName());
            }
		}
		else
		...
		if(bseLoginEnabled)
		{
			...
			{
				// Check to see if this is an STS token instead of a BSE token
				// There are two ways to check this: 1) New BSE tokens should start with "-35", 2) STS tokens should contain "Assertion"
				String securityToken = transactionContext.getBrokerSecurityToken();
				if ((! (securityToken.startsWith("-35"))) && (securityToken.contains("Assertion"))) {
					// Login via STS
					if (stsLogin(connectedSiteNumber, transactionContext, authenticatedSecurityRealm)) {
						return;
					}
				} else {
					// Login via BSE
					if (bseLogin(connectedSiteNumber, transactionContext, authenticatedSecurityRealm)) {
						return;
					}
				}
			}			
			...
			{
				logger.info("BSE login enabled but context does not have security token, cannot continue BSE login.");
			}
		}
	...
	private boolean stsLogin(String connectedSiteNumber, TransactionContext transactionContext, String authenticatedSecurityRealm) {
		logger.debug("STS credentials in context, STS token provided.");
        logger.info("Attempting STS login for user '{}'.", transactionContext.getName());
		try
		{
			if(connectedSiteNumber.equals(authenticatedSecurityRealm) && isBseTokenOnly()){
                logger.debug("Attempting STS local login for user '{}'.", transactionContext.getName());
				this.stsLogin(true);
				localLogin = true;
				logger.debug("Local STS login successful." );

			}
			else{
                logger.debug("Attempting STS remote login for user '{}'.", transactionContext.getName());
				this.stsLogin(false);
				localLogin = false;
				logger.debug("Remote STS login successful." );

			}
			this.loginTime = System.currentTimeMillis();
			vistaLoginComplete = true;
			transactionContext.setLoginMethod(VistaLoginMethod.sts.toString());
			logger.info("STS login successful." );
		...
		catch( Exception x )
		{
			if (logger.isDebugEnabled()) {
				logger.warn("Failed STS login", x);
			} else {
                logger.warn("Failed STS login: {}", x.getMessage());
			}
			// don't do anything with the error, its already been logged
			// will want to continue with CAPRI login if possible
			updateSiteBseStatus(VistaSiteBseStatus.bseUnknown);
		}

		return false;
	}

	private void stsLogin(boolean localLogin) throws ConnectionException, MethodException, SecurityCredentialsExpiredException {
		// Get the transaction context
		TransactionContext transactionContext = TransactionContextFactory.get();

		// Build the sign-on query
		VistaQuery signonQuery = new VistaQuery("XUS SIGNON SETUP");
		if (! (localLogin)) {
			...
			signonQuery.addParameter( VistaQuery.LITERAL, queryParameter );
		}

		// Call the sign-on query
		try {
			connectAndSignon(signonQuery);
		} catch (Exception e) {
			throw new MethodException("Error doing connect and sign-on for Vista", e);
		}

		// Get the Assertion from the provided security token
		String stsAssertion;
		try {
			stsAssertion = getStsAssertion(transactionContext.getBrokerSecurityToken());
		} catch (Exception e) {
			throw new MethodException("Error while retrieving Assertion from provided STS token", e);
		}

		// Build the query
		VistaQuery vistaQuery;
		try {
			vistaQuery = VistaCommonQueryFactory.createLocalStsLoginVistaQuery(stsAssertion);
		} catch (Exception e) {
			throw new MethodException("Error while building Vista Query for STS token", e);
		}

		// Call the validation
		String validationResult = null;
		try {
			validationResult = getVistaConnection().call(vistaQuery);
		} catch (Exception e) {
			throw new ConnectionException("Error while validating STS token", e);
		}

		// Validate the response
		if ((validationResult == null) || (validationResult.contains("Unable"))) {
			throw new SecurityCredentialsExpiredException("Could not validate STS token; token may be invalid or expired");
		}

		// Log the response
        logger.debug("Validated STS token. Got back: {}", validationResult);
	}

	/**
	 * Helper method to extract "Assertion" section from the given XML
	 *
	 * @param token 			whole STS token to operate from
	 * @return String			"Assertion" portion only
	...
	private String getStsAssertion(String token) throws IOException {
		try {
			...
			Node node = (Node) xpath.evaluate("(//*[local-name() = 'Assertion'])[1]", doc, XPathConstants.NODE);
			...
			String msg = "SecurityTokenFilter.getStsAssertion() --> Encountered exception [" + e.getClass().getSimpleName() + "] while getting 'Assertion'. Error: " + e.getMessage();
			logger.error(msg);
			throw new IOException(msg);
		}

		// If we've reached this point, we have no token to provide, so throw an error
		throw new IOException("SecurityTokenFilter.getStsAssertion() --> No 'Assertion' could be retrieved from the provided token");
	}

	private boolean bseLogin(String connectedSiteNumber, TransactionContext transactionContext, String authenticatedSecurityRealm) {
        logger.info("BSE credentials in context, BSE token '{}'.", transactionContext.getBrokerSecurityToken());
		if(isSiteBseTryable())
		{
            logger.info("Attempting BSE login for user '{}'.", transactionContext.getName());
			try
			{
				if(connectedSiteNumber.equals(authenticatedSecurityRealm) && isBseTokenOnly()){
                    logger.info("Attempting BSE local login for user '{}'.", transactionContext.getName());
					this.localBseLogin();
					localLogin = true;
					logger.info("Local BSE login successful." );

				}else if(isBseTokenOnly()){
                    logger.info("Attempting trunc BSE login for user '{}'.", transactionContext.getName());
					this.localBseLogin();
					localLogin = false;
					logger.info("Truncated BSE login successful." );
				}
				else{
				...
				logger.info("BSE login successful." );
				// only gets here if everything worked properly - if BSE not installed or token exception, won't get here
				updateSiteBseStatus(VistaSiteBseStatus.bseAvailable);
				getSiteBrokerTimeout();
				return true;
	...
	private boolean isBseTokenOnly() 
	{
		TransactionContext transactionContext = TransactionContextFactory.get();
		String brokerSecurityToken = transactionContext.getBrokerSecurityToken();
		if (brokerSecurityToken != null)
		{
			String[] tokenParse = StringUtils.Split(brokerSecurityToken, StringUtils.CARET);
			if (tokenParse.length == 1)
			{
				return true;
			}
		}
	...
	private void getSiteBrokerTimeout()
	{
		if(getVistaSessionConfiguration().isBrokerKeepAliveEnabled())
		{
			...
			try
			{
                ...
                logger.info("Got broker timeout value of '{}', setting value.", msg);
	...
	private void updateSiteBseStatus(VistaSiteBseStatus status)
	{
        ...
        logger.debug("Updating the site status for site '{}' to status '{}'.", siteNumber, status);
		VistaSessionBSECache.getVistaSessionBseCache(getVistaSessionConfiguration()).updateSiteStatus(siteNumber, status);
	}
	...
	private boolean isSiteBseTryable()
	{
		...
		VistaSiteBseStatus siteStatus = VistaSessionBSECache.getVistaSessionBseCache(getVistaSessionConfiguration()).getSiteStatus(siteNumber);
		String statusMsg = "";
		boolean result = true;
        ...
        logger.info("Cached BSE status for site '{}' is '{}'. {}", siteNumber, siteStatus, statusMsg);
		return result;
	}
	...
	public boolean isUserFullySignedOn()
	{
		try
		{
            logger.debug("Checking if user for VistA session ({}) is still signed on to site '{}'.", getSessionIndex(), getSite().getSiteNumber());
		...
		{
            logger.warn("InvalidVistaCredentialsException checking user session ({}) still signed on", getSessionIndex(), ivcX);
			return false;
		}
		catch(IOException ioX)
		{
            logger.warn("IOException checking user session ({}) still signed on", getSessionIndex(), ioX);
			return false;
		}
		catch(VistaMethodException vmX)
		{
            logger.warn("VistaMethodException checking user session ({}) still signed on", getSessionIndex(), vmX);
			return false;
		}
		catch(Exception ex)
		{
            logger.warn("Exception checking user session ({}) still signed on", getSessionIndex(), ex);
			return false;
		}
	}
	...
	public boolean keepAlive()
	{
		...
		try
		{
            logger.debug("Keeping VistA session ({}) alive to site '{}'.", identityHashcode, getSite().getSiteNumber());
		...
		{
            logger.warn("InvalidVistaCredentialsException keeping VistA session ({}) alive", identityHashcode, ivcX);
			return false;
		}
		catch(IOException ioX)
		{
            logger.warn("IOException keeping VistA session ({}) alive", identityHashcode, ioX);
			return false;
		}
		catch(VistaMethodException vmX)
		{
            logger.warn("VistaMethodException keeping VistA session ({}) alive", identityHashcode, vmX);
			return false;
		}
		catch(Exception ex)
		{
            logger.warn("Exception keeping VistA session ({}) alive", identityHashcode, ex);
			return false;
		}
	}
	...
	{
		if (logger.isDebugEnabled()) {
            logger.debug("RPC [{}] request: {}", vistaQuery.getRpcName(), StringUtils.displayEncodedChars(vistaQuery.toString()));
		}
	}
	
	private void displayVistaQueryResult(VistaQuery vistaQuery, String result)
	{
		// No truncation; if we've enabled debug and want the entire thing, log it out
		if (logger.isDebugEnabled()) {
            logger.debug("RPC [{}] response:\n{}", vistaQuery.getRpcName(), StringUtils.displayEncodedChars(result));
		}
	}
	...
	{
		TransactionContext transactionContext = TransactionContextFactory.get();
        logger.info("localConnect TransactionContext ({}).", transactionContext != null ? transactionContext.getDisplayIdentity() : "null transaction context");
	...
	throws ConnectionException, MethodException, InvalidVistaCredentialsException, SecurityCredentialsExpiredException
	{
		...
		try
		{
			...
			validateBSELoginResponse(vistaResult);			
			...
			{
                logger.warn("UnableToCreateContextException '{}', BSE does not allow adding context, cannot continue connection", getVistaConnection().getURL());
		...
		{
			logger.warn("Invalid credentials when doing remote BSE login", ivcX);
			throw ivcX;
		}
		catch (Exception ex)
		{
	...
	private void localBseLogin()
	throws ConnectionException, MethodException, InvalidVistaCredentialsException, SecurityCredentialsExpiredException
	{
		boolean success = false;			// will be true only when the connection and login RPCs have completed successfully
		TransactionContext transactionContext = TransactionContextFactory.get();

        logger.info("localBseLogin TransactionContext ({}).", TransactionContextFactory.get().getDisplayIdentity());

        logger.debug("TransactionContext: {}", transactionContext.toString());
		// perform a login
		VistaQuery vm = VistaCommonQueryFactory.createLocalBseSignonVistaQuery(transactionContext, getSite());
		...
		{
			logger.warn("Invalid credentials when doing local BSE login", ivcX);
			throw ivcX;
		}
		catch (Exception ex)
		{
		...
		finally 
		{
			if( ! success && getVistaConnection() != null ) 
				try 
				{
					logger.info("Unable to connect locally with BSE, disconnecting");
					// if the VistaConnection is still connected then the error occurred after the connectAndSignon and it needs to be disconnected
					// although maybe not immediately.  If there was an error during the connectAndSignon then it is disconnected immediately in there
					// and this disconnect should not be called on a VistaConnection that is not connected
					if(getVistaConnection().isConnected())
	...
	throws ConnectionException, MethodException
	{
		...
		try
		{
			...
			validateRemoteCAPRILoginResponse(vistaResult);
			// set the division for the user (if they have one assigned)
	...
	/**	
	 * If doing a remote login (CAPRI), the 6th piece should be a '1' to indicate a silent login was done properly.
	 * @param vistaResponse
	 * @throws InvalidVistaCredentialsException
	 */
	private void validateRemoteCAPRILoginResponse(String vistaResult)
	throws InvalidVistaCredentialsException
	{
		if(vistaResult == null)
			throw new InvalidVistaCredentialsException("Result of remote CAPRI login RPC was null, indicates failed remote CAPRI login attempt.");
		String [] lines = StringUtils.Split(vistaResult, StringUtils.NEW_LINE);
		if(lines == null)
			throw new InvalidVistaCredentialsException("Result of remote CAPRI login RPC was null array of lines, indicates failed remote CAPRI login attempt.");
		if(lines.length < 6)
			throw new InvalidVistaCredentialsException("Result of remote CAPRI login RPC does not contain at least 6 lines of response, indicates failed remote CAPRI login attempt.");
		if(!("1".equals(lines[5].trim())))
			throw new InvalidVistaCredentialsException("Result of 6th line of remote CAPRI login RPC '" + lines[5].trim() + "', not '1', indicates failed remote CAPRI login attempt.");
	}
	

	/**
	 * If doing a BSE login (local or remote), the 6th piece should be a '1' to indicate a silent login was done properly.
	 * @param vistaResponse
	 * @throws InvalidVistaCredentialsException
	 */
	private void validateBSELoginResponse(String vistaResult)
	throws InvalidVistaCredentialsException
	{
		if(vistaResult == null)
			throw new InvalidVistaCredentialsException("Result of local/remote BSE login RPC was null, indicates failed local/remote BSE login attempt.");
		String [] lines = StringUtils.Split(vistaResult, StringUtils.NEW_LINE);
		if(lines == null)
			throw new InvalidVistaCredentialsException("Result of local/remote BSE login RPC was null array of lines, indicates failed local/remote BSE login attempt.");
		if(lines.length < 6)
			throw new InvalidVistaCredentialsException("Result of local/remote BSE login RPC does not contain at least 6 lines of response, indicates failed local/remote BSE login attempt.");
		if(!("1".equals(lines[5].trim())))
			throw new InvalidVistaCredentialsException("Result of 6th line of local/remote BSE login RPC '" + lines[5].trim() + "', not '1', indicates failed local/remote BSE login attempt.");
	}
...
//						}
	...
	throws UnableToCreateContextException 
	{
        logger.info("setContext({}) TransactionContext ({}).", context, TransactionContextFactory.get().getDisplayIdentity());
	...
	throws VistaConnectionException, IOException, InvalidVistaCredentialsException, VistaMethodException
	{
		...
		VistaQuery msg = VistaCommonQueryFactory.createGetDUZVistaQuery(StringUtil.cleanString(userSSN));
		String rtn = call(msg);
	...
	throws ConnectionException, SecurityCredentialsExpiredException, BseFailedException
	{
		...
		for(retryCount=0; retryCount<this.failedSignonRetryMax; ++retryCount)
		{
			try
			{
				...
				getVistaConnection().connect();
                logger.info("Connected to Vista in {} milliseconds, signing on...", System.currentTimeMillis() - startConnect);
				...
				{
					int queryCount = 0;
                    logger.info("Calling '{}' signon queries.", signonQueries.length);
                ...
                logger.info("Signed on to Vista in {} milliseconds.", System.currentTimeMillis() - startSignon);
			...
			catch(InvalidVistaCredentialsException ivcX)
			{
				...
				long randomDelay = (long)(new SecureRandom().nextDouble() * (FAILED_SIGNIN_MAX_WAIT_TIME - FAILED_SIGNON_MIN_WAIT_TIME) + FAILED_SIGNON_MIN_WAIT_TIME);
                logger.warn("RPC_SIGNON failed [{}: {}] ({}), retrying in {} milliseconds.", ivcX.getClass().getSimpleName(), ivcX.getMessage(), retryCount, randomDelay);
			...
			catch (Exception ex)
			{
				...
				// delay for a random amount of time from 1 to 2 seconds
				long randomDelay = (long)(new SecureRandom().nextDouble() * (FAILED_SIGNIN_MAX_WAIT_TIME - FAILED_SIGNON_MIN_WAIT_TIME) + FAILED_SIGNON_MIN_WAIT_TIME);
                logger.warn("RPC_SIGNON failed [{}: {}] ({}), retrying in {} milliseconds.", ex.getClass().getSimpleName(), ex.getMessage(), retryCount, randomDelay);
				try{Thread.sleep(randomDelay);}
				catch(InterruptedException iX){}
			}
		}
