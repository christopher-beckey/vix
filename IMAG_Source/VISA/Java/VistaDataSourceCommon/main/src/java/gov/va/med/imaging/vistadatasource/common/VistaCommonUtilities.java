...
public class VistaCommonUtilities 
{
	...
	public static String getPatientDfn(PatientIdentifier patientIdentifier, 
			ResolvedArtifactSource resolvedArtifactSource, Site site, String protocol) throws IOException, ConnectionException, MethodException{
		
		VistaSession session = getVistaSession(resolvedArtifactSource, site, protocol);
		
		String patientDFN = VistaCommonUtilities.getPatientDfn(session, patientIdentifier);
		return patientDFN;
	}
	...
	throws MethodException, IOException, ConnectionException, PatientNotFoundException
	{
        logger.info("getPatientSensitivityValueFromIcn({}) TransactionContext ({}).", patientIcn, TransactionContextFactory.get().getDisplayIdentity());
	...
	{
        logger.info("getPatientSensitivityValueFromDfn({}) TransactionContext ({}).", patientDfn, TransactionContextFactory.get().getDisplayIdentity());
		try
		{
	...
	{
        logger.info("logRestrictedAccess({}), TransactionContext ({}).", patientDfn, TransactionContextFactory.get().getDisplayIdentity());
	...
	throws MethodException, IOException, ConnectionException, PatientNotFoundException
	{
        logger.info("getPatientMeansTestFromIcn({}) TransactionContext ({}).", patientIdentifier, TransactionContextFactory.get().getDisplayIdentity());
		String patientDfn = getPatientDfn(vistaSession, patientIdentifier);
	...
	throws MethodException, ConnectionException, IOException 
	{
        logger.info("getTreatingSitesUsingCprsRpc({}) TransactionContext ({}).", patientIdentifier, TransactionContextFactory.get().getDisplayIdentity());
	...
	private static VistaSession getVistaSession(ResolvedArtifactSource resolvedArtifactSource, Site site, 
			String protocol) 
		    throws IOException, ConnectionException, MethodException
		    {
			    return VistaSession.getOrCreate(resolvedArtifactSource.getMetadataUrl(protocol), 
			    		site, ImagingSecurityContextType.MAG_WINDOWS);
		    }
