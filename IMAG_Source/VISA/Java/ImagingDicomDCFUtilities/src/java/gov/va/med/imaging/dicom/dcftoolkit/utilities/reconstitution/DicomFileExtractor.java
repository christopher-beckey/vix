...
public class DicomFileExtractor {
    ...
    public IDicomDataSet getDDSFromDicomFile(String dicomFile)throws DicomFileException{
        ...
        try{
            ...
            if(fileInput.chapter10Encoding()){
                logger.debug("{}: Chapter 10 Encoding.", this.getClass().getName());
            }
            else{
                logger.debug("{}: Not Chapter 10 Encoding.", this.getClass().getName());
            }
            ...
            logger.error("{}: Dicom Toolkit layer: Exception thrown while getting Dicom Dataset from Dicom File.", this.getClass().getName());
        	...
        	if(fileInput != null){
        		try{
                    ...
                    logger.error("{}: Dicom Toolkit layer: Exception thrown while closing Dicom File {}.", this.getClass().getName(), dicomFile);
        		}
        		System.gc();
        	}
        }
    ...
    public IDicomDataSet getDDSFromDicomStream(SizedInputStream sizedDicomStream)throws DicomFileException {
    	...
    	// Fortify change: brought them out of try to close later
    	DicomStreamReader reader = null;
    	DicomFileInput dfi = null;
    	InputStream inputStream = null;
    	try {
    		DicomSessionSettings dss = new DicomSessionSettings();
    		inputStream = sizedDicomStream.getInStream();
    		// read InputStream into a ByteBuffer
    		reader = new DicomStreamReader(inputStream, dss);
    		// UID.TRANSFERLITTLEENDIAN, cpt 4/12/11 -- removed as DCF 3.3.22c figures the right TS of of a part10 file! (and we use only Part10 files)!
            dfi = new DicomFileInput(reader, null, dss );
            dDS = dfi.readDataSet();
        	...
        	// Fortify change: added finally block to close resources
        	// These two don't implement AutoClosable = no try-with-resources
        	try { if(reader != null) { reader.close(); } } catch(Exception exc) {/*unrecoverable so do nothing*/}
        	try { if(dfi != null) { dfi.close(); } } catch(Exception exc) {/*unrecoverable so do nothing*/}
    ...
    private void removeElement(String element){
        ...
        if (elementRemoved){
            logger.info("DICOM Element ({}) removed", element);
       }
    }
