    ...
    throws AxisFault 
    {
        ...
        try 
        {
            ...
            if (posting) 
            {
                ...
                {
                    log.trace("POST message created on client with credentials [{}, {}].", ((UsernamePasswordCredentials) cred).getUserName(), ((UsernamePasswordCredentials) cred).getPassword());
                }
            ...
            if(principal != null)
            {
                log.info("SecurityContext credentials for '{}' are available.", principal.getAccessCode());
    		...
    		try{
                log.info("Executing method [{}] on target [{}]", method.getPath(), hostConfiguration.getHostURL());}
            ...
            else if (msgContext.getSOAPConstants() == SOAPConstants.SOAP12_CONSTANTS) 
            {
                ...
                try{
                    log.warn("Method [{}] on target [{}] failed - '{}'.", method.getPath(), hostConfiguration.getHostURL(), statusMessage);}
                catch(IllegalStateException isX){}
            ...
            log.info("HTTPCommonsSender - " + HTTPConstants.HEADER_CONTENT_ENCODING + "={}", contentEncoding == null ? "null" : "" + contentEncoding.getValue());
            ...
            try{
                log.warn("Method [{}] on target [{}] succeeded, parsing response.", method.getPath(), hostConfiguration.getHostURL());}
            catch(IllegalStateException isX){}
            ...
            log.debug("Generic exception encountered", e);
