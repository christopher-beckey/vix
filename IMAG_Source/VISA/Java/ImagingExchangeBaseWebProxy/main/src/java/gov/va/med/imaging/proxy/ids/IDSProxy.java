...
public class IDSProxy 
{
	...
	{	
		//Anything before P207 will return 8. This is to get around problem with P177 where it always return 
		//the latest version of the callee's services
		int callerVersion = 8; 
		return getImagingServices(siteNumber, acceleratorServer, acceleratorPort, applicationName, callerVersion, version);
	}

	public SortedSet<IDSService> getImagingServices(String siteNumber, String acceleratorServer, 
			int acceleratorPort, String applicationName, int callerVersion, String version)
	{	
		if(serviceCache.isSiteOffline(siteNumber))
		{
            logger.info("Site '{}' has been previously tested and determined to be offline, returning null services.", siteNumber);
			return null;
		}
			...
			(version != null) && (version.length() > 0))
		{
			...
			if(service != null)
			{
				...
				logger.info("Service not found in cache.");
			}
			...
			{
                logger.info("Site '{}', version '{}', application '{}', was previously tested and no services found, returning null services.", siteNumber, version, applicationName);
				return null;
			}
		}		
		...
		String caller = "?callerVersion=" + Integer.toString(callerVersion);
		if((applicationName != null) && (applicationName.length() > 0))
		{
...
//			}
		...
		String idsServiceUrlString = getIdsProxyConfiguration().getIdsProtocolWithDefault() + "://" +  //$NON-NLS-1$
			...
			getIdsProxyConfiguration().getIdsServicePathWithDefault() + caller + type + versionQuery ; //$NON-NLS-1$
		try
		{
            logger.info("Querying IDS Service at site [{}] with URL [{}]", siteNumber, idsServiceUrlString);
		...
		if(response == HttpStatus.SC_OK)
		{
			try
			{
				...
				{
                    logger.info("Found {} services for site '{}', version '{}', {}', caching this status.", services == null ? "null" : "no", siteNumber, version, applicationName);
	...
	{
		int callerVersion = 999; //Use max version if no callerVersion passed in
		return getImagingServices(site, applicationName, callerVersion, version);
	}
	
	public SortedSet<IDSService> getImagingServices(Site site, String applicationName, int callerVersion, String version)
	{
		if(site == null)
			return null;
		if(!site.hasAcceleratorServer())
		{
            logger.info("Site [{}] does not have VIX defined in site service, cannot get imaging services", site.getSiteNumber());
			return null;
		}		
		return getImagingServices(site.getSiteNumber(), site.getAcceleratorServer(), 
				site.getAcceleratorPort(), applicationName, callerVersion, version);		
	}
