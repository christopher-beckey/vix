...
public abstract class ImagingProxy
{
	...
	*/
	...
	SecurityCredentialsExpiredException, ImageConversionException, MethodException, ConnectionException
	{
        logger.debug("ImagingProxy - getInstance for {}", imageUrn);
		...
		try
		{
			...
			{
                logger.debug("ImagingProxy - createTxtUrl for {}", imageUrn);
				imageUrlString = createTxtUrl(imageUrn, firstImageFormat.getMime());
			}
			else 
			{
                logger.debug("ImagingProxy - createImageUrl for {}", imageUrn);
        ...
        logger.info("Transaction [{}] initiated, retreiving [{}]", transactionIdentifier, imageUrlString);
		...
		{
			e.printStackTrace();
			responseMessage = "Exception on execute GET method, got response code '" + result + "', " + e.getMessage();
			throw new ConnectionException(responseMessage, e);
		}
		...
		if (result >= HttpStatus.SC_CONTINUE && result < HttpStatus.SC_OK)
		{
            ...
            logger.warn("Transaction [{}] response code = '{}' message is '{}'", transactionIdentifier, result, message);
		...
		else if(result == HttpStatus.SC_OK)
		{
			try
			{
                ...
                logger.info("Transaction [{}] returned SUCCESS. Content Length [{}]", transactionIdentifier, bytesRead);
		...
		else if (result >= HttpStatus.SC_INTERNAL_SERVER_ERROR)
		{
			...
			{
                logger.error("ConnectionException transaction [{}], response code = '{}' message is '{}'", transactionIdentifier, result, message);
				throw new ConnectionException(message);
			}
			else
			{
                logger.error("MethodException transaction [{}], response code = '{}' message is '{}'", transactionIdentifier, result, message);
	 ...
	 * @param client
	 * @param getMethod
	 * @param includeVistaSecurityContext
	 */
	protected void addSecurityContextToHeader(HttpClient client, GetMethod getMethod, 
			boolean includeVistaSecurityContext, ProxyServiceType proxyServiceType)
	{
		...
		try
		{
			ProxyService imageService = proxyServices.getProxyService(proxyServiceType);
		...
		if(includeVistaSecurityContext)
		{
            ...
            logger.debug("Setting DUZ in Request Header: {}", transactionContext.getDuz());
        	...
        	if(securityToken != null && securityToken.length() > 0)
        		getMethod.setRequestHeader(new Header(TransactionContextHttpHeaders.httpHeaderBrokerSecurityTokenId, 
        				securityToken));
            logger.debug("Setting Security Token in Request Header: {}", securityToken);
        ...
        logger.debug("Setting Site Number in Request Header: {}", transactionContext.getSiteNumber());
		...
		logger.debug("Setting SSN in Request Header.");
	...
	{
		addSecurityContextToHeader(client, getMethod, includeVistaSecurityContext, ProxyServiceType.image);
	}
