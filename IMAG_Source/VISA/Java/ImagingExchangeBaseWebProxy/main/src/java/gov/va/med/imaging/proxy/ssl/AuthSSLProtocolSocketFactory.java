 ...
 * 
 * <pre>
 * Protocol authhttps = new Protocol(&quot;https&quot;, new AuthSSLProtocolSocketFactory(new URL(&quot;file:my.keystore&quot;), 
 &quot;mypassword&quot;,
 * 	new URL(&quot;file:my.truststore&quot;), &quot;mypassword&quot;), 443);
	...
	implements SecureProtocolSocketFactory
{
	...
	private final int defaultFederationNioPort = 8442;

	private final String DUMMY = "DUMMY";
	
	private static Map<String, AuthSSLRemoteHostMap> hostMap = new HashMap<String, AuthSSLRemoteHostMap>();
	/**
	...
	public static synchronized void AddCvixHostMap(String host, AuthSSLRemoteHostMap sslHostInfo)
	{
		hostMap.put(host, sslHostInfo);
	}
	...
	private SSLContext createCvixSSLContext(String host)
	{
		try
		{
			URL cvixKeystoreUrl = hostMap.get(host).getKeystoreUrl(); 
			URL cvixTruststoreUrl = hostMap.get(host).getTruststoreUrl(); 
			String cvixKeystorePassword = hostMap.get(host).getKeystorePassword();
			String cvixTruststorePassword = hostMap.get(host).getTruststorePassword();
			
			KeyManager[] keymanagers = null;
			TrustManager[] trustmanagers = null;
			if (cvixKeystoreUrl != null)
			{
				KeyStore keystore = createKeyStore(cvixKeystoreUrl, cvixKeystorePassword);
				if (Logger.getLogger(AuthSSLProtocolSocketFactory.class).isDebugEnabled())
					logKeystoreContents("keystore", keystore);

				keymanagers = createKeyManagers(keystore, cvixKeystorePassword);
			}
			
			if (cvixTruststoreUrl != null)
			{
				KeyStore truststore = createKeyStore(cvixTruststoreUrl, cvixTruststorePassword);
				if (Logger.getLogger(AuthSSLProtocolSocketFactory.class).isDebugEnabled())
					logKeystoreContents("truststore", truststore);
			...
			Logger.getLogger(AuthSSLProtocolSocketFactory.class).debug("Create cvix SSL Context");
	...
	private void logKeystoreContents(String keystoreName, KeyStore keystore) throws KeyStoreException
	{
		...
		for (Enumeration<String> aliases = keystore.aliases(); aliases.hasMoreElements();)
		{
			...
			else
			{
                ...
                Logger.getLogger(AuthSSLProtocolSocketFactory.class).debug("Trusted Certificate Authority '{}':", alias);
				logCertificateContents(cert);
			}
		}
	}
	...
	private void logCertificateContents(Certificate cert)
	{
		if (cert instanceof X509Certificate)
		{
            ...
            Logger.getLogger(AuthSSLProtocolSocketFactory.class).debug("  Signature Algorithm: {}", x509Cert.getSigAlgName());
            Logger.getLogger(AuthSSLProtocolSocketFactory.class).debug("  Signature: {}", x509Cert.getPublicKey().toString());
	...
	private SSLContext getSSLContext(String host, int port)
	{
		if (hostMap.get(host) != null) //Cvix only
		{
			SSLContext cvixSslContext = hostMap.get(host).getSslContext(); 
			if (cvixSslContext == null)
			{
				cvixSslContext = createCvixSSLContext(host);
				hostMap.get(host).setSslContext(cvixSslContext);
			}
			return cvixSslContext;
		}
		
		if (port == defaultFederationNioPort) //Non-Blocking IO protocol must create SSLContext everytime
		{
			this.sslcontext = createSSLContext();
		}
		else //Blocking IO protocol
		{
	...
	throws IOException, UnknownHostException, ConnectTimeoutException
	{
		...
		{
			if (hostMap.get(host).getRemoteHostPort() != null) {
				newPort = hostMap.get(host).getRemoteHostPort();
			}
			if (hostMap.get(host).getTimeout() != null) {
				timeout = hostMap.get(host).getTimeout();
			}
		}

        Logger.getLogger(AuthSSLProtocolSocketFactory.class).debug("CreateSocket-  host:{} Port={} timeout={}", host, newPort, timeout);
		
		if(timeout == 0)
		{
			timeout = 60*1000; //Default: 1min
		}
		...
		try {
			...
			SocketAddress remoteaddr = new InetSocketAddress(host, newPort);
			socket.bind(localaddr);
			socket.connect(remoteaddr, timeout);
            Logger.getLogger(AuthSSLProtocolSocketFactory.class).debug("Socket.connect sucessfully to remote address [{}:{}]", host, newPort);
			return socket;
		} catch (Exception e) {
			...
			throw new IOException("Error opening and binding socket", e);
		}
	}
