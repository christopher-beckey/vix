...
extends ArrayList<ImageFormatQuality> 
{
	...
	public static ImageFormatQualityList createListFromAcceptString(String imageFormatAcceptString)
	{
		...
		for(StringTokenizer commaTokenizer = new StringTokenizer(imageFormatAcceptString, ","); commaTokenizer.hasMoreTokens();)
		{
			...
			ImageFormat imageFormat = ImageFormat.valueOfMimeType(StringUtil.MagPiece(imageFormatQualityString, StringUtil.SEMICOLON, 1));
			if(imageFormat != null)
			{
				if(imageFormatQualityString.contains(StringUtil.SEMICOLON))
				{
					String q = StringUtil.MagPiece(imageFormatQualityString, StringUtil.SEMICOLON, 2);
