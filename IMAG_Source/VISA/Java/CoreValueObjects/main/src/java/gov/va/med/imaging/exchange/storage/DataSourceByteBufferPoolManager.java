...
implements DataSourceByteBufferPoolManagerMBean
{
	...
	throws IOException
	{
		if(size > 0)
		{
            logger.info("Reading file [{}] into buffer of size [{}]", identifier, size);
            ...
            logger.info("Done reading file [{}] into buffer, read [{}] bytes", identifier, bytesRead);
	...
	throws IOException
	{
        logger.info("Reading file [{}] into buffer of unknown size", identifier);
	...
	private ByteBuffer getBuffer(int minSize)
	{
		...
		if(bufferList == null)
		{
            logger.error("Creating buffer of size [{}], not associated with a buffer list, new buffer list of minimum size might be needed", minSize);
