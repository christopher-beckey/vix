...
{
	private static final Logger logger = Logger.getLogger(ByteBufferBackedImageInputStream.class);
	...
	private void calculateImageFormat(boolean detectEmptyStream)
	{
		if(isInputStreamReadable())
		{
			...
			{
				logger.debug("ByteBufferBackedImageInputStream.calculateImageFormat() --> Determining image format from stream");
		...
		else if(buffer != null)
		{
			...
			try (InputStream inStream = getInputStreamFromBuffer();
				 FileTypeIdentifierStream ftis = new FileTypeIdentifierStream(inStream);)
			{
				logger.debug("ByteBufferBackedImageInputStream.calculateImageFormat() --> Determining image format from buffer");
			...
			{
                logger.warn("ByteBufferBackedImageInputStream.calculateImageFormat() --> Encountered IOException: {}", ioX.getMessage());
			}
		}
	}
