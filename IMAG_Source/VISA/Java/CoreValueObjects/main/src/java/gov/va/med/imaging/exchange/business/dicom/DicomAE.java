...
public class DicomAE implements PersistentEntity, Serializable
{
	...
	// Fields
	private int id;
	private String applicationName;		// max 128 char.: Unique (descriptive) DICOM Application Name
	private String remoteAETitle;		// max  16 char.: DICOM Application Entity Title (unique per local application instance)
	private String localAETitle;
	private String hostName;			// max  32 char.: the node this AE resides on (needed for remote SCPs only)
	private String port;				// max  10 char.: Port number of AE (needed for remote SCPs only)
	private String siteNumber;			// INSTITUTION file IEN of the site location where host of application reside
	...
	private boolean relaxValidation;	// if true, minimum validation is enough for incoming objects to pass validation
	private boolean resourceError = true;		// to remote AE on DB, device. etc. access error (default=true)
	private ArrayList<ServiceAndRolePair> services;
	private String imagingService;

	private int resultCode;
	private String resultMessage;
	private boolean remoteAEValid;
	private String studyAssociation;// max   8 char.: 'PSPA', 'CSPA', 'UNKOWN' (for remote C-STORE SCUs only)
	...
	public boolean isServiceAndRoleValid(String service, String role) {
		...
		if(services == null || services.isEmpty()){
			logger.warn("DicomAE.isServiceAndRoleValid() --> There are no DICOM Services assigned to this AE. Return false.");
			return false;
		}
	...
	public boolean containsConnectionData(){
		...
		if(this.port == null || this.port.equals(""))
			containsData = false;
		if(this.remoteAETitle == null || this.remoteAETitle.equals(""))
			containsData = false;
		if(this.localAETitle == null || this.localAETitle.equals(""))
			containsData = false;
