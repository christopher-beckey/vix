...
{
	private Logger logger = Logger.getLogger(RouterStatisticsAdvice.class);
	private Map<String, RouterMethodStatistics> methodStatistics = new HashMap<String, RouterMethodStatistics>();;

	public RouterStatisticsAdvice()
    {	
		logger.info("RouterStatisticsAdvice() --> Registering monitored router methods....");
		...
		logger.info("RouterStatisticsAdvice() --> Successfully registered monitored router methods");
	}
	...
	throws Throwable
	{
		...
		if(ms != null)
		{
            logger.trace("RouterStatisticsAdvice.before() --> Before calling {}({}).", method.getName(), createArgLoggable(args));
	...
	throws Throwable
	{
		...
		if(ms != null)
		{
			...
			{
                logger.trace("RouterStatisticsAdvice.afterReturning() --> After returning from {}({}) with null value.", method.getName(), createArgLoggable(args));
				ms.incrementNullResultCount();
			}
			else if(logger.isDebugEnabled()) {
                logger.trace("RouterStatisticsAdvice.afterReturning() --> After returning from {}({}), result is [{}]", method.getName(), createArgLoggable(args), createResultLoggable(result));
			}
	...
	throws Throwable
	{
		...
		if(ms != null)
		{
            logger.trace("RouterStatisticsAdvice.afterThrowing() --> After returning from {}({}) with exception [{}]", method.getName(), createArgLoggable(args), createThrowableLoggable(throwable));
	...
	throws AttributeNotFoundException, MBeanException, ReflectionException
    {
		...
		if(methodName != null && dataPointName != null)
		{
			...
			if(stats != null)
			{
                ...
                {
                    logger.warn("RouterStatisticsAdvice.getAttribute() --> Exception [{}], getting attribute [{}]", e.getMessage(), attribute);
                }
			}
		}
    ...
    public AttributeList getAttributes(String[] attributes)
    {
		...
		for(String attribute : attributes)
            ...
            {
                logger.warn("RouterStatisticsAdvice.getAttributes() --> Exception [{}], getting attribute [{}]", e.getMessage(), attribute);
            }
	...
	static
	{
		...
		{
            Logger.getLogger(RouterStatisticsAdvice.class).error("Exception '{}', initializing router statistics advice.", x.getMessage());
		}
	}
	...
	private MBeanOperationInfo[] getMBeanOperations()
    {
        ...
        {
            logger.warn("RouterStatisticsAdvice.getMBeanOperations() --> Exception [{}] getting bean operation.  Return null.", e.getMessage());
        	return null;
        }
    }
