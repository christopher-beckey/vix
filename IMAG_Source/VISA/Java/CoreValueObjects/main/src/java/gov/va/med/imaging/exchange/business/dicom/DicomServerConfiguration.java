...
extends AbstractBaseFacadeConfiguration 
{
	...
	public static Boolean isConfigurationExist()
	{
		String dir = getConfigurationDirectory();
		File dicomConfigFile = new File(StringUtil.cleanString(dir + "/" + DicomServerConfiguration.class.getName()));
		return dicomConfigFile.exists();
	}
    ...
    {
    	try {
			File folder = FileUtilities.getFile(folderPath);
			if (!folder.exists()) {
				folder.mkdirs();
			}
		} catch (Exception e) {
