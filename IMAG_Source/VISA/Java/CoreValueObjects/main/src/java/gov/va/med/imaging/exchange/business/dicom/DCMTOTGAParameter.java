...
public class DCMTOTGAParameter {
    ...
    private int bitsParameter;
    private boolean bitsSet;
    private int additionParameter;
    private boolean additionSet;
    private int subtractionParameter; 
    private boolean subtractionSet;
    private int floorParameter;
    private boolean floorSet;
    private int ceilingParameter;
    private boolean ceilingSet;
    private int reductionParameter;
    private boolean reductionSet;
    private boolean parametersSet;
    private boolean dicomParameterSet;
    ...
    private void decomposition(String settings) throws ParameterDecompositionException {
        ...
        if(settings == null || settings.length() == 0){
            throw new ParameterDecompositionException("DCMTOTGAParameter.decomposition() --> No DCMTOTGA Parameter data to decompose.");
        }
            try {
                ...
                for(int i = 0; i < pieces.length; i++){
                    ...
                    if(pieces[i].toUpperCase(Locale.ENGLISH).startsWith("A")){
                    ...
                    if(pieces[i].toUpperCase(Locale.ENGLISH).startsWith("S")){
                    ...
                    if(pieces[i].toUpperCase(Locale.ENGLISH).startsWith("R")){
            	...
            	throw new ParameterDecompositionException(nfe.getMessage());
            }
    }
    ...
    private int convertStringToInt(String argument)throws NumberFormatException{
      ...
      //Remove the First character, which is the letter.
        String argumentValue = argument.substring(1);
        ...
        try {
        	...
        	String msg = "DCMTOTGAParameter.convertStringToInt() --> Encountered NumberFormatException while converting value [" + argumentValue +"]";
        	logger.error(msg);
