...
implements Serializable, Comparable<Study>, Iterable<Series>, StudyFilterFilterable
{
	...
	/**
	 * 
	 * @param ObjectOrigin				object origin
	 * @param String					repository Id
	 * @param String 					study IEN
	 * @param PatientIdentifier			contains patient Id
	 * @param StudyLoadLevel			specifies the load level
	 * @param StudyDeletedImageState	delete state
	 * 
	 * @return Study					creation result
	 * @throws URNFormatException 		required
	 * 
	 */
	...
	throws URNFormatException
	{
		...
		{
			logger.warn("Study.create(1) --> Unable to create a Study object when given object origin is null. Return null.");
			return null;
		}
		if(repositoryId == null)
		{
			logger.warn("Study.create(1) --> Unable to create a Study object when given repository Id null. Return null.");
			return null;
		}
		if(studyIen == null)
		{
			logger.warn("Study.create(1) --> Unable to create a Study object when given study IEN is null. Return null.");
			return null;
		}
		if(patientIdentifier == null)
		{
			logger.warn("Study.create(1) --> Unable to create a Study object when given patient Id is null. Return null.");
			return null;
		}
		if(studyLoadLevel == null)
		{
			logger.warn("Study.create(1) --> Unable to create a Study object when given study load level is null. Return null.");
			return null;
		}
	 ...
	 * @param StudyLoadLevel			specifies the load level
	 * @param StudyDeletedImageState	delete state
	 * @param boolean					flag for new data source
	 * 
	 * @return Study					creation result
	 * @throws URNFormatException 		required
	 * 
	...
	public static Study create(
		...
		StudyLoadLevel studyLoadLevel,
		StudyDeletedImageState studyDeletedImageState,
		boolean isNewDataStructure)
	throws URNFormatException
	{
		if(objectOrigin == null)
		{
			logger.warn("Study.create(2) --> Unable to create a Study object when given object origin is null. Return null.");
			return null;
		}
		if(repositoryId == null)
		{
			logger.warn("Study.create(2) --> Unable to create a Study object when given repository Id null. Return null.");
			return null;
		}
		if(studyIen == null)
		{
			logger.warn("Study.create(2) --> Unable to create a Study object when given study IEN is null. Return null.");
			return null;
		}
		if(patientIdentifier == null)
		{
			logger.warn("Study.create(2) --> Unable to create a Study object when given patient Id is null. Return null.");
			return null;
		}
		if(studyLoadLevel == null)
		{
			logger.warn("Study.create(2) --> Unable to create a Study object when given study load level is null. Return null.");
			return null;
		}
			
		StudyURN studyUrn = isNewDataStructure ? StudyURNFactory.create(repositoryId, studyIen, patientIdentifier.getValue(), P34StudyURN.class)
											   : StudyURNFactory.create(repositoryId, studyIen, patientIdentifier.getValue(), StudyURN.class);
		// set the patient identifier type if it is necessary
		studyUrn.setPatientIdentifierTypeIfNecessary(patientIdentifier.getPatientIdentifierType());
		return create(studyUrn, studyLoadLevel, studyDeletedImageState, isNewDataStructure);
	}
	 ...
	 * @param StudyDeletedImageState	delete state
	 * @param boolean					flag for new data source
	 * @param boolean					flag for new MUSE
	 * 
	 * @return Study					creation result
	 * @throws URNFormatException 		required
	 * 
	...
	throws URNFormatException
	{
		if(objectOrigin == null)
		{
			logger.warn("Study.create(3) --> Unable to create a Study object when given object origin is null. Return null.");
			return null;
		}
		if(repositoryId == null)
		{
			logger.warn("Study.create(3) --> Unable to create a Study object when given repository Id null. Return null.");
			return null;
		}
		if(studyIen == null)
		{
			logger.warn("Study.create(3) --> Unable to create a Study object when given study IEN is null. Return null.");
			return null;
		}
		if(patientIdentifier == null)
		{
			logger.warn("Study.create(3) --> Unable to create a Study object when given patient Id is null. Return null.");
			return null;
		}
		if(studyLoadLevel == null)
		{
			logger.warn("Study.create(3) --> Unable to create a Study object when given study load level is null. Return null.");
			return null;
		}
		...
		if(isNewDataStructure){
			studyUrn = StudyURNFactory.create(repositoryId, studyIen, patientIdentifier.getValue(), P34StudyURN.class);			
		}
		else if(isMuse){
			studyUrn = StudyURNFactory.create(repositoryId, studyIen, patientIdentifier.getValue(), MuseStudyURN.class);					
		}
		else{
			studyUrn = StudyURNFactory.create(repositoryId, studyIen, patientIdentifier.getValue(), StudyURN.class);
		}
	...
	public static Study create(
		...
		if(objectOrigin == null)
		{
			logger.warn("Study.create(4) --> Unable to create a Study object when given object origin is null. Return null.");
			return null;
		}
		if(repositoryId == null)
		{
			logger.warn("Study.create(4) --> Unable to create a Study object when given repository Id null. Return null.");
			return null;
		}
		if(studyIen == null)
		{
			logger.warn("Study.create(4) --> Unable to create a Study object when given study IEN is null. Return null.");
			return null;
		}
		if(patientIdentifier == null)
		{
			logger.warn("Study.create(4) --> Unable to create a Study object when given patient Id is null. Return null.");
			return null;
		}
		if(studyLoadLevel == null)
		{
			logger.warn("Study.create(4) --> Unable to create a Study object when given study load level is null. Return null.");
			return null;
		}
		if(museServerId == null)
		{
			logger.warn("Study.create(4) --> Unable to create a Study object when given MUSE server Id null. Return null.");
			return null;
		}

		StudyURN studyUrn = isMuse ? StudyURNFactory.create(repositoryId, studyIen, patientIdentifier.getValue(), museServerId, MuseStudyURN.class)
								   : StudyURNFactory.create(repositoryId, studyIen, patientIdentifier.getValue(), StudyURN.class);
	...
	/**
	 * 
	 * @param GlobalArtifactIdentifier			contains patient Id
	 * @param StudyLoadLevel					specifies the load level
	 * @param StudyDeletedImageState			delete state
	 * 
	 * @return Study							creation result
			...
			StudyDeletedImageState studyDeletedImageState)
	{
		if(identifier == null)
		{
			logger.warn("Study.create(5) --> Unable to create a Study object when given patient Id is null. Return null.");
			return null;
		}
		if(studyLoadLevel == null)
		{
			logger.warn("Study.create(5) --> Unable to create a Study object when given study load level is null. Return null.");
			return null;
		}
	 ...
	 */
	public static Study create(GlobalArtifactIdentifier identifier, StudyLoadLevel studyLoadLevel,
			StudyDeletedImageState studyDeletedImageState, boolean isNewDataStructure)
	{
		if(identifier == null)
		{
			logger.warn("Study.create(6) --> Unable to create a Study object when given patient Id is null. Return null.");
			return null;
		}
		if(studyLoadLevel == null)
		{
			logger.warn("Study.create(6) --> Unable to create a Study object when given study load level is null. Return null.");
			return null;
		}
		
		return new Study(identifier, studyLoadLevel, studyDeletedImageState, isNewDataStructure);
	}
			...
			StudyDeletedImageState studyDeletedImageState, boolean isNewDataStructure)
	{
		...
		this.studyLoadLevel = studyLoadLevel;
		this.studyDeletedImageState = studyDeletedImageState;
		this.isStudyInNewDataStructure = isNewDataStructure;
...
//}
	...
	{
		if(this.consolidatedSiteNumber == null || this.consolidatedSiteNumber.length() <= 0)
			return false;
	...
	public String toString()
	{
		...
		if(getFirstImage() == null){
			sb.append("First Image is null.");
		}
		else{
			sb.append("First Image: "+getFirstImage().toString());
		}
			...
			if(!this.getSiteNumber().equals(this.consolidatedSiteNumber))
			{
				...
				if(studyUrn != null)
				{
					try
					{
                        ...
                        logger.debug("Updating study '{}' with new site ID, created new URN '{}'.", studyUrn.toString(), newUrn.toString());
						...
						study.alternateExamNumber = this.alternateExamNumber;
						study.groupIen = this.groupIen;
						study.contextId = this.contextId;
						study.accessionNumber = this.accessionNumber;
						study.isStudyInNewDataStructure = this.isStudyInNewDataStructure;
						study.museServerId = this.museServerId;

                        logger.info("Setting study.numberOfDicomImages to: {}, was: {}", this.numberOfDicomImages, study.numberOfDicomImages);
						study.numberOfDicomImages = this.numberOfDicomImages;
	...
	public boolean isStudyInNewDataStructure() {
		return isStudyInNewDataStructure;
	}

	public void setStudyInNewDataStructure(boolean isStudyInNewDataStructure) {
		this.isStudyInNewDataStructure = isStudyInNewDataStructure;
	}

	/**
	 * @return the museServerId
	 */
	...
	/**
	 * @param museServerId the museServerId to set
	 */
	public void setMuseServerId(String museServerId) {
		this.museServerId = museServerId;
        if(this.globalArtifactIdentifier instanceof MuseImageURN){
        	((MuseImageURN) this.globalArtifactIdentifier).setMuseServerId(this.museServerId);
        }
