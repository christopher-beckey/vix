...
extends LocalDataSourceSpi
{
	...
	/**
	 * Gets all transaction log entries which match the provided query and writes them to the provider writer
	 *
	 * @param writer The writer to receive matching transaction log entries
	 * @param query The query to evaluate against the transaction log store
	 * @throws MethodException In the event of any internal issues
	 * @throws ConnectionException In the event of any connection (external) issues
	 */
	void getLogEntries(TransactionLogWriter writer, TransactionLogQuery query) throws MethodException, ConnectionException;
}
