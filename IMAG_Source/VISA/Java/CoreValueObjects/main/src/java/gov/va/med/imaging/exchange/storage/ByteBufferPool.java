...
ByteBufferPoolMBean
{
	...
	{
        logger.info("Creating ByteBufferMinimumSizeList of [{} buffer sizes named [{}]", maxBufferSizeInbytes, name);
	...
	private ByteBufferPool(String name, int maxBufferSize, int preferredPoolSize)
	{
        ...
        logger.info("Scheduling byte buffer pool '{}' every [" + BYTE_BUFFER_POOL_TIMER_REFRESH + "] ms", name);
	...
	public void run() 
	{
		synchronized (byteBuffers) 
		{
			...
			{
                logger.info("ByteBuffer Pool '{}' has [{}] buffers, more than preferred amount [{}]", name, byteBuffers.size(), preferredBufferMaxCount);
				int buffersToRemove = byteBuffers.size() - preferredBufferMaxCount;
                logger.info("Removing [{}] buffers from buffer pool '{}'", buffersToRemove, name);
				...
				{
                    logger.debug("Removing buffer at index [{}] from buffer pool '{}'", i, name);
					byteBuffers.remove(i);
				}
			}
			else
			{
                logger.debug("ByteBuffer pool '{}' contains [{}] buffers, less than [{}] preferred buffers, not cleaning up", name, byteBuffers.size(), preferredBufferMaxCount);
			}
		}
	}
