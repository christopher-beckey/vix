...
{
	private final static Logger logger = Logger.getLogger(RetryableWorkUnitCommand.class);
	...
	public T executeWorkWithRetries() throws MethodException
	{
		...
		logger.info("RetryableWorkUnitCommand.executeWorkWithRetries() --> Starting operation....");
		...
		while (retryCount <= maximumNumberOfRetries && !success)
		{
			...
			catch (Exception e)
			{
				try
				{
                    logger.warn("RetryableWorkUnitCommand.executeWorkWithRetries() --> Encountered Exception during retryable work: {}", e.getMessage());

					if (retryCount == maximumNumberOfRetries)
					{
						String msg = "RetryableWorkUnitCommand.executeWorkWithRetries() --> Retry maximum reached [" + retryCount + "]";
						logger.error(msg);
						throw new MethodException(msg, e);
					}

                    logger.debug("RetryableWorkUnitCommand.executeWorkWithRetries() --> Sleeping for [{}] milliseconds before next retry...", delay);
				...
				catch (InterruptedException ie)
				{
					...
					String msg = "RetryableWorkUnitCommand.executeWorkWithRetries() --> InterruptedException while retrying: " + e.getMessage();
					logger.error(msg);
					throw new MethodException(msg, e);
				}
			}
