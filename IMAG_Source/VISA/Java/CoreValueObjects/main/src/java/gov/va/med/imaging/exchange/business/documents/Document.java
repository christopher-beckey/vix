...
implements Serializable, Comparable<Document>
{
	...
	throws URNFormatException
	{
		...
		{
            logger.warn("Image '{}' contains error message, excluding from document result.", image.getImageUrn().toString());
			return null;
		}
		if(image.isDeleted())
		{
            logger.warn("Image '{}' is deleted, excluding from document result.", image.getImageUrn().toString());
			return null;
		}
	...
	private static void populateMediaType(Document document, Image image)
	{
		if(document.mediaType == null)
		{
			...
			else
			{
				...
				if(document.mediaType == null)
				{
                    logger.debug("Could not find appropriate media type for filename '{}', using application/octet-stream", filename);
			...
			if(!this.getSiteNumber().equals(this.consolidatedSiteNumber))
			{
				...
				if(documentUrn != null)
				{
					try
					{
                        ...
                        logger.info("Updating document '{}' with new site ID, created new URN '{}'.", documentUrn.toString(), newDocumentUrn.toString());
