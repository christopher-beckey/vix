...
implements Comparable<DocumentSet>
{
	...
	throws URNFormatException
	{
		...
		{
            logger.warn("Study '{}' contains error message, excluding from document result.", study.getStudyUrn().toString());
			return null;
		}
		if(study.isDeleted())
		{
            logger.warn("Study '{}' is deleted, excluding from document result.", study.getStudyUrn().toString());
			return null;
		}
	...
	public DocumentSet cloneWithConsolidatedSiteNumber(Site site)
	{
		...
		{			
			...
			{
                logger.info("Updating documentSet '{}' with new repository ID '{}'.", this.identifier, this.consolidatedSiteNumber);
