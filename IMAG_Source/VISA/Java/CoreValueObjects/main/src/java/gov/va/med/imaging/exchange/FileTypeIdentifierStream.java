...
extends BufferedInputStream 
{
	...
	final static String MOV = "video/quicktime";
	final static String MP4 = "video/mp4";
	...
	private ClonableAdler32 runningChecksum;
	private ClonableAdler32 markedChecksum;
	
	private int runningLength;
	private int markedLength;
	private boolean multiImageGIF;
	
	private static final Logger logger = Logger.getLogger(FileTypeIdentifierStream.class);
	 ...
	 * @param byte1 The first byte of data read from the file, this function needs this first byte
	 * @throws IOException
	...
	throws IOException
	{
		...
		// QN: may not used locally but used to advance counter. Leave them.
		int byte7 = read();
		int byte8 = read();
		int byte9 = read();
		int byte10 = read();
		...
		if((byte2 == 73) && 
			...
			(byte6 == 97))						
		{
			//check if animation support which multi-image GIF
			if((byte25 == 33) && (byte26 == 249)){
				this.multiImageGIF = true;
			}
			return GIF;
		}
		
		return tryTGA(26, byte2, byte3);
	}
	...
	private static boolean compareWith(int FirstPossible, int SecondPossible, int CompareWith) {	
		return (FirstPossible == CompareWith || SecondPossible == CompareWith);
	}
	...
	private String tryMP4orMOV() throws IOException{
		...
		if(byte5 == 102 && byte6 == 116 && byte7 == 121 && byte8 == 112 && byte9 == 77 && byte10 == 83 &&
				byte11 == 78 && byte12 == 86) {
			return MP4;
		}//66 74 79 70 69 73 6F 6D
		else if(byte5 == 102 && byte6 == 116 && byte7 == 121 && byte8 == 112 && byte9 == 105 && byte10 == 115 &&
				byte11 == 111 && byte12 == 109) {
			return MP4;
		}
		else if(byte5 == 102 && byte6 == 116 && byte7 == 121 && byte8 == 112 && byte9 == 113 && byte10 == 116 &&
				byte11 == 32 && byte12 == 32) {
			return MOV;
		}
		return tryTGA(12, byte2, byte3);
	}
	...
	public ImageFormat getImageFormat()
	{
		...
		{
            logger.warn("FileTypeIdentifierStream.getImageFormat() --> Encountered ImageNotFoundException: {}. Not throwing back to use, return default format [{}]", infX.getMessage(), defaultImageFormat);
	...
	throws ImageNotFoundException
	{
		...
		else if(MOV.equals(imageFormat))
		{
			return ImageFormat.MOV;
		}
		else if(MP4.equals(imageFormat))
		{
	...
	throws ImageNotFoundException
	{
		...
		{    	
			...
			{	
				...
				default:	// not one of the above
					...
					{
						logger.error("FileTypeIdentifierStream.getImageType() --> Input stream has 0 bytes");
						throw new ImageNotFoundException("FileTypeIdentifierStream.getImageType() --> Input stream has 0 bytes");
					}
		...
		{	String msg = "FileTypeIdentifierStream.getImageType() --> Encountered IOException: " + e.toString();
			logger.error(msg);
		...
		finally
		{
			...
			{
                logger.warn("FileTypeIdentifierStream.getImageType() --> Unable to reset file pointer location: {}", e.toString());
			}
		}

        logger.info("FileTypeIdentifierStream.getImageType() --> File Type identified as: [{}]", ImgType);
	...
	public synchronized void mark(int readlimit)
	{
		...
		catch (CloneNotSupportedException cnsX)
		{
			...
			throw new RuntimeException("FileTypeIdentifierStream.getImageType() --> Checksum generator is not clonable, code has been modified inappropriately.");
		}
	}
	...
	public boolean isMultiImageGIF() {
		if(getImageFormat().equals(ImageFormat.GIF)) {
			return multiImageGIF;
		}
		else{
			return false;
		}
