...
extends ProcedureFilter 
{
	...
	{
        logger.info("Filter collection of '{} documents.", studies.size());
		for(Iterator<? extends StudyFilterFilterable> iter = studies.iterator(); iter.hasNext();)
		{
			...
			if(imageClass == null)
                logger.warn("Study [{}] has null first image class, not removing from list", study.getStudyIen());
        ...
        logger.info("resulting collection after filtering has '{} documents.", studies.size());
	}
