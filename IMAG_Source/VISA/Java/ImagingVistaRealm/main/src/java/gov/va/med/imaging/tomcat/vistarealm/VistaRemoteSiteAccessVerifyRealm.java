...
implements Realm, org.apache.catalina.Lifecycle, VistaRemoteSiteAccessVerifyRealmMBean
{
	...
	{
        logger.info("{} ctor()", VistaRemoteSiteAccessVerifyRealm.class.getCanonicalName());
	...
	public Principal authenticate(String username, String password) 
	{
        ...
        getLogger().info("Realm '{}'-authenticate ({}, <password not shown>)", getSiteNumber(), username);
		...
		try {
			...
			return synchronizedAuthenticate(password, principal, usernamePieces, realmSite);
		} finally {
			if (lock != null) {
				try {
					lock.unlock();
	...
	private VistaRealmPrincipal synchronizedAuthenticate(String password, VistaRealmPrincipal principal, UsernamePieces usernamePieces, VistaRemoteSiteAccessVerifyRealmSite realmSite) {
		String username;
		...
		if (isUsingPrincipalCache())
		{
			...
			if (cacheEntry != null)
			{
				...
				{
                    getLogger().info("User ({}, <password not shown>) found in VistaRealmPrincipal cache", username);
				...
				else
				{
                    ...
                    getLogger().warn("User ({}, <password not shown>) found in VistaRealmPrincipal cache WITH DIFFERENT PASSWORD!", username);
				}
			}
			else
				...
				try
				{
					...
					else
					{
						...
						if(getGenerateBseToken())
						{
							...
							{
                                getLogger().warn("Error getting BSE token from remote site '{}', BSE will be disabled.", realmSite.getSiteNumber(), rpcX);
							}
						}
					}
                    ...
                    getLogger().info("authenticate ({}, <password not shown>), user authenticated in Vista", username);
					principal.setAuthenticatedByVista(true);
				} catch (ConnectionFailedException e)
				{
                    getLogger().error("authenticate of ({}, <password not shown>), failed due to connection problem", username, e);
					principal = null;
				} catch (InvalidCredentialsException e)
				{
                    getLogger().info("authenticate of ({}, <password not shown>), failed due to invalid credentials", username);
					principal = null;
				} catch (MethodException e)
				{
                    getLogger().error("authenticate of ({}, <password not shown>), failed due to method exception (possible change in Vista version?)", username, e);
					principal = null;
				} finally
				{
					try
