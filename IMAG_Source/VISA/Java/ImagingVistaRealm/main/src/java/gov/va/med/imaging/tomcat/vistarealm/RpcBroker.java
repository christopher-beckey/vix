...
//	private final static String RPC_MAG_MAGG_SYS_GLOBAL_NODE = "MAGG SYS GLOBAL NODE";
	...
	throws ConnectionFailedException, InvalidCredentialsException, MethodException 
	{
		...
		{			
			...
			long authValue = Long.parseLong(authResults[0].trim());
			if(authValue <= 0) 
			{
	...
	throws UnableToConnectToBrokerException 
	{
		...
		for(int failureCount = 0; failureCount < FAILED_CONNECTION_MAX_COUNT && !connectionOK; ++failureCount) 
		{
			try 
			{
				...
				if(failureCount > 0)
                    logger.info("RpcBroker.connect() connected after [{}] attempts", failureCount);
				
				connectionOK = true;
			}
			catch(Exception eX) 
			{
                logger.info("Exception in RpcBroker.connect() [{}] on attempt [{}]", eX.getMessage(), failureCount);
				...
				else 
				{
					...
					// with our connection
					long sleepTime = (long)(new SecureRandom().nextDouble() * (FAILED_CONNECTION_MAX_WAIT_TIME - FAILED_CONNECTION_MIN_WAIT_TIME) + FAILED_CONNECTION_MIN_WAIT_TIME);
                    logger.info("RpcBroker.connect() sleeping for [{}] ms", sleepTime);
					try 
					{
