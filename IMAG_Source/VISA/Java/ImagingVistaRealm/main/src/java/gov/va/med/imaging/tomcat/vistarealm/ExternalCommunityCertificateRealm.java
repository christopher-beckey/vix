...
implements VistaRealm, VistaRealmSite, ExternalCommunityCertificateRealmMBean 
{
	...
	public Principal authenticate(X509Certificate[] certs) 
	{
        ...
        getLogger().info("Realm '{}'-authenticate ({}, <password not shown>)", getSiteNumber(), getServiceAccountUID());
		...
		try {
			if (lock != null) {
                ...
                getLogger().error("Unable to acquire lock for username '{}', this could potentially lead to concurrency problems.", getServiceAccountUID());
			}
			principal = authenticateSynchronized();
		} finally {
			if (lock != null) {
				try {
					lock.unlock();
	...
	private VistaRealmPrincipal authenticateSynchronized() {
		VistaRealmPrincipal principal = null;
		...
		if (isUsingPrincipalCache())
		{
			...
			if (cacheEntry != null)
			{
				...
				{
                    getLogger().info("User ({}, <password not shown>) found in VistaRealmPrincipal cache", getServiceAccountUID());
				...
				else
				{
                    ...
                    getLogger().warn("User ({}, <password not shown>) found in VistaRealmPrincipal cache WITH DIFFERENT PASSWORD!", getServiceAccountUID());
				}
			}
			else
		...
		if (isInitialized() )
		{
			...
			if (principal == null)
			{
				...
				try
				{
                    ...
                    getLogger().info("authenticate ({}, <password not shown>), user authenticated in Vista", getServiceAccountUID());
					principal.setAuthenticatedByVista(true);
				} catch (ConnectionFailedException e)
				{
                    getLogger().error("authenticate of ({}, <password not shown>), failed due to connection problem", getServiceAccountUID(), e);
					principal = null;
				} catch (InvalidCredentialsException e)
				{
                    getLogger().info("authenticate of ({}, <password not shown>), failed due to invalid credentials", getServiceAccountUID());
					principal = null;
				} catch (MethodException e)
				{
                    getLogger().error("authenticate of ({}, <password not shown>), failed due to method exception (possible change in Vista version?)", getServiceAccountUID(), e);
					principal = null;
				} finally
				{
					try
