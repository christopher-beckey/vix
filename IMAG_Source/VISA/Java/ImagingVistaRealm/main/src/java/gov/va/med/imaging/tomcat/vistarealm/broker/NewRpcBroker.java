...
implements Runnable
{
	...
	public static String getWorkstationId() {
		if(workstationId == null) {
			workstationId = NetUtilities.getUnsafeLocalHostName("DOD-VA_WebService");
	...
	public void localConnectWithoutImagingMagWin(VistaRealmSite site, VistaRealmPrincipal credentials) 
	throws ConnectionFailedException, InvalidCredentialsException, MethodException 
	{
		//logger.info("localConnectWoImaging started");
		this.credentials = credentials;
		...
		ds.setUid(credentials.getAccessCode());
		ds.setPassword(credentials.getVerifyCode());
		ds.setContext(MAG_WINDOWS_CONTEXT);
		...
		{			
			...
			connection = new VistaConnection(url);
			VistaQuery vm = new VistaQuery(RPC_SIGNON);
			//logger.info("localConnectWoImaging going to call VistaQuery "+RPC_SIGNON);
			connectAndSignon(vm);
			
			vm.clear();
			vm.setRpcName(RPC_AV_CODE);
			vm.addEncryptedParameter(VistaQuery.LITERAL, ds.getUid() + ';' + ds.getPassword());
			//logger.info("localConnectWoImaging going to call VistaQuery "+RPC_AV_CODE);
			vistaResult = connection.call(vm);
			//logger.info("localConnectWoImaging got vistaResult. "+vistaResult);
			...
			//logger.info("localConnectWoImaging calling setContext with "+MAG_WINDOWS_CONTEXT);
			setContext(MAG_WINDOWS_CONTEXT);
			//logger.info("localConnectWoImaging calling setUserProperties()");
			setUserProperties();
			//logger.info("localConnectWoImaging setting siteName and siteNumber");
			credentials.setSiteNumber(site.getSiteNumber());
			credentials.setSiteName(site.getSiteName());
		...
		{
            logger.error("RpcException: {}", rpcX);
			throw new MethodException(rpcX);
		}
		catch(InvalidCredentialsException icX) 
		{
		...
		finally 
		{
            ...
            logger.info("localConnectWoImaging() complete with success={}", success);
		}
	}
	...
	*/
	...
	throws Exception, MethodException
	{
		...
		for(retryCount=0; retryCount<FAILED_SIGNON_MAX_COUNT; ++retryCount)
		{
			...
			catch (Exception ex)
			{
                ...
                logger.warn("RPC_SIGNON failed ({}), retrying in {} milliseconds", retryCount, randomDelay);
	...
	public String runRpc(String query, String... params) 
			throws RpcException
	{
		return runRpc(query, VistaQuery.LITERAL, params);
	}
	
	public String runRpc(String query, int vistaQueryType, String... params) 
	throws RpcException
	{
		int[] qt = new int[params.length];
		Arrays.fill(qt,vistaQueryType);
		return runRpc(query, qt, params);
	}
	
	public String runRpc(String query, int[] vistaQueryType, String... params) 
			throws RpcException
	{
		VistaQuery vm = new VistaQuery(query);
		for(int i=0; i<params.length; i++) {
		   vm.addParameter(vistaQueryType[i], params[i]);
		}
	...
	public void run()
	{
		...
		{
            logger.warn("{}while disconnecting RPCBroker {}", x.getMessage(), this.toString());
		}
	}
}