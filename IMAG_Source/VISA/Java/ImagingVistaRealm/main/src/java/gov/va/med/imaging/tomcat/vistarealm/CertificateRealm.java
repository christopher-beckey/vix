...
implements Realm, org.apache.catalina.Lifecycle, AbstractVistaRealm, CertificateRealmMBean
{
	...
	{
        logger.info("{} ctor()", CertificateRealm.class.getCanonicalName());
	}
	...
	public boolean isInitialized()
	{
		...
		{
            logger.warn("VistaRealm[{}] - service account UID is not set and must be before authentication will succeed.", containerName);
			result = false;
		}
		if (getServiceAccountPWD() == null)
		{
            logger.warn("VistaRealm[{}] - service account PWD is not set and must be before authentication will succeed.", containerName);
			result = false;
		}
	...
	{
        logger.info("authenticate ({}, password)", username);
		return null;
	}
	...
	{
        logger.info("authenticate ({}, byte[])", username);
		return null;
	}
	...
	public Principal authenticate(X509Certificate certs[])
	{
		...
		for (X509Certificate cert : certs)
		{
			...
			{
                logger.warn("Authentication by certificate of {} failed due to {}", cert.getSubjectX500Principal().getName(), e.getMessage());
			} 
		...
		{
            logger.debug("User '{}' has been authenticated by X509Certificate.", certificateDistinguishedName);
