...
implements Realm, org.apache.catalina.Lifecycle, AbstractVistaRealm, javax.management.MBeanRegistration
{
	...
	// ===========================================================================================================
	private ObjectName desiredMBeanName = null;
	private ObjectName registeredMBeanName = null;
    ...
    throws Exception
    {
    	...
    	try
    	{
            getLogger().info("Realm instance '{}' being registered as '{}'.", getDesiredMBeanName().toString(), name == null ? getDesiredMBeanName().toString() : name.toString());
    	} 
    	catch (Exception e){}
    ...
    {
    	try{
            getLogger().info("Realm instance '{}' is now registered.", getDesiredMBeanName().toString()); }
    	catch (Exception e){}
    }
    ...
    {
    	try{
            getLogger().info("Realm instance '{}' being de-registered.", getDesiredMBeanName().toString()); }
    	catch (Exception e){}
    }
    ...
    {
    	try{
            getLogger().info("Realm instance '{}' has been de-registered.", getDesiredMBeanName().toString()); }
    	catch (Exception e){}
    }
		...
		if (mbs != null)
		{
			try
			{
				if(registeredMBeanName == null){
                    ...
                    getLogger().info("Realm '{}' registered as '{}'.", getDesiredMBeanName().toString(), registeredMBeanName);
				}
		...
		if (mbs != null)
		{
			try
			{
				if (registeredMBeanName != null) 
				{
                    ...
                    getLogger().info("Realm '{}' de-registered.", registeredMBeanName);
	...
	public void setSiteNumber(String localSiteNumber)
	{
		if (this.siteNumber == null)
            ...
            getLogger().error("The site number may not be changed once it has been set, attempt to change from '{}' to '{}' is being ignored.", this.siteNumber, localSiteNumber);
	}
	...
	public void start() throws LifecycleException
	{
		...
		notifyLifecycleListeners(new LifecycleEvent(this, Lifecycle.BEFORE_START_EVENT, null));
		...
		if (isInitialized())
		{
            getLogger().info("Realm[{}] initialized with authorization site {}.", this.getRealmName(), this.toString());

			notifyLifecycleListeners(new LifecycleEvent(this, Lifecycle.START_EVENT, null));
            getLogger().info("Realm [{}] is started.", this.getRealmName());
			notifyLifecycleListeners(new LifecycleEvent(this, Lifecycle.AFTER_START_EVENT, null));
		} else
            getLogger().warn("[{}] is NOT initialized, use JMX interface to set required fields and initialize.", this.getRealmName());
	...
	{
        getLogger().info("Realm[{}] stopping.", this.getRealmName());
		notifyLifecycleListeners(new LifecycleEvent(this, Lifecycle.BEFORE_STOP_EVENT, null));
		mBeanUnRegistration();
		notifyLifecycleListeners(new LifecycleEvent(this, Lifecycle.AFTER_STOP_EVENT, null));
	}
	...
	public boolean hasRole(Wrapper wrapper, Principal principal, String role)
	{
		...
		{
            getLogger().error("Expecting an instance of VistaRealmPrincipal and got an instance of {}, which VistaRealm does not understand", principal.getClass().getName());
			return false;
		}
	}
    ...
    {
        getLogger().info("Preemptively checking authorization '{}' to '{}:{}'.", principal.getName(), requestMethod, contextRequestPath);
	...
	throws IOException
	{
        ...
        getLogger().info("hasResourcePermission ({}, {},{}) rsID='{}'.", request.getRequestURI(), context.getName(), securityConstraints.length, getRealmSecurityIdentifier());
		...
		if (getLogger().isDebugEnabled())
			...
			{
                getLogger().debug("  SecurityConstraint '{}' roles", c.getDisplayName());
		...
		if(result)
		{
            getLogger().info("hasResourcePermission ({}...) rsID='{}' - permission GRANTED", requestUri, getRealmSecurityIdentifier());
		}
            ...
            Principal principal)
    {
		...
		{
            getLogger().debug("hasResourcePermission ({}...) rsID='{}' - no security constraints provided, permission GRANTED", requestUri, getRealmSecurityIdentifier());
			result = true;
		}
		else
		{
			...
			{
                getLogger().info("hasResourcePermission ({}...) rsID='{}' - bypassing security constraints for special form-based authentication resources, permission GRANTED", requestUri, getRealmSecurityIdentifier());
				result = true;
			}
			...
			if( principal == null )
			{
                ...
                getLogger().info("hasResourcePermission ({}...) rsID='{}' - no principal, permission {} on unauthenticated access.", requestUri, getRealmSecurityIdentifier(), result ? "is GRANTED" : "is NOT GRANTED");
			}
			else
			{
	...
	throws IOException
	{
        getLogger().debug("hasUserDataPermission({}) rsID='{}'", request.getRequestURI(), getRealmSecurityIdentifier());

		// Are there any security-constraint elements ?
		if( constraints == null || constraints.length == 0 )
		{
            getLogger().info("  hasUserDataPermission ({}) rsID='{}', no security constraints provided, returning true", request.getRequestURI(), getRealmSecurityIdentifier());
			return true;
		}
		...
		for( SecurityConstraint constraint : constraints )
		{
			...
			{
                getLogger().info("  hasUserDataPermission ({}) rsID='{}', applicable security constraint provided with no user constraint, returning true", request.getRequestURI(), getRealmSecurityIdentifier());
				return true;
			}
			if( Constants.NONE_TRANSPORT.equals(userConstraint) )
			{
                getLogger().info("  hasUserDataPermission ({}) rsID='{}', applicable security constraint provided with no transport security required, returning true", request.getRequestURI(), getRealmSecurityIdentifier());
				return true;
			}
		}
		...
		else
		{
			...
			if (redirectPort <= 0)
			{
                getLogger().info("  hasUserDataPermission ({}) rsID='{}', SSL redirect is disabled, returning false (HTTP 403)", request.getRequestURI(), getRealmSecurityIdentifier());
            ...
            getLogger().debug("  Redirecting request [{}] to [{}]", request.getRequestURI(), file.toString());
			response.sendRedirect(file.toString());

            getLogger().info("  hasUserDataPermission ({}) rsID='{} returning false", request.getRequestURI(), getRealmSecurityIdentifier());
			return false;
		}
            ...
            String contextRequestPath)
    {
        ...
        getLogger().debug("findSecurityConstraints({}, {}) rsID='{}'.", context.getName(), constraintKey.toString(), getRealmSecurityIdentifier());
        ...
        getLogger().debug("findSecurityConstraints({}) rsID='{}' located {} constraints in cache", constraintKey.toString(), getRealmSecurityIdentifier(), result == null ? 0 : result.length);
		...
		if( result == null )
		{
            ...
            getLogger().debug("findSecurityConstraints({}) rsID='{}'  {} security constraints defined for entire context '{}', all requests will be granted if no security constraints are defined.", constraintKey.toString(), getRealmSecurityIdentifier(), contextSecurityConstraints == null ? 0 : contextSecurityConstraints.length, context.getName());
			...
			{
                getLogger().debug("findSecurityConstraints finding security constraints applicable to path [{}] from {} constraints in context", contextRequestPath, contextSecurityConstraints.length);
                ...
                getLogger().debug("For '{}' adding {} security constraints to cache", constraintKey.toString(), contextMatchedSecurityConstraints.size());

				putToSecurityConstraintsCache(context, constraintKey, contextMatchedSecurityConstraints);

                getLogger().debug("findSecurityConstraints({}) rsID='{}' - returning {} security constraints.", constraintKey.toString(), getRealmSecurityIdentifier(), contextMatchedSecurityConstraints.size());
    	...
    	String requestMethod)
    {
		...
		{
            getLogger().debug("  Evaluating <security-constraint> '{}' against {}-{}", securityConstraint, requestMethod, requestPathInfo);
			...
			{
                getLogger().debug("  <security-constraint> '{}' has no web resources defined, ignoring ...", securityConstraint);
				continue;
			}
			...
			{
                getLogger().debug("    Checking <web-resource-collection> '{}' against {}-{}", webResourceCollection, requestMethod, requestPathInfo);
				...
				for (int urlPatternIndex = 0; urlPatternIndex < urlPatterns.length; urlPatternIndex++)
				{
                    ...
                    getLogger().debug("    Checking <url-pattern> '{}{}{}{}{}{}' against {} {}", urlPattern, patternIsDefaultMatch ? "' as default match" : "", patternIsAnyPathMatch ? "' as any path match" : "", patternIsExactMatch ? "' as exact match" : "", patternIsPathMatch ? "' as path match" : "", patternIsExtensionMatch ? "' as extension match" : "", requestMethod, requestPathInfo);

					if (patternIsDefaultMatch && webResourceCollection.findMethod(requestMethod))
					{
                        getLogger().debug("Default match '{}' found", urlPattern);
					...
					{
                        getLogger().debug("Any path pattern match '{}' found", urlPattern);
					...
					{
                        getLogger().debug("Exact pattern match '{}' found", urlPattern);
					...
					} else if (patternIsPathMatch)
					{
						        ...
						        && (!"*".equals(urlPatternPathComponents[patternComponentIndex])); ++patternComponentIndex)
						{
                            ...
                            getLogger().debug("      Checking <url-pattern> path component '{}' to request component '{}'", patternComponent, requestComponent);
						...
						if (matches)
						{
                            getLogger().debug("Path pattern match '{}' found", urlPattern);
							results.add(SecurityConstraintMatch.createPathSecurityConstraintMatch(securityConstraint, urlPattern));
						}
					} else if (patternIsExtensionMatch)
					{
						...
						{
                            getLogger().debug("Extension pattern match '{}' found", urlPattern);
