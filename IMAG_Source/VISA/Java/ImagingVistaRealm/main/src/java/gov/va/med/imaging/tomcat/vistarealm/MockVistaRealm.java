...
extends VistaAccessVerifyRealm
{
	...
	throws FileNotFoundException
	{
		...
		try {
			inStream = getClass().getClassLoader().getResourceAsStream(usernameFilename);
	...
	{
        logger.info("MockVistaRealm - authenticate ({}, <password not shown>)", username);
		...
		for(VistaRealmPrincipal principal: userList)
			...
			{
                logger.info("MockVistaRealm - authenticate ({}, <password not shown>) authenticated !", username);
				return principal;
			}

        logger.info("MockVistaRealm - authenticate ({}, <password not shown>) NOT authenticated !", username);
		return null;
	}
	...
	throws FileNotFoundException
	{
		...
		try {
			outStream = new FileOutputStream(usernameFilename);
