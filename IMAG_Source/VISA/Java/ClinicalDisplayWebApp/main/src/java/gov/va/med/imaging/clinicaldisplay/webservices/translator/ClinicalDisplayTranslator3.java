...
import gov.va.med.imaging.clinicaldisplay.webservices.soap.v3.ImageAccessLogEventTypeEventType;
...
{
	private final static Logger LOGGER = Logger.getLogger(ClinicalDisplayTranslator3.class);
	...
	public StudyFilter transformFilter(gov.va.med.imaging.clinicaldisplay.webservices.soap.v3.FilterType filterType) 
	{
		...
		if(filterType != null) 
		{
			...
			{
                LOGGER.warn("ClinicalDisplayTranslator3.transformFilter() --> Could not convert 'from date' [{}] to internal Date: {}", filterType.getFromDate(), x.getMessage());
			}
			...
			{
                LOGGER.warn("ClinicalDisplayTranslator3.transformFilter() --> Could not convert 'to date' [{}] to internal Date: {}", filterType.getToDate(), x.getMessage());
			}
	...
	throws URNFormatException 
	{
		...
		{
			LOGGER.warn("ClinicalDisplayTranslator3.transformStudyToShallowStudy() --> Setting empty procedure date for study");
	...
	throws URNFormatException 
	{
		...
		{
			LOGGER.warn("ClinicalDisplayTranslator3.transformImageToFatImage() --> Setting empty procedure date for study");
			result.setProcedureDate("");
		}
		else 
		{
	...
	public ImageAccessLogEventType transformLogEventType(ImageAccessLogEventTypeEventType eventType) {
		...
		if(eventType == ImageAccessLogEventTypeEventType.IMAGE_COPY) {
	...
	public PingServerTypePingResponse transformServerStatusToPingServerResponse(SiteConnectivityStatus siteStatus)
	{	
