...
import gov.va.med.imaging.federation.rest.types.FederationPatientIdentifierType;
import gov.va.med.imaging.federation.rest.types.FederationPatientIdentifierTypeType;
...
extends AbstractTranslator
{
	...
	static
	{
		...
		loadLevelMap.put(StudyLoadLevel.STUDY_ONLY_NOSERIES, FederationStudyLoadLevelType.STUDY_ONLY_NOSERIES);
	...
	public static FederationCprsIdentifiersType translateCprsIdentifierList(List<CprsIdentifier> cprsIdentifiers)
	{
		if(cprsIdentifiers == null)
			return null;

		String[] list = new String[cprsIdentifiers.size()];
		for(int i=0; i<cprsIdentifiers.size(); i++){
			list[i] = cprsIdentifiers.get(i).getCprsIdentifier();
		}		
		FederationCprsIdentifiersType result = 
			new FederationCprsIdentifiersType();
		
		result.setCprsIdentifiers(list);
		
		return result;
	}
	
	public static List<CprsIdentifier> translate(FederationCprsIdentifiersType cprsIdentifiers){
		if(cprsIdentifiers == null){
			return null;
		}
		
		String[] list = cprsIdentifiers.getCprsIdentifiers();
		List<CprsIdentifier> result = new ArrayList<CprsIdentifier>(list.length);
		for(int i=0; i<list.length; i++){
			result.add(new CprsIdentifier(list[i]));
		}
		return result;
	}
	...
	throws URNFormatException
	{
		if(examResult == null)
		{
			getLogger().error("FederationRestTranslator.translate(0) --> Null FederationExamResultType. Create default object to return.");
	...
	throws URNFormatException
	{
		if(exams == null)
		{
			getLogger().warn("FederationRestTranslator.translate(00) --> Received null exams, returning empty array of exams.");
	...
	throws TranslationException
	{
		...
		if((filterType.getStudyId() != null) && (filterType.getStudyId().length() > 0))
		{
			...
			{
				String msg = "FederationRestTranslator.translate(1) --> Study Id [" + filterType.getStudyId() + "] cannot be transformed into a GlobalArtifactIdentifier: " + x.getMessage();
				getLogger().error(msg);
	...
	public static FederationFilterType translate(StudyFilter filterType)
	{
		...
		if(filterType != null)
		{
			...
			filter.setIncludeAllObjects(filterType.isIncludeAllObjects());
			filter.setIncludePatientOrders(filterType.isIncludePatientOrders());
			filter.setIncludeEncounterOrders(filterType.isIncludeEncounterOrders());
			filter.setIncludeMuseOrders(filterType.isIncludeMuseOrders());
		}
		return filter;
	}
	...
	throws GlobalArtifactIdentifierFormatException
	{
		...
		if(filterType != null)
		{
			...
			if(requestedStudyIdAsString != null)
			{
				...
				{
					String msg = "FederationRestTranslator.translate(2) --> Study Id [" + requestedStudyIdAsString + "] cannot be transformed into a GlobalArtifactIdentifier: " + x.getMessage();
					getLogger().error(msg);
					throw new GlobalArtifactIdentifierFormatException(msg);
				}
			}
			else
				filter.setStudyId(null);
	...
	// #3
	//WFP-Need to fix this with respect to FederationFilterType and MaximumAllowedLevel. The current
	//	solution will not go thru the Federation because it's not a primitive.
	public static StudyFilter translate(FederationFilterType filterType)
	throws GlobalArtifactIdentifierFormatException
	{
		StudyFilter filter = new StudyFilter();
		filter.setMaximumAllowedLevel(filterType.getMaximumAllowedLevel());
		filter.setFromDate(filterType.getFromDate());
		filter.setToDate(filterType.getToDate());
		filter.setStudy_class(filterType.get_class() == null ? "" : filterType.get_class());
		filter.setStudy_event(filterType.getEvent() == null ? "" : filterType.getEvent());
		...
		if(requestedStudyIdAsString != null)
		{
			...
			{
				String msg = "FederationRestTranslator.translate(3) --> Study Id [" + requestedStudyIdAsString + "] cannot be transformed into a GlobalArtifactIdentifier: " + x.getMessage();
				getLogger().error(msg);
				throw new GlobalArtifactIdentifierFormatException(msg);
			}
		}
	...
	throws TranslationException
	{
		...
		{
			getLogger().error("FederationRestTranslator.translate(4) --> Cannot translate null FederationDocumentSetResultType.  Create default object to return.");
	...
	throws TranslationException
	{
		...
		{
			String msg = "FederationRestTranslator.translate(5) --> Unable to create Unable to create OID from home community Id [" + documentSet.getHomeCommunityId() + "]: " + oidfX.getMessage();
			getLogger().error(msg);
			throw new TranslationException(msg);
		}
	}
	...
	throws TranslationException
	{
		...
		{
			String msg = "FederationRestTranslator.translate(6) --> Unable to create GlobalArtifactIdentifier from identifier [" + document.getIdentifier() + "]: " + urnfX.getMessage();
			getLogger().error(msg);
			throw new TranslationException(msg);
		}
		catch(URNFormatException urnfX)
		{
			String msg = "FederationRestTranslator.translate(6) --> Unable to create PatientArtifactIdentifier from identifier [" + document.getIdentifier() + "] and ICN [" + documentSet.getPatientIcn() + "]: " + urnfX.getMessage();
			getLogger().error(msg);
			throw new TranslationException(msg);
		}
	}
	...
	throws TranslationException
	{
		...
		{
			getLogger().warn("FederationRestTranslator.translate(7) --> Cannot translate null FederationStudyResultType.  Create default object to return.");
	...
	throws TranslationException
	{
		...
		try
		{
			study = Study.create(ObjectOrigin.UNKNOWN, studyType.getSiteNumber(), 
					...
					studyLoadLevel, 
					translate(studyType.getStudyDeletedImageState()), studyType.isStudyInNewDataStructure());
		}
		catch (URNFormatException x)
		{
			String msg = "FederationRestTranslator.translate(8) --> Unable to create a Study from the given key elements: " + x.getMessage();
			getLogger().error(msg);
			throw new TranslationException(msg);
		}
		if((studyType.getAlternateArtifactId() != null) && (studyType.getAlternateArtifactId().length() > 0))
		{
			...
			{
                getLogger().warn("FederationRestTranslator.translate(8) --> GlobalArtifactIdentifierFormatException creating alternate artifact ID for study from value [{}]: {}", studyType.getAlternateArtifactId(), gaifX.getMessage());
			}
		}
		...
		if(studyType.getAlternateExamNumber() != null)
		{
			study.setAlternateExamNumber(studyType.getAlternateExamNumber());
		}
		else
		{
			study.setAlternateExamNumber("");
		}
		...
		study.setGroupIen(studyType.getGroupIen());
		study.setContextId(studyType.getContextId());
		study.setStudyInNewDataStructure(studyType.isStudyInNewDataStructure());
		study.setAccessionNumber((studyType.getAccessionNumber()==null) ? "" : studyType.getAccessionNumber());
		//getLogger().debug("Got Study "+study.getStudyUid()+" for patient "+study.getPatientId()+" accessionNumber="+studyType.getAccessionNumber());
		...
		study.setNumberOfDicomImages(studyType.getNumberOfDicomImages());
	...
	throws TranslationException
	{
		...
		FederationStudyType result = new FederationStudyType();
		...
		result.setStudyInNewDataStructure(study.isStudyInNewDataStructure());
		result.setAccessionNumber((study.getAccessionNumber()==null) ? "" : study.getAccessionNumber());
		//getLogger().debug("Study "+study.getStudyIen()+" for patient "+result.getPatientIcn()+" accessionNumber="+result.getAccessionNumber()+" altExamNumber="+result.getAlternateExamNumber());
		if(study.getAlternateArtifactIdentifier() != null)
		{
			result.setAlternateArtifactId(study.getAlternateArtifactIdentifier().toString());
		}
		
		result.setDescription(study.getDescription() == null ? "" : study.getDescription());
		...
		//WFP-I want to know why this now works correctly when I removed the exception.
		//	The study still contains the firstImage Image object.
		//	Yet at this point it is null.
		if(study.getFirstImage() == null)
			getLogger().warn("FederationRestTranslator.translate(9) --> First Image is null");
		...
		if(study.getModalities() != null)
		{
			...
			result.setStudyModalities(modalities);
		}
	...
	public static FederationSeriesType [] translate(Set<Series> seriesSet) 
	...
	public static FederationSeriesType translate(Series series)
	{
		...
		FederationSeriesType result = new FederationSeriesType();
		FederationImageType [] instances = new FederationImageType[series.getImageCount()];
	...
	//throws TranslationException
	{
		...
		try
		{
			if(instanceType.isImageInNewDataStructure()){
				image = Image.create(
						...
						instanceType.getFullArtifactIen(),
						instanceType.getAbsArtifactIen(),
						instanceType.getImageModality(),
						instanceType.isImageInNewDataStructure()
					);				
			}
			else{
				image = Image.create(
		...
		{
            getLogger().error("FederationRestTranslator.translate(10) --> Unable to create an Image instance from the given key elements: {}", x.getMessage());
			return null;
		}
		...
		image.setAccessionNumber(instanceType.getAccessionNumber() == null ? "" : "" + instanceType.getAccessionNumber());

//		if(instanceType.getDescription() != null){
//			String descriptionElement = instanceType.getDescription();
//			
//			image.setDescription(descriptionElement);
//			
//			String[] parts = StringUtil.split(descriptionElement, StringUtil.RIGHT_BRACKET);
//			String altExamNumber = parts[0];
//			altExamNumber = altExamNumber.substring(1).trim();
//			image.setAlternateExamNumber(altExamNumber);
//			logger.debug("WFP: Exam#: "+altExamNumber);
//		}
//		else{
...
//		}
		...
		if(instanceType.getDescription() != null)
		{
			...
			if (parts.length > 0)
			{
				...
				altExamNumber = altExamNumber.substring(1).trim();
				image.setAlternateExamNumber(altExamNumber);
                getLogger().debug("FederationRestTranslator.translate(10) --> Set alt exam # [{}]", altExamNumber);
			}
		}
		...
		image.setImageInNewDataStructure(instanceType.isImageInNewDataStructure());
		image.setAbsArtifactIen(instanceType.getAbsArtifactIen());
		image.setFullArtifactIen(instanceType.getFullArtifactIen());
		image.setAbsDiskVolumeIen(instanceType.getAbsDiskVolumeIen());
		image.setFullDiskVolumeIen(instanceType.getFullDiskVolumeIen());
		image.setFullFilespec(instanceType.getFullFilespec());
		image.setAbsFilespec(instanceType.getAbsFilespec());
		image.setFullFilepath(instanceType.getFullFilepath());
		image.setAbsFilepath(instanceType.getAbsFilepath());
		image.setAccessionNumber(instanceType.getAccessionNumber());
	...
	public static FederationImageType translate(Image image)
	{
		...
		{
			getLogger().warn("FederationRestTranslator.translate(11) --> Setting null procedure date for image");
		...
		result.setAbsArtifactIen(image.getAbsArtifactIen());
		result.setFullArtifactIen(image.getFullArtifactIen());
		...
		result.setImageInNewDataStructure(image.isImageInNewDataStructure());
		
		
		result.setAbsDiskVolumeIen(image.getAbsDiskVolumeIen());
		result.setFullDiskVolumeIen(image.getFullDiskVolumeIen());
		result.setFullFilespec(image.getFullFilespec());
		result.setAbsFilespec(image.getAbsFilespec());
		result.setFullFilepath(image.getFullFilepath());
		result.setAbsFilepath(image.getAbsFilepath());
		
		return result;
		
	}
	
	public static FederationPatientSensitiveType translate(PatientSensitiveValue patientSensitiveValue)
	{
	...
	public static PatientSensitiveValue translate(FederationPatientSensitiveType patientSensitiveValue)
	{
	...
	private static FederationPatientSensitivityLevelType translate(PatientSensitivityLevel patientSensitivityLevel)
	{
	...
	private static PatientSensitivityLevel translate(FederationPatientSensitivityLevelType patientSensitivityLevel)
	{
		for( Entry<PatientSensitivityLevel, FederationPatientSensitivityLevelType> entry : FederationRestTranslator.patientSensitiveLevelMap.entrySet() )
	...
	public static List<WorkItem> translate(FederationWorkItemType[] workItems) 
	{
		if(workItems == null)
			return null;
		
		List<WorkItem> result = new ArrayList<WorkItem>();
		for(FederationWorkItemType workItemType : workItems)
		{
			result.add(translate(workItemType));
		}
		return result;
	}


	private static WorkItem translate(FederationWorkItemType workItemType) 
	{
		WorkItem item = new WorkItem(
				workItemType.getType(), 
				workItemType.getSubtype(),
				workItemType.getStatus(),
				workItemType.getPlaceId(),
				workItemType.getCreatingUser(),
				workItemType.getCreatingApplication());
		
		item.addTag("patienticn", workItemType.getPatientIcn());
		item.addTag("contextid", workItemType.getContextId());
		return item;
	}


	public static FederationWorkItemType[] translateWorkItems(List<WorkItem> workItems) 
	{
		if(workItems == null)
			return null;
		
		FederationWorkItemType[] result = new FederationWorkItemType[workItems.size()];
		int i = 0;
		for(WorkItem workItem : workItems)
		{
			result[i] = translate(workItem);
			i++;
		}
		return result;
	}
	
	private static FederationWorkItemType translate(WorkItem workItem) 
	{
		if (workItem == null)
			return null;
		...
		if ((workItem.getTags() != null) && (workItem.getTags().getTags() != null))
		{
			for (WorkItemTag tag: (List<WorkItemTag>) workItem.getTags().getTags())
			{
				if (tag.getKey().equals("patienticn"))
				{
					patientIcn = tag.getValue();
				}
				else if (tag.getKey().equals("contextid"))
				{
					
					contextId = tag.getValue();
				}
			}
		}
		
		FederationWorkItemType item = new FederationWorkItemType(
				workItem.getType(), 
				workItem.getSubtype(),
				workItem.getStatus(),
				workItem.getPlaceId(),
				workItem.getCreatingUser(),
				workItem.getCreatingApplication(),
				patientIcn, contextId);
		
		return item;
	}
	
	public static FederationPatientIdentifierType translate(PatientIdentifier value){
		
		FederationPatientIdentifierType type = new FederationPatientIdentifierType();
		type.setPatientIdentifierSource(value.getPatientIdentifierSource());
		type.setPatientIdentifierType(translate(value.getPatientIdentifierType()));
		type.setValue(value.getValue());
		
		return type;
	}

	public static RestStringType translate(AbstractImagingURN value){
		RestStringType type = new RestStringType(value.toString());

		return type;
	}


	public static FederationNamespaceIdentifierType translate(NamespaceIdentifier value) {
		
		FederationNamespaceIdentifierType type = new FederationNamespaceIdentifierType();
		type.setNamespace(value.getNamespace());
		
		return type;
	}

	private static FederationPatientIdentifierTypeType translate(PatientIdentifierType value) {
		
		FederationPatientIdentifierTypeType type = new FederationPatientIdentifierTypeType();
		type.setLocal(value.isLocal());
		type.setRequiresSiteIdentifier(value.isRequiresSiteIdentifier());
	...
	public static RestBooleanReturnType translate(Boolean value){
		return new RestBooleanReturnType(value.booleanValue());
	}
	...
	public static boolean translate(RestBooleanReturnType value){
		return value.isResult();
	}
	
	public static Boolean translatedBooleanClass(RestBooleanReturnType value){
		return new Boolean(value.isResult());
	}
	...
	public static PatientPhotoIDInformation translate(FederationPatientIdentificationImageInformationType patientPhotoIdInformation) 
	throws URNFormatException
	{
		ImageURN imageUrn = ImageURN.create(
					patientPhotoIdInformation.getSiteId(), 
					patientPhotoIdInformation.getImageIen(), 
					patientPhotoIdInformation.getStudyIen(),
					patientPhotoIdInformation.getPatientIcn());
		
		PatientIdentifier patientIdentifier = PatientIdentifier.icnPatientIdentifier(patientPhotoIdInformation.getPatientIcn());
		String patientName = patientPhotoIdInformation.getPatientName();
		Date dateCaptured = patientPhotoIdInformation.getDateCaptured();
		ImageFormat imageFormat = ImageFormat.fromExtension(patientPhotoIdInformation.getImageExtension());

		return new PatientPhotoIDInformation(
				imageUrn,
				patientIdentifier,
				patientName, 
				dateCaptured, 
				imageFormat, 
				patientPhotoIdInformation.getFilename());
	}


	public static FederationPatientIdentificationImageInformationType translate(PatientPhotoIDInformation routerResult) 
	{
		FederationPatientIdentificationImageInformationType info = new FederationPatientIdentificationImageInformationType();
		info.setDateCaptured(routerResult.getDateCaptured());
		info.setFilename(routerResult.getFilename());
		info.setPatientIcn(routerResult.getPatientIdentifier().getValue());
		info.setPatientName(routerResult.getPatientName());
		info.setImageFormat(routerResult.getImageFormat().name());
		info.setImageExtension(routerResult.getImageFormat().getExtension());
		info.setSiteId(routerResult.getImageUrn().getOriginatingSiteId());
		info.setStudyIen(routerResult.getImageUrn().getStudyId());
		info.setImageIen(routerResult.getImageUrn().getImageId());
