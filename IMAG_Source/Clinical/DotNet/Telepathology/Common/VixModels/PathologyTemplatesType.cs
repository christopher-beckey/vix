// -----------------------------------------------------------------------
// <copyright file="PathologyTemplatesType.cs" company="Department of Veterans Affairs">
//  Package: MAG - VistA Imaging
//  WARNING: Per VHA Directive 2004-038, this routine should not be modified.
//  Date Created: Jul 2012
//  Site Name:  Washington OI Field Office, Silver Spring, MD
//  Developer: Duc Nguyen
//  Description: VIX Object to hold a list of template strings
//        ;; +--------------------------------------------------------------------+
...
namespace VistA.Imaging.Telepathology.Common.VixModels
{
    ...
    public class PathologyTemplatesType : INotifyPropertyChanged
    {
...
#pragma warning restore 0067
        ...
        public ObservableCollection<ReportTemplate> GetTemplateObjects()
        {
            ObservableCollection<ReportTemplate> templates = new ObservableCollection<ReportTemplate>();

            if (this.Items != null)
            {
                foreach (string template in Items)
                {
                    string trimData = template.Replace("\n", string.Empty);
                    ReportTemplate temp = ReportTemplate.DeserializeTemplateFromXML(trimData);
                    if (temp != null)
                    {
                        templates.Add(temp);
                    }
