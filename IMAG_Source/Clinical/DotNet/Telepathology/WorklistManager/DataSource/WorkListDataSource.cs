...
namespace VistA.Imaging.Telepathology.Worklist.DataSource
{
    public class WorkListDataSource : IWorkListDataSource
    {
        ...
        public string SiteName { get { return "Salk Lake City"; } }
        ...
        public List<string> GetInterpretationList(string siteID, string accessionNumber)
        {
            List<string> interpretationList = new List<string>();
            if ((siteID == "") || (accessionNumber == ""))
                return interpretationList;

            if (vistaClient != null)
                interpretationList = vistaClient.GetInterpretationList(siteID, accessionNumber);
