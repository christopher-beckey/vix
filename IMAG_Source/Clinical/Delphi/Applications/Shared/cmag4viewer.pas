...
Uses
  ...
  cmag4ViewerImageProxy,
  cMag4Vgear,
  cMag4ViewerEventScrollBox,
  cMagImageList,
  cMagSecurity,
  cMagUtilsDB,
  cMagVUtils,
...
Type
  ...
  Private
    ...
    FAutoRedraw: Boolean;
    FPanWindow: Boolean;
    FAnnotationsEnabled: Boolean;    {p129 - annot info}
    FZoomWindow: Boolean;
        //    FAbsWindow:         Boolean;
    ...
    FPrevTool : TMagImageMouse; {/p122 dmmn 8/25 - keep track of the previous mouse before annotation /}
    ...
    FImageChangedEvent: TMagImageChangedEvent; // JMW 4/17/2013 P131
...
//Was for RCA decouple magmsg, not now.    procedure MyLogMsg(msgType, msg: String; Priority: TMagMsgPriority = magmsgINFO);
  ...
  Public
    ...
    {/ P122 - JK 5/4/2011 /}
    procedure ShowAnnotToolbar;          {p129 - annot info}
    ...
    Function RGBChanger(CurrentState: Integer; ApplyAll : Boolean) : Integer; //p122rgb dmmn 12/12/11
...
//    Procedure UnSelectAll;
    ...
    Procedure SetCurrentTool(Value: TMagImageMouse);
     ...
     {  also above   procedure ShowAnnotToolbar;}
...
{/gek p117  added for the display of image status on the Abstracts}
    ...
    {/ P122 - JK 6/9/2011 /}
    procedure HideAnnotationToolbar;        {p129 - annot info}

    procedure SetDisplayedImage(ImageIndex : Integer; SetSelected: Boolean = True);

    // JMW 4/18/2013 P131 - an image has been selected, need to draw scout lines on other displayed images
    procedure DrawScoutLineFromSelectedImage(StudyRootImage: TImageData);
  ...
  Published
    ...
    // JMW 4/17/2013 p131 - event for when the selected image is changed
...
Begin
  ...
  FPrevTool := FCurrentTool; {/p122 dmmn 8/25 - mouse tracking /}
...
Begin
    ...
    //    ']');
  magAppMsg('s', 'ScrollAndDisplayImage proxyindex=[' + Inttostr(Proxyindex) +
    ']'); 
...
End;
 {7/12/12 gek Merge 130->129}
function TMag4Viewer.RGBChanger(CurrentState: Integer; ApplyAll : Boolean): Integer;
Var
  i: Integer;
  Iproxy: TImageProxy;
begin
  ...
  begin
    ...
    Begin
      Iproxy := FProxyList[i];
      If Iproxy.Image <> Nil Then
        Result := TMag4VGear(Iproxy.Image).RGBChanger(CurrentState,ApplyAll);
    End
  end
...
{/ P122 - JK 6/9/2011 /}
procedure TMag4Viewer.HideAnnotationToolbar;     {p129 - annot info}
var
  i: Integer;
begin
  for i := 0 To FProxyList.Count - 1 Do
    if TImageProxy(FProxyList[i]).Image <> nil then
      if TImageProxy(FProxyList[i]).Image.AnnotationComponent <> nil then
        if TImageProxy(FProxyList[i]).Image.AnnotationComponent.Visible then
        begin
          TImageProxy(FProxyList[i]).Image.CheckIfAnnotsModified;
          TImageProxy(FProxyList[i]).Image.AnnotationComponent.Hide;
        end;
end;
...
//  HideAnnotationToolbar; {/ P122 - JK 5/4/2011 /}
...
Begin
  ...
  If FProxyList.Count = 0 Then
    ...
    //p117  MagLogger.LogMsg('s', '###Realign Proxy'); 
  magAppMsg(magmsglvlDEBUG,'###Realign Proxy');
...
(*    if isAbs then
    ...
    Begin
      ...
      Begin
        ...
        magAppMsg('s', 'About to get text file for image [' +
          Proxy.ImageData.Mag0 + ']'); 
      ...
      End;
            ...
            //    proxy.ImageData.Mag0 + ']');
      magAppMsg('s', 'About to load DICOM header information for image [' +
        Proxy.ImageData.Mag0 + ']'); 
      ...
      VGear.LoadRADAnnotations();       //p122t11
      ...
      if TransferResult.FTransferStatus = IMAGE_COPIED then  {/ P117 JK 1/24/2011 - don't register a LogImageAccess event if the image wan not loaded /}
        ...
        Begin
          MagImageAccessLogManager1.LogImageAccess(Proxy.ImageData);
        End;
  ...
  If FViewStyle = MagViewerViewAbs Then
    if VGear <> nil then
      ...
      Proxy.Image.ShowAnnotIndicator;
...
Begin
  ...
  Begin
    ...
    result.SetPanWindowHolder(FPanWindowHolder);
  End
  Else
    If FViewStyle = MagViewerViewFull Then
    Begin
...
*)
  ...
  {/P122 - JK 1/23/2012 - turn off the Annotation Icon if this is radiology /}
  if GearAbility = MagGearAbilityRadiology then
    Result.pnlAnnotIcon.visible := False;
  Result.SmoothImage;
  Result.AutoRedraw := True;
  {129t10 PanWinError ...  capture needs this callback.}
...
Procedure TMag4Viewer.SetPanWindowWithActivateOption(Value: Boolean; Activate: Boolean);
Var
  x, y: Integer;
  Viewerpt, Imagept: TPoint;
  h, w: Integer;
Begin
  FPanWindow := Value;        {Display, each  click,  not in capture PanWinError}
    //  PanCloseAll;
  ...
  If (Value) And (Activate) Then
    Cur4Image.PanWindowSettings(h, w, x, y)           {Display each click, Not Capture PanWinError}
  Else
...
Begin
  SetPanWindowWithActivateOption(Value, True);                 {Display, Not Capture?  PanWinError}
...
Begin
  ...
  {/ P122 - JK 7/18/2011 - removed this because it was causing an exception with application shuts down.
     This is likely a memory leak but it needs investigation. I think the freeing of the annotation component
     if not automatic as its parent is destroy, should be elsewhere. /}
//  {/ P122 - JK 6/9/2011 /}
//  if VGear.AnnotationComponent <> nil then
//    VGear.AnnotationComponent.Close;
...
Begin
{debug94} magAppMsg('s', '**--** - in TMag4Viewer.ClearViewer.' + ' Fproxylist.count: ' + Inttostr(FProxyList.Count));
  If FProxyList.Count = 0 Then
    Exit;
  UnDoGroupingNew;
  PnlTop.caption := '';
  ...
  ImgStudyStatus.Visible := False;
{debug94} magAppMsg('s', '**--** - END TMag4Viewer.ClearViewer.' + ' Fproxylist.count: ' + Inttostr(FProxyList.Count));
End;
...
{This is called by the cMag4VGear in Display... doesn't get called from Capture.
   could be the Pan Window issue     PanWinError}
Procedure TMag4Viewer.ImageVMouseDown(Sender: Tobject; Button: TMouseButton;
  Shift: TShiftState; x, y: Integer);
Var
  clpt: TPoint;
Begin                          {  Here in Display  PanWinError}
  ...
  Begin
    ...
    Begin
      ImageVClick(Sender, TMag4VGear(Sender));           {display, and it is defined.    PanWinError} 
    End;
  End;
...
function TMag4Viewer.UnSelectAll : Boolean;
Var
  i: Integer;
  Proxy: TImageProxy;
Begin
  ...
  Begin
    ...
    If (Proxy.Image <> Nil) Then
      ...
      begin
        ...
        {/ P122 - JK 5/9/2011 /}
        if Proxy.Image.AnnotationComponent <> nil then
          if Proxy.Image.AnnotationComponent.Showing then
          begin
            {Check if annotations need to be saved in the present Gear windows before moving on}
//            Proxy.Image.AnnotationComponent.SaveAnnotationLayer(False);  //p122t5 require reload before hide
//            Proxy.Image.AnnotationComponent.Hide;
            if Proxy.Image.CheckIfAnnotsModified then
            begin
              Result := true;
              Proxy.Image.AnnotationComponent.Hide;
            end
            else     // user decided to cancel out
            begin
              Proxy.Image.Selected := True;           //p122t6 dmmn 10/18
              Result := false;
              Exit;
            end;
          end;
...
//        if Cur4Image.AnnotationComponent.AnnotsModified then
        begin
          // cancel save
//          Cur4Image.CheckIfAnnotsModified
...
//          Cur4Image.AnnotationComponent.SaveAnnotationLayer(True);
...
//p117  MagLogger.LogMsg('s', '###ScrollToImage index ' + Inttostr(ImageIndex)); 
magAppMsg(magmsglvlDEBUG,'###ScrollToImage index ' + Inttostr(ImageIndex)); //p117
...
Begin
  nm := Self.Name;
{debug94}
  magAppMsg('s', '**--** - in TMag4Viewer.UpDate_. ' + 'Name : ' + nm);
  magAppMsg('s', '**--** - in TMag4Viewer.UpDate_. ' + ' SubjectState: ' + SubjectState);
  ClearViewer;
  ...
  CheckButtons();
{debug94} magAppMsg('s', '**--** - END TMag4Viewer.UpDate_. ' + ' SubjectState: ' + SubjectState);
End;
...
Begin
{debug94} magAppMsg('s', '**--** - TMag4Viewer.AttachMyself ');
  If Assigned(FMagImageList) Then
  Begin
...
Begin
  Mag4ViewerOptions.Execute(Self);
End;
...
Procedure TMag4Viewer.ImagePrint;       {BM-ImagePrint-  TMag4Viewer calls TMagUtilsDB.ImagePrint}
Var
  Stat: Boolean;
  Xmsg: String;
Begin
  ...
  FMagUtilsDB.ImagePrint(Cur4Image, Stat, Xmsg);     {BM-ImagePrint- calls TMagUtilsDB.ImagePrint}
End;
...
Begin
  ...
  Else //if (SelectedCount > 0) then
    ...
    End;
                (*
  {/ P122 - JK 8/2/2011 - find the selected proxy and if there are
     annotations attached, refresh the current annotations.  It is possible that
     prior to reseting the image(s), the annotation view was on a history layer /}
  for i := 0 To FProxyList.Count - 1 Do
    if TImageProxy(FProxyList[i]).Image <> nil then
      if TImageProxy(FProxyList[i]).Image.AnnotationComponent <> nil then
        if TImageProxy(FProxyList[i]).Image.AnnotationComponent.Visible then
        begin
          TImageProxy(FProxyList[i]).Image.AnnotationComponent.LoadCurrentAnnotations;
        end;
        *)
...
Var
  i: Integer;
Begin
{debug94} magAppMsg('s', '**--** - in TMag4Viewer.ReFreshImages. ' + ' count: ' + Inttostr(FLoadedList.Count - 1));
  For i := 0 To FLoadedList.Count - 1 Do
  Begin
    TImageProxy(FLoadedList[i]).Image.RefreshImage;
  End;
{debug94} magAppMsg('s', '**--** - END TMag4Viewer.ReFreshImages. ' + ' count: ' + Inttostr(FLoadedList.Count - 1));
      (*
  {/P122t2 - dmmn 9/8/2011 - find the selected proxy and if there are
     annotations attached, refresh the current annotations.  It is possible that
     prior to reseting the image(s), the annotation view was on a history layer /}
  for i := 0 To FLoadedList.Count - 1 Do
    if TImageProxy(FLoadedList[i]).Image <> nil then
      if TImageProxy(FLoadedList[i]).Image.AnnotationComponent <> nil then
        if TImageProxy(FLoadedList[i]).Image.AnnotationComponent.Visible then
        begin
          TImageProxy(FLoadedList[i]).Image.AnnotationComponent.LoadCurrentAnnotations;
        end;        *)
End;
    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          VGear.DrawRadLetters();
          VGear.LoadRADAnnotations(); //p122t11
          VGear.SetAnnotationStyle(FAnnotationStyle);       {p129 - annot info}
          VGear.SetCurrentTool(FCurrentTool);
...
var
  i: Integer;
begin
  {/ P122 - JK 8/22/2011 - don't allow ApplyToAll while in annotation mode /}
  for i := 0 To FProxyList.Count - 1 Do
    if TImageProxy(FProxyList[i]).Image <> nil then
      if TImageProxy(FProxyList[i]).Image.AnnotationComponent <> nil then
        if TImageProxy(FProxyList[i]).Image.AnnotationComponent.Visible then
...
Begin
  Result := FAnnotationsEnabled;   {p129 - annot info}
End;
...
Begin
  ...
  Case FCurrentTool Of
    ...
    MactWinLev:
      AutoWinLevel();
    MactAnnotation:
      ShowAnnotationToolbar;       {p129 - annot info}
    MactMeasure:
      Measurements();
    MactProtractor:
      Protractor();
    MactAnnotationPointer:
      AnnotationPointer();          {p129 - annot info}
  End; {case}
End;

Procedure TMag4Viewer.ShowAnnotationToolbar;           {p129 - annot info}
Begin
  ...
  If Cur4Image = Nil Then
    Exit;
  Cur4Image.ShowAnnotationToolbar;        {p129 - annot info}
End;
...
Begin
  ...
  if (Tool <> MactAnnotationPointer) and (Tool <> MactAnnotation) then
    FPrevTool := FCurrentTool; {/p122 dmmn 8/25 - mouse tracking /}
...
{/ P122 - JK 5/4/2011 /}
procedure TMag4Viewer.ShowAnnotToolbar;            {p129 - annot info}
begin
  if Cur4Image <> nil then
    Cur4Image.btnAnnotationsClick(Self);         {p129 - annot info}
end;
...
Begin
  If Assigned(FPanWindowClose)
    Then  FPanWindowClose(Sender);
End;
/////////////////////////////
...
Begin
  If FCur4Image = Value Then
    Exit;

//  {/ P122 - JK 10/18/2011 - check if the current image is changing. If it is and the
//    annotation toolbar is open, see if annotation changes need to be saved before
//    moving on. /}
//  if FCur4Image <> nil then
//    if not FCur4Image.AbstractImage then
//      if FCur4Image.AnnotationComponent <> nil then
//        if FCur4Image.AnnotationComponent.AnnotsModified then
//          FCur4Image.AnnotationComponent.SaveAnnotationLayer(True);
  ...
  if FCur4Image = nil then
    Exit;
  if FCur4Image.AnnotationComponent <> nil then      {p129 - annot info}
  begin
    FCur4Image.AnnotationComponent.IGArtXCtl.OnCreateMarkNotify := FCur4Image.AnnotationComponent.FIGArtXCtlCreateMarkNotify;
    FCur4Image.AnnotationComponent.IGArtXCtl.OnDeleteMarkNotify := FCur4Image.AnnotationComponent.FIGArtXCtlDeleteMarkNotify;
    FCur4Image.AnnotationComponent.IGArtXCtl.OnModifyMarkNotify := FCur4Image.AnnotationComponent.FIGArtXCtlModifyMarkNotify;
    FCur4Image.AnnotationComponent.IGArtXCtl.OnPreCreateMarkNotify := FCur4Image.AnnotationComponent.FIGArtXCtlPreCreateMarkNotify;
  end;

  If Assigned(FImageChangedEvent) then
  begin
    FImageChangedEvent(self, FCur4Image);
  end;
End;
...
procedure TMag4Viewer.SetDisplayedImage(ImageIndex : Integer; SetSelected: Boolean = True);
Var
  State: TMagImageState;
Begin
  State := Cur4Image.GetState;
...
// JMW 4/18/2013 p131
{ The currently selected image was selected (already), now try to use that
  image to draw scout lines on other images}
procedure TMag4Viewer.DrawScoutLineFromSelectedImage(StudyRootImage: TImageData);
var
  currentImage : TMag4VGear;
  Proxy : TImageProxy;
  i : integer;
  currentImageData, iobj : TImageData;
  scoutLine : TMagScoutLine;
begin
  currentImage := GetCur4Image;
  currentImageData := currentImage.PI_ptrData;
  ...
  begin
    ...
    begin
      Proxy := FProxyList[i];
      If (Proxy.Image <> Nil)
        And (Proxy.Image.ImageLoaded)
        And (Proxy.Image.Visible)
        And (Proxy.ImageData <> nil) Then
      begin
        iobj := proxy.ImageData;
        if not iobj.IsSame(currentImageData) then
        begin
          // not the currently selected image
          scoutLine := uMagRadHeaderLoader.GetXRefLine(currentImageData,
            StudyRootImage, iobj, StudyRootImage);
          if scoutLine <> nil then
          begin
            Proxy.Image.DrawScoutLine(scoutLine);
          end
          else
          begin
            Proxy.Image.HideScoutLine();
          end;
        end;
      end;
    end;
...
procedure TMag4Viewer.HideScoutLines();
var
  currentImage : TMag4VGear;
  Proxy : TImageProxy;
  i : integer;
  currentImageData : TImageData;
begin
  currentImage := GetCur4Image;
  currentImageData := currentImage.PI_ptrData;
  if currentImage <> nil then
...
Procedure TMag4Viewer.SetScoutLineColor(Color : TColor);
var
  Proxy : TImageProxy;
  i : integer;
begin
  ...
  begin
    ...
    If (Proxy.Image <> Nil) Then
    begin
      Proxy.Image.SetScoutLineColor(Color);
    end;
  end;
  proxy := nil;
end;

Procedure TMag4Viewer.SetPanWindowHolder(PanWindowHolder : TMagPanWindowHolder);
begin
