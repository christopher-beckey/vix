...
Type
  ...
  /// this was in 129...not in MAIN    TMagImageChangedEvent = Procedure(Sender: Tobject; MagImage: TMag4VGear) Of Object;
  ...
  {/ P122 - JK 7/28/2011 - signal the frmRadViewer form to close the other stack viewer's annotation toolbar, if open /}
  TMagAnnotateButtonClickEvent = Procedure(Sender: TObject) of Object;
  ...
  TMag4StackViewer = Class(TFrame, IMag4Viewer, IMagRemoteinterface)
    ...
    procedure btnAnnotationsClick(Sender: TObject);
  Private
    ...
    {/ P122 - JK 7/28/2011 /}
    FAnnotationButtonClickEvent: TMagAnnotateButtonClickEvent;
    ...
    FCanShowAnnotationComponent : Boolean;  //p122t6

    // JMW 4/16/2013 P131
    FShowSeriesDropDown : Boolean;
    FImageMouseUp: TImageMouseUpEvent;
    FRootImage : TImageData;
    FShowScrollStartStop : Boolean;
...
//Was for RCA decouple magmsg, not now.        procedure MyLogMsg(msgType, msg: String; Priority: TMagMsgPriority = magmsgINFO);  //RCA
   ...
   // procedure LogMsg(MsgType : String; Msg : String; Priority : TMagLogPriority = MagLogINFO);  {JK 10/6/2009 - Maggmsgu refactoring - remove old method}
    ...
    procedure HideAnnotationToolbarStack;
    // JMW 4/16/2013 P131
    procedure setShowSeriesDropDown(Value : Boolean);
    procedure setImageMouseUpEvent(Value : TImageMouseUpEvent);
    procedure setShowScrollStartStop(Value : Boolean);
  Public
 ...
 {7/12/12 gek Merge 130->129}
    function RGBChanger(CurrentState: Integer; ApplyAll : Boolean) : Integer; //p122rgb
    ...
    Procedure ShowAnnotationToolbar;
    ...
    procedure SetDisplayedImage(ImageIndex : Integer; SetSelected: Boolean = True);
    { Another image has been selected (SelectedImage),
      determine if the currently displayed image is associated and draw the Scout Line}
    procedure DrawScoutLineFromSelectedImage(SelectedImage : TImageData;
      SelectedImageRootImage : TImageData);

    procedure HideScoutLines();
  ...
  Published
    ...
    {/ P122 - JK 7/28/2011 /}
    property OnAnnotateButtonClick: TMagAnnotateButtonClickEvent read FAnnotationButtonClickEvent write FAnnotationButtonClickEvent;
    ...
    Property CanShowAnnotToolbar : Boolean Read FCanShowAnnotationComponent Write FCanShowAnnotationComponent; //p122t6

    // JMW 4/16/2013 P131
    property ShowSeriesDropDown : Boolean read FShowSeriesDropDown write setShowSeriesDropDown;
    Property OnImageMouseUp: TImageMouseUpEvent Read FImageMouseUp Write setImageMouseUpEvent;
    Property ShowScrollStartStop : Boolean read FShowScrollStartStop write setShowScrollStartStop;
  End;
...
(*  procedure TMag4StackViewer.MyLogMsg(msgType, msg: String; Priority: TMagMsgPriority = magmsgINFO);
begin
...
Begin
  Inherited;
  magAppMsg('s','*** TMag4StackViewer.Create SELF = ' + IntToHex(Integer(Pointer(Self)),0) + ' ***');
...
Begin
  ...
  // JMW 5/1/2013 P131
  // this component automatically attaches itself during creation so detach it
  // during destroy to ensure it is no longer referenced after it is destroyed
...
Procedure TMag4StackViewer.OpenStudy(RootImg: TImageData; ImgList: Tlist;
  Studydesc: String; ImgIndex: Integer = -1; AppendImages: Boolean = False);
Begin
...
Begin
  ...
  If (cboSeries.Items.Count > 1) and (FShowSeriesDropDown) Then
  Begin
...
Var
  ...
  GetTxtFile: Boolean;
Begin
  FCanShowAnnotationComponent := true;
  {/ P122 - JK 7/24/2011 - When replacing images in the Gear, check if in annotation mode and if annotations need to be saved
     before moving on. /}
  if FFirstImageLoaded then
    if Mag4VGear1 <> nil then
      if Mag4VGear1.AnnotationComponent <> nil then
      begin
        if Mag4VGear1.AnnotationComponent.IsAnnotationMode and (not Mag4VGear1.AnnotationComponent.RADPackage) then  {/ P122 - JK 7/24/2011 /}
        begin
          {/p122t5 if the user choose to cancel, cancel the loading/}
          if Mag4VGear1.CheckIfAnnotsModified then
          begin
            Mag4VGear1.AnnotationComponent.IsAnnotationMode := False;
            Mag4VGear1.AnnotationComponent.Hide;
          end
          else
            Exit;
        end
        {/p122t5 dmmn 10/11/11 hide the toolbar for radiology package /}
        else if Mag4VGear1.AnnotationComponent.IsAnnotationMode and (Mag4VGear1.AnnotationComponent.RADPackage) then
        begin
          Mag4VGear1.AnnotationComponent.IsAnnotationMode := False;
          Mag4VGear1.AnnotationComponent.Hide;
        end;
      end;
  //p122t15 dmmn 7/11/12 - clear the manual calibrated values when new image loaded.
  if Mag4VGear1 <> nil then
    if Mag4VGear1.AnnotationComponent <> nil then
      Mag4VGear1.AnnotationComponent.ClearActualRatio();
  ...
  // don't want the win/lev from ref image for the big file
    ...
    Mag4VGear1.HideScoutLine();

    Mag4Vgear1.PI_ptrData := IObj; {/ P122 - JK 7/20/2011 - moved this up a few lines to give the TImageData info to
                                      Annotation initialization when it needs it. /}

    Mag4Vgear1.LoadTheImage(Oneimage, BigFile);
    // p122t15 dmmn 7/11/12 - since we are not loading the annotations for Vista RAD anytmore, we need
    // a way to notify the component that the image has annotations to display a message in the information
    // window and for printing (both normally and ROI)
    if (IObj.Package = 'RAD') then
      if Mag4VGear1.AnnotationComponent <> nil then
        if Iobj.Annotated then
        begin
          // use AnnotatedByVR instead of HasRADAnnotation because the other property
          // is being used by other condition and updating it improperly might cause issues.
          Mag4VGear1.AnnotationComponent.AnnotatedByVR := true;
        end
        else
        begin
          Mag4VGear1.AnnotationComponent.AnnotatedByVR := false;
        end;
//  Mag4Vgear1.FitToWindow();
    Screen.Cursor := crDefault;
    Mag4Vgear1.ImageLoaded := True;
//    Mag4Vgear1.PI_ptrData := IObj; // I think this is ok here  {/ P122 - JK 7/20/2011 - moved up a few lines /}
//  Mag4VGear1.ImageNumber := ImgIndex; // should this be +1 ? (I don't think so)
  ...
  // JMW 2/29/08 P72 - if the place IEN is not 200, then get TXT file
    ...
    Begin
      ...
      Else
      Begin
        // JMW 5/1/2013 P131 - for scout lines we need the text file even if
        // the DICOM header is fully updated, so always get the txt file for VA
        // images
        ...
        Begin
          magAppMsg('s', 'Image is from ViX and contains DICOM header, not getting TXT file', MagmsgINFO);
          GetTxtFile := False;
        End
        Else
        Begin
      ...
      //LogMsg('s','About to get text file for image [' + IObj.Mag0 + ']');
        magAppMsg('s', 'About to get text file for image [' + IObj.Mag0 + ']'); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    End;
{
  ...
  //LogMsg('s','About to load DICOM header information for image [' + IObj.Mag0 + ']');
    magAppMsg('s', 'About to load DICOM header information for image [' + IObj.Mag0 + ']'); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    //LogMsg('DEQA','Patient identifier from VistA does not match patient identifier from image.');
      magAppMsg('DEQA', 'Patient identifier from VistA does not match patient identifier from image.'); {JK 10/6/2009 - Maggmsgu refactoring}
    End
    Else
    Begin
...
//      if ((FPageSameSettings) or (FApplyGivenWindowSettings)) and (Fstate <> nil) and (FState.WinValue > 0) and (FState.LevValue > 0) then
  ...
  // maybe check if autoredraw = true, if true then don't do the updatepageview here (probably already done) - sorta weird and kludgy
    ...
    Mag4Vgear1.LoadRADAnnotations(); //p122t11
    ...
    if ImgResult.FTransferStatus = IMAGE_COPIED then  {/ P117 JK 1/24/2011 - don't register a LogImageAccess event if the image wan not loaded /}
      If MagImageAccessLogManager1 <> Nil Then
      Begin {get the error here, after <Paramater Error> MAGGTU6}
        MagImageAccessLogManager1.LogImageAccess(IObj);
      End;
...
//  Mag4VGear2.Selected := false;
  If Assigned(OnStackViewerClick) Then
    OnStackViewerClick(Self, Self, Mag4Vgear1);
  //p122t7 dmmn 11/2/11 - fix double selected stacks when user choose cancel in modified annotation mode and clicking the other stack
  if CanShowAnnotToolbar then

  Mag4Vgear1.Selected := True;
  // JMW 8/11/08 - hopefully this fixes the 'cannot focus a disabled or
  // invisible window' error message
  Try
    If (Mag4Vgear1.Visible) And (Mag4Vgear1.Enabled)
       and (CanShowAnnotToolbar) Then           //p122t7 dmmn
      Mag4Vgear1.SetFocus();
  Except
    On e: Exception Do
    Begin
      //LogMsg('s', 'Exception setting gear focus [' + e.Message + ']', MagLogERROR);
      magAppMsg('s', 'Exception setting gear focus [' + e.Message + ']', MagmsgERROR); {JK 10/6/2009 - Maggmsgu refactoring}
    End;
  End;
  If FPanWindow Then SetPanWindow(FPanWindow);
End;
...
Begin
  {/ P122 - JK 9/20/2011 - don't allow ApplyToAll while in annotation mode /}
  if FCurrentGearControl.AnnotationComponent <> nil then
    if FCurrentGearControl.AnnotationComponent.Visible then
    begin
      if Value = True then
      begin
        MessageDlg('Cannot set Apply To All setting while in Annotation Mode',
...
End;
 {7/12/12 gek Merge 130->129}
Function TMag4StackViewer.RGBChanger(CurrentState: Integer; ApplyAll : Boolean) : Integer;
begin
  //p122rgb dmmn 12/12/11 - rgb
  Result := FCurrentGearControl.RGBChanger(CurrentState, ApplyAll);
end;
...
Procedure TMag4StackViewer.ImagePrint();      {Not complete Function  NO Printing here}
Begin
...
Var
  ...
  IObj: TImageData;
Begin
  {/ P122 - JK 9/23/2011 - Split the OR into two lines. /}
//  If (FImageList = Nil) Or (FImageList.Count <= 0) Then Exit;
  try  {/ P122 - JK 9/26/2011 - try and quiet down the exception that we cannot figure out /}
    ...
    {/ P122 - JK 10/18/2011 - if the Items property is nil, exit too /}
    if FImageList.Items[0] = nil then
    begin
        magAppMsg('s', 'Bad FImageList.items[0]');
        Exit;
    end;
    
    //    if IsBadReadPtr(FImageList, 4) then
//    begin
//      magAppMsg('s', 'TMag4StackViewer.RIVRecieveUpdate_: FImageList.Items[0] = nil');
//      magAppMsg('s', 'Bad FImageList');
//      Exit;
//    end
//    else
//    begin
//      if IsBadReadPtr(FImageList.Items[0], 4) then
//      begin
//        magAppMsg('s', 'Bad FImageList.items[0]');
//        Exit;
//      end;
//    end;
    ...
    on E:Exception do
    begin
      magAppMsg('s','TMag4StackViewer.RIVRecieveUpdate_ Error = ' + E.Message);
    end;
...
Begin
  {/ P122 - JK 8/17/2011 /}
  if Mag4VGear1 <> nil then
    if Mag4VGear1.AnnotationComponent.IsAnnotationMode then
    begin
      {/p122t5 dmmn 10/11 - if user choose to cancel the save prompt, exit /}
      if Mag4VGear1.CheckIfAnnotsModified then
      begin
        Mag4VGear1.AnnotationComponent.IsAnnotationMode := False;
        Mag4VGear1.AnnotationComponent.Hide;
...
Procedure TMag4StackViewer.ShowAnnotationToolbar;
var
  P, PP: TPoint;
Begin
  FCurrentTool := MactAnnotation;
  // this event is fired when the toolbar is pressed
  if FCurrentGearControl = nil then
    Exit;

  {/ P122 - JK 5/4/2011 /}
  P.X := Mag4VGear1.Left + 10;
  P.Y := Mag4VGear1.Top + pnlTop.Height + 10;
  PP := Mag4VGear1.ClientToScreen(P);
  FCurrentGearControl.AnnotationComponent.ToolbarCoords := PP;

  FCurrentGearControl.AnnotationComponent.EditMode := True;

  {/P122 JK 8/19/2011 - if in DICOM cine mode, pass in the actual page number corresponding to the
    image frame that the annotation is to be added to /}
  FCurrentGearControl.AnnotationComponent.ShowArtPage(FCurrentGearControl.Page);

  magAppMsg('s', 'TMag4StackViewer.ShowAnnotationToolbar: >>> FCurrentGearControl name = ' + FCurrentGearControl.Name +
                   '. It''s AnnotationComponent name = ' + FCurrentGearControl.AnnotationComponent.Name);
...
{/ P122 - JK 6/9/2011 /}
procedure TMag4StackViewer.HideAnnotationToolbarStack;
var
  i: Integer;
begin
  {Fire the event to tell the rad viewer (parent) to close the
   other stack viewer annotation toolbar if open }
  If Assigned(OnAnnotateButtonClick) Then
    OnAnnotateButtonClick(Self);
end;

procedure TMag4StackViewer.btnAnnotationsClick(Sender: TObject);
var
  ServerInfo: String; {/ P122 JK 9/20/2011 /}
begin
  try
    {Turn off the scroll timer before the toolbar is displayed if "on".}
    if tmrScroll.Enabled then
    begin
      tmrScroll.Enabled := False;
      CheckStartStopButton();
    ...
    if GSess.AnnotationFontAvailable = False then
    ...
    if Mag4VGear1.ImageLoaded then
    begin
      {Close annotation toolbar in the other stack if it is open}
      HideAnnotationToolbarStack;

      if Not CanShowAnnotToolbar then
        Exit;
      
      {If the user is connected to a remote site and accesses an image, check the
       session variable to see if that site supports annotations (has installed Patch 122 KIDS)}
      {/p122t15 - dmmn 8/13/12 - loop around the check here for DOD site to restore the
      ability to take temporary measurements in radiology viewer of DOD images /}
      if (Mag4Vgear1.PI_ptrData.PlaceIEN <> '200') then
      begin

      {/ P122 - JK 10/2/2011 /}
      ServerInfo := Mag4Vgear1.PI_ptrData.ServerName + ':' + IntToStr(Mag4Vgear1.PI_ptrData.ServerPort);

      if GSess.RemoteSiteSupportsAnnotation(ServerInfo) = False then
      begin
        MessageDlg('This remote image located at ' + Mag4Vgear1.PI_ptrData.PlaceCode + ' cannot be annotated. Annotations are not supported at that site.',
          mtConfirmation, [mbOK], 0);
        Exit;
      end;

      {/ P122 - JK 10/2/2011 - If the user is connected to a remote site and accesses an image, check the
       session variable to see if that user has remote permissions (Kernel setup) to annotate. First
       check the pattern (~Y~Y) to see if the user has master key priv. /}
      ServerInfo := Mag4Vgear1.PI_ptrData.ServerName + ':' + IntToStr(Mag4Vgear1.PI_ptrData.ServerPort) + '~Y~Y';
      if GSess.RemoteSiteSupportsAnnotation(ServerInfo) = True then
        magAppMsg('s', 'User has MAG ANNOTATE MGR key at [' + Mag4Vgear1.PI_ptrData.PlaceCode + '] to annotate image IEN: ' + Mag4Vgear1.PI_ptrData.Mag0)
      else
      begin
        ServerInfo := Mag4Vgear1.PI_ptrData.ServerName + ':' + IntToStr(Mag4Vgear1.PI_ptrData.ServerPort) + '~Y';
        if GSess.RemoteSiteSupportsAnnotation(ServerInfo) = False then
        begin
          MessageDlg('You do not have permission to annotate images at ' + Mag4Vgear1.PI_ptrData.PlaceCode, mtConfirmation, [mbOK], 0);
          Exit;
        end;
      end;
        {/p122t15 - dmmn 8/13/12 - set flag to indicate that the image is not DOD /}
        if Mag4VGear1.AnnotationComponent <> nil then
        begin
          Mag4VGear1.AnnotationComponent.IsDODImage := false;
        end;

      end
      else
      begin
        {/p122t15 - dmmn 8/13/12 - set flag to indicate that the image is DOD /}
        if Mag4VGear1.AnnotationComponent <> nil then
        begin
          Mag4VGear1.AnnotationComponent.IsDODImage := true;
          Mag4VGear1.AnnotationComponent.RADPackage := false;
        end;
      end;

      {/ P122 - JK 9/21/2011 - be sure that AnnotsModified starts off as false. When a pre-annotated non-RAD package
         image opens and the user changes the view from 1x1 to something else and back again, the save challenge dialog
         pops up because it thinks some annotatation changed upon loading. /}
      if Mag4VGear1.AnnotationComponent <> nil then
        Mag4Vgear1.AnnotationComponent.AnnotsModified := False;
    ...
    on E:Exception do
      magAppMsg('s', 'TMag4StackViewer.btnAnnotationsClick exception = ' + E.Message);
  end;
end;
...
procedure TMag4StackViewer.setShowSeriesDropDown(Value : Boolean);
begin
  FShowSeriesDropDown := Value;
  if not FShowSeriesDropDown then
    cboSeries.Visible := false;
end;

procedure TMag4StackViewer.setShowScrollStartStop(Value : Boolean);
begin
  FShowScrollStartStop := Value;
  btnScrollStartStop.Visible := Value;
  TbStudySpeed.Visible := Value;
end;

procedure TMag4StackViewer.setImageMouseUpEvent(Value : TImageMouseUpEvent);
begin
  self.FImageMouseUp := Value;
  Mag4Vgear1.OnImageMouseUp := self.FImageMouseUp;

end;

procedure TMag4StackViewer.SetDisplayedImage(ImageIndex : Integer; SetSelected: Boolean = True);
begin
  LoadImage(ImageIndex, False, SetSelected);
end;

{ Another image has been selected (SelectedImage),
      determine if the currently displayed image is associated and draw the Scout Line}
procedure TMag4StackViewer.DrawScoutLineFromSelectedImage(SelectedImage : TImageData;
  SelectedImageRootImage : TImageData);
var
...
begin
  scoutLine :=
    uMagRadHeaderLoader.GetXRefLine(SelectedImage, SelectedImageRootImage,
      FCurrentGearControl.PI_ptrData, FRootImage);
  if scoutLine <> nil then
  begin
    FCurrentGearControl.DrawScoutLine(scoutLine);
  end
  else
  begin
    FCurrentGearControl.HideScoutLine();
  end;
  {
  // find the study associated with the images in the scout window
  study := uMagRadHeaderLoader.GetStudyHolder().getStudy(FRootImage);
  currentStudy := uMagRadHeaderLoader.GetStudyHolder().getStudy(SelectedImageRootImage);
  ...
  currentScoutImage := FCurrentGearControl.PI_ptrData;
  ...
  begin
    ...
    begin
      try
        ...
        FCurrentGearControl.DrawScoutLine(scoutLine.X1,
          scoutLine.Y1, scoutLine.X2, scoutLine.Y2);
      except
        on E : Exception do
        begin
...
procedure TMag4StackViewer.HideScoutLines();
begin
  if FCurrentGearControl <> nil then
    FCurrentGearControl.HideScoutLine();
end;

Procedure TMag4StackViewer.SetScoutLineColor(Color : TColor);
begin
  if FCurrentGearControl <> nil then
    FCurrentGearControl.SetScoutLineColor(Color);
end;

Procedure TMag4StackViewer.SetPanWindowHolder(PanWindowHolder : TMagPanWindowHolder);
begin
  FPanWindowHolder := PanWindowHolder;
  Mag4Vgear1.SetPanWindowHolder(FPanWindowHolder);
end;
