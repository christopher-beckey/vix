...
Type
  ...
  TMagViewerTB8 = Class(TFrame)
    ...
    procedure tbtnAnnotateClick(Sender: TObject);
...
//    FMagVUtils: TMagVUtils;
 ...
 {7/12/12 gek Merge 130->129}
    FCurrentRGBState : Integer;   {/p122rgb dmmn 12/13/11 - RGBChanger state /}
  ...
  Public
    ...
    procedure RefreshButtonVisible(value : boolean);
  Published
 ...
 {7/12/12 gek Merge 130->129}
  ...
  begin
    case ImageState.RGBState of
      0: tbRGBChanger.ImageIndex := 28;
      1: tbRGBChanger.ImageIndex := 29;
      2: tbRGBChanger.ImageIndex := 30;
      3: tbRGBChanger.ImageIndex := 31;
    end;
  end
  else
    tbRGBChanger.ImageIndex := 36;
End;
...
procedure TMagViewerTB8.RefreshButtonVisible(value : boolean);
begin
 tbRefresh.Visible := value;
end;
...
Begin
  If FCurViewer <> Nil Then
  begin
    tbRGBChanger.ImageIndex := 28;  {/ P130 - JK 4/21/2012 /} {7/12/12 gek Merge 130->129}
    FCurViewer.RemoveFromList;
  end;
...
Begin
  ...
  begin
    tbRGBChanger.ImageIndex := 28;  {/ P130 - JK 4/21/2012 /} {7/12/12 gek Merge 130->129}
    FCurViewer.ReFreshImages;
  end;
End;

procedure TMagViewerTB8.tbtnAnnotateClick(Sender: TObject);
begin
  If FCurViewer <> Nil Then
    FCurViewer.ShowAnnotToolbar;  {/ P122 - JK 5/4/2011 /}
end;
...
End;
 {7/12/12 gek Merge 130->129}
procedure TMagViewerTB8.tbRGBChangerClick(Sender: TObject);
begin
  {/p122rgb dmmn 12/13/11 - RGB changer for color image /}
  if TbApplyToAll.Down then
  begin
    //if apply all then the next state for all proxy will follow the current state
    // of the selected image
    If FCurViewer <> Nil Then
      FCurrentRGBState := FCurViewer.RGBChanger(FCurrentRGBState, True);
  end
  else
  begin
    If FCurViewer <> Nil Then
