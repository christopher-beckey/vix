...
(*    ***  ---   After refactoring this is a 'Well behaved'  'Non-Coupled' unit.  --- *** *)
...
procedure SetIniEntry(section, entry, value: string);
    {}
Function GetConfigFileName8(XDirectory: String = ''): String;
    {}
...
function isUrn(S: String): Boolean;  {/ P122 - JK 7/22/2011 /}

{ JMW 6/19/2013 P131 - get the actual object behind an interface}
...
Uses
  Inifiles,
//RCA out    Magguini,   getconfigfilename method copied here.
  Shellapi,
  StrUtils,
  SysUtils
  ;
{This function was copied here, in RCA  so that uMagUtils8, was not dependent on an 'Mag*' file. (maguini).
  only umagutils8 functions call this function, I renamed it to 2 in 129 so no confusion
  p129}
Function GetConfigFileName8(XDirectory: String = ''): String;
Var
  dir : string;
Begin
 {win7 change to AppData Directory}
   //// this is user specific      dir :=  GetEnvironmentVariable('AppData') + '\vista\imaging\' ;
...
var magini: Tinifile;
begin
  magini := Tinifile.create(GetConfigFileName8);
...
Begin
{$IFDEF DEBUGTOFILE}
  DebugDirectory = GetEnvironmentVariable('AppData') + '\VIDebug';
  DebugFile = 'VIDebugFile.Txt';
...
const
  VAImage = 'urn:vaimage:';
  VAStudy = 'urn:vastudy:';
  VAAnnot = 'urn:vaannotation:';
begin
  if (Pos(VAImage, S) > 0) or (Pos(VAStudy, S) > 0) or (Pos(VAAnnot, S) > 0)  then
    Result := True
  else
    Result := False;
end;
