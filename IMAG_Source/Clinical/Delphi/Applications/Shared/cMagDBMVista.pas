...
Uses
  ...
  UMagDefinitions,
  imaginterfaces ,
   //maggmsgu,  old cMagDBMVista for Import OCX had maggmsgu,  not imaginterfaces.{p158}
...
Type
  ...
  Private
    ...
    procedure LogRPCResultWithXML(Verbose: Boolean = False); Overload;
    Procedure LogRPCResultWithXML(t: TStrings; Verbose: Boolean = False); Overload;

//Was for RCA decouple magmsg, not now.      procedure MyLogMsg(msgType, msg: String; Priority: TMagMsgPriority = magmsgINFO);
//Was for RCA decouple magmsg, not now.      procedure MyLogMsgs(Code: String; Msgs: Tstringlist);

  Public
    // 129-140  Track Broker Calls.
    procedure SetLogRPCCalls(value : boolean) ; Override;
    function  GetLogRPCCalls() : boolean; Override;
...
{p121 adds division as parameter}
    ...
    Function RPMagTeleReaderUnreadlistGet(Var t: TStrings; Var Xmsg: String; AcquisitionPrimaryDivision, Sitecode, SpecialtyCode, ProcedureStrings, LastUpdate, UserDUZ, LocalSiteCode, SiteTimeOut,
      StatusOptions, LocalSiteStationNumber: String): Boolean; Override;  {/ P127 - NST 04/06/2012 - Send Reader Station Number (locking issie) /}
    Function RPMagTeleReaderUnreadlistLock(Var Xmsg: String; AcquisitionPrimaryDivision, AcquisitionSiteCode, ItemID, LockUnlockValue, UserFullName, UserInitials, LocalDUZ, LocalSiteCode, LocalSiteStationNumber: String):
      Boolean; Override;   {/ P127 - NST 04/06/2012 - Send Reader Station Number (locking issie) /}
...
//    Procedure RPMagPatInfo(Var Fstat: Boolean; Var Fstring: String; MagDFN: String; IsIcn: Boolean = False); Override;
    ...
    {/ P122 - JK 6/20/2011 - Get the annotation history information for a specific image IEN.
       Calls RPC: MAG ANNOT GET IMAGE which returns a stringlist of annotation layers (histories) stored for the image. /}
    procedure RPMagAnnotGetHistory(IEN: String; Var Fstat: Boolean; Var Rpcmsg: String; Var AnnotHistory: TStringList;
              ServerName: String; ServerPort: Integer); override;

    {/ P122 - JK 6/20/2011 - Get the annotation XML for an specific image IEN and annotation layer (history).
       Calls RPC: MAG ANNOT GET IMAGE DETAIL /}
    procedure RPMagAnnotGetXML(AnnotID: String; LayerNumber: String; var AnnotXML: TStringList;
              var Fstat: Boolean; var Rpcmsg: String; ServerName: String; ServerPort: Integer); override;

    {/ P122 - JK 6/20/2011 - Saves the annotation XML for an specific image IEN as the most recent layer (history).
       Calls RPC: MAG ANNOT STORE IMAGE DETAIL /}
    procedure RPMagAnnotSaveXML(IEN: String; AnnotSource: String; Version: String; AnnotXML: TStringList;
          Deleted: String; Var Fstat: Boolean; Var Rpcmsg: String; ServerName: String; ServerPort: Integer); override;

    {/ P122 - JK 7/6/2011 - gets the kernel parameter result for annotation permission.  Calls MAG ANNOT IMAGE ALLOW /}
    function RPKernelAnnotatePermission: Boolean; override;

    {/ P123 - JK 8/8/2011 - returns I = IHS or V = VA /}
    function RPMaggAgency: String; override;

    { P131 JMW 5/15/2013 - return a unique string to identify this broker connection}
    function GetBrokerIdentifier() : String; override;
...
{JK 10/5/2009 -  refactoring - removed old method}
...
{ TODO -chigh: Changed style of Broker Calls to be similiar in paramters.
...
{$ENDIF}
  ...
  Begin
    ...
    magAppMsg('s', 'cMagDBMVistA - a new patient has been selected, will not load remote images for patient');
    Exit;
  End;
  ...
  Begin
    ...
    Begin
      ...
      Try
        ...
        magAppMsg('s', 'DFN:' + RemoteDFN + ' Start RPC MAG4 PAT GET IMAGES on ' + RemoteBroker.GetServerDescription + ' ' +
          Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
 ...
 {7/12/12 gek Merge 130->129}
        ...
        RBrokerResult := RemoteBroker.GetPatientStudies(Pkg, Cls, Types, Event, Spec, FrDt, ToDt, Origin,
                             Filter.ShowDeletedImageInfo, RemoteList, PartialResult, PartialMsg);
        ...
        Begin
          ...
          Begin
            ...
            begin
              PR := '^Partial';
              magAppMsg('', 'Partial Result Message = [ ' + PartialMsg + ']'); {/ P117 NCAT - JK 1/17/2011 /}
            end
            else
              PR := '';
...
{$ENDIF}
                    ...
                    //    formatdatetime('mm/dd/yy@hh:nn:ss.zzz', now));
          magAppMsg('s', 'DFN:' + RemoteDFN + ' Done RPC MAG4 PAT GET IMAGES on ' + RemoteBroker.GetServerDescription + ' ' +
            Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
...
Procedure TMagDBMVista.RPMaggImageDelete(Var Fstat: Boolean; Var Rmsg: String; Var Flist: TStrings; Ien, ForceDel: String; Reason: String = ''; GrpDelOK:
  Boolean = False);
Begin
  Fstat := False;
  Rmsg := '';
...
{/ P122 - JK 6/20/2011 - Get the list of history layers for a image IEN. /}
procedure TMagDBMVista.RPMagAnnotGetHistory(IEN: String; var Fstat: Boolean;
  var RPCMsg: String; var AnnotHistory: TStringList; ServerName: String; ServerPort: Integer);
var
  ResStr: String;
  FList: TStringList;
//  i: Integer;
...
begin
  ...
  Begin
    Fstat := False;
    RPCMsg := 'No connection to the Server';
    Exit;
  End;
  ...
  begin
    FStat    := False;
    RPCMsg   := 'Failed';
    magAppMsg('s', 'TMagDBMVista.RPMagAnnotGetHistory failed. Servername = ' + Servername + ' ' +
      'ServerPort = ' + IntToStr(ServerPort) + ' IEN = ' + IEN);
    Exit;
  end;
  ...
  try
    ...
    begin
      try
        ...
        FBroker.PARAM[0].PTYPE := LITERAL;
        FBroker.REMOTEPROCEDURE := 'MAG ANNOT GET IMAGE';

        LogRPCParams;
        FBroker.LstCALL(AnnotHistory);
        FStat := True;

        ResStr := MagPiece(AnnotHistory[0], '^', 1);
        if ResStr = '0' then
          RPCMsg := 'Failed'
        else
          RPCMsg := 'Success';

        LogRPCResultWithXML(AnnotHistory, True);
      except
        on E:Exception do
        begin
          Rpcmsg := E.Message;
          FStat := False;
          magAppMsg('s', 'TMagDBMVista.RPMagAnnotGetHistory error = ' + E.Message);
        end;
      end;
    end
    ...
    begin
      {This is a remote image annotation load history operation through the remote broker (VIX or RPC)}

      RemoteBroker := MagRemoteBrokerManager1.GetBroker(ServerName, ServerPort);
      if RemoteBroker = nil Then
      begin
        MagRemoteBrokerManager1.LogBrokerNotFoundError(ServerName, ServerPort, 'GetAnnots');
        Exit;
      end;
      with RemoteBroker do
      begin
        magAppMsg('s', '--RPC : MAG ANNOT GET IMAGE'); 
        try
          FList.Clear;
          FList.Assign(RemoteBroker.GetAnnots(IEN));

          if FList.Count > 0 then
          begin
            FStat := True;
            RpcMsg := 'Remote Broker: MAG ANNOT GET IMAGE: Success';
            AnnotHistory.Assign(FList);
          end
          else
          begin
            FStat := False;
            RpcMsg := 'Remote Broker: MAG ANNOT GET IMAGE: Failed';
          end;
        except
          Messagedlg('Error attempting to get annotation history using the remote broker', MtWarning, [Mbok], 0);
        end;
      end;
    end;
...
procedure TMagDBMVista.RPMagAnnotGetXML(AnnotID: String; LayerNumber: String; var AnnotXML: TStringList; var Fstat: Boolean;
  var RPCMsg: String; ServerName: String; ServerPort: Integer);
var
  ResStr: String;
//  FList: TStringList;
//  i: Integer;
...
//  inherited;
  ...
  begin
    ...
    RPCMsg   := 'Failed';
    AnnotXML.Clear;
    magAppMsg('s', 'TMagDBMVista.RPMagAnnotGetXML failed. Servername = ' + Servername + ' ' +
      'ServerPort = ' + IntToStr(ServerPort) + ' AnnotID = ' + AnnotId + ' LayerNumber = ' + LayerNumber);
    Exit;
  end;
  ...
  begin
    try
      ...
      FBroker.PARAM[0].Value := AnnotID;
      FBroker.PARAM[0].PTYPE := LITERAL;

      FBroker.PARAM[1].Value := LayerNumber;
      FBroker.PARAM[1].PTYPE := LITERAL;

      FBroker.REMOTEPROCEDURE := 'MAG ANNOT GET IMAGE DETAIL';

      LogRPCParams;
      FBroker.LstCALL(AnnotXML);
      FStat := True;

      ResStr := MagPiece(AnnotXML[0], '^', 1);
      if ResStr = '0' then
        RPCMsg := 'Failed'
      else
        RPCMsg := 'Success';

      LogRPCResultWithXML(AnnotXML, True);
    except
      on E:Exception do
      begin
        Rpcmsg := E.Message;
        FStat := False;
        magAppMsg('s', 'TMagDBMVista.RPMagAnnotGetXML error = ' + E.Message);
      end;
    end;
  end
  else
  begin
    {This is a remote image annotation load XML operation through the remote broker (VIX or RPC)}

    RemoteBroker := MagRemoteBrokerManager1.GetBroker(ServerName, ServerPort);
    if RemoteBroker = nil Then
    begin
      MagRemoteBrokerManager1.LogBrokerNotFoundError(ServerName, ServerPort, 'GetAnnotDetails');
      Exit;
    end;
    with RemoteBroker do
    begin
      magAppMsg('s', '--RPC : MAG ANNOT GET IMAGE DETAIL'); 
      try
        S := RemoteBroker.GetAnnotDetails(AnnotID, LayerNumber);

        if Trim(S) <> '' then
        begin
          FStat := True;
          RpcMsg := 'Remote Broker: MAG ANNOT GET IMAGE DETAIL: Success';
          AnnotXML.Clear;

//          if ExtractStrings([chr(13)], [' '], PChar(S), AnnotXML) = 0 then
//            magAppMsg('s', 'TMagDBMVista.RPMagAnnotGetXML ExtractStrings(chr(13)) failed');
//
//          for i := 0 to AnnotXML.Count - 1 do
//            AnnotXML[i] := StringReplace(AnnotXML[i], chr(10), '', [rfReplaceAll]);
          {/p122t5 dmmn 10/11 - this portion for the VIX result /}
          if isURN(AnnotID) then
          begin
            AnnotXML.Add('1^2');  {add in the RPC status line, 1= Success, 2 = two lines appended to follow}
            AnnotXML.Add(MagPiece(S, '`', 1));
            AnnotXML.Add(MagPiece(S, '`', 2));
          end
          else {/p122t5 dmmn 10/11 - this portion for the RPC result (vix off)/}
          begin
            AnnotXML.Text := s;
          end;
        end
        ...
        begin
          FStat := False;
          AnnotXML.Add('0^0');  {add in the RPC status line, 0= Failure, 0 = no XML}
          RpcMsg := 'Remote Broker: MAG ANNOT GET IMAGE DETAIL: Failed';
        end;
      except
        Messagedlg('Error attempting to get annotation XML using the remote broker', MtWarning, [Mbok], 0);
      end;
    end;
  end;
end;

procedure TMagDBMVista.RPMagAnnotSaveXML(IEN: String; AnnotSource: String; Version: String;
                                         AnnotXML: TStringList;
                                         Deleted: String; var Fstat: Boolean; var RPCMsg: String;
...
var
  ...
  RemoteBroker: IMagRemoteBroker;
begin
  { If the server and port assignment for this xml is the same as the FBroker values, use the RPC broker for
    local communications.  Otherwise use the remotebroker. }
  ...
  begin
    ...
    RPCMsg   := 'Failed';
    AnnotXML.Clear;
    magAppMsg('s', 'TMagDBMVista.RPMagAnnotSaveXML failed. Servername = ' + Servername + ' ' +
      'ServerPort = ' + IntToStr(ServerPort) + ' IEN = ' + IEN + ' AnnotSource = ' + AnnotSource + ' Version = ' + Version);
    Exit;
  end;
  ...
  try
    ...
    begin
      try
        ...
        FBroker.PARAM[1].Value := AnnotSource;
        FBroker.PARAM[1].PTYPE := LITERAL;

        FBroker.PARAM[2].Value := Version;
        FBroker.PARAM[2].PTYPE := LITERAL;

        FBroker.PARAM[3].PTYPE := List;
        for i := 0 To AnnotXML.Count - 1 do
          FBroker.PARAM[3].Mult['' + IntToStr(i) + ''] := AnnotXML[i];

        FBroker.PARAM[4].Value := Deleted;
        FBroker.PARAM[4].PTYPE := LITERAL;

        FBroker.REMOTEPROCEDURE := 'MAG ANNOT STORE IMAGE DETAIL';

        LogRPCParams;
        FBroker.LstCALL(FList);
        FStat := True;

        ResStr := MagPiece(FList[0], '^', 1);
        if ResStr = '0' then
        begin
          {/p122 dmmn 8/22 - update the status of the save /}
          FStat := False;
          RPCMsg := 'MAG ANNOT STORE IMAGE DETAIL: Failed.';
          RPCMsg := RPCMsg + 'Error: ' + MagPiece(FList[0], '^', 3); //error message from Jemmy in 3rd piece
        end
        else
        begin
          FStat := True;
          RPCMsg := 'MAG ANNOT STORE IMAGE DETAIL: Success';
        end;

        LogRPCResultWithXML(FList, True);
      except
        on E:Exception do
        begin
          Rpcmsg := E.Message;
          FStat := False;
          magAppMsg('s', 'TMagDBMVista.RPMagAnnotSaveXML error = ' + E.Message);
        end;
      end;
    end
    ...
    begin
      {This is a remote image annotation save operation through the remote broker (VIX or RPC)}

      RemoteBroker := MagRemoteBrokerManager1.GetBroker(ServerName, ServerPort);
      if RemoteBroker = nil Then
      begin
        MagRemoteBrokerManager1.LogBrokerNotFoundError(ServerName, ServerPort, 'SaveAnnots');
        Exit;
      end;
      with RemoteBroker do
      begin
        magAppMsg('s', '--RPC : MAG ANNOT STORE IMAGE DETAIL'); 
        try
          if RemoteBroker.SaveAnnots(IEN, AnnotSource, Version, AnnotXML) = True then
          begin
            FStat := True;
            RpcMsg := 'Remote Broker: MAG ANNOT STORE IMAGE DETAIL: Success';
          end
          else
          begin
            FStat := False;
            RpcMsg := 'Remote Broker: MAG ANNOT STORE IMAGE DETAIL: Failed';
          end;
        except
          Messagedlg('Error attempting to save annotation xml using the remote broker', MtWarning, [Mbok], 0);
        end;
    End;
    end;
...
//  FBroker.REMOTEPROCEDURE := 'VAFCTFU GET TREATING LIST'; // this is 122 T11
  // JMW 1/19/2012 - use RPC from VistARad to get treating list // this is 122 T11
  FBroker.REMOTEPROCEDURE := 'MAGJ GET TREATING LIST';
  FBroker.PARAM[0].Value := MagDFN;
  FBroker.PARAM[0].PTYPE := LITERAL;
  FBroker.Call;

 // If MagPiece(FBroker.Results.Strings[0], '^', 1) = '-1' Then //Taken out in 122 T11
  If MagPiece(FBroker.Results.Strings[0], '^', 1) = '0' Then   //put IN in 122 T11
  Begin
        //LogMsg('s', 'DFN:' + MagDFN + ' Patient does not have any images at any remote sites ' + formatdatetime('mm/dd/yy@hh:nn:ss.zzz', now));
    magAppMsg('s', 'DFN:' + MagDFN + ' Patient does not have any images at any remote sites ' +
      Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
  ...
  Begin
    ...
    magAppMsg('s', 'Patient has been seen at the following facilities [' + FBroker.Results.Text + ']', MagmsgINFO); 
    ...
    End;
        ...
        //    now));
    magAppMsg('s', 'DFN:' + MagDFN + ' Patient has images at [' + Inttostr(RemoteSiteCount) + '] remote site(s) ' +
      Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
  ...
  magAppMsg('', 'Determining DFN at each Remote Site... ' + Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
  ...
  magAppMsg('', 'Counting images at each Remote Site... ' +
    Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
...
Begin
  ...
  Except
    On e: EBrokerError Do
      ...
      magAppMsg('de', 'Error Communicating with VistA : ' + e.Message);
    On e: Exception Do
  ...
  Else
    ...
    //p106 rlm 20101229 CR640 "Title of Dialog box"
    magAppMsg('de','UnKnown Error while Saving Settings.');
  End;
  ...
  Begin
    Try
      ...
      Begin
        ...
        magAppMsg('s', '--RPC : MAGG QUE IMAGE GROUP^' + Whichimages + '^' + IObj.Mag0); 
        s := STRCALL;
                //LogMsg('s', '--' + s);
        magAppMsg('s', '--' + s); 
      End;
...
Begin
  Inherited;
  FlogRPCCalls := true;  // for now defalut to TRUE, to keep things working as is.
...
Begin
  ...
  magAppMsg('s', 'Workstation Information sent to VistA:'); 
...
Begin
  ...
  Begin
    ...
    Context := 5;
  End;
  {p129t17  The call to DispDttoFM..  is corrupting the DateTime we send in FM Format.
     we don't want to change everything.. this is called from Display also....
     so Change : if Dtto or DtFrom has 2nd ^ piece,  then that is a FM DateTime and we use that
        else proceed as before.}
  if (maglength(Dtto,'^') > 1)
    then
       begin
       Dtto := magpiece(dtto,'^',2);
       if (length(magpiece(dtto,'.',2))=3) then dtto := dtto + '0';
       end
    else Dtto := DispDttoFM(Dtto);
  if (maglength(Dtfrom,'^') > 1)
    then
       begin
       Dtfrom := magpiece(Dtfrom,'^',2);
       if (length(magpiece(Dtfrom,'.',2))=3) then Dtfrom := Dtfrom + '0';
       end
    else Dtfrom := DispDttoFM(Dtfrom);
  t.Clear;  {just to be sure. }
...
{$ENDIF}
    ...
    Except
      ...
      Begin
        ...
        magAppMsg('s', 'ERROR (EXCEPTION) in VistA function:' + e.Message); 
      End;
    End;
  Finally
    Vlist.Free;
...
function TMagDBMVista.GetLogRPCCalls: boolean;
begin
  result := FlogRPCCalls;
end;
...
procedure TMagDBMVista.SetLogRPCCalls(value: boolean);
begin
  inherited;
   FlogRPCCalls := value;
end;
...
Begin
  if not FlogRPCCalls then exit;
  magAppMsg('s', ' --------- Results --------- string ');
  magAppMsg('s', s);
  magAppMsg('s', ' --------- End Results  -------- RPC : ' + FBroker.REMOTEPROCEDURE);
...
(*    94T1 - 94t13 changes above,
    ...
    magAppMsg('s', ' --------- End Results  --------- ' + Fbroker.RemoteProcedure); 
    *)
End;
...
Procedure TMagDBMVista.LogRPCResultWithXML(Verbose: Boolean = False);
Var
  i: Integer;
  OutputLineCnt: Integer;
Begin
  if not FlogRPCCalls then exit;
  ...
  if Verbose then
  begin
    OutputLineCnt := FBroker.Results.Count;
    magAppMsg('s', 'Displaying XML - Verbose - Full output = ' + IntToStr(OutputLineCnt) + ' lines');
  end
  else
  begin
    if FBroker.Results.Count < 10 then
      OutputLineCnt := FBroker.Results.Count
    else
      OutputLineCnt := 10;

    magAppMsg('s', 'Displaying XML - showing ' + IntToStr(OutputLineCnt) + ' of ' +  IntToStr(FBroker.Results.Count) + ' total lines');
  end;

  For i := 0 To OutputLineCnt - 1 Do
  Begin
    magAppMsg('s', FBroker.Results[i]);   End;
  magAppMsg('s', ' --------- End Results  --------- ' + FBroker.REMOTEPROCEDURE);

End;

Procedure TMagDBMVista.LogRPCResultWithXML(t: TStrings; Verbose: Boolean = False);
Var
  i: Integer;
  OutputLineCnt: Integer;
Begin
  ...
  begin
    ...
    magAppMsg('s', 'Displaying XML - showing ' + IntToStr(OutputLineCnt) + ' lines of ' +  IntToStr(t.Count) + ' total lines');
  end;
...
Begin
  ...
  magAppMsg('s', ' --------- RPC Exception ---- ' + FBroker.REMOTEPROCEDURE + ' ---- '); 
    //Logmsg('s', 'Error: ' + errstring);
  magAppMsg('s', 'Error: ' + Errstring); 
    //Logmsg('s', ' --------- End Exception --------- ');
  magAppMsg('s', ' --------- End Exception --------- '); 
...
Begin
  ...
  Try
    ...
    Begin
      ...
      magAppMsg('', 'Error Checking Status of Version: ' + Version); 
      Exit;
    End;
...
Function TMagDBMVista.RPMagTeleReaderUnreadlistGet(Var t: TStrings; Var Xmsg: String; AcquisitionPrimaryDivision, Sitecode, SpecialtyCode, ProcedureStrings,
  LastUpdate, UserDUZ, LocalSiteCode, SiteTimeOut, StatusOptions, LocalSiteStationNumber: String): Boolean;  {/ P127 - NST 04/06/2012 - Send Reader Station Number (locking issie) /}
Var
  RemoteBroker: IMagRemoteBroker;
  s: String;
  p: Integer;
  ...
  Try
    ...
    Begin
      ...
      FBroker.PARAM[7].Value := StatusOptions;
            // NST P127T1 04/04/2012
            // Local site Station Number
      FBroker.PARAM[8].PTYPE := LITERAL;
      FBroker.PARAM[8].Value := LocalSiteStationNumber;
      FBroker.LstCALL(t);
    End
    Else
    Begin
      ...
      RemoteBroker.RPMagTeleReaderUnreadlistGet(t, AcquisitionPrimaryDivision,
        Sitecode, SpecialtyCode, ProcedureStrings, LastUpdate, UserDUZ,
          LocalSiteCode, SiteTimeOut, StatusOptions, LocalSiteStationNumber);       // NST P127T1 04/04/2012 - Send Reader Station Number
    End;
  ...
  Except
    ...
    Begin
      ...
      Begin
        ...
        magAppMsg('s', 'Disconnecting remote site [' + Sitecode + ']'); 
...
Function TMagDBMVista.RPMagTeleReaderUnreadlistLock(Var Xmsg: String; AcquisitionPrimaryDivision, AcquisitionSiteCode, ItemID, LockUnlockValue, UserFullName,
  UserInitials, LocalDUZ, LocalSiteCode, LocalSiteStationNumber: String): Boolean;
Var
  RemoteDUZ: String;
  RemoteBroker: IMagRemoteBroker;
Begin
  ...
  Try
    ...
    Begin
      ...
      magAppMsg('s', 'Locking/Unlocking work item [' + ItemID + '] at Acq Site [' + AcquisitionSiteCode + '], Primary division [' +
        AcquisitionPrimaryDivision + ']'); 
            //LogMsg('s', 'User local DUZ [' + LocalDUZ + '], Acq Site DUZ [' + RemoteDUZ + ']');
      magAppMsg('s', 'User local DUZ [' + LocalDUZ + '], Acq Site DUZ [' + RemoteDUZ + ']'); 
      ...
      FBroker.PARAM[6].PTYPE := LITERAL;
      FBroker.PARAM[7].PTYPE := LITERAL;
      FBroker.PARAM[7].Value := LocalSiteStationNumber;  // P127T1 NST 04/06/2012 Send Reader Station Number
      Xmsg := FBroker.STRCALL;
    End
    Else
    Begin
      ...
      RemoteBroker.RPMagTeleReaderUnreadlistLock(Xmsg, AcquisitionPrimaryDivision,
        ...
        LocalDUZ, LocalSiteCode, LocalSiteStationNumber);      // NST P127T1 04/04/2012 - Send Reader Station Number
    End;
  Except
    On e: Exception Do
    Begin
  ...
  Begin
    ...
    Except
      ...
      Begin
        ...
        magAppMsg('s', 'Exception creating keep alive thread in cMagDBMVista, [' + e.Message + ']'); 
      End;
    End;
  End
  Else
    ...
    Begin
      ...
      Except
        ...
        Begin
          ...
          magAppMsg('s', 'Exception terminating keep alive thread in cMagDBMVista, [' + e.Message + ']'); 
        End;
      End;
    End;
  End;
...
//  F: Textfile;
  ...
  Annotated, Resulted, AnnotationStatus, AnnotationStatusDesc, Package: String;
  ...
  begin
    case i of
      ...
      31: Result := 'MagStatus';
      32: Result := 'Annotated';             {/ P122 JK 7/23/2011 /}
      33: Result := 'Resulted';              {/ P122 JK 7/23/2011 /}
      34: Result := 'AnnotationStatus';      {/ P122 JK 7/27/2011 /}
      35: Result := 'AnnotationStatusDesc';  {/ P122 JK 7/27/2011 /}
      36: Result := 'Package';               {/ P122 JK 7/23/2011 /}
...
//      writeln(f, '------ Study ' + IntToStr(j) + ' ------');
      ...
      {/ P122 - JK 7/28/2011 /}
      Annotated            := MagPiece(StudyPart2, '^', 32);
      Resulted             := MagPiece(StudyPart2, '^', 33);
      AnnotationStatus     := MagPiece(StudyPart2, '^', 34);
      AnnotationStatusDesc := MagPiece(StudyPart2, '^', 35);
      Package              := MagPiece(StudyPart2, '^', 36);
...
{$ENDIF}
 ...
 {7/12/12 gek Merge 130->129}
          ...
          if RemoteBroker.IsAnnotsSupported then
          begin
            magAppMsg('s', 'Annotation Patch 122 is installed at remote site ' + RemoteBroker.GetSiteCode + ' [' + RemoteBroker.GetSite.SiteAbbr + ']');

            if GSess.isRemoteSiteInAnnotateSiteList(RemoteBroker.GetSite.SiteAbbr) = False then {/ P122 - JK 10/2/2011 /}
            begin
              {/ P122 - JK 10/2/2011 /}
              Tmp := RemoteBroker.GetSite.SiteAbbr + '~' + RemoteBroker.GetSite.SiteNumber + '~' + RemoteBroker.GetSite.VistaServer + ':' + IntToStr(RemoteBroker.GetSite.VistaPort);

              if RemoteBroker.CanUserAnnotateRemotely then
              begin
                magAppMsg('s', 'User has permission to annotate at remote site: ' +  RemoteBroker.GetSite.SiteAbbr);
                Tmp := Tmp + '~Y';
              end
              else
              begin
                magAppMsg('s', 'User DOES NOT have permission to annotate at remote site: ' +  RemoteBroker.GetSite.SiteAbbr);
                Tmp := Tmp + '~N';
              end;

              if RemoteBroker.HasAnnotateMasterKey then
                Tmp := Tmp + '~Y'
              else
                Tmp := Tmp + '~N';

              {/ P122 - JK 10/4/2011 - Get the DUZ of the user as it is assigned to the user's remote account /}  
              Tmp := Tmp + '~' + RemoteBroker.GetUserAnnotDUZ;
            end;

          end
          else
            magAppMsg('s', 'Annotation Patch 122 is *NOT* installed at remote site ' + RemoteBroker.GetSiteCode + ' [' + RemoteBroker.GetSite.SiteAbbr + ']');

          {/ P122 - JK 10/2/2011 /}
          if Tmp <> '' then          
            GSess.SiteAnnotationInfo.Add(Tmp);
...
Begin
  ...
  Except
    ...
    Begin
      ...
      magAppMsg('s', 'Error retrieving security token, ' + e.Message, MagmsgERROR); 
      Result := '';
    End;
  End;
End;
...
Begin
  ...
  Except
    On e: Exception Do
    Begin
      magAppMsg('s', 'Error logging capri remote login, ' + e.Message, MagmsgERROR);
    End;
  End;
...
{/ P122- JK 7/6/2011 /}
function TMagDBMVista.RPKernelAnnotatePermission: Boolean;
Var
  s: String;
Begin
  try
    Result := False;
    FBroker.REMOTEPROCEDURE := 'MAG ANNOT IMAGE ALLOW';
    s := FBroker.STRCALL;
    If Copy(s, 1, 1) = '0' Then Exit; //error occurred during call
    If Copy(s, 3, 1) = '1' Then Result := True;
  except
    on E:Exception do
      magAppMsg('s', 'TMagDBMVista.RPKernelAnnotatePermission error = ' + E.Message);
  end;
end;

{/ P123- JK 8/8/2011 /}
function TMagDBMVista.RPMaggAgency: String;
Var
  s: String;
Begin
  try
    FBroker.REMOTEPROCEDURE := 'MAG DICOM GET AGENCY';
    s := FBroker.STRCALL;
    Result := S;
  except
    on E:Exception do
      magAppMsg('s', 'TMagDBMVista.RPMaggAgency error = ' + E.Message);
  end;
end;

{ P131 JMW 5/15/2013 - return a unique string to identify this broker connection}
function TMagDBMVista.GetBrokerIdentifier() : String;
begin
  result := '';
  if FBroker <> nil then
  begin
    result := fbroker.Server + ':' + inttostr(fbroker.ListenerPort);
  end;
end;
