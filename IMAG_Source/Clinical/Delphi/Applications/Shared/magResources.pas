...
/////////////  GARRETT TESTING...
(*
This is what was normally defined.
{$DEFINE NOVIEW}
($DEFINE CW}
..... others were not defined...  they didn't have the '$' in front of DEFINE
*)
Interface
...
Uses
  ...
  Dialogs,
  Stdctrls,
  uMagDefinitions, {/ P122 - JK 8/30/2011 - added to support the check for font family resource checking /}
  imaginterfaces,
...
Begin
 ...
 {$ENDIF}
  ...
  {/ P122 - JK 8/30/2011 - Check to be sure the client machine has type annotation font family installed.
  ...
  try
    ...
    begin
      MagAppMsg('d', 'This workstation does not have the ' + magAnnotationFont + ' font installed.' + #13#10 +
                       'Warning: Images that have textual annotations ' +
                       'may not display text or may display text with a different font.');  {/ P122 - JK 8/31/2011 WPR #32 /}
      GSess.AnnotationFontAvailable := False;  {JK - Put up a warning message upon logging in and keep Display working as it is}
      for i := 0 to FontList.Count - 1 do
         MagAppMsg('s', 'Installed Font: ' + FontList[i]);
    end;
  finally
    FontList.Free;
  end;
