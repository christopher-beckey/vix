...
Begin
  SecurityKeys.Clear;
  MagAppMsg('s', 'Retrieving user''s security keys: ');
  idmodobj.GetMagDBBroker1.RPMaggUserKeys(SecurityKeys);
  //InitializeKeyDependentObjects;

//(* RCA GEK out decouple    {JK 10/6/2009 - Set the MagLogger privilege level based on the Mag System key}
...
Begin
  ...
  Begin
    Result := False;
    MagAppMsg('de', 'Security Keys are not initialized in Key Manager.');
  End;
