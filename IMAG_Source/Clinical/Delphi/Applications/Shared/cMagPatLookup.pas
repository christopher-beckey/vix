...
//  WinMsg('Type patient name (partial is OK), or full SSN, or last initial, or last 4 of SSN. Then press <Enter>.'); {JK 1/4/2009 - spelling/capitalization}
  ...
  if GSess.Agency.IHS then
    WinMsg('Type patient HRN (partial is OK) or full HRN. Then press <Enter>.')
  else
    WinMsg('Type patient name (partial is OK), or full SSN, or last initial, or last 4 of SSN. Then press <Enter>.');
End;
...
//RCA TEST OUT
       PhotoImage := MagImageManager1.GetImageFile(PhotoImage, SiteAbbr, Xmsg);
        PhotoImage := PhotoImage;    //RCA IN when above is out.  .. have to CopyTheFile to get local
         {  Load the image}
