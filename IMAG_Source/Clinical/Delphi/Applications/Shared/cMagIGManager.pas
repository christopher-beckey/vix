...
Begin
  If MagIGManager = Nil Then
    ...
    //below is for Testing: TESTING
    //  showmessage('TMagIGMaganger Created.  Handle: ' + inttostr(MagIGManager.IGCoreCtrl.Handle));
    //  showmessage('TMagIGMaganger Created.  Handle: ' + inttostr(MagIGManager.AfterConstruction
    end;
  Result := MagIGManager;
End;
...
Begin
  UseDefault := True;
  DevOnly := False;
{p129T10 gek  when commented out,  error in Telereader
              unwanted coupling. Telereader <-> ArtX controls}
{$IFDEF USENEWANNOTS}
  IGArtXCtl := TIGArtXCtl.Create(Nil);
  IGArtXGUICtl := TIGArtXGUICtl.Create(Nil);
{$ENDIF}
  IGCoreCtrl := TIGCoreCtl.Create(Nil);
  //testing  showmessage('IGCoreCtrl Created.' + inttostr(igcorectrl.InstanceSize));
  IGCoreCtrl.Result.NotificationFlags := IG_ERR_OLE_ERROR;
  {Notification Flags   IG_ERR_EVENT_ERROR or IG_ERR_EVENT_WARNING ;  IG_ERR_OLE_ERROR}
  {Note :  Testing shows that we don't want to change Notification flags here.   it will have effect on Display}
  // IGCoreCtrl.Result.NotificationFlags := IG_ERR_EVENT_ERROR; //129 TESTING .. try IG_ERR_EVENT_ERROR
  // IGCoreCtrl.Result.NotificationFlags := IG_ERR_EVENT_ERROR or IG_ERR_EVENT_WARNING ; //129 TESTING .. try IG_ERR_OLE_ERROR or IG_ERR_EVENT_WARNING.

  //129 testing
  //showmessage(' IGManager notification flags  = ' + inttostr(IGCoreCtrl.Result.NotificationFlags)); 
  //129 testing End
...
{$ENDIF}
  ...
  Begin
    ...
    Else
      If Temp.ID = IG_FORMAT_PDF Then        {BM-ImagePrint-  CHANGING A PROPERTY FOR PRINT ???}
      Begin
