...
//    {/ P122 - JK 6/20/2011 - Get the annotation history information for an specific image IEN.
//       Calls RPC: MAG GET IMAGE ANNOTATIONS which returns a stringlist of annotation layers (histories) stored for the image. /}
//    procedure RPMagAnnotGetHistory(IEN: String; Var Fstat: Boolean; Var Rpcmsg: String; Var AnnotHistory: TStringList); Virtual;
//
//    {/ P122 - JK 6/20/2011 - Get the annotation XML for a specific image IEN and annotation layer (history).
//       Calls RPC: MAG GET IMAGE ANNOTATION /}
//    procedure RPMagAnnotGetXML(AnnotID: String; LayerNumber: String; var AnnotXML: TStringList; var Fstat: Boolean;
//                               var Rpcmsg: String); Virtual;
//
//    {/ P122 - JK 6/20/2011 - Saves the annotation XML for an specific image IEN as the most recent layer (history).
//       Calls RPC: MAG STORE IMAGE ANNOTATION DET /}
//    procedure RPMagAnnotSaveXML(IEN: String; AnnotSource: String; AnnotXML: TStringList;
//      Deleted: String; Var Fstat: Boolean; Var Rpcmsg: String; ServerName: String; ServerPort: Integer); Virtual;
    ...
    procedure RPMagAnnotGetXML(AnnotID: String; LayerNumber: String; var AnnotXML: TStringList; var Fstat: Boolean;
                               var Rpcmsg: String; ServerName: String; ServerPort: Integer); override;
...
//demolist.LoadFromFile(demodir+'\magdemoimagelist.txt');
  ...
  Exit;
//demo
 {TODO -clowPriority: needs activated for demo}
...
//      maggmsgf.magmsg('s', 'Increasing RPCTimeLimit to ' + inttostr(oldRPCTimelimit * 2));
    magAppMsg('s', 'Increasing RPCTimeLimit to ' + Inttostr(OldRpcTimeLimit * 2)); 
  ...
  Try
    ...
    magAppMsg('s', 'DFN:' + DFN + ' Start RPC MAG4 PAT GET IMAGES' + Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
    FBroker.LstCALL(Flist);
    //maggmsgf.MagMsg('s', 'DFN:' + DFN + ' Done RPC MAG4 PAT GET IMAGES' + formatdatetime('mm/dd/yy@hh:nn:ss.zzz', now));
    magAppMsg('s', 'DFN:' + DFN + ' Done RPC MAG4 PAT GET IMAGES' + Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); 
  ...
  magAppMsg('s', 'Restoring RPCTimeLimit to ' + Inttostr(OldRpcTimeLimit)); 
...
procedure TMagDBDemo.RPMagAnnotGetHistory(IEN: String; var Fstat: Boolean;
...
procedure TMagDBDemo.RPMagAnnotGetXML(AnnotID, LayerNumber: String;
  var AnnotXML: TStringList; var Fstat: Boolean; var Rpcmsg: String;
...
procedure TMagDBDemo.RPMagAnnotSaveXML(IEN, AnnotSource: String; Version: String;
  AnnotXML: TStringList; Deleted: String; var Fstat: Boolean;
...
Begin
{ TODO -clowPriority : Get a list of Rad Exams for one of the demo patients. }
...
//not needed for demo
  ...
  magAppMsg('s', '  -> Result = ' + FBroker.Results[0]); 
...
function TMagDBDemo.GetLogRPCCalls: boolean;
begin
//
end;
...
procedure TMagDBDemo.SetLogRPCCalls(value: boolean);
begin
  inherited;
  //
end;
...
{/ P122 - JK 7/6/2011 /}
function TMagDBDemo.RPKernelAnnotatePermission: Boolean;
Var
  s: String;
Begin
  Result := False;
...
{/ P123 - JK 8/8/2011 /}
function TMagDBDemo.RPMaggAgency: String;
Var
  s: String;
Begin
  try
    ...
    on E:Exception do
     magAppMsg('s', 'TMagDBDemo.RPMaggAgency error = ' + E.Message);
  end;
end;
