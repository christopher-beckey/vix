...
type
  ...
  protected
    ...
    FMore: boolean;
    //FResults,FDisplayFields,FFieldNumbers: TStrings; //*** BB ***
    FDisplayFields,FFieldNumbers: TStrings; //*** BB ***
    FResults: TStringList; //*** BB ***
  ...
  public
    ...
    //property Results: TStrings read FResults; //*** BB ***
...
begin
  ...
  for i := 0 to (FFDAList.Count - 1) do begin
    {$WARNINGS OFF}
    fda := TFMFDAObj(FFDAList.Items[i]);
    {$WARNINGS ON}
    fda.free;
    end;
  FFDAList.Clear;
...
begin
  ...
  if FFDAList.Count > 0 then Result := True;
end;
 ...
 if (Owner is Tform) and (Tform(Owner).ActiveControl <> nil) then begin
  ...
  try
    if BrokerOK(Self) then begin
      ...
      for index := 0 to (FFDAList.Count - 1) do begin
        {$WARNINGS OFF}
...
procedure Register;
begin
  //RegisterComponents('MagFileMan', [TFMLister]);
  //RegisterComponents('MagFileMan', [TFMGets]);
  //RegisterComponents('MagFileMan', [TFMValidator]);
  //RegisterComponents('MagFileMan', [TFMFiler]);
  //RegisterComponents('MagFileMan', [TFMHelp]);
  //RegisterComponents('MagFileMan', [TFMFinder]);
  //RegisterComponents('MagFileMan', [TFMFindOne]);
end;
