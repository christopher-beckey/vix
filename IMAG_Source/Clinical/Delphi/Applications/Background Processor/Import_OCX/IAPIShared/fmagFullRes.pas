...
Begin
  ...
  Begin
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Publishing ImageSelect from FullRes window'); {JK 10/6/2009 - Maggmsgu refactoring}
...
Begin
  ...
  If (IObj.ServerName <> Dmod.MagDBBroker1.GetServer) Or (IObj.ServerPort <> Dmod.MagDBBroker1.GetListenerPort) Then
//  if (iobj.PlaceCode <> wrksplacecode) then    // JMW 3/2/2005 p45
  Begin
    Showmessage('You cannot delete an image from a remote site');
    Exit;
  End;
  ImsgObj.MagMsg('s', 'Attempting deletion of ' + IObj.FFile + '  IEN ' + IObj.Mag0);
...
//  LogActions('FULL', 'DELETE', Iobj.Mag0);
  ...
  End;
  ImsgObj.LogMsg('s', 'Delete Result: ' + Rmsg);
End;
...
// MAIN MENU IS DIFFERENT THAN POPUOP
  ...
  Begin
    ...
    If Not MagImageManager1.IsImageCurrentlyCaching() Then Dmod.MagFileSecurity.MagCloseSecurity(Xmsg);
...
Begin
  ...
  MagLogger.LogMsg('s', 'horizScroll pos = ' + Inttostr(Mag4Viewer1.Scrlv.HorzScrollBar.Position)); {JK 10/6/2009 - Maggmsgu refactoring}
  //LogMsg('s','horizScroll ScrollPos = '+  inttostr(mag4viewer1.scrlV.HorzScrollBar.ScrollPos ));
  MagLogger.LogMsg('s', 'horizScroll ScrollPos = ' + Inttostr(Mag4Viewer1.Scrlv.HorzScrollBar.ScrollPos)); {JK 10/6/2009 - Maggmsgu refactoring}
  //LogMsg('s','horizScroll range = '+  inttostr(mag4viewer1.scrlV.HorzScrollBar.Range ));
  MagLogger.LogMsg('s', 'horizScroll range = ' + Inttostr(Mag4Viewer1.Scrlv.HorzScrollBar.Range)); {JK 10/6/2009 - Maggmsgu refactoring}
  //LogMsg('s','ScrollBox width = '+  inttostr(mag4viewer1.scrlV.Width ));
  MagLogger.LogMsg('s', 'ScrollBox width = ' + Inttostr(Mag4Viewer1.Scrlv.Width)); {JK 10/6/2009 - Maggmsgu refactoring}
...
Procedure TfrmFullRes.ImagePrintFullWin;
Var
  s: String;
  VGear: TMag4VGear;
Begin
