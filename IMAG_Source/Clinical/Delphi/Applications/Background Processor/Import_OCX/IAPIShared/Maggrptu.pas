...
{ TODO -cdemo :make the Health Summary Options in the main menu Disabled for Demo }
    Dmod.MagDBBroker1.RPMaggHSList(Rstat, Rmsg, Tlist, 'hs');
    If Not Rstat Then
    Begin
    //maggmsgf.MagMsg('','ERROR Compiling Health Summary list.');
      MagLogger.MagMsg('', 'ERROR Compiling Health Summary list.'); {JK 10/5/2009 - Maggmsgu refactoring}
...
Begin
  ...
  Try
    ...
    //MaggMsgf.magmsg('', 'Select a Health Summary Type', msg);
      MagLogger.MagMsg('', 'Select a Health Summary Type', Msg); {JK 10/5/2009 - Maggmsgu refactoring}
      Exit;
    End;
  //MaggMsgf.magmsg('', 'Building Health Summary', msg);
    MagLogger.MagMsg('', 'Building Health Summary', Msg); {JK 10/5/2009 - Maggmsgu refactoring}
    Msg.Update;
  ...
  { TODO -cdemo : make the Health Summary Options in the main menu Disabled for Demo}
    Dmod.MagDBBroker1.RPMaggHS(Rstat, Rmsg, Tlist, PatDFN.caption + '^' + Xst);
    LocShowreport(Tlist, PatDemog.caption, 'Health Summary   ' + Xst1 + '   ' + PatName.caption);
  //Maggmsgf.MagMsg('', 'Health Summary Complete.', msg);
    MagLogger.MagMsg('', 'Health Summary Complete.', Msg); {JK 10/5/2009 - Maggmsgu refactoring}
  Finally
    Tlist.Free;
  End;
End;
...
Begin
  ...
  Begin
    MagReplaceStrings(Dmod.MagPat1.M_PatName, Dmod.MagPat1.M_FakePatientName, t);
    MagReplaceStrings(Dmod.MagPat1.M_SSN, '000000000', t);
    MagReplaceStrings(Dmod.MagPat1.M_SSNdisplay, '000-00-0000', t);
    MagReplaceString(Dmod.MagPat1.M_PatName, Dmod.MagPat1.M_FakePatientName, Reporttitle);
  End;
