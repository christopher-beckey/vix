...

{
 ...
 Note:
  ...
  DebugMode :  is a more verbose wordy logging of events.
       lgdm(
  also
   ...
   Related and used in DebugMode.

   In the Initial install of OCX these Values are all FALSE, and do not Change.
   User can change them through the Registry.
...
type
  ...
  private
    { Patch  xxx CopyThread}
    mythread : TThreadCopyFile;
//    mythread : TThreadCopyInc;
    ThreadDone: boolean;
    ThreadStatus : integer;
    ThreadMessage : string;

     {p135 gek debugging controls.}
     {FDebugMode is Still under development, Not released.}
{NOT YET RELEASED }
{Future :
      FDebugMode is True/False to turn debugging on or off.
      If FDebugMode = True
        then debug messages will be logged to IMAGING WINDOWS SESSION FILE.
         or to a DOS File, that is defined in the MagImportX.ini file.
      }
      FLogDir : string;
    	FDebugON : boolean;
    	FDebugToFileON : boolean;
    	FDebugToFileName : string;
      FDebugLogSizeKB : integer;
    	Findent : string;
     {Still under development, Not released.}
     {Future :
...
//pxxx/    procedure StayAliveTimer(Sender: TObject);
...
(*   *** CheckDebugFromRegistry is called at from Create, and ImportQueue *)
    procedure CheckDebugFromRegistry;
      (* Next three functions/procedures are called from CheckDebugFromRegistry *)
      function GetRegistryFileNameIAPI(): string;
      function GetDebugFileName(logDir : string) : string;
      procedure LogFileInit(LogDir: string);    {For DebugMode}
      function AreWeDebugging() : boolean;
    ...
    function CopyThreadStart(FromFile, Tofile: string) : integer;
    procedure StayAliveTh;
    procedure LoopUntilDone;
    procedure WriteToLogFile(s: string);
    procedure ManageDiskFiles(logdir: string);
    procedure FormatResult(resstatus: boolean; var xmsg: string);
//    procedure OnThreadExit(Sender: TObject);
...
//pxxx/    function StayAlive(value : boolean): string;
...
begin
 ...
 // FGmsg := Tstringlist.create; {future.  Not used now}
  ...
  if not FileExists(FWaterMarkBitmap) then
    ...
    FDirImport := '' ;
    FDirImage := '' ;
//pxxx/  FTimerStayAlive := TTimer.Create(self);
//pxxx/  FTimerStayAlive.Enabled := false;
//pxxx/  FTimerStayAlive.OnTimer := StayAliveTimer;
end;
...
begin
  ...
  try
  try
    {vmsg0 is of format '0^...'  '1^...)'}           {result to BP}
    {vmsg0 is the message returned to BP or calling App as 0 node of array.}
    if not Validate(vmsg0) then   // etrap ok.        {result to BP 2}
    begin
    ...
    begin
      vmsg0 := '0^Error: The list of Images to be imported is Empty.';     {result to BP}
      Result := FALSE;
      exit
    end;
    ...
    begin
      ...
      if not CreateImageEntry(rmsgs, imageID, FImages[I]) then    {result to BP 2} // etrap ok
            begin
 ...
 // pxxx    rmsgs  message redesign   use rmsgs  for more detailed error. ?
            if not CopyImageToServer(rmsgs, imageID, imagefile) then   // dbg Copy1   {result to BP 2}
            begin
    ...
    if (ImageCopyOK.Count=0) then
    begin
        {.. this could be culprit, overwriting Error Msg from CopyImageToServer}
   {158  take out generic message}

     if (vmsg0 = '')  then vmsg0 := '0^ERROR: 0 Images copied';

    // vmsg0 := '0^ERROR: 0 Images copied';                            {result to BP}
     // BP takes out ':'  numbers (1,2,3...)   '-' is okay
      //vmsg0 := '0^THE 1 QUIC2K - B: ROWN FOX JUMPED OVER';                            {result to BP}
      Result := FALSE
    end
    else
    begin
...
begin
  E := ',' + UPPERCASE(fileext) + ',';
  Result := NOT (pos(E, ',MPG,ASC,AVI,DOC,HTM,RTF,WAV,TXT,MPEG,MHT,MHTML,HTML,MP3,MP4,XML,')>0)
end;
...
{ this call will set the Debug flags: FDebugON, FDebugToFileON, and FDebugFileName}
  ...
  begin
    ...
    lgdm('1- ' + xmsg);
    FLogList.insert(0,'0^No Connection to VISTA.');    {result to BP}
  ...
  If Not SetIAPIDirectories(Rmsg) Then
     ...
     resStatus := false;
            lgdm('1- '+ Rmsg);
     FLogList.insert(0,'0^'+rmsg);                        {result to BP}
     FLogList.insert(1, 'Queue Number : ' + Qnum);
     reslist.assign(FLogList);
     exit;
     end;
    ...
    lgdm('1- Param: QUEUE Number= '+Qnum);
    ...
    begin
      ...
      reslist.assign(Flist);     {result to BP}
...
if FImportActionValue = 'RESCIND' then
               begin
                 ...
                 on e:exception do
                   begin
                     ...
                     resStatus := false;
                     FLogList.insert(0,'0^EXCEPTION : CreateAccusoftControls.')  ;   {result to BP}
                     FLogList.insert(1,FtrkNum);
                     FLogList.Insert(2,Qnum);
                     reslist.assign(FLogList);
                     exit;
      ...
      begin
        ...
        lgdm('1- Tracking ID: ' + FTrkNum);

// Calling *******************************************
//         *******************************************
//                   SAVEToVistA
        resStatus := SAVEToVistA(xmsg);
// SAVEToVistA  *******   has been called.
    ...
    FormatResult(resstatus,xmsg);
    FLogList.insert(0, xmsg);                            {result to BP 1}  {xmsg from SaveToVistA}
    FLogList.insert(1, FTrkNum);
    FLogList.insert(2, Qnum);
...
procedure TMagImport.FormatResult(resstatus : boolean; var xmsg : string);
var p1,l1 ,p2      : string;
begin
 p1 := magpiece(xmsg,'^',1);
 p2 := magpiece(xmsg,'^',2);
 ...
 if ((p1 <> '0') and (p1 <> '1') and (p1 <> '2')) then
   begin
      if Resstatus  then p1 := '1'
                    else p1 := '0';
   xmsg := p1 + '^'  + p2;
   end;
end;
...
begin
  ...
  try
    ...
    //fUnStrings.Add(Inttostr(Funstrings.count)); // force error
    Result := CopyTheFile(xmsg, oFullName, nFullName)   // dbg Copy2      {result to BP 3}

  except
  on E:Exception do
   begin
   Result := false;
   xmsg := '0^EXCEPTION: CopyImageToServer';            {result to BP 2}
   lgm(xmsg);
   lgm('msg: ' + e.message);
   end;
  end;
...
begin
 ...
 indentinc;
       ...
       lgdm('5a-  Fusername: ' + Fusername + ' FPassword: ' + Fpassword);
        ...
        on E : Exception do
          begin
            ...
            xmsg := '-1^Error: Invalid Password.';      {result to BP 2}
  ...
  if tmpPass = '' then displayPass := '<null>';
    ...
    begin
      lgdm('5a- Attempt OpenSecurePath - FAILED. '+ xmsg);   {result to BP 2}
    ...
    begin
      ...
      xmsg := '0^File doesn''t exist. ' ;       {result to BP 2}
      exit
    end;
    magfilesize := (fmxutils.GetFileSize(FromFile));
    if magfilesize < 1 then
            begin
            lgdm('5a- File Size Zero: ' + FromFile);
            xmsg := '0^File Size Zero. ' ;       {result to BP 2}
            exit
            end;
    try
    lgdm('5a- Attempting OpenSecurePath: ToFile='+ToFile);
    lgdm('5a- tmpUser = null    tmpPass = null.');
    if not FMagSecurity.MagOpenSecurePath(ToFile, xmsg, '', '', true) then      {result to BP 2}
    begin
    ...
    on E:exception do
      begin
        result := false;
        xmsg := '0^EXCEPTION:  during Open Network Connection';        {result to BP 2}
        lgm(xmsg);
        lgm('msg: ' + e.message);
      end;
    end;
...
{HERE : use getlasterror....}
      ...
      begin
        ...
        xmsg := '0^Can''t create directory: ' + ExtractFileDir(ToFile);     {result to BP 2}
    ...
    try
      ...
      // THIS Is the windows Call :     HERE HERE
//pxxx/     {$I-}  //pxxx/
      //pre-threadCopy
//pxxx/      osCopyOK := Copyfile(pchar(FromFile), pchar(ToFile), true);    // dbg Copy3 //pxxx/
      thCopyRes := CopyThreadStart(fromfile,tofile);   //thFix
//pxxx/      {$I+}  //pxxx/
//pxxx/      osError := GetLastError;  //pxxx/
 //pxxx/     if osError <> 0 then lgdm('cp-2 GetLastError = ' + inttostr(osError));  //pxxx/
 //pxxx/     if osCopyOK then  //pxxx/  //if (thCopyRes = 0) then   //thFix
      if (thCopyRes = 0) then   //thFix
        begin
        lgdm('5a- "ThreadCopy " :   SUCCESS');
        Result := true;
       ...
       begin
       //pxxx/osErrMsg := SysErrorMessage(OSError);
        lgdm('5a- Attempt to "CopyFile" :  FAILED ');
        lgdm('5a- ThreadCopy: ' + inttostr(thCopyRes) + ' - ' + threadMessage);
        Result := FALSE;
        xmsg := '0^FAILED to Copy File: ' + FromFile + ' to: ' + ToFile ;    {result to BP 2}
        lgm('Last Error: ' + inttostr(osError) + ' - ' +  threadMessage);
       end;
 ...
 if Result then
   begin
        ...
        if magfilesize < 1  then
          begin
           result := false;
           lgm('Error:  FileSize 0 after copy: ' + FromFile );
           lgm(' - - -  to: ' + ToFile);
           lgdm('5a- Attempt to "CopyFile" :  FAILED ');
           lgdm('5a- ' + ' Error:  FileSize 0 after copy: ' + ToFile);

           xmsg := '0^Error: After copy : FileSize Zero' ;  {result to BP 2}
          end;
   end;
      ...
      on E : Exception do
        begin
          xmsg := '0^Exception CopyFile: ' + E.Message;    {result to BP 2}
          FMagSecurity.MagCloseSecurity(zmsg)
        end;
    end;
  ...
  lgdm(Fmagsecurity.Msglist); // in for debug
   ...
   on E: exception do
    begin
      xmsg := '0^Exception - CopyTheFile: ' + e.Message;     {result to BP 2}
      lgm(e.message);
    end;
  end;
 finally
...
if FDBBroker.IsConnected then
  ...
  lgdm('2a- VistA Connection is needed...');
    if (vserver='') or (vport='') then
      ...
      begin
      if AreWeDebugging then  {... for now if Debug On,  we Prompt.  it's not built to run continuously}
        begin
          lgdm('IAPI login values:');
          lgdm('vserver: ' + vserver + '  vport: ' + vport);
          lgdm('Accesscode: ' + halfdisplay(accesscode) + '  Verifycode: ' + halfdisplay(verifycode));
          lgdm('Division: ' + division );
        end;
...
{this isn't a true full path to a file name,  this result is used
   by Delphi RegistryINI functions to read/write to Registry
   This is in \HKEY_CURRENT_USER\    }

     result :=  'software\vista\imaging\ImportOCX';

 {Registry entries that will be checked later are :
    FDebugON :=  reg.ReadString('DebugOptions','DebugON',value);
    FDebugToFileON := Reg.ReadString('DebugOptions','DebugToLogFileON',value);
    Logdir := reg.ReadString('DebugOptions','LogFileDirectory',valdir) ;

    FDebugToFileName  uses datetime stamp. }
end;

procedure TMagImport.CheckDebugFromRegistry;
var
  value : string;
  pf,ipath,  reglogdir, secOn: string;
  reg : TRegINiFile;
  oserror : integer;
  runDT : string;
  runTDate : TdateTime;
  DaysOld : integer;
begin
  //158   We allways write to BackProc\log\utility
  pf :=  GetEnvironmentVariable('PROGRAMFILES');
  FLogdir :=   pf +'\vista\imaging\backproc\log\utility\';

  try
  reg := TRegIniFile.create(GetRegistryFileNameIAPI);

  value := '';
  value := reg.ReadString('DebugOptions','DebugON',value);
  FDebugON := (UPPERCASE(value) = 'TRUE');
  if value = '' then    // write initial values to Registry.
    begin
      reg.WriteString('DebugOptions','DebugON','FALSE');
      FDebugON := false;
      reg.WriteString('DebugOptions','DebugToLogFileON','FALSE');
      FDebugToFileON := false;
      reg.WriteString('DebugOptions','LogFileDirectory',FLogdir);
      FDebugLogSizeKB := 300;
      reg.WriteInteger('DebugOptions','LogFileSizeKB',FDebugLogSizeKB);
      runDt := '';
      reg.WriteString('DebugOptions','LastDebugRunTime',runDT);
      EXIT;
    end;
  ...
  if FDebugON then
    begin // we're here so FDebugON := true;..  check the Run Time Quit if too long.
    runDt := reg.ReadString('DebugOptions','LastDebugRunTime','');
    if runDt = '' then
      begin
      ...
      begin
       runTdate := strtoDateTime(runDt);
       DaysOLD := DaysBetween(Now, runTDate);
       if  (DaysOld > 0 ) then
         begin
           FDebugON := false;
           reg.WriteString('DebugOptions','DebugON','FALSE');
           reg.WriteString('DebugOptions','LastDebugRunTime','');
         end;
      end;
    end;

  value := 'FALSE';
  value := Reg.ReadString('DebugOptions','DebugToLogFileON',value);
  FDebugToFileON := (UPPERCASE(value) = 'TRUE');


  if NOT FDebugToFileON then EXIT;
  { all following code concerns FDebugToFileON .  Independent of FDebugON}
  FDebugLogSizeKB := reg.ReadInteger('DebugOptions','LogFileSizeKB',FDebugLogSizeKB);
  if FDebugLogSizeKB  = 0 then
    begin
    FDebugLogSizeKB := 500;
     reg.WriteInteger('DebugOptions','LogFileSizeKB',FDebugLogSizeKB);
    end;
  reglogdir := reg.ReadString('DebugOptions','LogFileDirectory','');
  if reglogdir <> FLogdir then
    begin
    reg.WriteString('DebugOptions','LogFileDirectory',FLogdir);
    end;

  {logfileinit could turn debugging off  if any problems initializing the Log file}
  LogFileInit(FLogDir);
...
function TmagImport.GetDebugFileName(logDir : string) : string;
var
       dir : string;
...
if FDebugToFileName = '' then
     begin
     if (copy(FLogDir,length(FLogDir),1) <> '\')
       then FLogDir := FLogDir +'\';
     FDebugToFileName := FLogDir +  'MagOCX_'+FormatDateTime('yymmdd_hhnnss', NOW)+'.log';
  ...
  if fileexists(FDebugToFileName) then
      begin
          curLogfilesize := (fmxutils.GetFileSize(FDebugToFileName) Div 1024) + 1 ;
          if curLogfilesize < FDebugLogSizeKB then    //100
             begin
             result := FDebugToFileName;
             exit;   //comment
             end
...
procedure TmagImport.LogFileInit(Logdir : string);
var
...
begin
   ...
   if not DirectoryExists(LogDir) then
     ...
     // FDebugToFileName is set in next Call
  if GetDebugFileName(LogDir) = '' then
    begin
      FDebugToFileON := False;
      EXIT;
    end;

  AssignFile(vDebugLogFile,FDebugToFileName);


//   Lbfilesize.caption := Inttostr((Getfilesize(AppName) Div 1024) + 1) + ' KB';

  if not FileExists(FDebugToFileName)
  ...
  close(vDebugLogFile);
  ManageDiskFiles(logdir);
end;

procedure TMagImport.ManageDiskFiles(logdir : string);
Var
  DT : TDateTime;
  DaysOld : integer;
...
{ Clean out the Log directory any file more than 1 day  old. }
  ...
  If FindFirst(LogDir + 'MagOCX*.*', FaAnyFile, SearchRec) = 0 Then
  Begin
    ...
    if (DaysOld > 0 ) then
      begin
      DeleteFile(logdir + SearchRec.Name);
...
var
  I: integer;
begin
If AreWeDebugging then            ///  Question here...  do we need to test both loggings.
...
procedure TMagImport.WriteToLogFile(s:string);
var afile : TextFile;
begin

if NOT	FDebugToFileON  then EXIT;
if FDebugToFileName = ''  then EXIT;
...
try
 ...
 on e:exception do
   begin
     FDebugToFileON := FALSE;
     FDebugToFileName := '';
     lgm('Exception writing to log file:  ' + e.Message);
...
function TMagImport.AreWeDebugging: boolean;
begin
  result := ( FDebugON or FDebugToFileON );
end;
...
function TmagImport.CopyThreadStart(FromFile, Tofile: string) : integer;
begin
    StayAliveTh;

  if (assigned(myThread) and (myThread <> nil))
    then
      begin
      lgm('CopyThread is assigned, and not Nil..  FreeAndNil it.');
      FreeAndNil(myThread);
      end;
  application.processmessages;

    mythread := TThreadCopyFile.create(FromFile, ToFile);
    //ThInOcxNoGood //mythread.OnTerminate := OnThreadExit;
    ThreadDone := false;
    ThreadStatus := -99;
    lgm('ThreadStart for  ------------------  ' + FromFile);
    mythread.resume;
...
procedure TmagImport.StayAliveTh();
var s : string;
begin
  ConnectIfNeeded(FSAvserver, FSAvport, FSAaccesscode, FSAverifycode,FSAdivision);
  ...
  try
  s := MagBroker.strCall;
  lgm('IM HERE: ' + s + '   '  + formatdatetime('nn:ss:zzz',now));
  except
   on e:exception do
     begin
       lgm('Exception in XWB IM HERE call: ' + e.message);
     end;
  end;
end;

(*procedure TMainXXX.Close;    Web Answer.
  begin
...
The asynchronous nature of the thread execution means that you run a risk
of not freeing the thread or (much worse) doing it twice.
There is no risk in keeping the thread object around
after it has finished the execution, and there is no risk in
calling Terminate() on a thread that has already finished either.*)

procedure TmagImport.LoopUntilDone();
 var sleepmil, j, i, ct : integer;
 done : boolean;
...
begin
  ...
  sttm := now;
  sleepmil := 100;
  lgm('Loop START at  ' + formatdatetime('nn:ss:zzz',sttm) );
  while not done do
     begin
     inc(ct);
     if ct > 60 then  sleepmil := 2000 // long copy, switch to every 2 seconds.
     else if ct > 40 then sleepmil := 200 ;    // 4 seconds of 1/5 sec
     // sleepmil starts at 100   (1/10 sec)  for 4 seconds.
     sleep(sleepmil);  //  1/10 sec... 1/5 sec  ..  2 sec
     application.processmessages;
     if ct > 60 then self.StayAliveTh;  // if over 12 seconds..  start Stayalive RPC call.
     if ((ct mod 5) = 0)
     then lgm('Percent: ' + inttostr(myThread.Percent) + '   Done: ' + inttostr(mythread.Done)
                       + '   Block: ' + inttostr(mythread.block) + '   todo: ' + inttostr(mythread.ToDo));

     if myThread.ThState = 0 then
       begin
         ThreadStatus := myThread.ThStatus;
         ThreadMessage := myThread.ThMessage;
         ThreadDone := true;
         lgm('Thstate = 0,   thread has finished.  ');
         mythread.Terminate;
         FreeandNil(mythread)  ;  //mythread.free;
       end;

     if ThreadDone then   Done := true;

     end;
  finally
    endtm := now;
    lgm('Loop END.  elapsed seconds: ' + inttostr(secondsbetween(endtm,sttm))
               + ' -----------  msg: ' + Threadmessage);
    lgdm('Loop  DONE at  ' + formatdatetime('nn:ss:zzz',endtm)
                  + '  St : ' +  inttostr(ThreadStatus)
                  + '  Msg : ' +  ThreadMessage
                  + '  Elapsed time: '  + inttostr(secondsbetween(endtm,sttm)));
  end;
end;


(*  not beign used now.  the Thread Exit and Sync with Main thread doesn't work nice when
    begin run in OCX  (ActiveX)  ... known issue in Delphi  *)
(*
procedure TMagImport.OnThreadExit(Sender: TObject);
begin
lgm('Thread Exit');
ThreadStatus := myThread.ThStatus;
ThreadMessage := myThread.ThMessage;
