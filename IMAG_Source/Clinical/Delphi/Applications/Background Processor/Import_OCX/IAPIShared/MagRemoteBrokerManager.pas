...
Type
  ...
  Private
  {
    ...
    FLocalSiteCode: String; // the local site code
...
//    function isSiteDOD(SiteCode : String) : boolean;
    ...
    Function GetLocalSite(): TVistaSite;
...
//procedure DoneMakingConnections();
   ...
   //46
    ...
    Function GetPatientStudies(Pkg, Cls, Types, Event, Spc, FrDate, ToDate, Origin: String; Var Results: TStrings; Var Partial_Returned: Boolean; Var Partial_Msg: String): Boolean;
...
//procedure TMagRemoteBrokerManager.LogMsg(MsgType : String; Msg : String; Priority : TMagLogPriority = MagLogINFO);
//begin
...
//  maggmsgf.MagMsg('s', 'Unable to find broker at [' + Server + ', ' + inttostr(Port) + '] to run function [' +  RPCFunction + '], function will not continue. ' + formatdatetime('mm/dd/yy@hh:nn:ss.zzz', now));
  //LogMsg('s', 'Unable to find broker at [' + Server + ', ' + inttostr(Port) + '] to run function [' +  RPCFunction + '], function will not continue. ' + formatdatetime('mm/dd/yy@hh:nn:ss.zzz', now), MagLogWARN);
  MagLogger.LogMsg('s', 'Unable to find broker at [' + Server + ', ' +
    Inttostr(Port) + '] to run function [' + RPCFunction + '], function will not continue. ' +
    Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now), MagLogWARN); {JK 10/6/2009 - Maggmsgu refactoring}
End;
...
// 46 had all code from here.
    ...
    //LogMsg('s', 'MagRemoteBrokerManager - a new patient has been selected, setting new patient selected to true');
                      MagLogger.LogMsg('s', 'MagRemoteBrokerManager - a new patient has been selected, setting new patient selected to true'); {JK 10/6/2009 - Maggmsgu refactoring}
...
Begin
  ...
  Begin
    ...
    MagLogger.LogMsg('s', 'MagRemoteBrokerManager - a new patient has been selected, will not retrieve patient DFN at remote sites'); {JK 10/6/2009 - Maggmsgu refactoring}
    Exit;
  End;
     //46 end
  For i := 0 To BrokerCount - 1 Do
...
//    if RemoteBrokerArray[i].getSiteName() = SiteName then
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Connection to Broker [' + Server + ', ' +
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Creating Connection to Broker [' + RemoteBrokerArray[i].GetServerDescription() + ']...'); {JK 10/6/2009 - Maggmsgu refactoring}
      ...
      Begin
        ...
        MagLogger.LogMsg('s', 'Failed to connect to broker [' + RemoteBrokerArray[i].GetServerDescription() + ']'); {JK 10/6/2009 - Maggmsgu refactoring}
        ...
        Begin
          ...
          MagLogger.LogMsg('s', 'Remote broker [' + RemoteBrokerArray[i].GetServerDescription() +
            '] was WS and does not require ViX, attempting to connect to remote VA site with RPC Broker'); {JK 10/6/2009 - Maggmsgu refactoring}
          ...
          //RemoteBrokerArray[i].OnLogEvent := OnLogEvent;   {JK 10/6/2009 - Maggmsgu refactoring}
      ...
      Dmod.MagFileSecurity.ShareList.AddStrings(Shares);
      FreeAndNil(Shares);
        //LogMsg('s', 'Connection to Broker [' + RemoteBrokerArray[i].getServerDescription() + '] has been established.');
      MagLogger.LogMsg('s', 'Connection to Broker [' + RemoteBrokerArray[i].GetServerDescription() + '] has been established.'); {JK 10/6/2009 - Maggmsgu refactoring}
...
Begin
  ...
  Begin
    ...
    MagLogger.LogMsg('s', 'MagRemoteBrokerManager - a new patient has been selected, will not count patient images at remote sites'); {JK 10/6/2009 - Maggmsgu refactoring}
    Exit;
  End;
  For i := 0 To BrokerCount - 1 Do
  Begin
...
Begin
  ...
  Begin
    ...
    MagLogger.LogMsg('s', 'Demo Site file [' + DemoFilename + '] not found, unable to connect to demo remote brokers'); {JK 10/6/2009 - Maggmsgu refactoring}
...
Begin
  ...
  Begin
    ...
    UserCredentials.LocalSiteNumber := GSess.Wrksinst.ID;
...
//      SiteData := getDemoSites(SiteCodes, SiteData);
      ...
      Begin
        ...
        Begin
          ...
          MagLogger.LogMsg('s', 'Server details for a demo site was not found, unable to connect to demo remote broker.'); {JK 10/6/2009 - Maggmsgu refactoring}
          Status := False;
        End
        Else
        Begin
   ...
   //72 only had the one param,
          ...
          Begin
            ...
            Begin
              ...
              MagLogger.LogMsg('s', 'MagRemoteBrokerManager - The user has cancelled connections to remote sites, no longer connecting to remote sites'); {JK 10/6/2009 - Maggmsgu refactoring}
            End
            Else
              ...
              Begin
                ...
                MagLogger.LogMsg('s', 'MagRemoteBrokerManager - A new patient was selected, no longer connecting to remote sites'); {JK 10/6/2009 - Maggmsgu refactoring}
              End;
          End;
  ...
  Begin
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Site Service has been disabled.  Remote Image Views will not work without this service.'); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'VistA Site Services has already failed, will not attempt again.  Remote Image Views will not work without this service' +
        Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    Try
      ...
      MagLogger.LogMsg('s', 'Retrieving remote site information for sites [' + SiteCodes + ']'); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    Except
      ...
      Begin
          //LogMsg('s', 'Unable to access Remote Site Services Server.  Remote Image Views will not work without this service. Exception=[' + e.Message + '].', MagLogERROR);
        MagLogger.LogMsg('s', 'Unable to access Remote Site Services Server.  Remote Image Views will not work without this service. Exception=[' +
          e.Message + '].', MagLogERROR); {JK 10/6/2009 - Maggmsgu refactoring}
        RIVNotifyAllListeners(Self, 'SiteServiceFailed', SiteServiceURL);
        Result := False;
...
Begin
  ...
  MagLogger.LogMsg('s', 'Local Site Code=[' + FLocalSiteCode + '].'); {JK 10/6/2009 - Maggmsgu refactoring}
  ...
  Begin
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Unable to access Remote Site Services Server to get local site details.  Remote Image Views will not work without this service.'); {JK 10/6/2009 - Maggmsgu refactoring}
      Result := False;
      Exit;
    End;
    ...
    Try
      ...
      MagLogger.LogMsg('s', 'Retrieving sites in local VISN. ' + Formatdatetime('mm/dd/yy@hh:nn:ss.zzz', Now)); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    Except
      On e: Exception Do
      Begin
        //LogMsg('s','Exception getting local VISN, Exception=[' + e.Message + ']', MagLogError);
        MagLogger.LogMsg('s', 'Exception getting local VISN, Exception=[' +
          e.Message + ']', MagLogERROR); {JK 10/6/2009 - Maggmsgu refactoring}
         // do something here
      End;
    End;
  ...
  MagLogger.LogMsg('s', 'Determining if Remote Site [' + Sitecode + '] is in local VISN.'); {JK 10/6/2009 - Maggmsgu refactoring}
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        MagLogger.LogMsg('s', 'Already connected to Broker [' + Sitecode + '], no new connection required'); {JK 10/6/2009 - Maggmsgu refactoring}
      ...
      Begin
        ...
        MagLogger.LogMsg('s', 'Broker [' + Sitecode + '], has been disconnected, information will not be loaded from this site.'); {JK 10/6/2009 - Maggmsgu refactoring}
      End;
      Exit;
    End;
  End;
  ...
  begin
    ...
    begin
      ...
      begin
        LogMsg('s','Requesting access to DOD site but user does not have required key to view DOD data', MagLogINFO);
        useViX := false;
      end;
    end;
  end;
  ...
  If GetLocalSite().IsSiteVixEnabled() Then
  Begin
    //LogMsg('s', 'Local site has a VIX defined in the site service, will attempt to use', MagLogInfo);
    MagLogger.LogMsg('s', 'Local site has a VIX defined in the site service, will attempt to use', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    Begin
      ...
      //  + getLocalSite().getVixUrl() + ']', MagLogInfo);
      MagLogger.LogMsg('s', 'Requesting list of interface versions available from local VIX at url ['
        + GetLocalSite().GetVixUrl() + ']', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
      VixServiceAvailable :=
        cMagRemoteWSBrokerFactory.IsImagingServiceAvailable(GetLocalSite().GetVixUrl());

      If VixServiceAvailable Then
      Begin
        //LogMsg('s', 'Local VIX has interface versions available, will attempt to use VIX', MagLogInfo);
        MagLogger.LogMsg('s', 'Local VIX has interface versions available, will attempt to use VIX', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
        ...
        Begin
          //LogMsg('s','Site is DOD but Allowed DOD Sites is false', MagLogINFO);
          MagLogger.LogMsg('s', 'Site is DOD but Allowed DOD Sites is false', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
          UseViX := False;
        End
        Else
        Begin
          ...
          Begin
            ...
            MagLogger.LogMsg('s', 'Requested site [' + Site.SiteNumber + '] is VA and there is a local VIX, but access to local VIX for VA sites is not allowed, not using VIX.', MagLogINFO);
            UseViX := False;
          End
          Else
          Begin
            // the remote site can use the ViX and its allowed for this site
            //LogMsg('s', 'Requested site [' + site.SiteNumber + '] will attempt to connect using the VIX', MagLogInfo);
            MagLogger.LogMsg('s', 'Requested site [' + Site.SiteNumber + '] will attempt to connect using the VIX', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
            UseViX := True;
            Port := GetLocalSite().VixPort;
            Server := GetLocalSite().VixServer;
          End;
      ...
      Else
      Begin
        //LogMsg('s', 'Cannot find appropriate interface version from VIX - cannot communicate with IDS on VIX to get acceptable version', MagLogWARN);
        MagLogger.LogMsg('s', 'Cannot find appropriate interface version from VIX - cannot communicate with IDS on VIX to get acceptable version', MagLogWARN); {JK 10/6/2009 - Maggmsgu refactoring}
      End;
    End;
  End;
  ...
  Begin
      ...
      //LogMsg('s', 'Getting remote WS broker for site [' + site.SiteNumber + '], local VIX URL [' + getLocalSite().getVixUrl() + ']', MagLogInfo);
    MagLogger.LogMsg('s', 'Getting remote WS broker for site [' + Site.SiteNumber + '], local VIX URL [' + GetLocalSite().GetVixUrl() + ']', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
  ...
  Else
    ...
    Begin
      ...
      //LogMsg('s','Site [' + site.SiteNumber + '] requires a ViX but none is available, creating dummy broker', MagLogINFO);
      MagLogger.LogMsg('s', 'Site [' + Site.SiteNumber + '] requires a ViX but none is available, creating dummy broker', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
      RemoteBrokerArray[BrokerCount] := TMagRemoteDummyBroker.Create(Site);
    End
    Else // site does not require a ViX and no ViX is available
    Begin
      //LogMsg('s', 'No local VIX is available, but is not necessary for site [' + site.SiteNumber + '], creating RPC Broker', MagLogInfo);
      MagLogger.LogMsg('s', 'No local VIX is available, but is not necessary for site [' + Site.SiteNumber + '], creating RPC Broker', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    //RemoteBrokerArray[BrokerCount].OnLogEvent := OnLogEvent;  {JK 10/6/2009 - Maggmsgu refactoring}
  ...
  if not SkipOver then begin
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Broker [' + Sitecode + '] has been marked to NOT auto-connect to.'); {JK 10/6/2009 - Maggmsgu refactoring}
      Exit;
    End;
    ...
    Begin
      ...
      begin
        ...
        MagLogger.LogMsg('s', 'Broker [' + Sitecode + '] will not auto-connect because it is not in your local VISN.'); {JK 10/6/2009 - Maggmsgu refactoring}
        Exit;
      end;
    End;
    ...
    begin
      ...
      BrokerCount := BrokerCount + 1;
      RIVNotifyAllListeners(Self, 'AddDisconnected', Sitecode);
      MagLogger.LogMsg('s', 'Broker [' + Sitecode + '], user preference for DoD is to not auto-connect.');
      Exit;
    end;
  end; {SkipOver}
  ...
  MagLogger.LogMsg('s', 'Creating Connection to Broker [' + Sitecode + ']...'); {JK 10/6/2009 - Maggmsgu refactoring}
  ...
  Begin
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Failed to connect to remote site [' + RemoteBrokerArray[BrokerCount].GetServerDescription() + '] using ViX'); {JK 10/6/2009 - Maggmsgu refactoring}
      ...
      Begin
        ...
        MagLogger.LogMsg('s', 'Connection to remote site [' + RemoteBrokerArray[BrokerCount].GetServerDescription() + '] failed, site requires VIX and cannot connect through other methods');
      ...
      Begin
        Site := TVistaSite.Create(RemoteBrokerArray[BrokerCount].GetSite()); // necessary because the site that was passed to remote broker is about it be destroyed
          //LogMsg('s','Creating RPC broker to connect to site [' + site.SiteNumber + '] since VIX failed', MagLogInfo);
        MagLogger.LogMsg('s', 'Creating RPC broker to connect to site [' + Site.SiteNumber + '] since VIX failed', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
        ...
        Begin // now the rpc broker connection failed, can no longer continue
          ...
          MagLogger.LogMsg('s', 'Second attempt to connect to remote site [' + RemoteBrokerArray[BrokerCount].GetServerDescription() + '] now through rpc broker failed');
            {JK 10/6/2009 - Maggmsgu refactoring}
          Result := False;
          BrokerCount := BrokerCount + 1;
          RIVNotifyAllListeners(Self, 'AddDisconnected', Sitecode);
    ...
    Begin
        //LogMsg('s', 'Failed to connect to remote site [' + SiteCode + '], already using RPC broker, cannot do anything else', MagLogError);
      MagLogger.LogMsg('s', 'Failed to connect to remote site [' + Sitecode + '], already using RPC broker, cannot do anything else', MagLogERROR); {JK 10/6/2009 - Maggmsgu refactoring}
      Result := False;
      BrokerCount := BrokerCount + 1;
      RIVNotifyAllListeners(Self, 'AddDisconnected', Sitecode);
      Exit;
  ...
  MagLogger.LogMsg('s', 'Connection to Broker [' + Sitecode + '] Created'); {JK 10/6/2009 - Maggmsgu refactoring}
  BrokerCount := BrokerCount + 1;
End;
...
Begin
  ...
  If InputData.Count = 1 Then
  Begin
    SiteCount := MagUtilities.Maglength(InputData.Strings[0], '^'); // - 1;
    If SiteCount = 0 Then
    Begin
      //p59  AddStrings is new.  also param treatinglist is now Tstringlist
      //treatinglist := inputdata; //46,72
      TreatingList.AddStrings(InputData); //59
    End
    Else
    Begin
      i := 1;
      While (i + 4) <= SiteCount Do
      Begin
        // not totally sure about this... these are the values I have seen that seem to cause trouble when looking up Site information
        // there might be other values that are in the site codes from the treating facility list.
        Sitecode := MagUtilities.MagPiece(InputData.Strings[0], '^', i);
        {
        if pos('VAMC', SiteCode) = 1 then
        begin
          SiteCode := copy(SiteCode, 5, length(SiteCode));
        end;
        loc := pos('OC', SiteCode);
        if loc > 0 then
        begin
          SiteCode := copy(SiteCode, loc + 2, length(SiteCode));
        end;
        loc := pos('OPC', SiteCode);
        if loc > 0 then
        begin
          SiteCode := copy(SiteCode, loc + 3, length(SiteCode));
        end;
        }
        // DPC200HD, VAMC200HD, probably more
        If Pos('200HD', Sitecode) = 0 Then
//        if SiteCode <> 'VAMC200HD' then // completely ignore this site code (HDR)
        Begin
          Sitecode := ParseSiteCode(Sitecode);
          SiteName := MagUtilities.MagPiece(InputData.Strings[0], '^', i + 1);
          // JMW 8/13/08 - if the site number is 200 then just show as DOD, don't
          // allow the other names to appear (HDR Available Soon, EDS, etc)
          If Sitecode = '200' Then
            SiteName := 'DOD';
          {72gekQF1    Quick Fix to Replace 'AUSTIN' with 'DOD'}
          //72gekQF1 f uppercase(SiteName) = 'AUSTIN' then SiteName := 'DOD';
          SiteName := AnsiReplaceText(SiteName, 'Austin', 'DOD'); //72gekQF1
          Part3 := MagUtilities.MagPiece(InputData.Strings[0], '^', i + 2);
          Part4 := MagUtilities.MagPiece(InputData.Strings[0], '^', i + 3);
          SiteDetails := Sitecode + '^' + SiteName + '^' + Part3 + '^' + Part4 + '^';
  ...
  End
  Else
  Begin // already in list form

    //treatingList := inputdata; //this was in 46 but out in 72
    // check for hdr, parse it out
    For i := 0 To InputData.Count - 1 Do
    Begin
      SiteDetails := InputData.Strings[i];
      Sitecode := MagUtilities.MagPiece(InputData.Strings[0], '^', 1);
      If Sitecode <> 'VAMC200HD' Then
      Begin
        TreatingList.Add(SiteDetails);
        {TODO -c5972 : what is happenning in this function ? }
           // was in 59, ?   treatinglist.AddStrings(inputdata);//in 59
      End;
    End;
  End;
End;
...
Begin
 //46 put in -> if self.NewPatientSelected then
  ...
  Begin
    ...
    MagLogger.LogMsg('s', 'MagRemoteBrokerManager - a new patient has been selected, will not check patient sensitive at remote sites. ' +
      'Setting all remote sites to PATIENT_INACTIVE'); {JK 10/6/2009 - Maggmsgu refactoring}
    For i := 0 To BrokerCount - 1 Do
    Begin
      If RemoteBrokerArray[i].GetPatientStatus = STATUS_PATIENTACTIVE Then
      Begin
  ...
  Begin {1}
    ...
    //46 put in -> if self.NewPatientSelected then
      ...
      Begin {3}
        ...
        MagLogger.LogMsg('s', 'Remote sensitive code of [' + Inttostr(RemoteCode)
          + '] current sensitive code is  [' +
        ...
        Case RemoteCode Of {5}
            ...
            Begin
              ...
              Begin
                ...
                Begin
                  ...
                  MagLogger.LogMsg('s', 'User has chosen not to view restricted patient @ ' + RemoteBrokerArray[i].GetSiteName); {JK 10/6/2009 - Maggmsgu refactoring}
                End
                Else
                Begin
                  Xmsg := 'Access to this restricted patient has been logged.';
                  //LogMsg('s', 'User has chosen to view restricted patient @ ' + RemoteBrokerArray[i].getSiteName);
                  MagLogger.LogMsg('s', 'User has chosen to view restricted patient @ ' + RemoteBrokerArray[i].GetSiteName); {JK 10/6/2009 - Maggmsgu refactoring}
          ...
          3:
            Begin
              ...
              MagLogger.LogMsg('s', 'Access to this patient @ [' + RemoteBrokerArray[i].GetSiteName + '] is not allowed.'); {JK 10/6/2009 - Maggmsgu refactoring}
            End;
        End; (*CASE *) {5}
...
Function TMagRemoteBrokerManager.GetPatientStudies(Pkg, Cls, Types, Event, Spc, FrDate, ToDate, Origin: String;
                                                   Var Results: TStrings; Var Partial_Returned: Boolean; Var Partial_Msg: String): Boolean;  {/ P117 NCAT - JK 11/30/2010 /}
Var
  i: Integer;
...
Begin
  ...
  Begin
    Try
      ...
      MagLogger.LogMsg('s', 'Getting ImagingExchangeSiteService, URL [' + ExchangeSiteServiceURL + ']'); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    Except
      ...
      Begin
        ...
        MagLogger.LogMsg('s', 'Exception getting site service proxy, URL [' + ExchangeSiteServiceURL + '], Exception [' + e.Message + ']'); {JK 10/6/2009 - Maggmsgu refactoring}
        ExchangeSiteService := Nil;
        Result := Nil;
        Exit;
      End;
...
Begin
  ...
  Begin
    MagLogger.LogMsg('s', 'Site [' + Site.SiteNumber + '] returned from site service with error [' + Site.FaultTO.message_ + ']', MagLogERROR);
    Result := Nil;
    Exit;
  End;
  ...
  Begin
    ...
    Begin
      ...
      Begin
        ...
        Try
          MagLogger.LogMsg('s', 'Finding local site information from site service with local site number [' + SNumber + ']', MagLogINFO);
          ...
          Begin
            ...
            MagLogger.LogMsg('s', 'Got nil local site from site service, indicates local site number not in site service, trying again with primary site station number  [' + SNumber + ']',
              MagLogINFO);
        ...
        Except
          ...
          Begin
            ...
            //LogMsg('s','Exception getting local site, [' + e.Message + ']', MagLogERROR);
            MagLogger.LogMsg('s', 'Exception getting local site, [' + e.Message + ']', MagLogERROR); {JK 10/6/2009 - Maggmsgu refactoring}
      ...
      Begin
        FLocalSite := TVistaSite.Create();
        FLocalSite.SiteNumber := SNumber;
        //LogMsg('s','Site service not found - null', MagLogERROR);
        MagLogger.LogMsg('s', 'Site service not found - null', MagLogERROR); {JK 10/6/2009 - Maggmsgu refactoring}
        CheckOverrideLocalVix(); // not sure if the override should be done in this error situation...
      End;
    End;
    If FLocalSite <> Nil Then
      MagLogger.LogMsg('s', 'Local site set to site [' + FLocalSite.SiteNumber + ']', MagLogINFO);
  End;
  Result := FLocalSite;
End;
...
Begin
  ...
  Begin
    ...
    //LogMsg('s','Overriding local VIX Server from Site Service with [' + LocalVixServerOverride + ']', MagLogINFO);
    MagLogger.LogMsg('s', 'Overriding local VIX Server from Site Service with [' + LocalVixServerOverride + ']', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
  End;
  If LocalVixPortOverride > 0 Then
  Begin
    FLocalSite.VixPort := LocalVixPortOverride;
    //LogMsg('s','Overriding local VIX Port from Site Service with [' + inttostr(LocalVixPortOverride) + ']', MagLogINFO);
    MagLogger.LogMsg('s', 'Overriding local VIX Port from Site Service with [' + Inttostr(LocalVixPortOverride) + ']', MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
  End;
End;
...
Begin
  ...
  Begin
    ...
    //LogMsg('s','Could not find local site in demosites files', MagLogERROR);
    MagLogger.LogMsg('s', 'Could not find local site in demosites files', MagLogERROR); {JK 10/6/2009 - Maggmsgu refactoring}
  End
  Else
  Begin
    Try
    ...
    Except
      ...
      Begin
        Result := TVistaSite.Create();
        Result.SiteNumber := SNumber;
        //LogMsg('s','Could not find local site in demosites files, Exception=[' + e.Message + ']', MagLogERROR);
        MagLogger.LogMsg('s', 'Could not find local site in demosites files, Exception=[' + e.Message + ']', MagLogERROR); {JK 10/6/2009 - Maggmsgu refactoring}
      End;
    End;
  End;
