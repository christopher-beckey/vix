...
Begin
  ...
  Try
    ...
    Begin
      ...
      Dmod.MagDBBroker1.RPFilterSave(Rstat, Rmsg, t);
      If Not Rstat Then
...
//ClearAll;
  ...
  Try
    Dmod.MagDBBroker1.RPFilterListGet(Rstat, Rmsg, Rlist, PDuz);
    If Not Rstat Then
...
Begin
  ...
  Begin
    Dmod.MagDBBroker1.RPFilterDelete(Rstat, Rmsg, FcurFltIEN);
    If Not Rstat Then
          //maggmsgf.MagMsg('ed', rmsg)
      MagLogger.MagMsg('ed', Rmsg) {JK 10/5/2009 - Maggmsgu refactoring}
    Else
    Begin
      WinMsg(Rmsg);
      ClearAll;
...
Begin
  ...
  Try
    ...
    t := GetFilterDesc(Filter, Dmod.MagDBBroker1);
...
//  result.add('');
    ...
    Begin
      ...
      Begin
        ...
        If Ixien <> '' Then t.Add(' --- ^' + Dmod.MagDBBroker1.RPXWBGetVariableValue('$P($G(^MAG(2005.83,' + Ixien + ',0)),U,1)'));
      End;
    End;
 // t.add('Types^' + s);
//  result.add('');
    ...
    Begin
      ...
      Begin
        ...
        If Ixien <> '' Then s := s + Dmod.MagDBBroker1.RPXWBGetVariableValue('$P($G(^MAG(2005.84,' + Ixien + ',0)),U,1)') + ',';
      End;
    End;
...
//  result.add('[Procedure/Event]:');
    ...
    Begin
      ...
      Begin
        ...
        If Ixien <> '' Then s := s + Dmod.MagDBBroker1.RPXWBGetVariableValue('$P($G(^MAG(2005.85,' + Ixien + ',0)),U,1)') + ',';
      End;
    End;
...
//  result.add('');
  ...
  result.add('Origin :       '+s);}
    ...
    If Ixien <> '' Then
      s := Dmod.MagDBBroker1.RPXWBGetVariableValue('$P($G(^VA(200,' + Ixien + ',0)),U,1)')
    Else
      s := 'Any';
    t.Add('Saved By^' + s);
...
Begin
  FMSet := TImageFMSet.Create;
  s := Dmod.MagDBBroker1.RPXWBGetVariableValue('$P($G(^DD(2005,45,0)),U,3)');
  FMSet.DBSetDefinition := s;
