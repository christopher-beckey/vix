...
Function CopyTheFile(FromFile, ToDir: String): Boolean;
        {}
Procedure ImagePrintEsigReason(MagVGear: TMag4VGear);
        {}
...
Function MgrImageDelete(IObj: TImageData; Var Xmsg: String): Boolean;
...
Begin
    { this'll be gone with MultiProcedure listing which will use a TMagListView
       and/or TMagTreeView.}
  Radlistwin.DFN.Text := Dmod.MagPat1.M_DFN;
...
Var
  ...
  Rstat: Boolean;
Begin
  MagLogger.MagMsg('', 'Checking Exam  : ' + MagPiece(Radstring, '^', 3) + '  for Images...');
  ...
  Try
    ...
    Dmod.MagDBBroker1.RPMaggRadImage(Rstat, Rmsg, t, Radstring);
    If Not Rstat Then
    Begin
      If MagPiece(t[0], '^', 1) = '-2' Then
        MagLogger.MagMsg('DEQA', Rmsg)
      Else
        MagLogger.MagMsg('de', Rmsg);
      MagLogger.MagMsg('s', 'Rad Report info : ' + Sysradstring);
      Exit;
    End;
    MagLogger.MagMsg('', MagPiece(t[0], '^', 1) + ' ' + MagPiece(t[0], '^', 2));
  ...
  Try
    ...
    Begin
      Sl := 'Deletion Canceled : ' + #13 + Xmsg + #13 + 'Image ID#: ' + IObj.Mag0;
        //LogMsg('',sl);
      MagLogger.LogMsg('d', Sl); {JK 10/6/2009 - Maggmsgu refactoring} {JK 1/20/2010 - added the 'd' param}
      Exit;
    End;

    //LogMsg('', 'Deleting Image ID# ' + Iobj.Mag0 + '...');
    MagLogger.LogMsg('', 'Deleting Image ID# ' + IObj.Mag0 + '...'); {JK 10/6/2009 - Maggmsgu refactoring}

    Dmod.MagDBBroker1.RPMaggImageDelete(Rstat, Xmsg, Rlist, IObj.Mag0, '', Reason, AllowGrpDel);
    If Not Rstat Then
    Begin
      For i := 0 To Rlist.Count - 1 Do
        ...
        //LogMsg('de', sl);
      MagLogger.LogMsg('de', Sl); {JK 10/6/2009 - Maggmsgu refactoring}
      For i := 0 To Rlist.Count - 1 Do
          //LogMsg('s', rlist[i]);
        MagLogger.LogMsg('s', Rlist[i]); {JK 10/6/2009 - Maggmsgu refactoring}
      Exit;
    End;
    Result := True;
    //LogMsg('d', xmsg);
...
try
if not  dmod.MagFileSecurity.MagOpenSecurePath(textfile,xmsg) then
  begin
    t.Add(xmsg);
    exit;
  end;
...
Procedure PatientIDMismatchLog(MagVGear: TMag4VGear);
Begin
  Dmod.MagDBBroker1.RPLogCopyAccess('IMGMM^^' +
...
Begin
  ...
  If Not Dmod.MagUtilsDB1.GetEsig(Xmsg) Then Exit;
  InitializeKeyDependentObjects; // for display app
  If Not Dmod.MagUtilsDB1.GetReason(1, Reason) Then Exit;
  Dmod.MagUtilsDB1.ImagePrint(MagVGear, Stat, Xmsg);
 // dmod.MagDBBroker1.RPLogCopyAccess(reason);
 // if dmod.MagDBBroker1 <> nil then
  reascode := MagPiece(Reason, '^', 1);
  Dmod.MagDBBroker1.RPLogCopyAccess(reascode + '^^' +
...
Begin
  ...
  If Not Dmod.MagUtilsDB1.GetReason(2, Reason) Then Exit;
  Dmod.MagUtilsDB1.ImageCopy(MagVGear, Stat, Xmsg, True);
...
Begin
  ...
  If Dmod.MagPat1.M_UseFakeName Then IObj.PtName := Dmod.MagPat1.M_FakePatientName;

  Try { finally will assure Cursor is default }
  //LogMsg('', '');
    MagLogger.LogMsg('', ''); {JK 10/6/2009 - Maggmsgu refactoring}
  ...
  {   Quit if this image is of Questionable Integrity {}
    ...
    Begin
      ...
      MagLogger.LogMsg('DEQA', IObj.QAMsg); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    Begin
      ...
      Begin
       ...
       //logmsg('s',Iobj.GetViewStatusMsg,MagLogInfo);
          MagLogger.LogMsg('s', IObj.GetViewStatusMsg, MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
       //LogMsg('s', 'Image IEN in Question: ' + Iobj.Mag0);
          MagLogger.LogMsg('s', 'Image IEN in Question: ' + IObj.Mag0); {JK 10/6/2009 - Maggmsgu refactoring}
          Exit;
        End;
    ...
    Begin
      ...
      Result := '0^' + IObj.GetViewStatusMsg;
       //logmsg('DI',Iobj.GetViewStatusMsg,MagLogInfo);
      MagLogger.LogMsg('DI', IObj.GetViewStatusMsg, MagLogINFO); {JK 10/6/2009 - Maggmsgu refactoring}
       //LogMsg('s', 'Image IEN in Question: ' + Iobj.Mag0);
      MagLogger.LogMsg('s', 'Image IEN in Question: ' + IObj.Mag0); {JK 10/6/2009 - Maggmsgu refactoring}
      Exit;
    End;
...
//  maggmsgf.magmsg('','Queue a Copy from JukeBox');
  //LogMsg('','Queue a Copy from JukeBox');
    MagLogger.LogMsg('', 'Queue a Copy from JukeBox'); {JK 10/6/2009 - Maggmsgu refactoring}
  {TODO: this could be after the Load , need all speed in loading image}
    Dmod.MagDBBroker1.RPMaggQueImage(IObj);
...
//        maggmsgf.MagMsg('s','Exception resolving screen resolution.  Continuing...');
      ...
      MagLogger.LogMsg('s', 'Exception resolving screen resolution.  Continuing...'); {JK 10/6/2009 - Maggmsgu refactoring}
    End;
...
//    maggmsgf.magmsg('s', 'File Does Not Exist : ' + IObj.FFile);
    //LogMsg('de', 'File Does Not Exist !, Notify IRM');
     ...
     if not overrideblock then    {/p117 bb, gek stop the Dialog box if we are overriding block}
         MagLogger.LogMsg('de', 'File Does Not Exist !, Notify IRM'); {JK 10/6/2009 - Maggmsgu refactoring}
    //LogMsg('s', 'File Does Not Exist : ' + IObj.FFile);
      MagLogger.LogMsg('s', 'File Does Not Exist : ' + IObj.FFile); {JK 10/6/2009 - Maggmsgu refactoring}
...
//                                '  desc -  ' + Iobj.ExpandedDescription(false));
  ...
  //                              '  desc -  ' + Iobj.ExpandedDescription(false));
    MagLogger.LogMsg('s', 'opening image type -' + Inttostr(IObj.ImgType) +
  ...
  { log Action 'IMG^'  = image has been viewed }
      ...
      Dmod.MagDBBroker1.RPMag3Logaction('IMG^' + Dmod.MagPat1.M_DFN + '^' + IObj.Mag0, IObj);
  {NetWorkFile}

      If IObj.DFN = Dmod.MagPat1.M_DFN Then
        IObj.SSN := Dmod.MagPat1.M_SSNdisplay
      Else
        IObj.SSN := '';
...
Begin
  ...
  Begin
    ...
    ImgList := Dmod.MagImageListManager.GetOrCreateStudy(StudyObj);
    t := ImgList.ObjList;
...
Begin
  ...
  //gek testing to fix error found by jerry 9/8/2011 put second paren (  ) around 'FrmImagelist...  er<> nil'
  ...
  //  two above instead of below
   // {
    ...
    Begin
      ...
      MagLogger.LogMsg('DE', ' Acrobat Reader error: # ' + Inttostr(PDFHandle)); {JK 10/6/2009 - Maggmsgu refactoring}
    End;
    //}
  End
  Else
 ...
 { TODO -oGarrett -crefactor : Need to use WhichViewer parameter from all calls, then optimize this code. }
  ...
  begin
    If FrmImageList.Pnlfullres.Visible Then
      FormToNormalSize(FrmImageList)
    Else
      If Doesformexist('frmFullRes') Then FormToNormalSize(FrmFullRes);
   ...
   {JK 2/11/2009 - Fixes D75.  ImgType 15 = document bmp. ImgType 104 = PDF}
    ...
    Except
      ...
      Begin
        ...
        MagLogger.LogMsg('s', 'Ignoring exception: ' + e.Message); {JK 10/6/2009 - Maggmsgu refactoring}
...
//  maggmsgf.magmsg('s', 'Creating MUSE settings Window');
  ...
  MagLogger.LogMsg('s', 'Creating MUSE settings Window'); {JK 10/6/2009 - Maggmsgu refactoring}
...
Begin
  ...
  FrmRadViewer.CurrentPatient := Dmod.MagPat1;
  //frmRadViewer.OnLogEvent := dmod.MagLogManager.OnLogEvent;   {JK 10/6/2009 - Maggmsgu refactoring}
  ...
  FrmRadViewer.SetUtilsDB(Dmod.MagUtilsDB1);
...
Begin
  ...
  Rstat := False;
  Dmod.MagDBBroker1.RPCTPresetsGet(Rstat, Rmsg, Presets);
  If Rstat Then
  Begin
  ...
  Begin
    ...
    FrmCTConfigure.Broker := Dmod.MagDBBroker1;
  End;
End;
...
Begin
  ...
  {This loads the study (if not already loaded) and then opens the first image in the study}
    ObjList := Tlist.Create();
    ImgList := Dmod.MagImageListManager.GetOrCreateStudy(IObj);
    ...
    Try
      Begin
        ...
        if frmImageList.FCurrentFilter.ShowDeletedImageInfo then
          Dmod.MagDBBroker1.RPMaggGroupImages(IObj, Temp, False, 'D')
        else
          Dmod.MagDBBroker1.RPMaggGroupImages(IObj, Temp, False, '');

//      maggmsgf.magmsg('', 'Image Group selected, accessing Group Images...');
      //LogMsg('', 'Image Group selected, accessing Group Images...');
        MagLogger.LogMsg('', 'Image Group selected, accessing Group Images...'); {JK 10/6/2009 - Maggmsgu refactoring}
        If Temp.Count = 1 Then //maggmsgf.magmsg('s', magpiece(temp.strings[0], '^', 2));
        //LogMsg('s', magpiece(temp.strings[0], '^', 2));
          MagLogger.LogMsg('s', MagPiece(Temp.Strings[0], '^', 2)); {JK 10/6/2009 - Maggmsgu refactoring}
        Dmod.MagDBBroker1.RPMaggQueImageGroup('A', IObj);
      End;
...
//        maggmsgf.magmsg('D', 'ERROR: Accessing Group Images.  See VistA Error Log.');
        ...
        MagLogger.LogMsg('D', 'ERROR: Accessing Group Images.  See VistA Error Log.'); {JK 10/6/2009 - Maggmsgu refactoring}
    ...
    //LogMsg('', 'Loading Images to group window');
      MagLogger.LogMsg('', 'Loading Images to group window'); {JK 10/6/2009 - Maggmsgu refactoring}
    {------------------------}
...
Procedure SetDBDemo;
Begin
 //outUntil Demo is Reimplemented dmod.MagDBBroker1 := dmod.MagDBDemo1;
  SetAppDBPointers;
End;
...
Begin
  ...
  Begin
    ...
    Dmod.MagDBSysUtils1.Broker := MagDBBroker1.GetBroker;
    FrmImageList.MagImageList1.MagDBBroker := Dmod.MagDBBroker1;
    If Doesformexist('frmimagelist_1') Then Showmessage(' 2 frmImageList windows');
 {p94t2 moved back to frmMain...  where it belongs  gek.}
 // frmMain.magpatphoto1.MagDBBroker := dmod.MagDBBroker1;
  End;
End;
...
Begin
  ...
  Begin
    ...
    If Msgdlg Then //maggmsgf.magmsg('d', 'You''ve deleted : ' + IObj.imgdes);
      ...
      MagLogger.LogMsg('d', 'You''ve deleted : ' + IObj.ImgDes); {JK 10/6/2009 - Maggmsgu refactoring}
  End
  Else
    Result := False;
End;
...
Procedure SetDBMVista;
Begin
 //outUntil Demo is Reimplemented  dmod.MagDBBroker1 := dmod.MagDBMVista1;
  SetAppDBPointers;
End;
...
result := nil;
    ...
    dmod.MagDBBroker1.RPMaggImageInfo(rstat,rmsg,t,ien,true);
    if not rstat then   exit;
  result :=  result.StringToTImageData(rmsg,dmod.MagDBBroker1.GetServer,dmod.MagDBBroker1.GetListenerPort);
finally
end;
end;
*)
...
Begin
  ...
  Try
    ...
    Begin
      If Magien <> '' Then
     ////p93t8  dmod.MagDBBroker1.RPMaggImageInfo(rstat,rmsg,rlist,magien);
     ////p93t8  if not rstat then exit;
     ////p93t8  Iobj := Iobj.StringToTImageData(rmsg,dmod.MagDBBroker1.GetServer,dmod.MagDBBroker1.GetListenerPort);
     // ? compare patients, if Diff Patient than MagPat1, then open in diff window.
        TempIobj := Dmod.MagDBMVista1.IENtoTImageData(Magien, Rstat, Rmsg);
      If TempIobj = Nil Then Exit;
      If TempIobj.DFN = Dmod.MagPat1.M_DFN Then
      Begin
        NewFullRes := TfrmFullRes.Create(Nil);
        NewFullRes.caption := 'Opening Image(s) by ID #';
        NewFullRes.Color := clPurple;
        NewFullRes.Mag4Viewer1.Align := alnone;
...
Begin
  ...
  Else
    s := Inttostr(FrmImageList.MagListView1.Items.Count) + ' of ' +
      Dmod.MagPat1.M_TotalImageCount + ' Images match Filter';
  ...
  If Doesformexist('frmMagAbstracts') Then
    ...
    Begin
      ...
      Begin
        ...
        caption := 'Abstracts: ' + Dmod.MagPat1.M_NameDisplay;
      End;
    End;
  If Doesformexist('frmImageList') Then
    With FrmImageList Do
...
Begin
  ...
  If IObj.FullLocation <> '' Then
    ...
    Begin
      Maglogger.LogMsg('di', 'Image : ' + IObj.ImgDes + #13 +
        'Date  : ' + IObj.Date + '     Procedure : ' + IObj.Proc + #13 +
        #13 +
        'for Patient :    ' + IObj.PtName + #13 +
        'resides on a JUKEBOX that is OFFLINE.');
      Maglogger.LogMsg('s', '**JukeBox is OffLine.  ' + IObj.FFile);
      Dmod.MagDBBroker1.RPMaggOffLineImageAccessed(IObj);
      Result := True
    End;
End;
