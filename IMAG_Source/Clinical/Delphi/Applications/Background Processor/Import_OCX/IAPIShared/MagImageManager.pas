...
Type
  TMagCannedAbs = (MagAbsPacGroup, MagAbsPacImage, MagAbsEKG, MagAbsCine, MagAbsAVI, MagAbsHTML, MagAbsDoc, MagAbsText, MagAbsPDF, MagAbsRTF, MagAbsWav,
...
Begin
  ...
  Begin
    ...
    MagLogger.LogMsg('s', 'Image [' + IObj.FFile + '] is already cached'); {JK 10/6/2009 - Maggmsgu refactoring}
    Result := NewList;
    Exit;
  End;
  ...
  CacheInformation := AbsFileName + '^' + FCacheDirectory + '\' +
    SiteAbbr + '\' + ExtractFileName(AbsFileName) + '^' + Inttostr(ImageType);
  ;
  NewList.Add(CacheInformation);
...
Begin
  ...
  Begin
    ...
    MagLogger.LogMsg('s', 'Study is already cached'); {JK 10/6/2009 - Maggmsgu refactoring}
    Result := NewList;
    Exit;
  End;
...
Begin
 ...
 {/p117 gek 1/17/11.  This is what we need to change for the photo cache problem..
    ...
    Begin
      ...
      MagLogger.LogMsg('s', 'Image is already being cached, about to wait for thread to complete.'); {JK 10/6/2009 - Maggmsgu refactoring}
      WaitForImageThread();
            //LogMsg('s', 'Image caching thread is complete, loading image...');
      MagLogger.LogMsg('s', 'Image caching thread is complete, loading image...'); {JK 10/6/2009 - Maggmsgu refactoring}
      Result := DestinationFilename;
      Exit;
    End;
        {}
    If Not Dmod.MagFileSecurity.MagOpenSecurePath(NetworkFilename, Xmsg) Then
    Begin
      Result := '';
      Exit;
    End;
 ...
 {HERE HERE PATCH 117 FIXING BUG FOR PHOTO IMAGES CACHED IN WRONG DIRECTORY, AND NOT BEGIN CLEANED UP.
   ...
   This may go away, if we Set FCacheDirectory to the Correct Direotory...  }
    ...
    MagLogger.LogMsg('s', 'Caching image [' + ExtractFileName(NetworkFilename) + ']'); {JK 10/6/2009 - Maggmsgu refactoring}
    Try
    ...
    Except
      ...
      Begin
        ...
        MagLogger.LogMsg('s', 'Error caching image [' + NetworkFilename + ']. Error=[' + e.Message + '].'); {JK 10/6/2009 - Maggmsgu refactoring}
...
Begin //p94t10 gek  Put back in the comparion of PlaceCode (3 letter site code)
  ...
  If (IObj.PlaceCode <> Gsess.WrksPlaceCODE)
    ...
    Or (IObj.ServerName <> Dmod.MagDBBroker1.GetServer)
    Or (IObj.ServerPort <> Dmod.MagDBBroker1.GetListenerPort) Then
    Result := False
  Else
    Result := True;
End;
...
Begin
  ...
  Begin
    ...
    Else
      If ImgFormatType = 21 Then // motion video
            // for the avi player to work, file must have an avi extension.
        DestinationFilename := DestinationFilename + '.avi'; // what about other extensions?
  End
  Else
    DestinationFilename := FCacheDirectory + '\' + SiteAbbr + '\' + ExtractFileName(NetworkFilename);
  If IsFileCurrentlyCaching(NetworkFilename) Then
  ...
  Begin
    Forcedirectories(ExtractFileDir(DestinationFilename));
  End;
  ...
  Begin
    ...
    Begin
      ...
      MagLogger.LogMsg('', 'Error getting image from ViX server'); {JK 10/6/2009 - Maggmsgu refactoring}
    End
    Else
      If Result.FTransferStatus = IMAGE_UNAVAILABLE Then
      Begin
            //LogMsg('', 'Image unavailable from the ViX server (409 error)');
        MagLogger.LogMsg('', 'Image unavailable from the ViX server (409 error)'); {JK 10/6/2009 - Maggmsgu refactoring}
      End;
  End;
End;
