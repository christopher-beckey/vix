...
Begin
  ...
  //p94  gek, IENToTImageData function moved to magDBMVista
      TempIobj := Dmod.MagDBMVista1.IENtoTImageData(IObj.Mag0, Rstat, Rmsg);
...
*)
 ...
 // now moved to OnShow Mag4VGear1.LoadTheImage(Iobj,true);
      Dmod.MagDBBroker1.RPMag4GetImageInfo(IObj, Fimageinfo);
      Memimageinfo.Lines.AddStrings(Fimageinfo);
      Memimageinfo.Lines.Add(Divider);
      Dmod.MagDBBroker1.RPMaggReasonList(Rstat, Rmsg, TmpList, 'D');
      If Rstat Then
      Begin
        TmpList.Delete(0);
        Loadreasons(TmpList);
...
Begin
  ...
  Oneimage := Dmod.MagVUtils1.ResolveAbstract(XImageObj, Msghint);
  ...
  If Not Dmod.MagFileSecurity.MagOpenSecurePath(Oneimage, Xmsg) {// was proxy.imagedata.AFile} Then
    Oneimage := Dmod.MagVUtils1.GetAppPath + '\bmp\AbsError.bmp'
  Else
    If Not FileExists(Oneimage) Then Oneimage := Dmod.MagVUtils1.GetAppPath + '\bmp\NotExist.bmp';
...
//exit;
 ...
 //mag4vgear2.AutoRedraw := true;
  ...
  If Not Dmod.MagFileSecurity.MagOpenSecurePath(Oneimage, Xmsg) {// was proxy.imagedata.AFile} Then
    Oneimage := Dmod.MagVUtils1.GetAppPath + '\bmp\FullResFileOpenError.bmp'
  Else
    If Not FileExists(Oneimage) Then Oneimage := Dmod.MagVUtils1.GetAppPath + '\bmp\NotExist.bmp';
...
Begin
  ...
  Dmod.MagUtilsDB1.ImageReport(XImageObj, Rstat, Rmsg, Memimageinfo, True);
  Memimageinfo.Lines.Add(Divider);
...
Begin
  ...
  End;
  Try
    Dmod.MagDBBroker1.RPMaggGroupImages(XImageObj, Temp, True, '');   {/ P117 - JK 9/2/2010 /}
    WinMsg('', 'Image Group selected, accessing Group Images...');
    If Temp.Count = 1 Then WinMsg('s', MagPiece(Temp.Strings[0], '^', 2));
    Dmod.MagDBBroker1.RPMaggQueImageGroup('A', XImageObj);
