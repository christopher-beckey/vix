...
Begin
   {debug94} If (ImsgObj <> Nil) Then ImsgObj.LogMsg('s', '**--** ||TMag4Pat.Notify_  Start: ' + 'subjectstate : ' + SubjectState);
  For i := 0 To (High(FObserversList)) Do
  Begin
    If FObserversList[i] <> Nil Then
    Begin
      IvObserver := (FObserversList[i]);
    {debug94} If (ImsgObj <> Nil) Then ImsgObj.LogMsg('s', '**--** TMag4Pat.Notify_ Observer[' + Inttostr(i) + ']   state ' + SubjectState);
...
Begin
  FDFN := State;
  { TODO : Look into this, could be Notifying Twice.}
  Notify_(State);
End;
