...
Begin
  For i := 0 To Dmod.MagFileSecurity.Msglist.Count - 1 Do
    Memo1.Lines.Add(Dmod.MagFileSecurity.Msglist[i]);
End;
...
Begin
  ...
  Try
    Dmod.MagDBSysUtils1.RPMaggDevFldValues(EMagNode.Text, EFlags.Text, CurrentServer, CurrentPort, t);
        //45
    (*
...
Var
  Xmsg: String;
Begin
  Dmod.MagFileSecurity.MagOpenSecurePath(CurImageRecord.AFile, Xmsg);
  ShowSecurityMsgs;
  Memo1.Lines.Add('{--------------------}');
  Dmod.MagFileSecurity.MagOpenSecurePath(CurImageRecord.FFile, Xmsg);
  ShowSecurityMsgs;
  Memo1.Lines.Add('{--------------------}');
End;
...
Begin
  ...
  Begin
    ...
    TempIobj := Dmod.MagDBMVista1.IENtoTImageData(Ien, Rstat, Rmsg);
  ...
  if vobj <> nil  then
    ...
    if vObj.IsImageDeleted then
       ...
       // TESTING   currentImageJBPath := vObj.Mag0 + ' JB JB JB JB JB JB JB ';
        dmod.MagDBBroker1.RPMagJukeBoxPath(rstat,rmsg,vObj.mag0);
...
Begin
  ...
  s := Dmod.MagDBBroker1.RPXWBGetVariableValue(mcode);
  EMagNode.Text := s;
  If EMagNode.Text = '' Then
  Begin
...
Begin
  ...
  t := Tstringlist.Create;
  Try
    Dmod.MagDBSysUtils1.RPMaggSysGlbNode(EMagNode.Text, CurrentServer, CurrentPort, t); //45
    For i := 0 To t.Count - 1 Do
      Memo1.Lines.Add(t[i]);
    Memo1.Lines.Add('{--------------------}');
