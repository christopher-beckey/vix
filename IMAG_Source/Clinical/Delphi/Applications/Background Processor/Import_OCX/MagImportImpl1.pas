...
//    function Get_PatientHasPhoto: OleVariant; safecall;
  ...
  public
    ...
    {/p135}
//pxxx/    function ResetStayAlive(value : boolean): string;
...
{/p135 FDelayConnectUntilImport
     This conditional code is put in front of the code to process the import.
     We will connect using the Global  "Private" variables Fso*...}
 {
 FsoServer
  FsoPort
  FsoAccess
...
{135. Here, if we've set the DelayUntilImport.  We will check the connection before
            we call Import Queue...  we want this functionality in the TmagImport component itself, not
            just in the OCX interface.  In Progress.}

(*  135T56   we're trying this out.
    because the function ImportQueue will also test the connection, and connected if needed, also
    return a more detailed list of messages.
try
if  FDelayConnectUntilImport then
  ...
  on e:exception do
    begin
      {p135  change Error description, and made it two lines.}
      //reslist[0] := 'Error Connection to VistA : ' + e.Message;
      vararrayRedim(reslist,1);
      reslist[0] := '0^Exception: connecting to VistA.';
      reslist[1] := e.Message;
      xstat := false;
      exit;
    end;
 ...
 {p135 Code for Stay Alive call.  We decided to use same logic as BP.
  ...
  { TODO -oGarrett : further tests need done to see if we really need this. with the autoconnect and
                     capturing the OnPulseError event.}
//pxxx/  rstayalive := ResetStayAlive(FDelayConnectUntilImport);
  try
  // From here on, is the code before 135
 ...
 { now convert the data that needs conversion to the format of the passed parameters.}
  ...
  {p135  The Finally block, had no code. and it's N/A  here
     changed to Exception with descriptive error message.
                               (probably won't ever occur) }
  except
  on e:exception do
     begin
       reslist[0] := '0^Exception: defining result Array';
     end;
  end;
  finally
  t.free;
...
begin
  ...
  {The VistAInit call doesn't return a 'list' of detail.  But the ImportQueue call does.}
  FDelphiControl.VistAInit(xstat, xmsg, FsoServer, FsoPort, FsoAccess, FsoVerify, FsoDivision);
  zstat := xstat;
  zmsg   := xmsg;
  {If Error,  then exit. }
   If not zstat then exit;

{/p135  Added this code below.
    A)  we always try to connect, to make sure the passed variables are valid and a
        connection is possible.
    B)  But (NEW) if FDelayConnectUntilImport, then we will save the connection parameters,
        and Disconnect, and only ReConnect if we are doing an Import.}
if  FDelayConnectUntilImport then
  begin
