...
uses
  Classes, ImgList, ToolWin, Graphics, Buttons,
  cMag4VGear,
  cMag4Viewer,
  cMagImageList,
  cMagListView,
...
type
  TfrmImageList = class(TForm, ImagObserver, IMagRemoteinterface)
    ...
    mnuROIStatuses: TMenuItem;
    tbtnEKG: TToolButton;
    ToolButton1: TToolButton;
    mnuFilePrintOptions: TMenuItem;          {p149}
    ...
    procedure MagListView1InfoTip(Sender: TObject; Item: TListItem;
      var InfoTip: string);
    procedure mnuROIStatusesClick(Sender: TObject);
    procedure tbtnEKGClick(Sender: TObject);
    procedure DeSkewSmooth1Click(Sender: TObject);   {p149}
    procedure mnuFullResSpecialClick(Sender: TObject);     {p149}
    procedure PrintPreview1Click(Sender: TObject);        {p149}
    procedure mnuFilePrintOptionsClick(Sender: TObject);      {p149}
  private
    ...
    lastHintNode: TTreeNode; {/ P122 T15 - JK 7/11/2012 /}

    {/p130t12 - dmmn 5/9/13 - Use this to check if the user is in ROI mode instead
    of using the mnuROISelectImagesToPrint.Checked because it's not updated correctly}
    FIsSelectingImagesForROI : Boolean;
    ...
    //procedure LogMsg(MsgType: string; Msg: string; Priority: TMagLogPriority = magmsginfo);{JK 10/5/2009 - Maggmsgu refactoring - removed old method}
...
//    procedure CurrentImageStatusChange(value: integer); {/ P122 T15 - moved and refactored this to umagdisplaymgr /}
  ...
  procedure CountCheckedImagesToPrint;
    ...
    {/ P122 T15 - JK 7/25/2012 - make a common method to update the tree and list with the proper status icons. /}
    procedure ShowQAReviewStatus(IObj: TImageData; QAReviewState, QAReviewStatus: Integer; rmsg: String; NeedsReview: Boolean);
    function PubMsg(NewsCode, NewsTopic: Integer; Msg: String): Boolean;

    procedure CheckIfInROIMode;  {/ P130 - JK 9/5/2012 - New Routine to fix Remedy Ticket 726792 /}
    procedure ShowEKGWindow;     {/ P130 - JK 9/5/2012 - New Routine to Fix Remedy Ticket 726792 /}
    procedure OpenPrintPreview; {p149}
    procedure OpenInAbs;         {p149}
  public
...
procedure TfrmImageList.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  {/ P122 - JK 6/9/2011 /}
  ListWinFullViewer.HideAnnotationToolbar;
end;

procedure TfrmImageList.FormCreate(Sender: TObject);
begin

  FIsSelectingImagesForROI := false; //p130t12
...
{$ENDIF}
  ...
  idmodobj.GetCCOWManager.Attach_(self); //93
  ...
  self.ListWinFullViewer.MagUtilsDB := idmodobj.GetMagUtilsDB1;
  ...
  if not assigned(self.magImageList1.magdbbroker) then
    self.magImageList1.magdbbroker := idmodobj.GetMagDBBroker1;
  if not assigned(self.magImageList1.magPat) then
    self.magImageList1.magPat := idmodobj.GetMagPat1;
  if not assigned(self.magObserverLabel1.MagPat) then
    self.magObserverLabel1.MagPat := idmodobj.GetMagPat1;
  SetDefaultFullViewerAlignment;

  if self.MagPatPhoto1.MagDBBroker = nil then
  begin
    self.magpatphoto1.MagDBBroker := idmodobj.GetMagDBBroker1;
    self.magpatphoto1.MagSecurity := idmodobj.GetMagFileSecurity;
    self.magpatphoto1.MagPat := idmodobj.GetMagPat1;
  end;
  //TEST
...
begin
  ...
  else if Action = 'PatientChangeProcessComplete' then
    CheckIfInROIMode;  {/ P130 - JK 9/5/2012 - Fixes Remedy Ticket 726792 /}
...
begin
  ...
  begin
    ...
    begin
      ...
      MagAppMsg('s', 'Image : ' + Iobj.ExpandedDescription + #13 +
  ...
  if preview then //buildreport(Iobj.Mag0, '', 'Image Report', memReport)
    idmodobj.GetMagUtilsDB1.ImageReport(IObj, rstat, rmsg, memReport)
...
begin
  ...
  try
    idmodobj.GetMagDBBroker1.RPMag4GetImageInfo(Iobj, t, FCurrentFilter.ShowDeletedImageInfo); {/ P117 - JK 9/20/2010 - Added 3rd parameter /}
  ...
  {     JMW 7/30/08 p72 - not necessary. Image Network share access is
    ...
    //   old way =>   idmodobj.GetMagFileSecurity.MagCloseSecurity(xmsg);
  winmsg(mmsglistwin, Iobj.ExpandedDescription);
end;
...
{$ENDIF}
        ...
        begin
          ...
          magappmsg('','After Set News:  newsobj.NewsStrValue = '+newsobj.NewsStrValue);
        end;
      end;
...
begin
  ...
  if (idmodobj.GetMagPat1.M_DFN <> '') and (vFilter <> nil) then
  begin
    UpdateFilteredList;
  end;
  //RefreshAllFilters;
...
var
  ...
  vCCOWState: byte;
begin
{debug94 } MagAppMsg('s','**--**-- -- -- TfrmImageList.Update_  state ' + subjectstate);
  ...
  {/ P130 - JK 11/7/2012 - Remedy ticket HD 726792 - TFS 54724/}
  //if self.mnuROISelectImagestoPrint.Checked then
  {p130t12 - use this var to check for ROI in order to fix Cliff's issue on the
  checkboxes returns when changing filters } 
  if FIsSelectingImagesForROI then
  begin
    ImageListCheckBoxCheckNone;
    { comment these lines out because it's been done in ImageListCheckBoxCheckNone }
    //for i := 0 to maglistview1.Items.Count - 1 do
    //  maglistview1.Items[i].Checked := true;
...
var
  ...
  statdesc: string;
  controlled: string;
  controlledcaption: string;          {p149}
  rmsg: string;
begin
{$IFDEF TESTMESSAGES} MagAppMsg('', 'In mnuUtilitiesClick(...');
{$ENDIF}
  usercanedit := (UserHasKey('MAG EDIT') or UserHasKey('MAG SYSTEM'));
  selectedImage := umagdisplaymgr.FCurSelectedImageObj <> nil;

  if selectedImage then
    localimage := IsThisImageLocaltoDB(FcurSelectedImageObj, idmodobj.GetMagDBBroker1, rmsg);
{$IFDEF TESTMESSAGES}
   MagAppMsg('', 'usercanedit: ' + magbooltostr(usercanedit));
   MagAppMsg('', 'selectedImage: ' + magbooltostr(selectedimage));
   MagAppMsg('', 'IsThisImageLocaltoDB: ' + magbooltostr(localimage));
{$ENDIF}
...
{/p117 gek  added QA REVIEW check}
  ...
  mnuROIStatuses.Enabled     := (UserHasKey('MAG SYSTEM') or UserHasKey('MAG ROI'));  {/ P130 JK 4/20/2012 /}
  ...
  begin
    ...
    mnuSetControlledImage.Caption := '"Controlled Status" () =';          {p149}
...
//    controlled := MagBooltostr(fcurselectedimageobj.MagSensitive);   //out in 149
    if FcurselectedImageobj.MagSensitive                         {p149}
       then controlledcaption := 'Controlled'
       else controlledcaption := 'Not Controlled';

    mnuSetControlledImage.Caption := '"Controlled Status" (' + controlledcaption + ')'; // mod in  {p149}
...
//   IOBJ := MagListView1.GetSelectedImageObj   // TESTING 117
  ...
  if not IsThisImageLocaltoDB(iobj, idmodobj.GetMagDBBroker1, rmsg) then
    // if (iobj.ServerName <> idmodobj.GetMagDBBroker1.GetServer) or (iobj.ServerPort <>
    //     idmodobj.GetMagDBBroker1.GetListenerPort) then
  begin
...
procedure TfrmImageList.DeSkewSmooth1Click(Sender: TObject);             {p149}
begin
//            {p149}
end;

{/ P122 T15 - JK 7/25/2012 - make a common method to update the tree and list with the proper status icons. /}
procedure TfrmImageList.ShowQAReviewStatus(IObj: TImageData; QAReviewState, QAReviewStatus: Integer; rmsg: String; NeedsReview: Boolean);
begin
  WinMsgClear;
  ...
  if NeedsReview then
  begin
    maglistview1.ImageStateChange(Iobj, QAReviewState); //umagdefinitions.mistateNeedsRefresh);
    maglistview1.ImageStatusChange(Iobj, QAReviewStatus);

    magTreeview1.ImageStateChange(Iobj, QAReviewState); //umagdefinitions.mistateNeedsRefresh);
    magTreeview1.ImageStatusChange(Iobj, QAReviewStatus);
  end;
//  else
//  begin
...
begin
  ...
  begin
    ...
    mnuFilePrintOptions.Enabled := false;          {p149}
  ...
  begin
    ...
    mnuFileImagePrint.Enabled := (not isdel) and isOpen ; //pnlfullres.Visible;
     mnuFilePrintOptions.Enabled := (not isdel) and isOpen ;          {p149}
    ...
    begin
     ...
     if mnuFilePrintOptions.Enabled then mnuFilePrintOptions.Enabled := UserHasKey('MAG REVIEW NCAT');          {p149}
    end;
...
{/ P130 - JK 9/19/2012 - PubMsg should be moved to a common unit so all the forms can
   take advantage of easy message publishing. It is in fmagImageList for now because
   we have run out of time to generalize this further. TODO: move this to a common
   unit and also add a publisher component. This will centralize the use of
   the publish side of Publish/Subscribe and free coupling of fMagImageList. TODO:
   Add more publish messages to remove the tight coupling to fmagMain.  There are
   numerous calls to frmMain.<some function> still in this unit and in other
   units. These can be changed to Pub/Sub calls. /}
function TfrmImageList.PubMsg(NewsCode: Integer; NewsTopic: Integer; Msg: String): Boolean;
var
  NewsObj: TMagNewsObject;  {/ P130 - 9/19/2012 /}
begin
  NewsObj := TMagNewsObject.Create;
  NewsObj.Newscode := MpubMainFormMsgs;
  NewsObj.NewsTopic := mmsgpublish;
  NewsObj.NewsStrValue := Msg;
  MagPublisher1.I_SetNews(NewsObj);
end;
...
//  frmMain.WinMsg('', 'Current Image Filter: " ' + filter.Name + ' "');   {/ P130 - JK 9/19/2012 - removed winmsg for Pub/Sub /}
  {/ P130 - JK 9/19/2012 /}
  PubMsg(MpubMainFormMsgs, mmsgpublish, 'Current Image Filter: " ' + filter.Name + ' "');
...
begin
  ...
  idmodobj.GetMagDBBroker1.RPFilterDetailsGet(rstat, rmsg, fltdetails, '', fltname,
    fduz);
  if not rstat then
  begin
    //      MagAppMsg('d', rmsg + #13 + #13 +
    //LogMsg('d', rmsg + #13 + #13 +
...
begin
  {/ P122 - JK 10/18/2011 /}
  if ListWinFullViewer.Cur4Image <> nil then
    if ListWinFullViewer.Cur4Image.AnnotationComponent <> nil then    
      if ListWinFullViewer.Cur4Image.AnnotationComponent.IsAnnotationMode then
        Exit;
...
begin
  ...
  frmMain.ChangeToPatient(idmodobj.GetMagPat1.M_DFN);
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        //    e.message + ']');
        MagAppMsg('s', 'Exception auto-caching group images. Error=[' +
...
begin
  ...
  if UserHasKey('MAG ROI') or UserHasKey('MAG SYSTEM') then  {/ P130 - JK 7/10/2012 - Remedy ticket HD 726792 /}
    mnuROISelectImagestoPrintClick(Self);
end;
...
procedure TfrmImageList.MagListView1InfoTip(Sender: TObject; Item: TListItem;
  var InfoTip: string);
begin
  ...
  if Item.StateIndex = 9 then
    if (TMagListViewData(Item.Data).Iobj.Package = 'RAD') and
       (TMagListViewData(Item.Data).Iobj.ImgType = 11) then
      InfoTip := 'One or more images in this group were annotated in VistARad. To view' + slinebreak +
                 'the annotations, open the image(s) in VistARad.';

  if Item.StateIndex = 8 then
  if (TMagListViewData(Item.Data).Iobj.Package = 'RAD') and
     ((TMagListViewData(Item.Data).Iobj.ImgType = 3) or
     (TMagListViewData(Item.Data).Iobj.ImgType = 100)) then
    InfoTip := 'This image was annotated in VistARad. To view' + slinebreak +
...
{/ P130 - JK 9/5/2012 - Fixes Remedy Ticket 726792 /}
procedure TfrmImageList.CheckIfInROIMode;
begin
  if not MagListView1.Visible then
    Exit;

  if UserHasKey('MAG ROI') or UserHasKey('MAG SYSTEM') then  
  begin
    //if Self.mnuROISelectImagestoPrint.Checked then
    if FIsSelectingImagesForROI then    //p130t12
    begin
...
begin
  ...
  begin
    ...
    MagAppMsg('d', 'You must first select a Patient '); {JK 10/5/2009 - Maggmsgu refactoring}
...
procedure TfrmImageList.tbtnEKGClick(Sender: TObject);
begin
  {/ P130 - JK 1/21/2013 - TFS #60839 - added EKG button to Image List window. /}
  ShowEKGWindow;
end;
...
begin
{$IFDEF TESTMESSAGES}MagAppMsg('', 'In MagTreeView1click(...');
{$ENDIF}
...
var
  ...
  ocurs: Tcursor;
begin
{$IFDEF TESTMESSAGES}MagAppMsg('', 'In OpenSelectedTreeImage(...');
{$ENDIF}
  try
    CursorChange(ocurs, crHourGlass); //        screen.cursor := crHourGlass;
  ...
  selected images from tree, then call openselectedimages from here. }
    if (magtreeview1.SelectionCount > 1) then
     //// this is not used. RCA OUT so we can decouple.
...
{$IFDEF TESTMESSAGES}MagAppMsg('', 'called OpenSelectImage(IEN: ' + vIobj.Mag0);
      MagAppMsg('', 'result OpenSelectImage.. ' + retmsg);
{$ENDIF}
...
{/ P122 T15 - JK 7/11/2012 /}
function TfrmImageList.NodeHint(tn: TTreeNode): String;
var
  vIObj: TImageData;
begin
  Result := '';
  ...
  begin
    ...
    begin
      vIobj := (TMagTreeViewData(tn.Data).Iobj);
      if tn.StateIndex = 9 then
        if (vIobj.Package = 'RAD') and
           (vIobj.ImgType = 11) then
          Result := 'One or more images in this group were annotated in VistARad. To view' + slinebreak +
                    'the annotations, open the image(s) in VistARad.';

      if tn.StateIndex = 8 then
      if (vIobj.Package = 'RAD') and
         ((vIobj.ImgType = 3) or
         (vIobj.ImgType = 100)) then
        Result := 'This image was annotated in VistARad. To view' + slinebreak +
                  'the annotations, open this image in VistARad.';
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    MagAppMsg('s', 'Mouse Move  TreeNode = NIL');
    exit;
  end
  else MagAppMsg('s', 'Mouse Move ' + tn.Text); //gmaglog.LogMsg_('Mouse Move ' + tn.Text);
  *)
...
begin
  ...
  try
    ...
    {=============}
    MagAppMsg('s', 'Section of code shouldn''t execute.'); {JK 10/6/2009 - Maggmsgu refactoring}
    retmsg := OpenSelectedImage(Iobj, radcode, 0, 1, 0);
    if magpiece(retmsg, '^', 1) = '0' then
...
Procedure TfrmImageList.ShowEKGWindow;
Var
  Tssn: String;
Begin
  If idmodobj.GetMagPat1.M_DFN = '' Then
  Begin
    WinMsg(mmsglistwin, 'You must enter a Patient before viewing MUSE EKGs.');
    Exit;
  End;
  ...
  begin
    WinMsg(mmsglistwin, 'The patient doesn''t have a social security number for the MUSE EKGs.');
    Exit;
  end;
  ...
  TbtnEkg.Enabled := False;
  TbtnEkg.Cursor := crHourGlass;
  WinMsg(mmsglistwin, idmodobj.GetMagPat1.M_NameDisplay +
...
procedure TfrmImageList.mnuFullResSpecialClick(Sender: TObject);          {p149}
begin
  OpenPrintPreview;
end;

procedure TfrmImageList.OpenPrintPreview;
Var
  Magien: String;
  TempIobj: TImageData;
...
Begin
  Try
    Magien := '';
     if FCurSelectedImageObj <> nil
         then TempIobj := FCurSelectedImageObj
         else
         begin
...
mpubImageUnSelectAll = 2003;  }
  ...
  case NewsObj.NewsCode of
    MpubImageNeedsReview: {/ P122 T15 - JK 8/9/2012 - Adding in new feature to refresh Needs Review from other forms using Pub/Subscr method. /}
      begin
        IObj := TImageData(newsObj.NewsChangeObj);
        MagAppMsg('s', 'TfrmImageList.MagSubscriber1SubscriberUpdate: Needs Review message received for IEN: ' +
                         IObj.Mag0);
        ShowQAReviewStatus(IObj, umagdefinitions.mistateNeedsRefresh, umagdefinitions.MistNeedsReview, rmsg, True);
      end;
...
begin
  ...
  idmodobj.GetMagFileSecurity.SecurityOn := mnuImageFileNetSecurityON1.checked;
...
begin
  mnuImageFileNetSecurityON1.checked := idmodobj.GetMagFileSecurity.SecurityOn;
end;
...
begin
  for i := 0 to idmodobj.GetMagFileSecurity.msglist.count - 1 do
    //MagAppMsg('s', idmodobj.GetMagFileSecurity.msglist[i]);
    MagAppMsg('s', idmodobj.GetMagFileSecurity.msglist[i]);
...
begin
  ...
  begin
    for i := 0 to maglistview1.Items.Count - 1 do
      if maglistview1.items[i].Checked then
        inc(ct);
    ...
    begin
      ...
      frmReleaseOfInfoPrint.Execute(true, MagListView1, self.FCurrentFilter, upref, True); //*** BB 08/24/2010 ***
    end;
  end
  else
  begin
...
begin
  ...
  //ccowstate := idmodobj.GetCCOWManager.
  idmodobj.GetCCOWManager.GetState_(ccowstr);
...
begin
  try
    ...
    if FcurSelectedImageObj.IsImageDeleted then
      MagAppMsg('DI', FcurSelectedImageObj.GetViewStatusMsg, magmsginfo)
    else
    begin

      if not IsThisImageLocaltoDB(FCurSelectedImageObj, idmodobj.GetMagDBBroker1, rmsg) then

        // if ((FCurSelectedImageObj.ServerName <> idmodobj.GetMagDBBroker1.GetServer)
         //    or (FCurSelectedImageObj.ServerPort <> idmodobj.GetMagDBBroker1.GetListenerPort)) then
     ...
     // t := Tstringlist.create;
      ...
      if frmIndexEdit.Execute(idmodobj.GetMagDBBroker1, FCurSelectedImageObj, rmsg) then
      begin
        //Success.
  ...
  //ListView.ImageStatusChange(Iobj, umagdefinitions.mistNeedsRefresh);
    ...
    on e: exception do
    begin
      //MagAppMsg('dI', e.Message);
      MagAppMsg('dI', E.Message); {JK 10/5/2009 - Maggmsgu refactoring}
    end;
  end;
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        //oldprototype  idmodobj.GetMagDBBroker1.RPMag4FieldValueGet(rstat,rmsg,rlist,magienlist[0],'IENR','10;40:45');
        //93
        //  idmodobj.GetMagDBBroker1.RPMag4FieldValueGet(rstat,rmsg,rlist,magienlist[0],'IENR','10;40:45');

        //HERE
        idmodobj.GetMagDBBroker1.RPMaggImageGetProperties(rstat, rmsg, rlist,
          magienlist[0], '*');
        ...
        {  if frmIndexEdit.execute(IIndexObj,idmodobj.GetMagDBMVista1,ctmsg) then  }
        chmsg := #13 + #13;

        /// frmIndex Edit has been modified, not called like this now...      if frmIndexEdit.Execute(IIndexObj, idmodobj.GetMagDBBroker1, ctmsg, Iobj) then
        if true then
        begin
...
begin
  ...
  FCurrentFilter.ShowDeletedImageInfo             := upref.UseDelImagePlaceHolder;
  idmodobj.GetMagPat1.M_IncDeletedImageCount             := upref.UseDelImagePlaceHolder;
...
begin
  ...
  begin
    MagAppMsg('DI', TMag4Vgear(popupAbstracts.PopupComponent).PI_ptrData.GetViewStatusMsg, magmsginfo);
  end
  else
  begin
...
var
  Iobj: TImageData;
  rmsg: string;
  NeedsReview: Boolean; {/ P130 T15 - JK 7/18/2012 /}
begin
  Iobj := TMag4Vgear(popupAbstracts.PopupComponent).PI_ptrData;
  ...
  if TMag4Vgear(popupAbstracts.PopupComponent).PI_ptrData.IsImageDeleted then
  begin
    MagAppMsg('DI', IObj.GetViewStatusMsg, magmsginfo);
  end
  else
  begin
    { JMW 6/24/2005 p45t3 compare based on server and port (more accurate)}
...
var
  ...
  newsObj: TMagNewsObject;
begin
  //frmMain.Winmsg('', ''); {/ P130 - JK 9/19/2012 - removed winmsg for Pub/Sub /}
  {/ P130 - JK 9/19/2012 /}
  PubMsg(MpubMainFormMsgs, mmsgpublish, '');
...
var
  ...
  isdel : boolean;    {CodeCR710}
begin
{$IFDEF TESTMESSAGES}MagAppMsg('', 'In PopupTreePopup(...');
{$ENDIF}
...
begin
  ...
  {/ P130 - JK 12/10/2012 - TFS Task: 57749  Remedy: 0438339 -
     When a group is clicked in the image listview, check if the FCurSelectedImageObj is a child of a group.
     if so, replace the IObj pointer with the IObj of the parent /}
  if IObj.IGroupIEN <> 0 then
    IObj := TMagListViewData(TListItem(MagListView1.Selected).Data).Iobj;
...
begin
  if FCurSelectedImageObj <> nil then
  begin
    if (FCurSelectedImageObj.ServerName <> idmodobj.GetMagDBBroker1.GetServer) or
      (FCurSelectedImageObj.ServerPort <> idmodobj.GetMagDBBroker1.GetListenerPort) then
    begin
      //MagAppMsg('DI', 'You cannot delete an image from a remote site');
      MagAppMsg('DI', 'You cannot delete an image from a remote site'); {JK 10/5/2009 - Maggmsgu refactoring}
      exit;
    end;
    //logmsg('s', 'Attempting deletion of ' + FCurSelectedImageObj.ffile +
    //    '  IEN ' + FCurSelectedImageObj.Mag0);
    MagAppMsg('s', 'Attempting deletion of ' + FCurSelectedImageObj.ffile +
      '  IEN ' + FCurSelectedImageObj.Mag0); {JK 10/6/2009 - Maggmsgu refactoring}
  {/ P130 T15 - JK 7/18/2012 /}
  if MgrImageDelete(FCurSelectedImageObj, rmsg, NeedsReview) then
  begin
    if NeedsReview then
    begin
//      CurrentImageStatusChange(FCurSelectedImageObj, umagdefinitions.MistNeedsReview, rstat, rmsg2);
      {/ P122 T15 - JK 7/18/2012 - added back in the messaging that was in CurrentImageStatusChange when it was in this unit /}
      ShowQAReviewStatus(FCurSelectedImageObj, umagdefinitions.mistateNeedsRefresh, umagdefinitions.MistNeedsReview, rmsg, NeedsReview)
    end
    else
    begin
...
var
  ...
  fieldlist: tstrings;
  Iobj: TImageData;
  displayMsg : string;          {p149}
  ocurs: Tcursor;
begin
if value then displayMsg := '  Controlled Display'          {p149}
         else displayMsg := '  Not Controlled(Normal Display)';          {p149}
  if FCurSelectedImageObj = nil then
  begin
    //MagAppMsg('DI', 'You need to select an Image.');
  ...
  try
    WinMsgClear;
          {p149  next line mod in 149}
    if messagedlg('Change ''Controlled Status'' to: ' + displayMsg, mtconfirmation, [mbok, mbcancel], 0) = mrcancel then
    begin
      exit;
    end;
    CursorChange(ocurs, crHourGlass); //        screen.Cursor := crhourglass;
    ...
    idmodobj.GetMagDBBroker1.RPMaggImageSetProperties(rstat, rmsg, rlist, fieldlist, FCurSelectedImageObj.Mag0,
      '');
    WinMsgClear;
    winmsg(mmsglistwin, rmsg);
    if rstat then
    ...
    begin
      messagedlg('Change ''Controlled Status'' Failed : ' + rmsg, mtconfirmation, [mbok], 0);  //mod in           {p149}
    end;
  finally
    cursorRestore(ocurs); //         screen.Cursor := crDefault;
  end;
...
var
  ...
  rmsg2: String;  {/ P122 T15 - JK 7/25/2012 /}
begin
  CurrentImageStatusChange(FCurSelectedImageObj, umagdefinitions.mistVerified, rstat, rmsg2);
end;
...
{$IFDEF TESTMESSAGES}MagAppMsg('', 'In MagTreeView1MouseDown(...');
{$ENDIF}
  try
    CursorChange(ocurs, crHourGlass);
    timersynctimer.Enabled := false;
...
{$IFDEF TESTMESSAGES}MagAppMsg('', 'End of MagTreeView1MouseDown(...');
{$ENDIF}
...
begin
  ...
  mnuSetControlledImageTrue.Enabled := not fcurselectedimageobj.MagSensitive;
  mnuSetControlledImageFalse.checked := not fcurselectedimageobj.MagSensitive;          {p149}
  mnuSetControlledImageTrue.checked := fcurselectedimageobj.MagSensitive;;          {p149}
end;
...
var
  ...
  rmsg2: String;  {/ P122 T15 - JK 7/25/2012 /}
begin
  CurrentImageStatusChange(FCurSelectedImageObj, umagdefinitions.mistViewable, rstat, rmsg2);
end;
...
begin
  ...
  WinMsgClear;
  winmsg(mmsglistwin, rmsg2);
  if rstat then
  begin
    maglistview1.ImageStateChange(FCurSelectedImageObj, umagdefinitions.mistateNeedsRefresh);
    maglistview1.ImageStatusChange(FCurSelectedImageObj, umagdefinitions.mistNeedsReview);
    magTreeview1.ImageStateChange(FCurSelectedImageObj, umagdefinitions.mistateNeedsRefresh);
    magTreeview1.ImageStatusChange(FCurSelectedImageObj, umagdefinitions.mistNeedsReview);

  end
  else
  begin
    messagedlg('Change Status Failed : ' + rmsg2, mtconfirmation, [mbok], 0);
  end;
end;
...
procedure TfrmImageList.mnuFilePrintOptionsClick(Sender: TObject);          {p149}
begin
(*try
...
procedure TfrmImageList.ImagePrintListWin;        {BM-ImagePrint- calls ImagePrintEsigReason}
var
  s: string;
  vgear: TMag4VGear;
  haveGear: boolean;
begin
  ...
  begin
    //MagAppMsg('di','To Print, you need to select an Open Image.');
    MagAppMsg('di', 'To Print, you need to select an Open Image.'); {JK 10/5/2009 - Maggmsgu refactoring}
  end;
end;
...
(*  if mnuShowDeletedImageInformation.Checked then
  ...
  idmodobj.GetMagPat1.M_IncDeletedImageCount := FCurrentFilter.ShowDeletedImageInfo; {/ P117 - JK 10/5/2010 /}
  frmmain.ChangeToPatient(idmodobj.GetMagPat1.M_DFN);
...
begin
  ...
  begin
    //MagAppMsg('di','To Copy, you need to select an Open Image.');
    MagAppMsg('di', 'To Copy, you need to select an Open Image.'); {JK 10/5/2009 - Maggmsgu refactoring}
  end;
end;
...
begin
  ...
  MagAppMsg('s', 'Publishing mpubImageUnSelectAll in ImagesForTIUNote'); {JK 10/6/2009 - Maggmsgu refactoring}
  frmImageList.MagPublisher1.I_SetNews(newsobj); //procedure ImagesForCPRSTIUNote
	{/117 gek  decouple frmVerify from idmodobj.Get}
...
begin
  ...
  mnuROISelectImagestoPrint.Checked := FIsSelectingImagesForROI;          //p130t12
end;

procedure TfrmImageList.mnuROISelectImagestoPrintClick(Sender: TObject);
begin
  //if self.mnuROISelectImagestoPrint.Checked then
  //p130t12 update the ROI flag
  FIsSelectingImagesForROI := self.mnuROISelectImagesToPrint.Checked;
...
procedure TfrmImageList.mnuROIStatusesClick(Sender: TObject);
//var
//  frm_ROI_Statuses: Tfrm_ROI_Statuses;
begin
//  frm_ROI_Statuses := Tfrm_ROI_Statuses.Create(Self);
//  try
//    frm_ROI_Statuses.ShowModal;
//  finally
//    frm_ROI_Statuses.Free;
//  end;
  frm_ROI_Statuses.Execute(Self);
//  frm_ROI_Statuses := nil;   {Added by JK on 5/17/2012}
end;
...
begin
  ...
  ImageListCheckBoxCheckNone;  {/ P130 - JK 7/10/2012 - Remedy ticket HD 726792 /}
  ...
  maglistview1.Checkboxes := true;  {/ P130 - JK 7/10/2012 - Remedy ticket HD 726792 /}
...
begin
  for i := 0 to maglistview1.Items.Count - 1 do
    maglistview1.Items[i].Checked := false;
end;
...
if value then
  ...
  maglistview1.StateImages := idmodobj.GetImageListStateIcons;
  maglistview1.SmallImages := idmodobj.GetImageListStatusIcons;
  maglistview1.Invalidate;
  end;
end;
...
begin
  ...
  frmReleaseOfInfoPrint.Execute(false, MagListView1, self.FCurrentFilter,upref, False); //*** BB 08/24/2010 ***
end;

procedure TfrmImageList.PrintPreview1Click(Sender: TObject);              {p149}
begin
...
(*   TESTING - OPEN THE PRINT PREVIEW (ANY WINDOW) INSIDE A
     TABSHEET OF A PAGE  CONTROL *)
procedure TfrmImageList.OpenInAbs();
var
  ObjList: Tlist;
  tempiobj : TimageData;
  newSpecial : TfrmFullResSpecial;
  tabSheet : TTabSheet;
begin
(* for TESTING, first have to  put a PageControl1 on the Abstract window..
   set align := albottom
   have a Splitter also  *)
   tabSheet := TTabSheet.Create(frmMagAbstracts.PageControl1) ;
   tabSheet.PageControl := frmMagAbstracts.PageControl1;
   ...
   newspecial :=  TfrmFullResSpecial.create(tabsheet);
   newspecial.parent := tabsheet;
   newspecial.caption := 'Image Print Options';
   newspecial.Align := alClient;
   newspecial.BorderStyle := bsNone;
   newspecial.Visible := true;
   tabSheet.Caption := newspecial.Caption;
   ///  frmFullResSpecial.FormStyle := fsStayOnTop; //new
   //   frmFullResSpecial.FSpecialIobj := TempIobj;
   //  frmFullResSpecial.ShowImageInfo;
   Application.Processmessages;
   ObjList.Add(TempIobj);
   newspecial.Mag4Viewer1.ClearBeforeAddDrop := true;
   newspecial.Mag4Viewer1.ImagesToMagView(ObjList);
   finally
     objlist.Free;
   end;
end;
...
begin
  ...
  ImageListCheckBoxEnable(False);
  pnlROIoptions.Visible := False;
  FIsSelectingImagesForROI := false;        //p130t12 cancelling the ROI options
end;
...
begin
  ...
  lbCheckedImageCount.Caption := inttostr(ctr) + ' Item(s) selected to process';
