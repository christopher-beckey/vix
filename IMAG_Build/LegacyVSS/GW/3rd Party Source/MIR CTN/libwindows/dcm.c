...
DCM_AddElement(DCM_OBJECT ** callerObject, DCM_ELEMENT * element)
{
	...
	* groupItem;
	...
	if (element->representation != DCM_UNKNOWN &&
...
DCM_ScanParseObject(DCM_OBJECT ** callerObject, void *buf, size_t bufferSize,
		    DCM_FLAGGED_ELEMENT * vector, int vectorLength,
		    CONDITION(*callback) (), void *ctx)
{
	...
	* elementItem;
	...
	while (elementItem != NULL && !done) {
		...
		if (elementItem->element.tag == vector[i].e.tag) {
		    found = TRUE;
		    (void) copyData(object, elementItem, &vector[i].e, &l);
		    *vector[i].flagAddress |= vector[i].flag;

		    if (DCM_IsString(vector[i].e.representation)) {
			vector[i].e.d.string[l] = '\0';
			p = vector[i].e.d.string + l - 1;
			while (p >= vector[i].e.d.string && (*p == ' '))
			    *p-- = '\0';
		    }
		}
	    }
...
CONDITION(*callback) (void *buf, U32 bytesExported, int lastFlag, void *ctx),
		 void *ctx)
{
	       ...
	       U32 * rtnLength)
{
    ...
    *rtnLength += 2;
    ...
    if (vr == DCM_OB || vr == DCM_OW || vr == DCM_SQ) {
	*b++ = 0x00;
	*b++ = 0x00;
...
*/
	...
	while (elementItem != NULL) {
	    if (elementItem->element.tag == DCM_MAKETAG(0x003a, 0x01000)) {
		printf("Found waveform for export\n");
	    }
	    if (c <= 20) {
...
**		Pad the string with ' '
**	    break;
**	case DCM_CS, DCM_AS, DCM_DS, DCM_IS, DCM_LO, DCM_SH:
	     ...
	     PRIVATE_OBJECT ** object, U32 * scannedLength, DCM_ELEMENT * e)
{
    ...
    *fileOffset += (off_t) 4;
	...
	if (vrPtr->representation != e->representation) {
		...
		/* the object */
	    } else if (e->representation == DCM_UNKNOWN ||
	  ...
	  CONDITION(*sk) (void *ctx, int offset, int flag))
{
	...
	*scannedLength = 0;
	...
	if ((e.representation == DCM_UNKNOWN) &&
	...
	cond = checkAttributeOrder(&e, &lastGroup, &lastElement);
	if (cond != DCM_NORMAL)
...
DCM_GetCompressedValue(DCM_OBJECT ** callerObject, DCM_TAG tag, void *buf,
		       size_t bufSize,
		       CONDITION(*callback) (void *buf, U32 bytesExported, int index, int startFlag, int lastFlag, void *ctx),
		       void *ctx)
{
    PRIVATE_OBJECT
	** object;
	...
	* elementItem;
	...
	if (firstBuffer) {
		...
		callback(offsetBuffer, e.length, index, 1, 0, ctx);
		streamOffset = 0;
	    } else {
