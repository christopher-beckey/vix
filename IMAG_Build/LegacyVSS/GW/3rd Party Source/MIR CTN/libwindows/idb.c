...
static char rcsid[] = "$Revision: 1 $ $RCSfile: idb.c,v $";
#include <stdio.h>
#ifndef IRIX
/*#include <unistd.h> */
#endif
...
updateLimitsTable(TBL_HANDLE * limits,
		  int imageSize, int patientCountDelta,
		  int studyCountDelta, int imageCountDelta)
{
...
#if 1
	    /* Changed this on 1-Feb-1998.  Use reference counting to */
	    /* allow multiple users of the IDB facility/tables */
	    idbc->refCount++;
...
static CONDITION
deleteInstanceFileCallback(TBL_FIELD * fp, long count, void *ctx)
{
...
CONDITION
CBDel_CollectImages(TBL_FIELD * field, long count, void *ctx)
{
    LST_NODE
	* temp_node;
    char
...
CONDITION
CBDel_CollectSeries(TBL_FIELD * field, long count, void *ctx)
{
    LST_NODE
	* temp_node;
    char
...
CONDITION
CBDel_CollectStudies(TBL_FIELD * field, long count, void *ctx)
{
    LST_NODE
	* temp_node;
    char
...
IDB_Select(IDB_HANDLE ** handle, IDB_QUERY_MODEL model, long begin_level,
	   long end_level, IDB_Query * pssi, long *count,
	   CONDITION(callback()), void *ctx)
{
	...
	return IDB_Select_NoView(handle, begin_level, end_level, pssi,
				 count, callback, ctx);
    }
		...
		CONDITION(callback()), void *ctx)
{
	...
	* idbc;
	...
	if ((numsercrits = IDB_ConvertDicomQuerytoSQL(pssi, IDB_SERIES_LEVEL)) == 0)
	    criteria = (TBL_CRITERIA *) NULL;
	...
	if ((numimacrits = IDB_ConvertDicomQuerytoSQL(pssi, IDB_IMAGE_LEVEL)) == 0)
	    criteria = (TBL_CRITERIA *) NULL;
	if (GS_NumSerNodes != 0) {
	    criteria = GS_imacl;
	...
	for (i = 0; i < GS_NumImaNodes; i++) {
	    ...
	    ret_val = TBL_Select(&inshandle, inscrit, field, NULL, CBSel_CollectInstances,
		  ...
		  CONDITION(callback()), void *ctx)
{
	...
	* idbc;
	...
	criteria = GS_patcl;
	if ((numpatcrits = IDB_ConvertDicomQuerytoSQL(pssi, IDB_PATIENT_LEVEL)) == 0)
	    criteria = (TBL_CRITERIA *) NULL;
	...
	if ((numstucrits = IDB_ConvertDicomQuerytoSQL(pssi, IDB_STUDY_LEVEL)) == 0)
	    criteria = (TBL_CRITERIA *) NULL;
...
CONDITION
CBSel_CollectPatientsStudies(TBL_FIELD * fp, long count, void *ctx)
{
...
CONDITION
CBSel_CollectPatients(TBL_FIELD * fp, long count, void *ctx)
{
...
CONDITION
CBSel_CollectStudies(TBL_FIELD * fp, long count, void *ctx)
{
    int
        i;
...
CONDITION
CBSel_CollectSeries(TBL_FIELD * fp, long count, void *ctx)
{
    int
        i;
...
CONDITION
CBSel_CollectImages(TBL_FIELD * fp, long count, void *ctx)
{
...
CONDITION
CBSel_CollectInstances(TBL_FIELD * fp, long count, void *ctx)
{
...
IDB_ConvertDicomQuerytoSQL_PSView(IDB_Query * pssi)
{
	...
	if (!(pssi->PatientNullFlag & QF_PAT_PatNam)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Pat_PatNam", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->PatientNullFlag & QF_PAT_PatID)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Pat_PatID", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->PatientNullFlag & QF_PAT_PatSex)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Pat_PatSex", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
/* 11-Aug-1997
...
*/
	...
	if (!(pssi->PatientNullFlag & QF_PAT_Owner)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Pat_Owner", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->PatientNullFlag & QF_PAT_GroupName)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Pat_GroupName", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->PatientNullFlag & QF_PAT_Priv)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Pat_Priv", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
    /*
	...
	if (!(pssi->StudyNullFlag & QF_STU_AccNum)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_AccNum", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_StuID)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_StuID", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_StuInsUID)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_StuInsUID", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_RefPhyNam)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_RefPhyNam", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_StuDes)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_StuDes", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_PatAge)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_PatAge", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_PatSiz)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_PatSiz", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_PatWei)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_PatWei", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
/* 12-Aug-1997
...
*/
	...
	if (!(pssi->StudyNullFlag & QF_STU_Owner)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_Owner", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_GroupName)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_GroupName", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
	...
	if (!(pssi->StudyNullFlag & QF_STU_Priv)) {
	    ...
	    TBL_CRITERIA_LOAD_BYTE(GS_patstucl[i], "Stu_Priv", cs, TBL_STRING, TBL_LIKE);
	    i++;
	}
    }
    if (i != 0) {
...
IDB_ConvertDicomQuerytoSQL(IDB_Query * pssi, long level)
{
	...
	if (pssi->PatientQFlag & QF_PAT_PatNam) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_patcl[i], "PatNam", cs, TBL_STRING, TBL_LIKE);
		/*
	...
	if (pssi->PatientQFlag & QF_PAT_PatID) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_patcl[i], "PatID", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->PatientQFlag & QF_PAT_PatBirTim) {
	...
	if (pssi->PatientQFlag & QF_PAT_PatSex) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_patcl[i], "PatSex", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
/* 11-Aug-1997
...
*/
	...
	if (pssi->PatientQFlag & QF_PAT_Owner) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_patcl[i], "Owner", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->PatientQFlag & QF_PAT_GroupName) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_patcl[i], "GroupName", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->PatientQFlag & QF_PAT_Priv) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_patcl[i], "Priv", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (i != 0) {
	...
	if (pssi->StudyQFlag & QF_STU_AccNum) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "AccNum", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_StuID) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "StuID", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_StuInsUID) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "StuInsUID", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_RefPhyNam) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "RefPhyNam", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_StuDes) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "StuDes", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_PatAge) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "PatAge", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_PatSiz) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "PatSiz", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_PatWei) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "PatWei", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
/* 12-Aug-1997
...
*/
	...
	if (pssi->StudyQFlag & QF_STU_Owner) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "Owner", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_GroupName) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "GroupName", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->StudyQFlag & QF_STU_Priv) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_stucl[i], "Priv", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (i != 0) {
	...
	if (pssi->SeriesQFlag & QF_SER_Mod) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "Mod", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_SerNum) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "SerNum", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_SerInsUID) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "SerInsUID", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_ProNam) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "ProNam", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_SerDes) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "SerDes", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_BodParExa) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "BodParExa", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_ViePos) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "ViePos", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
/* 12-Aug-1997
...
*/
	...
	if (pssi->SeriesQFlag & QF_SER_Owner) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "Owner", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_GroupName) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "GroupName", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->SeriesQFlag & QF_SER_Priv) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_sercl[i], "Priv", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (i != 0) {
	...
	if (pssi->ImageQFlag & QF_IMA_ImaNum) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "ImaNum", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->ImageQFlag & QF_IMA_SOPInsUID) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "SOPInsUID", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->ImageQFlag & QF_IMA_SOPClaUID) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "SOPClaUID", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->ImageQFlag & QF_IMA_PhoInt) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "PhoInt", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->ImageQFlag & QF_IMA_PatOri) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "PatOri", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	...
	if (pssi->ImageQFlag & QF_IMA_Owner) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "Owner", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->ImageQFlag & QF_IMA_GroupName) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "GroupName", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	if (pssi->ImageQFlag & QF_IMA_Priv) {
		...
		TBL_CRITERIA_LOAD_BYTE(GS_imacl[i], "Priv", cs, TBL_STRING, TBL_LIKE);
		i++;
	    }
	}
	...
	* idbc;
	...
	if (strcmp(buf, pssi->patient.PatID) != 0) {
	    ...
	    return COND_PushCondition(IDB_ERROR(IDB_PATIDMISMATCH),
			    pssi->study.StuInsUID, pssi->patient.PatID, buf,
				      "IDB_InsertImage");
	}
...
/*	File in pointer to value in loop below */
	...
	if ((ret_val == TBL_NORMAL) && (ret_valb == TBL_NORMAL)) {
	    updateLimitsTable(limits,
	     (NeedImageInsert == 1) ? pssi->image.Size / SCALING_FACTOR : 0,
...
CONDITION
CBIns_CollectInstances(TBL_FIELD * field, long count, void *ctx)
{
    LST_NODE
	* temp_node;
    char
