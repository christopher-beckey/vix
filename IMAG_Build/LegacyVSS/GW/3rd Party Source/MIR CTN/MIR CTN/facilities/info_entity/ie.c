...
static char rcsid[] = "$Revision: 1 $ $RCSfile: ie.c,v $";
 ...
 */
static CTNBOOLEAN
findElement(DCM_OBJECT * object, DCM_TAG tag,
	    DCM_ELEMENT * element);
static SOP_CLASS findModality(DCM_OBJECT * object);
static char *findSOPClassUID(DCM_OBJECT * object);
static SOP_CLASS lookupModality(char *classUID);
...
IE_ExamineObject(DCM_OBJECT ** dcmObject, IE_OBJECT ** ieObject)
{
	...
	flag;
    CTNBOOLEAN			/* Flag indicates the object is incomplete */
	objectIncomplete = FALSE;
    CTNBOOLEAN			/* Flag which indicates an IE is incomplete */
	ieIncomplete = FALSE;
    CTNBOOLEAN			/* Flag which indicates that an IE exists */
	ieExists = FALSE;
    DCM_ELEMENT			/* Handle to DCM_ELEMENT */
	element;
    TAGS			/* Handle to table of TAGS */
	* tags_tbl;
	...
	* table;
    char			/* The SOPClassUID of the DCM object */
       *UID;
	...
	return COND_PushCondition(IE_ILLEGALDCMOBJECT,
		       IE_Message(IE_ILLEGALDCMOBJECT), "IE_ExamineObject");
    /*
	...
	return COND_PushCondition(IE_MALLOCFAILURE, IE_Message(
				     IE_MALLOCFAILURE), "IE_ExamineObject");
	...
	return COND_PushCondition(IE_LISTFAILURE, IE_Message
				  (IE_LISTFAILURE), "IE_ExamineObject");
    /*
	...
	for (k = 0; k < table[loop].size_tbl; k++) {
		 ...
		 */
		else if (!flag && (tags_tbl[j].requirement == IE_K_TYPE1) &&
			 (table[loop].requirement == IE_K_REQUIRED) &&
		 (table[loop].mod_fields[k].requirement == IE_K_REQUIRED)) {
		 ...
		 */
		    ...
		    cond = DCM_GetElementSize(dcmObject, tags_tbl[j].tag,
					      &length);
		    /*
	...
	if (ie_ie == NULL)
	    return COND_PushCondition(IE_MALLOCFAILURE,
			  IE_Message(IE_MALLOCFAILURE), "IE_ExamineObject");
	 ...
	 */
	if ((ie_ie->requirement != IE_K_REQUIRED) &&
	    (ie_ie->status == IE_MISSING))
	    free(ie_ie);
	else {			/* Push this IE into the list if it is not
				 * the above case */
		...
		return COND_PushCondition(IE_LISTFAILURE, IE_Message
				      (IE_LISTFAILURE), "IE_ExamineObject");
	    if (ieIncomplete)
		cond = COND_PushCondition(IE_IEINCOMPLETE, IE_Message
			     (IE_IEINCOMPLETE), table[loop].ie_description);
	    ieIncomplete = FALSE;
	    ieExists = FALSE;
	}
    }
    if (objectIncomplete)
	ie_object->status = IE_INCOMPLETE;
    *ieObject = ie_object;
     ...
     */
    if (objectIncomplete)
	return COND_PushCondition(IE_OBJECTINCOMPLETE, IE_Message
				  (IE_OBJECTINCOMPLETE), "IE_ExamineObject");
    else
	return IE_NORMAL;
}
...
CONDITION
IE_ExamineInformationEntity(DCM_OBJECT ** dcmObject,
			 IE_IETYPE ieType, IE_INFORMATIONENTITY ** ieEntity)
{
    CONDITION			/* Return value from DUL and ACR routines */
	cond;
	...
	* tags_tbl;
    ...
    if (ie_ie->moduleList == NULL)
	return COND_PushCondition(IE_LISTFAILURE, IE_Message
			   (IE_LISTFAILURE), "IE_ExamineInformationEntity");
    /*
	...
	for (k = 0; k < modFields_tbl[loop].size_tbl; k++) {
	     ...
	     */
	    else if (!flag && (tags_tbl[k].requirement == IE_K_TYPE1) &&
		     (modFields_tbl[loop].requirement == IE_K_REQUIRED)) {
		ieIncomplete = TRUE;
		...
		cond = DCM_GetElementSize(dcmObject, tags_tbl[k].tag,
					  &length);
		/*
	...
	if (moduleExists && moduleIncomplete)
	 ...
	 * push it into the list.
	 */
	if ((ie_module->requirement != IE_K_REQUIRED) &&
	    (ie_module->status == IE_MISSING))
	    free(ie_module);
		...
		return COND_PushCondition(IE_LISTFAILURE, IE_Message
			   (IE_LISTFAILURE), "IE_ExamineInformationEntity");
	    if (moduleIncomplete)
		cond = COND_PushCondition(IE_MODULEINCOMPLETE, IE_Message
					  (IE_MODULEINCOMPLETE),
				       modFields_tbl[loop].mod_description);
	...
	ie_ie->status = IE_INCOMPLETE;
	*ieEntity = ie_ie;
	return COND_PushCondition(IE_IEINCOMPLETE, IE_Message
				  (IE_IEINCOMPLETE), ie_ie->ieDescription);
    } else if (ieMissing) {
	ie_ie->status = IE_MISSING;
	*ieEntity = ie_ie;
	return COND_PushCondition(IE_IEMISSING, IE_Message(IE_IEMISSING),
				  ie_ie->ieDescription);
    } else {
	*ieEntity = ie_ie;
	return IE_NORMAL;
...
CONDITION
IE_ExamineModule(DCM_OBJECT ** dcmObject, IE_IETYPE ieType,
		 IE_MODULETYPE moduleType, IE_MODULE ** ieModule)
{
	...
	* tag_tbl = NULL;
    ...
    if (SOPClass == ILLEGAL)
	return COND_PushCondition(IE_ILLEGALDCMOBJECT,
		       IE_Message(IE_ILLEGALDCMOBJECT), "IE_ExamineModule");
    /*
    ...
    if (ie_module == NULL)
	return COND_PushCondition(IE_MALLOCFAILURE, IE_Message
				  (IE_MALLOCFAILURE), "IE_ExamineModule");
    ie_module->structureType = IE_K_MODULE;
    ie_module->moduleType = moduleType;
    ie_module->status = IE_COMPLETE;
    ie_module->attributeList = LST_Create();
    if (ie_module->attributeList == NULL)
	return COND_PushCondition(IE_LISTFAILURE, IE_Message
				  (IE_LISTFAILURE), "IE_ExamineModule");
    /*
	...
	if (ie_attr == NULL)
	    return COND_PushCondition(IE_MALLOCFAILURE,
			  IE_Message(IE_MALLOCFAILURE), "IE_ExamineModule");
	ie_attr->structureType = IE_K_ATTRIBUTE;
	ie_attr->requirement = tag_tbl[loop].requirement;
	flag = findElement(*dcmObject, tag_tbl[loop].tag,
			   &(ie_attr->element));
	if (flag) {
	    attrExists = TRUE;
	    modMissing = FALSE;
	    ...
	    cond = DCM_GetElementSize(dcmObject, tag_tbl[loop].tag,
				      &length);
	    if (cond == DCM_NORMAL) {
		modMissing = FALSE;
		attrExists = TRUE;
	 ...
	 */
	if ((ie_attr->status == IE_MISSING) && (ie_attr->requirement
		       != IE_K_TYPE1 || ie_attr->requirement != IE_K_TYPE2))
	    free(ie_attr);
	else {
	...
	ie_module->status = IE_INCOMPLETE;
	*ieModule = ie_module;
	return COND_PushCondition(IE_MODULEINCOMPLETE, IE_Message(
			IE_MODULEINCOMPLETE), ie_module->moduleDescription);
    } else if (modMissing) {
	ie_module->status = IE_MISSING;
	*ieModule = ie_module;
	return COND_PushCondition(IE_MODULEMISSING,
		IE_Message(IE_MODULEMISSING), ie_module->moduleDescription);
    } else {
	*ieModule = ie_module;
	return IE_NORMAL;
    }
		      ...
		      IE_OBJECT ** object)
{
	...
	* table;
    ...
    if (SOPClass == ILLEGAL)
	return COND_PushCondition(IE_ILLEGALDCMOBJECT,
		  IE_Message(IE_ILLEGALDCMOBJECT), "IE_ObjectRequirements");
    /*
     * Allocate memory for the IE_OBJECT, fill in the structure.
     */
    ie_object = malloc(sizeof(IE_OBJECT));
    if (ie_object == NULL)
	return COND_PushCondition(IE_MALLOCFAILURE, IE_Message(
				IE_MALLOCFAILURE), "IE_ObjectRequirements");
    ie_object->structureType = IE_K_INFORMATIONOBJECT;
    ie_object->ieList = LST_Create();
    if (ie_object->ieList == NULL)
	return COND_PushCondition(IE_LISTFAILURE, IE_Message
				  (IE_LISTFAILURE), "IE_ObjectRequirements");
    /*
	...
	if (table[loop].requirement == IE_K_REQUIRED) {
	    ...
	    if (ie_ie == NULL)
		return COND_PushCondition(IE_MALLOCFAILURE,
		     IE_Message(IE_MALLOCFAILURE), "IE_ObjectRequirements");
	    ie_ie->structureType = IE_K_INFORMATIONENTITY;
	    ie_ie->ieType = table[loop].type;
	    strcpy(ie_ie->ieDescription, table[loop].ie_description);
	    ie_ie->requirement = table[loop].requirement;
	    ...
	    if (cond != LST_NORMAL)
		return COND_PushCondition(IE_LISTFAILURE,
		       IE_Message(IE_LISTFAILURE), "IE_ObjectRequirements");
	}
    }
...
CONDITION
IE_IERequirements(char *classUID, IE_IETYPE ieType,
		  IE_INFORMATIONENTITY ** ieEntity)
{
	...
	* modFields_tbl;
    ...
    if (SOPClass == ILLEGAL)
	return COND_PushCondition(IE_ILLEGALDCMOBJECT,
		      IE_Message(IE_ILLEGALDCMOBJECT), "IE_IERequirements");
    /*
     * Allocate memory for the IE_INFORMATIONENTITY, fill in the structure.
     */
    ie_ie = malloc(sizeof(IE_INFORMATIONENTITY));
    if (ie_ie == NULL)
	return COND_PushCondition(IE_MALLOCFAILURE, IE_Message(
				    IE_MALLOCFAILURE), "IE_IERequirements");
    ie_ie->structureType = IE_K_INFORMATIONENTITY;
    ie_ie->ieType = ieType;
    ie_ie->moduleList = LST_Create();
    if (ie_ie->moduleList == NULL)
	return COND_PushCondition(IE_LISTFAILURE, IE_Message
				  (IE_LISTFAILURE), "IE_IERequirements");
    /*
     * Get the handle to the table of the information entities.
     */
    loop = 0;
	...
	if (modFields_tbl[loop].requirement == IE_K_REQUIRED) {
	    ...
	    if (ie_module == NULL)
		return COND_PushCondition(IE_MALLOCFAILURE,
			 IE_Message(IE_MALLOCFAILURE), "IE_IERequirements");
	    ie_module->structureType = IE_K_MODULE;
	    ie_module->moduleType = modFields_tbl[loop].type;
	    strcpy(ie_module->moduleDescription,
		   modFields_tbl[loop].mod_description);
	    ...
	    if (cond != LST_NORMAL)
		return COND_PushCondition(IE_LISTFAILURE,
			   IE_Message(IE_LISTFAILURE), "IE_IERequirements");
	}
    }
...
CONDITION
IE_ModuleRequirements(char *classUID, IE_IETYPE ieType,
		      IE_MODULETYPE moduleType, IE_MODULE ** ieModule)
{
    CONDITION			/* Return value from DUL and ACR routines */
	cond;
	...
	* tags_tbl;
    ...
    if (SOPClass == ILLEGAL)
	return COND_PushCondition(IE_ILLEGALDCMOBJECT,
		  IE_Message(IE_ILLEGALDCMOBJECT), "IE_ModuleRequirements");
    /*
     * Allocate memory for the IE_INFORMATIONENTITY, fill in the structure.
     */
    ie_module = malloc(sizeof(IE_MODULE));
    if (ie_module == NULL)
	return COND_PushCondition(IE_MALLOCFAILURE, IE_Message(
				 IE_MALLOCFAILURE), "IE_ModuleRequirement");
    ie_module->structureType = IE_K_MODULE;
    ie_module->moduleType = moduleType;
    ie_module->attributeList = LST_Create();
    if (ie_module->attributeList == NULL)
	return COND_PushCondition(IE_LISTFAILURE, IE_Message
				  (IE_LISTFAILURE), "IE_ModuleRequirements");
    /*
     * Get the handle to the table of the information entities.
     */
    loop = 0;
	...
	if (tags_tbl[loop].requirement == IE_K_TYPE1 ||
	    ...
	    if (ie_attr == NULL)
		return COND_PushCondition(IE_MALLOCFAILURE,
		     IE_Message(IE_MALLOCFAILURE), "IE_ModuleRequirements");
	    ie_attr->structureType = IE_K_ATTRIBUTE;
...
IE_Free(void **object)
{
	...
	* attribute;
	...
	if (cond == LST_NORMAL)
	    free(*object);
    } else if (((IE_INFORMATIONENTITY *) * object)->structureType ==
	...
	return COND_PushCondition(IE_ILLEGALDCMOBJECT,
				IE_Message(IE_ILLEGALDCMOBJECT), "IE_Free");
...
/*lint +e527 */
     ...
     */
    ctx = NULL;
    cond = DCM_GetElementValue(&object, &dcm_element,
			       &(dcm_element.length), &ctx);
	...
	if (isString)
	    dcm_element.d.string[dcm_element.length] = '\0';
	*element = dcm_element;
    }
    return flag;
}
...
findModality(DCM_OBJECT * object)
{
    ...
    while (loop < size &&
	   strcmp(element.d.string, modality_tbl[loop].key))
	loop++;
...
findSOPClassUID(DCM_OBJECT * object)
{
    ...
    size = (int) DIM_OF(modality_tbl);
    while (loop < size &&
	   strcmp(element.d.string, modality_tbl[loop].classUID))
	loop++;
...
lookupModality(char *classUID)
{
    ...
    while (loop < (int) DIM_OF(modality_tbl) &&
	   strcmp(classUID, modality_tbl[loop].classUID))
	loop++;
