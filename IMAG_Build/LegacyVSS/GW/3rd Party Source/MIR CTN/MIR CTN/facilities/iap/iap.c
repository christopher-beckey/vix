...
static char rcsid[] = "$Revision: 1 $ $RCSfile: iap.c,v $";
...
IAP_SendImage(DUL_ASSOCIATIONKEY ** association,
	    ...
	    char *fileName, char *moveAETitle, unsigned short moveMessageID,
	      CONDITION(*sendCallback) (MSG_C_STORE_REQ * request, MSG_C_STORE_RESP * response,
		 ...
		 void *callbackCtx)
{
	...
	* object;
    ...
    cond = DCM_OpenFile(fileName, DCM_ORDERLITTLEENDIAN | DCM_NOGROUPLENGTH,
			&object);
    if (cond != DCM_NORMAL)
	return COND_PushCondition(IAP_OBJECTACCESSFAILED,
		      IAP_Message(IAP_OBJECTACCESSFAILED), "IAP_SendImage");
    ...
    (void) DCM_CloseObject(&object);
    if (cond != SRV_NORMAL)
	return COND_PushCondition(IAP_SENDFAILED, IAP_Message(IAP_SENDFAILED),
				  "IAP_SendImage");
		   ...
		   CONDITION(*sendCallback) (), void *callbackCtx)
{
	...
	* object;
    ...
    if (cond != DCM_NORMAL)
	return COND_PushCondition(IAP_OBJECTACCESSFAILED,
		 IAP_Message(IAP_OBJECTACCESSFAILED), "IAP_SendInfoObject");
...
static DCM_FLAGGED_ELEMENT list[] = {
	...
	sizeof(q.Patient.BirthDate), (void *) &q.Patient.BirthDate[0],
    DB_K_QBIRTHDATE, &q.Patient.Query_Flag},
    {DCM_PATNAME, DCM_PN, "", 1,
	sizeof(q.Patient.Name), (void *) &q.Patient.Name[0],
    DB_K_QNAME, &q.Patient.Query_Flag},
    {DCM_PATID, DCM_LO, "", 1,
	sizeof(q.Patient.PatID), (void *) &q.Patient.PatID[0],
    DB_K_QID, &q.Patient.Query_Flag},

    {DCM_IDSTUDYDATE, DCM_DA, "", 1,
	sizeof(q.Study.StudyDate), (void *) &q.Study.StudyDate[0],
    DB_K_QSTUDYDATE, &q.Study.Query_Flag},
    {DCM_IDSTUDYTIME, DCM_TM, "", 1,
	sizeof(q.Study.StudyTime), (void *) &q.Study.StudyTime[0],
    DB_K_QSTUDYTIME, &q.Study.Query_Flag},
	...
	sizeof(q.Study.StudyUID), (void *) &q.Study.StudyUID[0],
    DB_K_QSTUDYUID, &q.Study.Query_Flag},

    {DCM_IDMODALITY, DCM_CS, "", 1,
	sizeof(q.Series.Modality), (void *) &q.Series.Modality[0],
    DB_K_QMODALITY, &q.Series.Query_Flag},
    {DCM_RELSERIESNUMBER, DCM_IS, "", 1,
	sizeof(q.Series.SeriesNumber), (void *) &q.Series.SeriesNumber[0],
    DB_K_QSERIESNUMBER, &q.Series.Query_Flag},
    {DCM_RELSERIESINSTANCEUID, DCM_UI, "", 1,
	sizeof(q.Series.SeriesUID), (void *) &q.Series.SeriesUID[0],
    DB_K_QSERIESUID, &q.Series.Query_Flag},

    {DCM_RELIMAGENUMBER, DCM_IS, "", 1,
	sizeof(q.Image.ImageNumber), (void *) &q.Image.ImageNumber[0],
    DB_K_QIMAGENUMBER, &q.Image.Query_Flag},
/*    {DCM_IDSOPINSTANCEUID, DCM_UI, "", 1,
	sizeof(q.Image.ImageUID), (void *) &q.Image.ImageUID[0],
    DB_K_QIMAGEUID, &q.Image.Query_Flag},
*/
    {DCM_IDSOPCLASSUID, DCM_UI, "", 1,
	sizeof(q.Image.ClassUID), (void *) &q.Image.ClassUID[0],
    DB_K_QCLASSUID, &q.Image.Query_Flag}
};
...
CONDITION
IAP_ObjectToQuery(DCM_OBJECT ** object, char *SOPClass, Query * query,
		  int *elementCount)
{
    CONDITION
	cond,
    ...
    long
        flag;
    static char
        queryLevelString[48] = "";	/* Initialize for AIX compiler bug */
    DCM_ELEMENT
	queryLevelElement =
    {
	DCM_IDQUERYLEVEL, DCM_CS, "", 1,
	    sizeof(queryLevelString), (void *) &queryLevelString[0]
    };
    ...
    *elementCount = 0;
    ...
    if (cond != DCM_NORMAL)
	return COND_PushCondition(IAP_OBJECTACCESSFAILED,
		  IAP_Message(IAP_OBJECTACCESSFAILED), "IAP_ObjectToQuery");
...
CONDITION
IAP_QueryToObject(Query * query, DCM_OBJECT ** object, char *SOPClass,
		  int *elementCount)
{
    CONDITION
	cond,
    ...
    *elementCount = 0;
	...
	if (cond != DCM_NORMAL)
	    return COND_PushCondition(IAP_OBJECTCREATEFAILED,
		  IAP_Message(IAP_OBJECTCREATEFAILED), "IAP_QueryToObject");
    }
    returnCondition = IAP_NORMAL;
    flag = q.QueryState &
    ...
    if (cond != DCM_NORMAL)
	return COND_PushCondition(IAP_OBJECTACCESSFAILED,
		  IAP_Message(IAP_OBJECTACCESSFAILED), "IAP_QueryToObject");

    cond = DCM_ModifyElements(object, NULL, 0, localList,
			      (int) DIM_OF(localList), elementCount);
    if (cond != DCM_NORMAL)
	return COND_PushCondition(IAP_OBJECTACCESSFAILED,
		  IAP_Message(IAP_OBJECTACCESSFAILED), "IAP_QueryToObject");
...
parseImageInstanceUID(DCM_OBJECT ** object, int *elementCount)
{
    ...
    if (e.multiplicity == 1) {
	cond = DCM_ParseObject(object, NULL, 0, localList,
			       (int) DIM_OF(localList), elementCount);
	if (cond != DCM_NORMAL)
	    return COND_PushCondition(IAP_OBJECTACCESSFAILED,
	      IAP_Message(IAP_OBJECTACCESSFAILED), "parseImageInstanceUID");
	...
	if (c == NULL)
	    return COND_PushCondition(IAP_MALLOCFAILURE,
				      IAP_Message(IAP_MALLOCFAILURE),
				(l + 1) + (e.multiplicity * sizeof(char *)),
				      "parseImageInstanceUID");
