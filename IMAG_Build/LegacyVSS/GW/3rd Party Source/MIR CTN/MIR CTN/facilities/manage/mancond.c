...
static char rcsid[] = "$Revision: 1 $ $RCSfile: mancond.c,v $";
...
static VECTOR messageVector[] = {
    ...
    {DMAN_APPLICATIONVERIFICATIONFAILED,
    "Failed to verify application (%s, %s) in %s"},
    {DMAN_TITLENOTFOUND, "Application title %s not found in database in %s"},
    {DMAN_APPLICATIONNODEMISMATCH,
    "Application title (%s, %s) not found on expected node (%s) in %s"},
    {DMAN_ILLEGALCONNECTION,
    "Illegal connection (%s, %s) request in %s"},
    {DMAN_STORAGEACCESSDENIED,
    "Database access denied (%s, %s) by %s"},
    {DMAN_FILEGENERATIONFAILED,
    "Failed to generate a file name for (%s, %s) in %s"},
    {DMAN_FILENAMETOOLONG,
    "Generated file name length (%d) greater than allocated length (%d) in %s"},
