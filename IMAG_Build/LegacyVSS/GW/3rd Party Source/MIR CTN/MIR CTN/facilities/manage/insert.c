...
static char rcsid[] = "$Revision: 1 $ $RCSfile: insert.c,v $";
...
DMAN_Insert(DMAN_HANDLE ** handle, DMAN_GENERICRECORD * workRecord)
{
	...
	* prv;
	...
	cond = insertAE(&prv->applicationEntityHandle,
			(DMAN_APPLICATIONENTITY *) workRecord);
	break;
    case DMAN_K_STORAGECONTROL:
	cond = insertStorageControl(&prv->storageControlHandle,
				    (DMAN_STORAGECONTROL *) workRecord);
	break;
    case DMAN_K_SECURITYMATRIX:
	cond = insertSecurityMatrix(&prv->securityMatrixHandle,
	...
	cond = insertStorageAccess(&prv->storageAccessHandle,
				   (DMAN_STORAGEACCESS *) workRecord);
	break;
    case DMAN_K_FISACCESS:
	cond = insertFISAccess(&prv->FISAccessHandle,
			       (DMAN_FISACCESS *) workRecord);
	break;
    case DMAN_K_PRINTSERVERCFG:
	cond = insertPrintServerCFG(&prv->printServerCFGHandle,
				    (DMAN_PRINTSERVERCFG *) workRecord);
	break;
    case DMAN_K_VIDEOIMAGEDEST:
	cond = insertVideoImageDest(&prv->VideoImageDestHandle,
...
insertStorageControl(TBL_HANDLE ** handle, DMAN_STORAGECONTROL * workRecord)
{
	...
	TBL_FIELD_LOAD_BYTE(fields[i], "RequestingTitle",
			    workRecord->RequestingTitle, TBL_STRING);
	i++;
    }
    if (workRecord->Flag & DMAN_K_STORAGECONTROL_RESPONDING) {
	TBL_FIELD_LOAD_BYTE(fields[i], "RespondingTitle",
			    workRecord->RespondingTitle, TBL_STRING);
	i++;
    }
    if (workRecord->Flag & DMAN_K_STORAGECONTROL_MEDIUM) {
	TBL_FIELD_LOAD_BYTE(fields[i], "Medium",
			    workRecord->Medium, TBL_STRING);
	i++;
    }
    if (workRecord->Flag & DMAN_K_STORAGECONTROL_ROOT) {
	TBL_FIELD_LOAD_BYTE(fields[i], "Root",
			    workRecord->Root, TBL_STRING);
	i++;
    }
    fields[i].FieldName = NULL;
...
#ifdef SMM
static CONDITION
insertGroupNames(TBL_HANDLE ** handle,
		 DMAN_GROUPNAMES * workRecord, DMAN_CRITERIA criteria,
...
insertVideoImageDest(TBL_HANDLE ** handle, DMAN_VIDEOIMAGEDEST * workRecord)
{
	...
	TBL_FIELD_LOAD_BYTE(fields[i], "ImageType",
			    workRecord->ImageType, TBL_STRING);
	i++;
    }
    fields[i].FieldName = NULL;
