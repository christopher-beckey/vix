...
static char rcsid[] = "$Revision: 1 $ $RCSfile: delete.c,v $";
...
DMAN_Delete(DMAN_HANDLE ** handle, DMAN_GENERICRECORD * criteriaRecord)
{
	...
	* prv;
	...
	cond = deleteAE(&prv->applicationEntityHandle,
			(DMAN_APPLICATIONENTITY *) criteriaRecord);
	break;
    case DMAN_K_GROUPNAMES:
	cond = deleteGroup(&prv->groupNamesHandle,
			   (DMAN_GROUPNAMES *) criteriaRecord);
	break;
    case DMAN_K_STORAGECONTROL:
	cond = deleteStorageControl(&prv->storageControlHandle,
				    (DMAN_STORAGECONTROL *) criteriaRecord);
	break;
    case DMAN_K_SECURITYMATRIX:
	cond = deleteSecurityMatrix(&prv->securityMatrixHandle,
				    (DMAN_SECURITYMATRIX *) criteriaRecord);
	break;
    case DMAN_K_STORAGEACCESS:
	cond = deleteStorageAccess(&prv->storageAccessHandle,
				   (DMAN_STORAGEACCESS *) criteriaRecord);
	break;
    case DMAN_K_FISACCESS:
	cond = deleteFISAccess(&prv->FISAccessHandle,
			       (DMAN_FISACCESS *) criteriaRecord);
	break;
    case DMAN_K_PRINTSERVERCFG:
	cond = deletePrintServerCFG(&prv->printServerCFGHandle,
				    (DMAN_PRINTSERVERCFG *) criteriaRecord);
	break;
    case DMAN_K_VIDEOIMAGEDEST:
	cond = deleteVideoImageDest(&prv->VideoImageDestHandle,
				    (DMAN_VIDEOIMAGEDEST *) criteriaRecord);
	break;
    default:
	break;
    }
...
deleteStorageAccess(TBL_HANDLE ** handle, DMAN_STORAGEACCESS * criteriaRecord)
{
	...
	TBL_CRITERIA_LOAD_BYTE(criteria[i], "Title",
			       criteriaRecord->Title, TBL_STRING, TBL_EQUAL);
	i++;
    }
    criteria[i].FieldName = NULL;
...
deletePrintServerCFG(TBL_HANDLE ** handle, DMAN_PRINTSERVERCFG * criteriaRecord)
{
    ...
    static TBL_CRITERIA
        criteria[4];		/* 3 fields and one sentinel (NULL) */
    int
        i;
	...
	TBL_CRITERIA_LOAD_NUM(criteria[i], "GQId", criteriaRecord->GQId,
...
deleteVideoImageDest(TBL_HANDLE ** handle, DMAN_VIDEOIMAGEDEST * criteriaRecord)
{
	...
	TBL_CRITERIA_LOAD_BYTE(criteria[i], "ImageType",
			  criteriaRecord->ImageType, TBL_STRING, TBL_EQUAL);
	i++;
    }
    criteria[i].FieldName = NULL;
