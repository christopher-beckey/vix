...
static char rcsid[] = "$Revision: 1 $ $RCSfile: select.c,v $";
...
static CONDITION
queryAE(TBL_HANDLE ** handle,
	DMAN_APPLICATIONENTITY * workRecord,
	DMAN_APPLICATIONENTITY * criteriaRecord, LST_HEAD * head,
	CONDITION(*callback) (), long *count, void *ctx);
static CONDITION
    callbackAE(TBL_FIELD * field, int count, void *ctx);
static CONDITION
queryGroupNames(TBL_HANDLE ** handle,
		DMAN_GROUPNAMES * workRecord,
		DMAN_GROUPNAMES * criteriaRecord, LST_HEAD * head,
		CONDITION(*callback) (), long *count, void *ctx);
static CONDITION
queryStorageAccess(TBL_HANDLE ** handle,
		   DMAN_STORAGEACCESS * workRecord,
		   DMAN_STORAGEACCESS * criteriaRecord, LST_HEAD * head,
		   CONDITION(*callback) (), long *count, void *ctx);
static CONDITION
...
static CONDITION
querySecurityMatrix(TBL_HANDLE ** handle, DMAN_SECURITYMATRIX * workRecord,
		    DMAN_SECURITYMATRIX * criteriaRecord, LST_HEAD * l,
		    CONDITION(*callback) (), long *count, void *ctx);
static CONDITION
    callbackSecurityMatrix(TBL_FIELD * field, int count, void *ctx);
static CONDITION
queryFISAccess(TBL_HANDLE ** handle,
	       DMAN_FISACCESS * workRecord,
	       DMAN_FISACCESS * criteriaRecord, LST_HEAD * head,
	       CONDITION(*callback) (), long *count, void *ctx);
static CONDITION
    callbackFA(TBL_FIELD * field, int count, void *ctx);
static CONDITION
queryPrintServerCFG(TBL_HANDLE ** handle, DMAN_PRINTSERVERCFG * workRecord,
		    DMAN_PRINTSERVERCFG * criteriaRecord, LST_HEAD * l,
		    CONDITION(*callback) (), long *count, void *ctx);
static CONDITION
    callbackPrintServerCFG(TBL_FIELD * field, int count, void *ctx);

static CONDITION
queryVideoImageDest(TBL_HANDLE ** handle, DMAN_VIDEOIMAGEDEST * workRecord,
		    DMAN_VIDEOIMAGEDEST * criteriaRecord, LST_HEAD * l,
		    CONDITION(*callback) (), long *count, void *ctx);
static CONDITION
...
CONDITION
DMAN_Select(DMAN_HANDLE ** handle, DMAN_GENERICRECORD * workRecord,
	    DMAN_GENERICRECORD * criteriaRecord,
	    LST_HEAD * head, CONDITION(*callback) (), long *count, void *ctx)
{
    CONDITION
	cond;
    PRIVATE_HANDLE
	* prv;
	...
	cond = queryAE(&prv->applicationEntityHandle,
		       (DMAN_APPLICATIONENTITY *) workRecord,
		       (DMAN_APPLICATIONENTITY *) criteriaRecord, head,
		       callback, count, ctx);
	break;
    case DMAN_K_GROUPNAMES:
	cond = queryGroupNames(&prv->groupNamesHandle,
			       (DMAN_GROUPNAMES *) workRecord,
	    (DMAN_GROUPNAMES *) criteriaRecord, head, callback, count, ctx);
	break;
    case DMAN_K_STORAGEACCESS:
	cond = queryStorageAccess(&prv->storageAccessHandle,
				  (DMAN_STORAGEACCESS *) workRecord,
		      (DMAN_STORAGEACCESS *) criteriaRecord, head, callback,
				  count, ctx);
	break;
    case DMAN_K_STORAGECONTROL:
	cond = queryStorageControl(&prv->storageControlHandle,
				   (DMAN_STORAGECONTROL *) workRecord,
		     (DMAN_STORAGECONTROL *) criteriaRecord, head, callback,
				   count, ctx);
	break;
    case DMAN_K_SECURITYMATRIX:
	cond = querySecurityMatrix(&prv->securityMatrixHandle,
				   (DMAN_SECURITYMATRIX *) workRecord,
		     (DMAN_SECURITYMATRIX *) criteriaRecord, head, callback,
				   count, ctx);
	break;
    case DMAN_K_FISACCESS:
	cond = queryFISAccess(&prv->FISAccessHandle,
			      (DMAN_FISACCESS *) workRecord,
			  (DMAN_FISACCESS *) criteriaRecord, head, callback,
			      count, ctx);
	break;
    case DMAN_K_PRINTSERVERCFG:
	cond = queryPrintServerCFG(&prv->printServerCFGHandle,
				   (DMAN_PRINTSERVERCFG *) workRecord,
		     (DMAN_PRINTSERVERCFG *) criteriaRecord, head, callback,
				   count, ctx);
	break;

    case DMAN_K_VIDEOIMAGEDEST:
	cond = queryVideoImageDest(&prv->VideoImageDestHandle,
				   (DMAN_VIDEOIMAGEDEST *) workRecord,
		     (DMAN_VIDEOIMAGEDEST *) criteriaRecord, head, callback,
				   count, ctx);
	break;
	...
	CONDITION(*callback) (), long *count, void *ctx)
{
    ...
    cond = TBL_Select(handle, criteriaList,
		      fields, count, callbackAE, &context);
    if (cond == TBL_NORMAL)
	return DMAN_NORMAL;
    else
	return 0;
		...
		CONDITION(*callback) (), long *count, void *ctx)
{
    ...
    cond = TBL_Select(handle, criteriaList,
		      fields, count, callbackGroup, &context);
    if (cond == TBL_NORMAL)
	return DMAN_NORMAL;
    else
	return 0;
		   ...
		   CONDITION(*callback) (), long *count, void *ctx)
{
    ...
    i = 0;
    TBL_FIELD_DECLARE_STRING(fields[i], "Title", workRecord->Title,
			     sizeof(workRecord->Title));
    i++;
    TBL_FIELD_DECLARE_STRING(fields[i], "DbKey", workRecord->DbKey,
			     sizeof(workRecord->DbKey));
    i++;
    TBL_FIELD_DECLARE_STRING(fields[i], "Owner", workRecord->Owner,
			     sizeof(workRecord->Owner));
    i++;
    TBL_FIELD_DECLARE_STRING(fields[i], "GroupName", workRecord->GroupName,
			     sizeof(workRecord->GroupName));
    i++;
    TBL_FIELD_DECLARE_STRING(fields[i], "Comment", workRecord->Comment,
			     sizeof(workRecord->Comment));
    i++;
    TBL_FIELD_DECLARE_NUM(fields[i], "Access", TBL_SIGNED4, workRecord->Access,
			  sizeof(workRecord->Access));
...
static CONDITION
querySecurityMatrix(TBL_HANDLE ** handle, DMAN_SECURITYMATRIX * workRecord,
		    DMAN_SECURITYMATRIX * criteriaRecord, LST_HEAD * head,
		    CONDITION(*callback) (), long *count, void *ctx)
{
    CONDITION
    cond;
    ...
    context.record = (DMAN_GENERICRECORD *) workRecord;
    cond = TBL_Select(handle, criteriaList, fields, count,
		      callbackSecurityMatrix, &context);
    if (cond == TBL_NORMAL)
	return DMAN_NORMAL;
    else
	return 0;
...
callbackSecurityMatrix(TBL_FIELD * field, int count, void *ctxArg)
{
    ...
    *localMatrix;
	...
	if (strcmp(field->FieldName, "RequestingTitle") == 0) {
	    if (!field->Value.IsNull)
		matrix->Flag |= DMAN_K_SECURITY_REQUESTING;
	} else if (strcmp(field->FieldName, "RespondingTitle") == 0) {
	    if (!field->Value.IsNull)
		matrix->Flag |= DMAN_K_SECURITY_RESPONDING;
	}
	field++;
    }
...
static CONDITION
queryPrintServerCFG(TBL_HANDLE ** handle, DMAN_PRINTSERVERCFG * workRecord,
		    DMAN_PRINTSERVERCFG * criteriaRecord, LST_HEAD * head,
		    CONDITION(*callback) (), long *count, void *ctx)
{
    CONDITION
    cond;
	...
	TBL_CRITERIA_LOAD_NUM(criteriaList[i], "GQId", criteriaRecord->GQId,
			      TBL_SIGNED4, TBL_EQUAL);
	i++;
    }
    criteriaList[i].FieldName = NULL;

    i = 0;
    TBL_FIELD_DECLARE_STRING(fields[i], "RequestingTitle",
	  workRecord->RequestingTitle, sizeof(workRecord->RequestingTitle));
    i++;
    TBL_FIELD_DECLARE_STRING(fields[i], "RespondingTitle",
	  workRecord->RespondingTitle, sizeof(workRecord->RespondingTitle));
    i++;
    TBL_FIELD_DECLARE_NUM(fields[i], "GQId", TBL_SIGNED4,
			  workRecord->GQId, sizeof(workRecord->GQId));
    i++;
    fields[i].FieldName = NULL;
    ...
    context.record = (DMAN_GENERICRECORD *) workRecord;
    cond = TBL_Select(handle, criteriaList, fields, count,
		      callbackPrintServerCFG, &context);
    if (cond == TBL_NORMAL)
	return DMAN_NORMAL;
    else
	return 0;
...
callbackPrintServerCFG(TBL_FIELD * field, int count, void *ctxArg)
{
    ...
    *localCfg;
	...
	if (strcmp(field->FieldName, "RequestingTitle") == 0) {
	    if (!field->Value.IsNull)
		cfg->Flag |= DMAN_K_PRINTSERVER_REQUESTING;
	} else if (strcmp(field->FieldName, "RespondingTitle") == 0) {
	    if (!field->Value.IsNull)
		cfg->Flag |= DMAN_K_PRINTSERVER_RESPONDING;
	} else if (strcmp(field->FieldName, "GQId") == 0) {
	    if (!field->Value.IsNull)
		cfg->Flag |= DMAN_K_PRINTSERVER_GQID;
	}
	field++;
    }
...
static CONDITION
queryVideoImageDest(TBL_HANDLE ** handle, DMAN_VIDEOIMAGEDEST * workRecord,
		    DMAN_VIDEOIMAGEDEST * criteriaRecord, LST_HEAD * head,
		    CONDITION(*callback) (), long *count, void *ctx)
{
    CONDITION
    cond;
    ...
    TBL_FIELD_DECLARE_STRING(fields[i], "RespondingTitle",
	  workRecord->RespondingTitle, sizeof(workRecord->RespondingTitle));
    i++;
    TBL_FIELD_DECLARE_STRING(fields[i], "ImageType",
		      workRecord->ImageType, sizeof(workRecord->ImageType));
    i++;
    fields[i].FieldName = NULL;
    ...
    context.record = (DMAN_GENERICRECORD *) workRecord;
    cond = TBL_Select(handle, criteriaList, fields, count,
		      callbackVideoImageDest, &context);
    if (cond == TBL_NORMAL)
	return DMAN_NORMAL;
    else
	return 0;
...
callbackVideoImageDest(TBL_FIELD * field, int count, void *ctxArg)
{
    ...
    *localCfg;
	...
	if (strcmp(field->FieldName, "RequestingTitle") == 0) {
	    if (!field->Value.IsNull)
		cfg->Flag |= DMAN_K_VIDEOIMAGE_REQUESTING;
	} else if (strcmp(field->FieldName, "RespondingTitle") == 0) {
	    if (!field->Value.IsNull)
		cfg->Flag |= DMAN_K_VIDEOIMAGE_RESPONDING;
	} else if (strcmp(field->FieldName, "ImageType") == 0) {
	    if (!field->Value.IsNull)
		cfg->Flag |= DMAN_K_VIDEOIMAGE_IMAGETYPE;
	}
	field++;
    }
