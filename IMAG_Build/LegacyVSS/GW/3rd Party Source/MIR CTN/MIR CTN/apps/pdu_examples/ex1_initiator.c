...
** Author, Date:	Stephen M. Moore, 20-Nov-93
** Intent:		This program is an example and test program which
**			demonstrates the DICOM packages developed at MIR.
**			It establishes an Association with a server for the
**			purpose of generating Associate RQ and RP PDUs to
**			be used as examples.
**			This program requests 3 presentation contexts
**			representing 3 different SOP Classes, each with
...
** Source File:		$RCSfile: ex1_initiator.c,v $
** Revision:		$Revision: 1 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1 $ $RCSfile: ex1_initiator.c,v $";
		      ...
		      DUL_ASSOCIATESERVICEPARAMETERS * params)
{
    ...
    * association = NULL;
    char
       *SOPClasses[] = {DICOM_SOPCLASSCT, DICOM_SOPCLASSMR,
	DICOM_SOPCLASSCOMPUTEDRADIOGRAPHY
    };

    if (requestAssociation(network, &association, params, SOPClasses, 3) == 0) {
	myExit(NULL);
    }
