/*---------------------------------------------------------------------
 * $Date: 11/04/10 9:18a $             $Revision: 1 $
 *---------------------------------------------------------------------
 *
 *
...
#include <stdlib.h>
#include <Xm/Xm.h>
#include "SaPopCalls.h"		/*** added for Sapphire ***/

#ifdef UIL_CODE
#include <Mrm/MrmPublic.h>
#endif				/* UIL_CODE */

#if defined(__STDC__) && !defined(SOLARIS) && !defined(sun4) && !defined(IRIX) && !defined(HPUX)
...
#else
 /*
  * In the absence of an ORB implementation, these minimal definitions
  * satisfy our method dispatch code.
  */
typedef enum {
    NO_EXCEPTION,
    USER_EXCEPTION,
...
#ifndef UX_INTERPRETER		/* Omit this section when interpreting the
				 * code */
...
extern void
UxFreeClientDataCB(Widget wgt, XtPointer client_data,
		   XtPointer call_data);
...
extern XtArgVal
UxRemoveValueFromArgList(Arg * args,
