...
** Last Update:		$Author: Vhaiswstarkm $, $Date: 11/04/10 9:18a $
** Source File:		$RCSfile: idbmb.h,v $
** Revision:		$Revision: 1 $
** Status:		$State: Exp $
*/
...
#define	IDB_PN_QLENGTH	250
#define	IDB_LO_QLENGTH	250
...
typedef struct {
    S32 DBSize;			/* Current size of database in bytes */
    S32 DBLimit;		/* Limit on database size in MB */
    S32 PatientCount;		/* Number of patients in the database */
    S32 StudyCount;		/* Number of studies in the database */
    S32 ImageCount;		/* Number of images in the database */
} IDB_Limits;
...
#define	QF_PAT_InternalID	0x00002000
...
CONDITION IDB_Open(const char *databaseName, IDB_HANDLE ** handle);

CONDITION IDB_Close(IDB_HANDLE ** handle);

CONDITION IDB_InsertImage(IDB_HANDLE ** handle, const IDB_Insertion * pssi);

CONDITION
IDB_InsertImageInstance(IDB_HANDLE ** handle, char *imageuid,
			IDB_InstanceListElement * iie);

CONDITION IDB_Delete(IDB_HANDLE ** handle, long level, const char *uid, CTNBOOLEAN flag);

CONDITION
IDB_Select(IDB_HANDLE ** handle, IDB_QUERY_MODEL model, long begin_level,
       long end_level, const IDB_Query * pssi, long *count, CONDITION(callback()),
	   void *ctx);
CONDITION
...
CONDITION MBCBDel_CollectStudies(TBL_FIELD * field, int count, void *ctx);
CONDITION MBCBDel_CollectSeries(TBL_FIELD * field, int count, void *ctx);
CONDITION MBCBDel_CollectImages(TBL_FIELD * field, int count, void *ctx);

CONDITION MBCBIns_CollectInstances(TBL_FIELD * field, int count, void *ctx);

void IDB_DestroyGlobalLists(void);

CONDITION MBCBSel_CollectPatients(TBL_FIELD * fp, int count, void *ctx);
CONDITION MBCBSel_CollectStudies(TBL_FIELD * fp, int count, void *ctx);
CONDITION MBCBSel_CollectPatientsStudies(TBL_FIELD * fp, int count, void *ctx);
CONDITION MBCBSel_CollectSeries(TBL_FIELD * fp, int count, void *ctx);
CONDITION MBCBSel_CollectImages(TBL_FIELD * fp, int count, void *ctx);
CONDITION MBCBSel_CollectInstances(TBL_FIELD * fp, int count, void *ctx);

long IDB_ConvertDicomQuerytoSQL(IDB_Query * pssi, long level,
				const char* encodingString);
long IDB_ConvertDicomQuerytoSQL_PSView(IDB_Query * pssi,
				       const char* encodingString);
char *IDB_ConvertDicomMetatoSQL(char *str);

void IDB_SetCharSet(IDB_HANDLE** handle, const char* codeSet);
void IDB_GetCharSet(IDB_HANDLE** handle, char* codeSet);

#define IDB_FREE_STRINGS(index,list,limit)                  \
        for(index=0;index<limit;index++)                \
            if( list[index].Value.Type == TBL_STRING )  \
                free(list[index].Value.Value.String);
...
#define	IDB_CODESETFAILURE	FORM_COND(FAC_IDB, SEV_ERROR, 24)
