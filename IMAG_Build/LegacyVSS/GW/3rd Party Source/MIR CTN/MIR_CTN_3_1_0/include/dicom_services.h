...
SRV_ProposeSOPClassWithXfer(const char*SOPClass, DUL_SC_ROLE role,
			     ...
			     DUL_ASSOCIATESERVICEPARAMETERS* params);
CONDITION
SRV_ReceiveCommand(DUL_ASSOCIATIONKEY ** association,
		   DUL_ASSOCIATESERVICEPARAMETERS * params,
...
CONDITION
SRV_TestForCancel(DUL_ASSOCIATIONKEY ** association,
		  DUL_BLOCKOPTIONS block, int timeout,
		  DUL_PRESENTATIONCONTEXTID ctxID, unsigned short *command,
		  MSG_TYPE * messageType, void **messageArg);
CONDITION
SRV_ReceiveDataSet(DUL_ASSOCIATIONKEY ** association,
		   DUL_PRESENTATIONCONTEXT * presentationCtx,
		   DUL_BLOCKOPTIONS block, int timeout, char *dirName,
		   DCM_OBJECT ** dataSet);
CONDITION
...
typedef
CONDITION(SRV_C_ECHO_REQ_CALLBACK) (MSG_C_ECHO_REQ * echoRequest,
				  MSG_C_ECHO_RESP * echoResonse, void *ctx);
CONDITION
SRV_CEchoRequest(DUL_ASSOCIATIONKEY ** association,
		 DUL_ASSOCIATESERVICEPARAMETERS * params,
	       MSG_C_ECHO_REQ * echoRequest, MSG_C_ECHO_RESP * echoResponse,
		 SRV_C_ECHO_REQ_CALLBACK * cEchoCallback, void *ctx,
		 char *dirName);
typedef
CONDITION(SRV_C_ECHO_RESP_CALLBACK) (MSG_C_ECHO_REQ * echoRequest,
    MSG_C_ECHO_RESP * echoResonse, void *ctx, DUL_PRESENTATIONCONTEXT * pc);
CONDITION
SRV_CEchoResponse(DUL_ASSOCIATIONKEY ** association,
		  DUL_PRESENTATIONCONTEXT * presentationCtx,
		  MSG_C_ECHO_REQ ** echoRequest, MSG_C_ECHO_RESP * echoReply,
		  SRV_C_ECHO_RESP_CALLBACK * cEchoCallback, void *ctx,
		  char *dirName);

typedef
CONDITION(SRV_C_STORE_REQ_CALLBACK) (MSG_C_STORE_REQ * storeRequest,
				     MSG_C_STORE_RESP * storeResonse,
				     unsigned long bytesTransmitted,
				     unsigned long totalBytes, void *ctx);
CONDITION
SRV_CStoreRequest(DUL_ASSOCIATIONKEY ** association,
		  DUL_ASSOCIATESERVICEPARAMETERS * params,
	   MSG_C_STORE_REQ * storeRequest, MSG_C_STORE_RESP * storeResponse,
		  SRV_C_STORE_REQ_CALLBACK * callback,
		  void *callbackCtx, char *dirName);

typedef
CONDITION(SRV_C_STORE_RESP_CALLBACK) (MSG_C_STORE_REQ * storeRequest,
				      MSG_C_STORE_RESP * storeResonse,
				      unsigned long bytesTransmitted,
				      unsigned long totalBytes,
				      DCM_OBJECT ** object, void *callerCtx,
...
typedef
CONDITION(SRV_C_MOVE_REQ_CALLBACK) (MSG_C_MOVE_REQ * moveRequest,
			   MSG_C_MOVE_RESP * moveResonse, int responseCount,
			 char *abstractSyntax, char *queryLevel, void *ctx);
CONDITION
SRV_CMoveRequest(DUL_ASSOCIATIONKEY ** association,
		 DUL_ASSOCIATESERVICEPARAMETERS * params,
	       MSG_C_MOVE_REQ * moveRequest, MSG_C_MOVE_RESP * moveResponse,
		 SRV_C_MOVE_REQ_CALLBACK * statusCallback, void *statusCtx,
		 char *dirName);

typedef
CONDITION(SRV_C_MOVE_RESP_CALLBACK) (MSG_C_MOVE_REQ * moveRequest,
			   MSG_C_MOVE_RESP * moveResonse, int responseCount,
			 char *abstractSyntax, char *queryLevel, void *ctx);
CONDITION
SRV_CMoveResponse(DUL_ASSOCIATIONKEY ** association,
		  DUL_PRESENTATIONCONTEXT * ctx,
	      MSG_C_MOVE_REQ ** moveRequest, MSG_C_MOVE_RESP * moveResponse,
	     SRV_C_MOVE_REQ_CALLBACK * nextFileCallback, void *nextImageCtx,
		  char *dirName);

typedef
CONDITION(SRV_C_GET_REQ_CALLBACK) (MSG_C_GET_REQ * getRequest,
			     MSG_C_GET_RESP * getResonse, int responseCount,
			 char *abstractSyntax, char *queryLevel, void *ctx);
CONDITION
SRV_CGetRequest(DUL_ASSOCIATIONKEY ** association,
		DUL_ASSOCIATESERVICEPARAMETERS * params,
		MSG_C_GET_REQ * getRequest, MSG_C_GET_RESP * getResponse,
		SRV_C_GET_REQ_CALLBACK * getCallback, void *getCtx,
		CONDITION(*storageCallback) (), void *storageCtx,
		char *dirName);

typedef
CONDITION(SRV_C_GET_RESP_CALLBACK) (MSG_C_GET_REQ * getRequest,
				    MSG_C_GET_RESP * getResonse,
	   MSG_C_STORE_REQ * storeRequest, MSG_C_STORE_RESP * storeResponse,
...
SRV_CGetResponse(DUL_ASSOCIATIONKEY ** association,
		 ...
		 DUL_PRESENTATIONCONTEXT * ctx,
		 MSG_C_GET_REQ ** getRequest, MSG_C_GET_RESP * getResponse,
	     SRV_C_GET_RESP_CALLBACK * nextFileCallback, void *nextImageCtx,
		 char *dirName);

typedef
CONDITION(SRV_C_FIND_REQ_CALLBACK) (MSG_C_FIND_REQ * findRequest,
			   MSG_C_FIND_RESP * findResonse, int responseCount,
			 char *abstractSyntax, char *queryLevel, void *ctx);
CONDITION
SRV_CFindRequest(DUL_ASSOCIATIONKEY ** association,
		 DUL_ASSOCIATESERVICEPARAMETERS * params,
	       MSG_C_FIND_REQ * findRequest, MSG_C_FIND_RESP * findResponse,
		 SRV_C_FIND_REQ_CALLBACK * nextRecordCallback,
		 void *nextRecordCtx, char *dirName);

typedef
CONDITION(SRV_C_FIND_RESP_CALLBACK) (MSG_C_FIND_REQ * findRequest,
			   MSG_C_FIND_RESP * findResonse, int responseCount,
			 char *abstractSyntax, char *queryLevel, void *ctx);
CONDITION
SRV_CFindResponse(DUL_ASSOCIATIONKEY ** association,
		  DUL_PRESENTATIONCONTEXT * ctx,
		  MSG_C_FIND_REQ ** findRequest, MSG_C_FIND_RESP * findReply,
		  SRV_C_FIND_RESP_CALLBACK * findCallback, void *findCtx,
		  char *dirName);

typedef
CONDITION(SRV_N_GET_REQ_CALLBACK) (MSG_N_GET_REQ * getRequest,
				   MSG_N_GET_RESP * getResonse, void *ctx);
CONDITION
SRV_NGetRequest(DUL_ASSOCIATIONKEY ** association,
		DUL_ASSOCIATESERVICEPARAMETERS * params, char *SOPClass,
		MSG_N_GET_REQ * getRequest, MSG_N_GET_RESP * getResponse,
		SRV_N_GET_REQ_CALLBACK * getCallback, void *getCtx,
		char *dirName);

typedef
CONDITION(SRV_N_GET_RESP_CALLBACK) (MSG_N_GET_REQ * getRequest,
				    MSG_N_GET_RESP * getResonse, void *ctx,
				    DUL_PRESENTATIONCONTEXT * pc);
CONDITION
SRV_NGetResponse(DUL_ASSOCIATIONKEY ** association,
		 DUL_PRESENTATIONCONTEXT * ctx,
		 MSG_N_GET_REQ ** ngetRequest, MSG_N_GET_RESP * ngetReply,
		 SRV_N_GET_RESP_CALLBACK * ngetCallback, void *ngetCtx,
		 char *dirName);

typedef
CONDITION(SRV_N_CREATE_REQ_CALLBACK) (MSG_N_CREATE_REQ * createRequest,
			      MSG_N_CREATE_RESP * createResonse, void *ctx);
CONDITION
SRV_NCreateRequest(DUL_ASSOCIATIONKEY ** association,
		   DUL_ASSOCIATESERVICEPARAMETERS * params,
		   char *SOPClass,
		   MSG_N_CREATE_REQ * createRequest,
		   MSG_N_CREATE_RESP * createResponse,
		SRV_N_CREATE_REQ_CALLBACK * createCallback, void *createCtx,
		   char *dirName);

typedef
CONDITION(SRV_N_CREATE_RESP_CALLBACK) (MSG_N_CREATE_REQ * createRequest,
			       MSG_N_CREATE_RESP * createResonse, void *ctx,
				       DUL_PRESENTATIONCONTEXT * pc);
CONDITION
SRV_NCreateResponse(DUL_ASSOCIATIONKEY ** association,
		    DUL_PRESENTATIONCONTEXT * presentationCtx,
		    MSG_N_CREATE_REQ ** createRequest,
		    MSG_N_CREATE_RESP * createReply,
	       SRV_N_CREATE_RESP_CALLBACK * createCallback, void *createCtx,
		    char *dirName);

typedef
CONDITION(SRV_N_EVENTREPORT_REQ_CALLBACK)
(MSG_N_EVENT_REPORT_REQ * eventReportRequest,
 MSG_N_EVENT_REPORT_RESP * eventReportResonse,
 void *ctx);
CONDITION
SRV_NEventReportRequest(DUL_ASSOCIATIONKEY ** association,
			DUL_ASSOCIATESERVICEPARAMETERS * params,
			MSG_N_EVENT_REPORT_REQ * eventRequest,
			MSG_N_EVENT_REPORT_RESP * eventResponse,
			SRV_N_EVENTREPORT_REQ_CALLBACK * eventCallback,
			void *eventCtx, char *dirName);

typedef
CONDITION(SRV_N_EVENTREPORT_RESP_CALLBACK)
(MSG_N_EVENT_REPORT_REQ * eventReportRequest,
 MSG_N_EVENT_REPORT_RESP * eventReportResonse,
 void *ctx, DUL_PRESENTATIONCONTEXT * pc);
CONDITION
SRV_NEventReportResponse(DUL_ASSOCIATIONKEY ** association,
			 DUL_PRESENTATIONCONTEXT * presentationCtx,
			 MSG_N_EVENT_REPORT_REQ ** eventRequest,
			 MSG_N_EVENT_REPORT_RESP * eventReply,
			 SRV_N_EVENTREPORT_RESP_CALLBACK * eventCallback,
			 void *eventCtx, char *dirName);

typedef
CONDITION(SRV_N_SET_REQ_CALLBACK)
(MSG_N_SET_REQ * setRequest, MSG_N_SET_RESP * setResonse,
 void *ctx);
CONDITION
SRV_NSetRequest(DUL_ASSOCIATIONKEY ** association,
		...
		SRV_N_SET_REQ_CALLBACK * setCallback, void *setCtx,
		char *dirName);

typedef
CONDITION(SRV_N_SET_RESP_CALLBACK)
(MSG_N_SET_REQ * setRequest,
 MSG_N_SET_RESP * setResonse,
 void *ctx, DUL_PRESENTATIONCONTEXT * pc);
CONDITION
SRV_NSetResponse(DUL_ASSOCIATIONKEY ** association,
		 DUL_PRESENTATIONCONTEXT * presentationCtx,
		 MSG_N_SET_REQ ** setRequest, MSG_N_SET_RESP * setResponse,
		 SRV_N_SET_RESP_CALLBACK * setCallback, void *setCtx,
		 char *dirName);

typedef
CONDITION(SRV_N_DELETE_REQ_CALLBACK)
(MSG_N_DELETE_REQ * deleteRequest,
 MSG_N_DELETE_RESP * deleteResonse, void *ctx);
CONDITION
...
typedef
CONDITION(SRV_N_DELETE_RESP_CALLBACK)
(MSG_N_DELETE_REQ * deleteRequest,
 MSG_N_DELETE_RESP * deleteResonse,
 void *ctx, DUL_PRESENTATIONCONTEXT * pc);
CONDITION
...
typedef
CONDITION(SRV_N_ACTION_REQ_CALLBACK)
(MSG_N_ACTION_REQ * actionRequest,
 MSG_N_ACTION_RESP * actionResonse, void *ctx);
CONDITION
...
typedef
CONDITION(SRV_N_ACTION_RESP_CALLBACK)
(MSG_N_ACTION_REQ * actionRequest,
 MSG_N_ACTION_RESP * actionResonse,
 void *ctx, DUL_PRESENTATIONCONTEXT * pc);
CONDITION
...
void SRV_SetFlagBoolean(const char* flag, CTNBOOLEAN val);
...
#define	SRV_REQUESTEDXFER		FORM_COND(FAC_SRV, SEV_WARN,  41)
#define	SRV_SUPPORTEDXFER		FORM_COND(FAC_SRV, SEV_WARN,  42)
