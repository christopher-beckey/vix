...
** Source File:		$Source: /home/smm/ctn/ctn/facilities/messages/cbuild.c,v $
** Revision:		$Revision: 1 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1 $ $RCSfile: cbuild.c,v $";
...
MSG_BuildCStoreRequest(MSG_C_STORE_REQ * store, DCM_OBJECT ** object)
{
	...
	return COND_PushCondition(MSG_ZEROLENGTHCLASSUID,
	     MSG_Message(MSG_ZEROLENGTHCLASSUID), "MSG_BuildCStoreRequest");
    if (strlen(store->instanceUID) == 0)
	return COND_PushCondition(MSG_ZEROLENGTHINSTANCEUID,
	  MSG_Message(MSG_ZEROLENGTHINSTANCEUID), "MSG_BuildCStoreRequest");
...
CONDITION
MSG_BuildCStoreResponse(MSG_C_STORE_RESP * store,
			DCM_OBJECT ** object)
{
    CONDITION
	cond;
    ...
    if (strlen(store->classUID) == 0)
	return COND_PushCondition(MSG_ZEROLENGTHCLASSUID,
	    MSG_Message(MSG_ZEROLENGTHCLASSUID), "MSG_BuildCStoreResponse");
    if (strlen(store->instanceUID) == 0)
	return COND_PushCondition(MSG_ZEROLENGTHINSTANCEUID,
	  MSG_Message(MSG_ZEROLENGTHINSTANCEUID), "MSG_BuildCStoreReponse");
...
MSG_BuildCFindRequest(MSG_C_FIND_REQ * find, DCM_OBJECT ** object)
{
	...
	return COND_PushCondition(MSG_ZEROLENGTHCLASSUID,
	      MSG_Message(MSG_ZEROLENGTHCLASSUID), "MSG_BuildCFindRequest");
...
MSG_BuildCMoveRequest(MSG_C_MOVE_REQ * move, DCM_OBJECT ** object)
{
	...
	sizeof(type), (void *) &type},
	{DCM_GROUPCOMMAND, DCM_CMDPRIORITY, DCM_US, "", 1,
	0, (void *) &request.priority},
	{DCM_GROUPCOMMAND, DCM_CMDMOVEDESTINATION, DCM_AE, "", 1,
	0, (void *) request.moveDestination},
    };

    if (strlen(move->classUID) == 0)
	return COND_PushCondition(MSG_ZEROLENGTHCLASSUID,
	      MSG_Message(MSG_ZEROLENGTHCLASSUID), "MSG_BuildCMoveRequest");
