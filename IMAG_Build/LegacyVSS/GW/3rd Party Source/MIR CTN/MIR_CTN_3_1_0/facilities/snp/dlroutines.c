...
static char rcsid[] = "$Revision: 1 $ $RCSfile: dlroutines.c,v $";
...
dlbindreq(int fd, u_long sap, u_long max_conind, u_long service_mode, u_long conn_mgmt, u_long xidtest)
{
	...
	return COND_PushCondition(SNP_PUTMSGFAIL,
				  SNP_Message(SNP_PUTMSGFAIL), "dlbindreq");
    else
	return SNP_NORMAL;
}
...
dlunbindreq(int fd)
{
    ...
    if (putmsg(fd, &ctl, (struct strbuf *) NULL, flags) < 0)
	return COND_PushCondition(SNP_PUTMSGFAIL,
				SNP_Message(SNP_PUTMSGFAIL), "dlunbindreq");
    else
	return SNP_NORMAL;
}
...
dldetachreq(int fd)
{
    ...
    if (putmsg(fd, &ctl, (struct strbuf *) NULL, flags) < 0)
	return COND_PushCondition(SNP_PUTMSGFAIL,
				SNP_Message(SNP_PUTMSGFAIL), "dldetachreq");
    else
	return SNP_NORMAL;
}
...
dlbindack(int fd, char *bufp)
{
	...
	return COND_PushCondition(SNP_DLPIFAIL,
				  SNP_Message(SNP_DLPIFAIL), "dlbindack");
...
dlattachreq(int fd, u_long ppa)
{
    ...
    if (putmsg(fd, &ctl, (struct strbuf *) NULL, flags) < 0)
	return COND_PushCondition(SNP_PUTMSGFAIL,
				SNP_Message(SNP_PUTMSGFAIL), "dlattachreq");
    else
	return SNP_NORMAL;
}
...
dlokack(int fd, char *bufp)
{
    ...
    if (cond != SNP_NORMAL)
	return COND_PushCondition(SNP_DLPIFAIL,
				  SNP_Message(SNP_DLPIFAIL), "dlokack");
...
dlpromisconreq(int fd, u_long level)
{
    ...
    if (putmsg(fd, &ctl, (struct strbuf *) NULL, flags) < 0)
	return COND_PushCondition(SNP_PUTMSGFAIL,
			     SNP_Message(SNP_PUTMSGFAIL), "dlpromisconreq");
    else
	return SNP_NORMAL;
...
strgetmsg(int fd, struct strbuf * ctlp, struct strbuf * datap, int *flagsp, char *caller)
{
    ...
    if (alarm(MAXWAIT) < 0)
	return COND_PushCondition(SNP_ALARMSET,
				  SNP_Message(SNP_ALARMSET), "strgetmsg");
    ...
    *flagsp = 0;
	...
	return COND_PushCondition(SNP_GETMSGFAIL,
				  SNP_Message(SNP_GETMSGFAIL), "strgetmsg");
...
expecting(int prim, union DL_primitives * dlp)
{
	...
	return COND_PushCondition(SNP_DLPIEXPECT, SNP_Message(SNP_DLPIEXPECT),
				  dlprim(prim), dlprim(dlp->dl_primitive));
    return SNP_NORMAL;
}
