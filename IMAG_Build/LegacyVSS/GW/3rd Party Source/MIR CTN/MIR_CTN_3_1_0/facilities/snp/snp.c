...
static char rcsid[] = "$Revision: 1 $ $RCSfile: snp.c,v $";
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "In SNP_Init\n");
#endif
    handle = (SNP_HANDLE *) malloc(sizeof(SNP_HANDLE));
    if (handle == NULL)
	return COND_PushCondition(SNP_MALLOCERROR, SNP_Message(SNP_MALLOCERROR),
				  sizeof(SNP_HANDLE), "SNP_Init");
    return SNP_NORMAL;
}
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "In SNP_Terminate\n");
#endif
    free(handle);
    return SNP_NORMAL;
}
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "In SNP_RegisterCallback\n");
#endif
    switch (callbackType) {
...
SNP_Start(char *device, int ppa, char *initiator, char *acceptor, int port, int timeOutCon, int timeOutBuf, int bufferSpace)
{
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "In SNP_Start\n");
#endif
    /*
     * Make sure callbacks are registered
     */
    if ((handle->callback_state == NULL)
	|| (handle->callback_itoa == NULL)
	|| (handle->callback_atoi == NULL))
	return COND_PushCondition(SNP_CALLBACKSMISSING,
				  SNP_Message(SNP_CALLBACKSMISSING));

    /*
     * Validate arguments.
     */
    if (ppa < 0)
	return COND_PushCondition(SNP_ARGERROR, SNP_Message(SNP_ARGERROR),
				"ppa", "SNP_Start", "Must be 0 or greater");
    if (!(timeOutCon > 0))
	return COND_PushCondition(SNP_ARGERROR, SNP_Message(SNP_ARGERROR),
				  "timeOutCon", "SNP_Start", "Must be > 0");
    if (!(timeOutBuf > 0))
	return COND_PushCondition(SNP_ARGERROR, SNP_Message(SNP_ARGERROR),
				  "timeOutBuf", "SNP_Start", "Must be > 0");
    if (!(bufferSpace > 0))
	return COND_PushCondition(SNP_ARGERROR, SNP_Message(SNP_ARGERROR),
				  "bufferSpace", "SNP_Start", "Must be > 0");
    if (!(timeOutCon > timeOutBuf))
	return COND_PushCondition(SNP_ARGERROR, SNP_Message(SNP_ARGERROR),
				  "timeOutCon and timeOutBuf", "SNP_Start",
				  "timeOutCon must be > timeOutBuf");
    ...
    if (host == NULL)
	return COND_PushCondition(SNP_ARGERROR, SNP_Message(SNP_ARGERROR),
		       "initiator", "SNP_Start", "Can not find IP address");
#ifdef DEBUG
    if (debug) {
    ...
    if (host == NULL)
	return COND_PushCondition(SNP_ARGERROR, SNP_Message(SNP_ARGERROR),
			"acceptor", "SNP_Start", "Can not find IP address");
#ifdef DEBUG
    if (debug) {
	...
	return COND_PushCondition(SNP_OPENERROR, SNP_Message(SNP_OPENERROR),
				  "DLPI STREAM", "SNP_Start");
	...
	return COND_PushCondition(SNP_STREAMSETUP,
				  SNP_Message(SNP_STREAMSETUP), "SNP_Start");

    /*
     * Create LSTs to use for segments awaiting acknowledgements
     */
    if ((handle->tcp_list[ITOA] = LST_Create()) == NULL)
	return COND_PushCondition(SNP_LSTCREATFAIL,
			SNP_Message(SNP_LSTCREATFAIL), "ITOA", "SNP_Start");
    if ((handle->tcp_list[ATOI] = LST_Create()) == NULL)
	return COND_PushCondition(SNP_LSTCREATFAIL,
			SNP_Message(SNP_LSTCREATFAIL), "ATOI", "SNP_Start");
    ...
    if (ioctl(handle->fd_in, I_SETSIG, S_RDNORM | S_HIPRI) < 0)
	return COND_PushCondition(SNP_SIGSETERROR, SNP_Message(SNP_SIGSETERROR),
				  "SIGPOLL", "SNP_Start");
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "Leaving SNP_Start - STREAM set up, ready for monitoring\n");
#endif
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "In SNP_Stop - closing STREAM and cleaning up\n");
#endif
    /*
     * Close incoming DLPI stream
     */
    if ((close(handle->fd_in)) == -1)
	return COND_PushCondition(SNP_CLOSEERROR, SNP_Message(SNP_CLOSEERROR),
				  "DLPI STREAM", "SNP_Stop");
...
sigpoll_func()
{
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "In sigpoll_func()\n");
#endif
    if (sigflag) {
...
#ifdef DEBUG
	    if (debug)
		fprintf(stderr, "Data length of buffer = %d\n",
			data.len);
#endif
	    if (data.len) {
		for (p = data.buf; p < (char *) (data.buf + data.len);
		     p += bp->sbh_totlen) {
		    bp = (struct sb_hdr *) p;
		    if (bp->sbh_drops) {
			/* Call CALLBACK STATE and report packets dropped */
			cond = handle->callback_state(DROPPEDPACKETS,
						      handle->state_ctx);
#ifdef DEBUG
			if (debug)
	 ...
	 */
	if (((handle->tcp_list[ITOA]) && (LST_Count(&(handle->tcp_list[ITOA])))) ||
	    ((handle->tcp_list[ATOI]) && (LST_Count(&(handle->tcp_list[ATOI]))))) {
#ifdef DEBUG
	    if (debug)
		fprintf(stderr, "Connection time out alarm set\tTime : %d\n",
			handle->timeOutCon);
#endif
...
#ifdef DEBUG
	if (debug)
	    fprintf(stderr, "Made it through sigpoll_func()\n");
#endif
	/*
...
streamSetup(int ppa, char *device, int sap, int bufferSpace, int timeOutBuf)
{
    ...
    u_short port_filt[] = {ENF_PUSHLIT, ACC_PORT, ENF_PUSHWORD + 17, ENF_COR, ENF_PUSHLIT,
    ACC_PORT, ENF_PUSHWORD + 18, ENF_CAND};
    /*
     * Packet filter for 2nd half of SRC and DST address with 2nd half of INI
     * address
     */
    struct packetfilt for_ini_add2;
    u_short ini_add2_filt[] = {ENF_PUSHLIT, 0 /* INI_ADD2 */ , ENF_PUSHWORD + 14, ENF_COR,
    ENF_PUSHLIT, 0 /* INI_ADD2 */ , ENF_PUSHWORD + 16, ENF_CAND};
    ...
    u_short acc_add2_filt[] = {ENF_PUSHLIT, 0 /* ACC_ADD2 */ , ENF_PUSHWORD + 14, ENF_COR,
    ENF_PUSHLIT, 0 /* ACC_ADD2 */ , ENF_PUSHWORD + 16, ENF_CAND};
    ...
    u_short ini_add1_filt[] = {ENF_PUSHLIT, 0 /* INI_ADD1 */ , ENF_PUSHWORD + 13, ENF_COR,
    ENF_PUSHLIT, 0 /* INI_ADD1 */ , ENF_PUSHWORD + 15, ENF_CAND};
    /*
     * Packet filter for 1st half of SRC and DST address with 1st half of ACC
     * address
     */
    struct packetfilt for_acc_add1;
    u_short acc_add1_filt[] = {ENF_PUSHLIT, 0 /* ACC_ADD1 */ , ENF_PUSHWORD + 13, ENF_COR,
    ENF_PUSHLIT, 0 /* ACC_ADD1 */ , ENF_PUSHWORD + 15, ENF_CAND};
...
#ifdef DEBUG
    if (debug)
	fprintf(stderr, "Performing streamSetup\n");
#endif
    ...
    if (cond != SNP_NORMAL)
	return COND_PushCondition(SNP_DLPIFAIL,
				  SNP_Message(SNP_DLPIFAIL), "streamSetup");
     ...
     */
    if (strioctl(handle->fd_in, DLIOCRAW, -1, 0, NULL) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "DLIOCRAW", "streamSetup");
	...
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "I_PUSH pfmod for port", "streamSetup");
    ...
    if (strioctl(handle->fd_in, PFIOCSETF, -1,
		 sizeof(for_port), (char *) &for_port) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "PFIOCSETF for port", "streamSetup");
    ...
    if (ioctl(handle->fd_in, I_PUSH, "pfmod") < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		    "I_PUSH pfmod for 2nd half ini address", "streamSetup");
    ...
    if (strioctl(handle->fd_in, PFIOCSETF, -1,
		 sizeof(for_ini_add2), (char *) &for_ini_add2) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		       "PFIOCSETF for 2nd half ini address", "streamSetup");
    ...
    if (ioctl(handle->fd_in, I_PUSH, "pfmod") < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		    "I_PUSH pfmod for 2nd half acc address", "streamSetup");
    ...
    if (strioctl(handle->fd_in, PFIOCSETF, -1,
		 sizeof(for_acc_add2), (char *) &for_acc_add2) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		       "PFIOCSETF for 2nd half acc address", "streamSetup");
    ...
    if (ioctl(handle->fd_in, I_PUSH, "pfmod") < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		    "I_PUSH pfmod for 1st half ini address", "streamSetup");
    ...
    if (strioctl(handle->fd_in, PFIOCSETF, -1,
		 sizeof(for_ini_add1), (char *) &for_ini_add1) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		       "PFIOCSETF for 1st half ini address", "streamSetup");
    ...
    if (ioctl(handle->fd_in, I_PUSH, "pfmod") < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		    "I_PUSH pfmod for 1st half acc address", "streamSetup");
    ...
    if (strioctl(handle->fd_in, PFIOCSETF, -1,
		 sizeof(for_acc_add1), (char *) &for_acc_add1) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
		    "PFIOCSETF for 1st half of acc address", "streamSetup");

    /* Filter for TCP segements in the stream */

    if (ioctl(handle->fd_in, I_PUSH, "pfmod") < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "I_PUSH pfmod for TCP", "streamSetup");
    ...
    if (strioctl(handle->fd_in, PFIOCSETF, -1,
		 sizeof(for_tcp), (char *) &for_tcp) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "PFIOCSETF for TCP", "streamSetup");
	...
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "SBIOCSTIME", "streamSetup");

    /*
     * Set chunksize
     */
    if (strioctl(handle->fd_in, SBIOCSCHUNK, -1,
		 sizeof(u_int), (char *) &chunksize) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "SBIOCSCHUNK", "streamSetup");

    /*
     * * Set up the bufmod flags
     */
    if (strioctl(handle->fd_in, SBIOCGFLAGS, -1,
		 sizeof(u_long), (char *) &flag) < 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "SBIOCGFLAGS", "streamSetup");
    flag |= SB_NO_DROPS;
    if (strioctl(handle->fd_in, SBIOCSFLAGS, -1,
		 sizeof(u_long), (char *) &flag) != 0)
	return COND_PushCondition(SNP_IOCTLFAIL, SNP_Message(SNP_IOCTLFAIL),
				  "SBIOCSFLAGS", "streamSetup");
...
SNP_StateMsg(int state)
{
	...
	if (state == message_st[index].state)
	    return message_st[index].message;
...
#else				/* If SNOOP is not defined .... just return
				 * the unimplemented code */
...
SNP_Init()
{
    ...
    return COND_PushCondition(SNP_UNIMPLEMENTED,
			      SNP_Message(SNP_UNIMPLEMENTED), "SNP_Init");
}
...
SNP_Terminate()
{
    ...
    return COND_PushCondition(SNP_UNIMPLEMENTED,
			   SNP_Message(SNP_UNIMPLEMENTED), "SNP_Terminate");
...
SNP_RegisterCallback(CONDITION(*callback) (), int callbackType, void *ctx) {
    ...
    return COND_PushCondition(SNP_UNIMPLEMENTED,
		    SNP_Message(SNP_UNIMPLEMENTED), "SNP_RegisterCallback");
...
SNP_Stop()
{
    ...
    return COND_PushCondition(SNP_UNIMPLEMENTED,
			      SNP_Message(SNP_UNIMPLEMENTED), "SNP_Stop");
