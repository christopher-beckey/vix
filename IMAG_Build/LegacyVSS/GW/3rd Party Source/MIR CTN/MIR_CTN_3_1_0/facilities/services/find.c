...
static char rcsid[] = "$Revision: 1 $ $RCSfile: find.c,v $";
...
SRV_CFindRequest(DUL_ASSOCIATIONKEY ** association,
		 ...
		 SRV_C_FIND_REQ_CALLBACK * callback,
		 void *callbackCtx, char *dirName)
{
	...
	* localResponse;
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			   SRV_Message(SRV_NOCALLBACK), "SRV_CFindRequest");
    if (findRequest->type != MSG_K_C_FIND_REQ)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
		  SRV_Message(SRV_ILLEGALPARAMETER), "type", "FIND Request",
				  "SRV_CFindRequest");
    if (findRequest->dataSetType == DCM_CMDDATANULL)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
	   SRV_Message(SRV_ILLEGALPARAMETER), "dataSetType", "FIND Request",
				  "SRV_CFindRequest");

    presentationCtx = SRVPRV_PresentationContext(params,
						 findRequest->classUID);
    if (presentationCtx == NULL) {
	return COND_PushCondition(SRV_NOSERVICEINASSOCIATION,
	     SRV_Message(SRV_NOSERVICEINASSOCIATION), findRequest->classUID,
				  "SRV_CFindRequest");
    }
    /*
    ...
    cond = DCM_ParseObject(&findRequest->identifier, &queryLevelElement, 1,
			   NULL, 0, NULL);
    if (cond != DCM_NORMAL) {
	...
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
			 SRV_Message(SRV_OBJECTBUILDFAILED), "FIND Request",
				  "SRV_CFindRequest");
	...
	return COND_PushCondition(SRV_REQUESTFAILED,
			SRV_Message(SRV_REQUESTFAILED), "SRV_CFindRequest");

    cond = SRV_SendDataSet(association, presentationCtx,
			   &findRequest->identifier, NULL, NULL, 0);
    if (cond != SRV_NORMAL)
	return COND_PushCondition(SRV_REQUESTFAILED,
			SRV_Message(SRV_REQUESTFAILED), "SRV_CFindRequest");
	...
	cond = MSG_StatusLookup(localResponse->status, MSG_K_C_FIND_RESP,
				&statusDescription);
	if (cond != MSG_NORMAL)
	    return COND_PushCondition(SRV_REQUESTFAILED,
			SRV_Message(SRV_REQUESTFAILED), "SRV_CFindRequest");
	...
	if (!cancelled) {
	    cond = callback(findRequest, localResponse, responseCount,
	...
	if (localResponse->identifier != NULL)
	    (void) DCM_CloseObject(&localResponse->identifier);
...
SRV_CFindResponse(DUL_ASSOCIATIONKEY ** association,
	      ...
	      MSG_C_FIND_REQ ** findRequest, MSG_C_FIND_RESP * findResponse,
		  SRV_C_FIND_RESP_CALLBACK * findCallback,
		  void *findCtx, char *dirName)
{
    int
        flag,
        responseCount = 0;
...
a status of other than pending and a data set that is not null.\n";
	...
	* cancelMessage;
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			  SRV_Message(SRV_NOCALLBACK), "SRV_CFindResponse");
    }
    ...
    cond = SRV_ReceiveDataSet(association, presentationCtx, DUL_BLOCK, 0,
			      dirName, &(*findRequest)->identifier);

    if (PRVSRV_debug && (cond == SRV_NORMAL))
	(void) DCM_DumpElements(&(*findRequest)->identifier, 0);

    if (cond != SRV_NORMAL) {
	(void) MSG_Free((void **) findRequest);
	return COND_PushCondition(SRV_RESPONSEFAILED,
		      SRV_Message(SRV_RESPONSEFAILED), "SRV_CFindResponse");
    }
    cond = DCM_ParseObject(&(*findRequest)->identifier, &queryLevelElement, 1,
			   NULL, 0, NULL);
	...
	(void) COND_PushCondition(SRV_ILLEGALQUERYLEVELATTRIBUTE,
				SRV_Message(SRV_ILLEGALQUERYLEVELATTRIBUTE),
				  queryLevelString, "SRV_CFindResponse");
	return COND_PushCondition(SRV_RESPONSEFAILED,
		      SRV_Message(SRV_RESPONSEFAILED), "SRV_CFindResponse");
    }
	...
	if (cond == SRV_NORMAL) {
	    ...
	    (void) findCallback(*findRequest, findResponse, 0,
			  presentationCtx->abstractSyntax, queryLevelString,
				findCtx);
	    findResponse->status = MSG_K_CANCEL;
	    findResponse->messageIDRespondedTo = (*findRequest)->messageID;
	    ...
	    cond = findCallback(*findRequest, findResponse, responseCount,
			  presentationCtx->abstractSyntax, queryLevelString,
				findCtx);
	}
	...
	if (findResponse->dataSetType != DCM_CMDDATANULL) {
	    cond = SRV_SendDataSet(association, presentationCtx,
				   &findResponse->identifier, NULL, NULL, 0);
	    if (cond != SRV_NORMAL) {
		(void) MSG_Free((void **) findRequest);
		return COND_PushCondition(SRV_RESPONSEFAILED,
		      SRV_Message(SRV_RESPONSEFAILED), "SRV_CFindResponse");
