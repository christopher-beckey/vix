...
static char rcsid[] = "$Revision: 1 $ $RCSfile: nset.c,v $";
		 ...
		 char *dirName)
{
	...
	* responseObject = NULL;
	...
	rtnCond = COND_PushCondition(SRV_NOCALLBACK,
			   SRV_Message(SRV_NOCALLBACK), "SRV_NSetResponse");
    ...
    cond = SRV_ReceiveDataSet(association, presentationCtx,
			    DUL_BLOCK, 0, dirName, &(*setRequest)->dataSet);
    if (cond != SRV_NORMAL) {
	rtnCond = COND_PushCondition(SRV_RECEIVEDATASETFAILED,
		 SRV_Message(SRV_RECEIVEDATASETFAILED), "SRV_NSetResponse");
	...
	rtnCond = COND_PushCondition(SRV_OBJECTBUILDFAILED,
				     ...
				     "NSet Response Data Set",
				     "SRV_NSetResponse");
	setResponse->status = MSG_K_PROCESSINGFAILURE;
	goto SEND_RESPONSE;
    }
    cond = setCallback(*setRequest, setResponse, setCtx,
		       presentationCtx);
    if (cond != SRV_NORMAL) {
	...
	rtnCond = COND_PushCondition(SRV_ILLEGALPARAMETER,
		SRV_Message(SRV_ILLEGALPARAMETER), "type", "N-SET Response",
				     "SRV_NSetResponse");
	goto SEND_RESPONSE;
    }
SEND_RESPONSE:
	...
	if (setResponse->dataSet)
	    (void) DCM_CloseObject(&setResponse->dataSet);
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
		       SRV_Message(SRV_OBJECTBUILDFAILED), "N-SET Response",
				  "SRV_NSetResponse");
    }
	...
	cond = SRV_SendDataSet(association, presentationCtx,
			       &setResponse->dataSet, NULL, NULL, 0);
	if (cond != SRV_NORMAL) {
	    (void) DCM_CloseObject(&setResponse->dataSet);
	    return COND_PushCondition(SRV_SENDFAILED,
