...
static char rcsid[] = "$Revision: 1 $ $RCSfile: srvcond.c,v $";
...
static VECTOR messageVector[] = {
    ...
    "SRV Unexpected presentation context ID: %x in %s"},
    ...
    "SRV Transfer Syntax (%s) is not supported for abstract syntax (%s) in %s"},
    ...
    "SRV In %s response, suspicious combination of status (%s) and data set type (%s) in %s"},
    {SRV_OPERATIONCANCELLED, "SRV Callback or peer cancelled operation"},
    {SRV_PDVRECEIVEDOUTOFSEQUENCE,
    "SRV PDV Received out of sequence.  Expected (%s), Received (%s %d) in %s"},
