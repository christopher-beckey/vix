...
static char rcsid[] = "$Revision: 1 $ $RCSfile: move.c,v $";
...
SRV_CMoveRequest(DUL_ASSOCIATIONKEY ** association,
		 ...
		 SRV_C_MOVE_REQ_CALLBACK * callback, void *callbackCtx,
		 char *dirName)
{
    DCM_OBJECT
	...
	* localResponse;
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			   SRV_Message(SRV_NOCALLBACK), "SRV_CMoveRequest");
    if (moveRequest->type != MSG_K_C_MOVE_REQ)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
		  SRV_Message(SRV_ILLEGALPARAMETER), "type", "MOVE Request",
				  "SRV_CMoveRequest");
    if (moveRequest->dataSetType == DCM_CMDDATANULL)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
	   SRV_Message(SRV_ILLEGALPARAMETER), "dataSetType", "MOVE Request",
				  "SRV_CMoveRequest");

    ctx = NULL;
    cond = DCM_GetElementValue(&moveRequest->identifier, &queryLevelElement,
			       &l, &ctx);
    if (cond != DCM_NORMAL)
	...
	return COND_PushCondition(SRV_REQUESTFAILED,
			SRV_Message(SRV_REQUESTFAILED), "SRV_CMoveRequest");
    }
    cond = SRV_SendDataSet(association, presentationCtx,
			   &moveRequest->identifier, NULL, NULL, 0);
    if (cond != SRV_NORMAL)
	return COND_PushCondition(SRV_REQUESTFAILED,
			SRV_Message(SRV_REQUESTFAILED), "SRV_CMoveRequest");
	...
	cond = MSG_StatusLookup(localResponse->status, MSG_K_C_MOVE_RESP,
				&statusDescription);
	if (cond != MSG_NORMAL)
	    return COND_PushCondition(SRV_REQUESTFAILED,
			SRV_Message(SRV_REQUESTFAILED), "SRV_CMoveRequest");
	...
	responseCount++;
	if (!cancelled) {
	    cond = callback(moveRequest, localResponse, responseCount,
	...
	if (moveResponse != NULL) {
	    *moveResponse = *localResponse;
	    ...
	    moveResponse->conditionalFields &= !(MSG_K_C_MOVE_SUCCESSUID |
			  MSG_K_C_MOVE_FAILEDUID | MSG_K_C_MOVE_WARNINGUID);
#else
...
SRV_CMoveResponse(DUL_ASSOCIATIONKEY ** association,
	      ...
	      MSG_C_MOVE_REQ ** moveRequest, MSG_C_MOVE_RESP * moveResponse,
		  SRV_C_MOVE_RESP_CALLBACK * moveCallback, void *moveCtx,
		  char *dirName)
{
    int
        flag,
	...
	* responseObject;
	...
	DCM_QUERYLEVELPATIENT, DCM_QUERYLEVELSTUDY,
    DCM_QUERYLEVELSERIES, DCM_QUERYLEVELIMAGE};
...
a status of pending and data set that is not null.\n";
	...
	* cancelMessage;
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			  SRV_Message(SRV_NOCALLBACK), "SRV_CMoveResponse");
    }
	...
	return COND_PushCondition(SRV_LISTCREATEFAILURE,
		   SRV_Message(SRV_LISTCREATEFAILURE), "SRV_CMoveResponse");
	...
	return COND_PushCondition(SRV_RESPONSEFAILED,
		      SRV_Message(SRV_RESPONSEFAILED), "SRV_CMoveResponse");
#endif

    cond = SRV_ReceiveDataSet(association, presentationCtx, DUL_BLOCK, 0,
			      dirName, &(*moveRequest)->identifier);

    if (PRVSRV_debug && (cond == SRV_NORMAL))
	(void) DCM_DumpElements(&(*moveRequest)->identifier, 0);
	...
	(void) COND_PushCondition(SRV_ILLEGALQUERYLEVELATTRIBUTE,
				SRV_Message(SRV_ILLEGALQUERYLEVELATTRIBUTE),
				  queryLevelString, "SRV_CMoveResponse");
	return COND_PushCondition(SRV_RESPONSEFAILED,
		      SRV_Message(SRV_RESPONSEFAILED), "SRV_CMoveResponse");
    }
    ...
    flag = 0;
    while (!flag) {
	cond = SRV_TestForCancel(association, DUL_NOBLOCK, 0,
		     presentationCtx->presentationContextID, &cancelCommand,
				 &cancelMsgType, (void **) &cancelMessage);
	if (cond == SRV_NORMAL) {
	    rtnCond = SRV_OPERATIONCANCELLED;
	    moveResponse->status = MSG_K_CANCEL;
	    (void) MSG_Free((void **) &cancelMessage);
	    (void) moveCallback(*moveRequest, moveResponse, 0,
			  presentationCtx->abstractSyntax, queryLevelString,
				moveCtx);
	    moveResponse->status = MSG_K_CANCEL;
	    moveResponse->messageIDRespondedTo = (*moveRequest)->messageID;
	    ...
	    cond = moveCallback(*moveRequest, moveResponse, responseCount,
			  presentationCtx->abstractSyntax, queryLevelString,
				moveCtx);
	}
	if (cond == SRV_NORMAL) {
	...
	if (moveResponse->dataSetType != DCM_CMDDATANULL) {
	    cond = SRV_SendDataSet(association, presentationCtx,
				   &moveResponse->dataSet, NULL, NULL, 0);
	    if (cond != SRV_NORMAL) {
#if STANDARD_VERSION < VERSION_JUL1993
		clearUIDList(&moveResponse->successUIDList);
		clearUIDList(&moveResponse->failedUIDList);
...
createUIDList(LST_HEAD ** l)
{
    ...
    if ((*l = LST_Create()) == NULL)
	return SRV_LISTCREATEFAILURE;
