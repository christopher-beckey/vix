...
static char rcsid[] = "$Revision: 1 $ $RCSfile: storage.c,v $";
...
static CTNBOOLEAN gStoreAsPart10 = FALSE;
...
static CONDITION
localCallback(unsigned long bytesTransmitted, unsigned long totalBytes,
...
callbackFunction(void* buf, U32 bytesExported, int lastFlag, void* ctx)
{
  ...
  if (write(fd, buf, bytesExported) != (int)bytesExported) {
    return 0;
  }
  return DCM_NORMAL;
}
...
static int
setFileMeta(DCM_FILE_META* fileMeta, MSG_C_STORE_REQ* msg,
	    const char* xferSyntax)
{
...
void
SRV_SetFlagBoolean(const char* flag, CTNBOOLEAN val)
{
  if (strcmp(flag, "STOREASPART10") == 0) {
    gStoreAsPart10 = val;
  }
}
	      ...
	      STORAGE_CTX * storageCtx)
{
	...
	cond = storageCtx->callback(
				    storageCtx->storeRequest,
				    storageCtx->storeResponse,
				    bytesTransmitted,
				    totalBytes,
				    storageCtx->userCtx);
		   ...
		   char *dirName)
{
	...
	* object;
    ...
    if (gStoreAsPart10) {
      storeAsPart10 = 1;
    } else if (strcmp(ctx->acceptedTransferSyntax, DICOM_TRANSFERLITTLEENDIAN) != 0) {
      storeAsPart10 = 1;
    } else {
...
genericImageSize(char *SOPClass)
{
	...
	{DICOM_SOPCLASSENHANCEDCT, 2 * 512 * 512 * 512},
	{DICOM_SOPCLASSMR, 2 * 256 * 256},
	{DICOM_SOPCLASSENHANCEDMR, 2 * 256 * 256 * 512},
	{DICOM_SOPCLASSENCAPSULATEDPDF, 2 * 256 * 256},
	...
	{"1.3.12.2.1107.5.9.1", 2 * 512 * 512 * 32},
	{"1.2.840.10008.5.1.4.1.1.66", 2 * 512 * 512 * 32},	/* Raw data */
	{"1.2.840.10008.5.1.4.1.1.4.2", 2 * 512 * 512 * 32},	/* Spectroscopy data */
	{"1.2.840.10008.5.1.4.1.1.66.1", 5 * 1024 },	/* Spatial registration */
        { "1.2.840.10008.5.1.4.1.1.1.2", 2 * 512 * 512 * 32 },    /* "for presentation" mammo image */
        { "1.2.840.10008.5.1.4.1.1.1.2.1", 2 * 512 * 512 * 32 },   /* "for processing" mammo image */
