...
static char rcsid[] = "$Revision: 1 $ $RCSfile: srv2.c,v $";
		   ...
		   MSG_TYPE * messageType, void **messageArg)
{
    ...
    *ctxID = 0;
    if (PRVSRV_debug)
	fprintf(DEBUG_DEVICE, "Beginning to read a COMMAND PDV\n");
	...
	void *ctx;
	U32 elementLength;
	DCM_ELEMENT element = {DCM_CMDCOMMANDFIELD,
	DCM_US, "", 1, sizeof(dcmCommand), NULL};
	...
	if (command != NULL)
	    *command = dcmCommand;
	if (PRVSRV_debug)
	    fprintf(DEBUG_DEVICE, "Command received: %04x\n", (int) dcmCommand);
	/*
...
#ifdef _WIN32
	  fd1 = _open(scratch, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY,
			_S_IREAD | _S_IWRITE);
#else
