...
** Last Update:		$Author: Vhaiswstarkm $, $Date: 3/28/11 4:43p $
** Source File:		$Source: /home/smm/ctn/ctn/facilities/services/get.c,v $
** Revision:		$Revision: 1 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1 $ $RCSfile: get.c,v $";
...
processGetResponse(
		   ...
		   DUL_PRESENTATIONCONTEXT * presentationCtx, char *dirName,
		 MSG_C_GET_REQ * getRequest, MSG_C_GET_RESP * localResponse,
	 char *queryLevelString, CONDITION(*callback) (), void *callbackCtx,
    CTNBOOLEAN * done, CTNBOOLEAN * cancelled, MSG_C_GET_RESP * getResponse,
...
static CONDITION
sendStoreRequest(DUL_ASSOCIATIONKEY ** association,
...
CONDITION
SRV_CGetRequest(DUL_ASSOCIATIONKEY ** association,
		DUL_ASSOCIATESERVICEPARAMETERS * params,
		MSG_C_GET_REQ * getRequest, MSG_C_GET_RESP * getResponse,
		SRV_C_GET_REQ_CALLBACK * getCallback, void *getCallbackCtx,
		CONDITION(*storageCallback) (), void *storageCallbackCtx,
		char *dirName)
{
    DCM_OBJECT
	...
	* presentationCtx,	/* Presentation context for GET service */
	*storePresentationCtx;	/* for the arriving store request */
	...
	* storeRequest;
    void
       *ctx;
    char
        queryLevelString[48] = "";	/* Initialization for AIX compiler */
    DCM_ELEMENT
	queryLevelElement = {
	DCM_IDQUERYLEVEL,
	    DCM_CS, "", 1, sizeof(queryLevelString), NULL
    };
    unsigned short
        command;
    CTNBOOLEAN
    ...
    MSG_C_CANCEL_REQ
	cancelRequest = {
	MSG_K_C_CANCEL_REQ, 0, 0, DCM_CMDDATANULL
    };

    queryLevelElement.d.string = queryLevelString;

    if ((getCallback == NULL) || (storageCallback == NULL))
	return COND_PushCondition(SRV_NOCALLBACK,
			    SRV_Message(SRV_NOCALLBACK), "SRV_CGetRequest");
    if (getRequest->type != MSG_K_C_GET_REQ)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
		   SRV_Message(SRV_ILLEGALPARAMETER), "type", "GET Request",
				  "SRV_CGetRequest");
    if (getRequest->dataSetType == DCM_CMDDATANULL)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
	    SRV_Message(SRV_ILLEGALPARAMETER), "dataSetType", "GET Request",
				  "SRV_CGetRequest");

    ctx = NULL;
    cond = DCM_GetElementValue(&getRequest->identifier, &queryLevelElement,
			       &l, &ctx);
    if (cond != DCM_NORMAL)
	return COND_PushCondition(SRV_OBJECTACCESSFAILED,
		    SRV_Message(SRV_OBJECTACCESSFAILED), "Query Identifier",
				  "SRV_CGetRequest");
	...
	return COND_PushCondition(SRV_NOSERVICEINASSOCIATION,
	      SRV_Message(SRV_NOSERVICEINASSOCIATION), getRequest->classUID,
				  "SRV_CGetRequest");

    cond = MSG_BuildCommand(getRequest, &commandObject);
    if (cond != MSG_NORMAL) {
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
			  SRV_Message(SRV_OBJECTBUILDFAILED), "GET Request",
				  "SRV_CGetRequest");
    }
	...
	return COND_PushCondition(SRV_REQUESTFAILED,
			 SRV_Message(SRV_REQUESTFAILED), "SRV_CGetRequest");
    }
    cond = SRV_SendDataSet(association, presentationCtx,
			   &getRequest->identifier, NULL, NULL, 0);
    if (cond != SRV_NORMAL)
	return COND_PushCondition(SRV_REQUESTFAILED,
			 SRV_Message(SRV_REQUESTFAILED), "SRV_CGetRequest");

    while (!done) {		/* do until there are no more pending
				 * requests */
	...
	case MSG_K_C_GET_RESP:
	    cond = processGetResponse(association, presentationCtx, dirName,
				      getRequest, (MSG_C_GET_RESP *) message,
		       queryLevelString, getCallback, getCallbackCtx, &done,
				   &cancelled, getResponse, &responseCount);
	    (void) MSG_Free(&message);
	    if ((CTN_FATAL(cond)) || (CTN_ERROR(cond)))
		return cond;
	    break;
	case MSG_K_C_STORE_REQ:
	    storeRequest = (MSG_C_STORE_REQ *) message;
	    storePresentationCtx = SRVPRV_PresentationContext(params,
	    ...
	    cond = processStoreRequest(association, storePresentationCtx,
				       storeRequest, storageCallback,
				       storageCallbackCtx);
	    if ((CTN_FATAL(cond)) || (CTN_ERROR(cond)))
		return cond;
	    break;
...
CONDITION
SRV_CGetResponse(DUL_ASSOCIATIONKEY ** association,
		 DUL_ASSOCIATESERVICEPARAMETERS * params,
		 DUL_PRESENTATIONCONTEXT * getPresentationCtx,
		 MSG_C_GET_REQ ** getRequest, MSG_C_GET_RESP * getResponse,
		 SRV_C_GET_RESP_CALLBACK * getCallback, void *getCtx,
		 char *dirName)
{
    int
        flag,
	...
	rtnCond = SRV_NORMAL;
    DCM_OBJECT
	* responseObject;	/* get response object */
    void
       *ctx;
    DCM_ELEMENT
	queryLevelElement = {
	...
	DCM_QUERYLEVELPATIENT, DCM_QUERYLEVELSTUDY, DCM_QUERYLEVELSERIES,
    DCM_QUERYLEVELIMAGE};
...
a status of pending and data set that is not null.\n";
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			   SRV_Message(SRV_NOCALLBACK), "SRV_CGetResponse");
    }
	...
	return COND_PushCondition(SRV_RESPONSEFAILED,
		       SRV_Message(SRV_RESPONSEFAILED), "SRV_CGetResponse");

    cond = SRV_ReceiveDataSet(association, getPresentationCtx, DUL_BLOCK, 0,
			      dirName, &(*getRequest)->identifier);

    if (PRVSRV_debug && (cond == SRV_NORMAL))
	(void) DCM_DumpElements(&(*getRequest)->identifier, 0);
    ...
    if (cond != DCM_NORMAL) {
	(void) MSG_Free((void **) getRequest);
	(void) COND_PushCondition(SRV_QUERYLEVELATTRIBUTEMISSING,
				SRV_Message(SRV_QUERYLEVELATTRIBUTEMISSING),
				  "SRV_CGetResponse");
	return COND_PushCondition(SRV_RESPONSEFAILED,
		       SRV_Message(SRV_RESPONSEFAILED), "SRV_CGetResponse");
    }
	...
	if (strcmp(queryLevelString, allowedQueryLevels[l]) == 0)
	    flag = 1;

    if (!flag) {
	(void) MSG_Free((void **) getRequest);
	(void) COND_PushCondition(SRV_ILLEGALQUERYLEVELATTRIBUTE,
				SRV_Message(SRV_ILLEGALQUERYLEVELATTRIBUTE),
				  queryLevelString, "SRV_CGetResponse");
	return COND_PushCondition(SRV_RESPONSEFAILED,
		       SRV_Message(SRV_RESPONSEFAILED), "SRV_CGetResponse");
    }
     ...
     */
    cond = getCallback(*getRequest, getResponse, &storeRequest, NULL, 0,
	      getPresentationCtx->abstractSyntax, queryLevelString, getCtx);
    if (cond != SRV_NORMAL) {
	return COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
		SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_CGetRespose");
    }
	...
	if (cond != SRV_NORMAL) {
	    return COND_PushCondition(SRV_SENDFAILED, SRV_Message(SRV_SENDFAILED)
				   ,"sendStoreRequest", "SRV_CGetResponse");
	}
    } else {
	/*
	...
	if (cond != MSG_NORMAL) {
	    return COND_PushCondition(SRV_OBJECTBUILDFAILED, SRV_Message
		(SRV_OBJECTBUILDFAILED), "getResponse", "SRV_CGetResponse");
	}
	cond = SRV_SendCommand(association, getPresentationCtx, &responseObject);
	if (cond != SRV_NORMAL) {
	    return COND_PushCondition(SRV_SENDFAILED, SRV_Message(SRV_SENDFAILED)
				    ,"SRV_SendCommand", "SRV_CGetResponse");
	}
	if (getResponse->dataSetType != DCM_CMDDATANULL) {
	    cond = SRV_SendDataSet(association, getPresentationCtx,
				   &getResponse->identifier, NULL, NULL, 0);
	    if (cond != SRV_NORMAL) {
		return COND_PushCondition(SRV_SENDFAILED, SRV_Message
					(SRV_SENDFAILED), "SRV_SendDataSet",
					  "SRV_CGetResponse");
	    }
	}
	return SRV_NORMAL;
	 ...
	 */
	cond = SRV_ReceiveCommand(association, params, DUL_BLOCK, 0,
			    &ctxID, NULL, &messageType, (void **) &message);
	if (cond != SRV_NORMAL) {
	    return COND_PushCondition(SRV_RECEIVEFAILED, SRV_Message
				   (SRV_RECEIVEFAILED), "SRV_CGetResponse");
	}
	switch (messageType) {
	case MSG_K_C_CANCEL_REQ:
	    ...
	    (void) MSG_Free(&message);
	    cond = getCallback(*getRequest, getResponse, NULL, NULL,
			  responseCount, getPresentationCtx->abstractSyntax,
			       queryLevelString, getCtx);
	    if (cond != SRV_NORMAL) {
		return COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
		SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_CGetRespose");
	    }
	...
	case MSG_K_C_STORE_RESP:
	    ...
	    storeRequest.priority = 0;
	    storeRequest.moveMessageID = (*getRequest)->messageID;
	    cond = getCallback(*getRequest, getResponse, &storeRequest,
			       (MSG_C_STORE_RESP *) message, responseCount,
		       getPresentationCtx->abstractSyntax, queryLevelString,
			       getCtx);
	    if (cond != SRV_NORMAL) {
		return COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
		SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_CGetRespose");
		...
		if (cond != SRV_NORMAL) {
		    return COND_PushCondition(SRV_SENDFAILED,
			    SRV_Message(SRV_SENDFAILED), "sendStoreRequest",
					      "SRV_CGetResponse");
		}
	    }
	    break;
	...
	if (cond == SRV_NORMAL) {
	    cond = MSG_StatusLookup(getResponse->status, MSG_K_C_GET_RESP,
				    &statusDescription);
	    if (cond != MSG_NORMAL)
		return COND_PushCondition(SRV_RESPONSEFAILED,
		       SRV_Message(SRV_RESPONSEFAILED), "SRV_CGetResponse");
		   ...
		   unsigned long *responseCount)
{
	...
	* commandObject;
    ...
    cond = MSG_StatusLookup(localResponse->status, MSG_K_C_GET_RESP,
			    &statusDescription);
    if (cond != MSG_NORMAL)
	return COND_PushCondition(SRV_REQUESTFAILED,
			 SRV_Message(SRV_REQUESTFAILED), "SRV_CGetRequest");
    ...
    if (statusDescription.statusClass != MSG_K_CLASS_PENDING)
	*done = TRUE;		/* No more pending requests */

    if (localResponse->dataSetType != DCM_CMDDATANULL) {
	/* Some data set (identifier) has been sent back */
	cond = SRV_ReceiveDataSet(association, presentationCtx,
			 DUL_BLOCK, 0, dirName, &localResponse->identifier);
	if (cond != SRV_NORMAL) {
	    return COND_PushCondition(SRV_REQUESTFAILED,
			 SRV_Message(SRV_REQUESTFAILED), "SRV_CGetRequest");
	}
    ...
    *responseCount++;
	...
	cond = callback(getRequest, localResponse, *responseCount,
			presentationCtx->abstractSyntax, queryLevelString,
			callbackCtx);
	if (cond == SRV_OPERATIONCANCELLED) {
	    *cancelled = TRUE;
	    ...
	    if (cond != MSG_NORMAL)
		return COND_PushCondition(SRV_OBJECTBUILDFAILED,
		       SRV_Message(SRV_OBJECTBUILDFAILED), "CANCEL Request",
					  "SRV_CGetRequest");
	    cond = SRV_SendCommand(association, presentationCtx, &commandObject);
	    (void) DCM_CloseObject(&commandObject);
	    if (cond != SRV_NORMAL)
	    ...
	    *done = TRUE;	/* callback did not return SRV_NORMAL. Hence
				 * we opt to quit the loop */
    }
    if (getResponse != NULL) {
		    ...
		    CONDITION(*callback) (), void *callbackCtx)
{
    ...
    int
        fd;			/* file descriptor */
    CTNBOOLEAN
	first = TRUE,		/* to tell the callback function if it was
				 * invoked the first time or not */
	done = FALSE;
    unsigned long
        total,
        estimatedSize,
	...
	* object;
    char
        fileName[1024];		/* name of image file */
    MSG_C_STORE_RESP
	storeResponse;
    ...
    storeResponse.status = MSG_K_SUCCESS;	/* we initialize it to
						 * success but the callback
						 * can change it */

    /* invoke the call back for the first time and get the file name */
    total = 0;
    estimatedSize = genericImageSize(storeRequest->classUID);
    cond = callback(storeRequest, &storeResponse, first, fileName, total,
		    estimatedSize, NULL, callbackCtx);
    first = FALSE;
    if (cond != SRV_NORMAL) {
	return COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
	    SRV_Message(SRV_CALLBACKABORTEDSERVICE), "processStoreRequest");
    }
#ifdef _MSC_VER
...
#endif
    if (fd < 0) {
	return COND_PushCondition(SRV_FILECREATEFAILED,
	...
	if (cond != SRV_NORMAL) {
	    return COND_PushCondition(SRV_RESPONSEFAILED,
			SRV_Message(SRV_RESPONSEFAILED), "SRV_CGetRequest");
	}
	...
	if (total > nextCallback) {
	    cond = callback(storeRequest, &storeResponse, first, NULL, total,
			    estimatedSize, NULL, callbackCtx);
	    if (cond != SRV_NORMAL) {
		return COND_PushCondition(SRV_RESPONSEFAILED,
			SRV_Message(SRV_RESPONSEFAILED), "SRV_CGetRequest");
    ...
    cond = callback(storeRequest, &storeResponse, first, NULL, total,
		    estimatedSize, &object, callbackCtx);

    if (cond != SRV_NORMAL) {
	(void) DCM_CloseObject(&object);
	return COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
		SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_CGetRequest");
    }
    (void) DCM_CloseObject(&object);
    ...
    if (cond != MSG_NORMAL)
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
			SRV_Message(SRV_OBJECTBUILDFAILED), "storeResponse",
				  "processStoreRequest");
...
genericImageSize(char *SOPClass)
{
	...
	{DICOM_SOPCLASSENHANCEDCT, 2 * 512 * 512 * 32},
	{DICOM_SOPCLASSMR, 2 * 256 * 256},
	{DICOM_SOPCLASSENHANCEDMR, 2 * 256 * 256 * 32},
	...
	{"1.3.12.2.1107.5.9.1", 2 * 1024 * 1024 * 32},
	{"1.2.840.10008.5.1.4.1.1.66", 2 * 1024 * 1024 * 32},	/* Raw Data */
	{"1.2.840.10008.5.1.4.1.1.4.2", 2 * 1024 * 1024 * 32},	/* Spectroscopy Data */
    };
    int
        index;

    for (index = 0; index < (int) DIM_OF(size_map); index++)
	if (strcmp(SOPClass, size_map[index].classUID) == 0)
	    return size_map[index].imageSize;
...
static CONDITION
sendStoreRequest(DUL_ASSOCIATIONKEY ** association,
    DUL_ASSOCIATESERVICEPARAMETERS * params, MSG_C_STORE_REQ * storeRequest)
{
    DUL_PRESENTATIONCONTEXT
    * storePresentationCtx;
    DCM_OBJECT
	* storeRequestObject;
    CONDITION
	cond;
    ...
    storePresentationCtx = SRVPRV_PresentationContext(params,
						    storeRequest->classUID);
    if (!storePresentationCtx) {
	return COND_PushCondition(SRV_PRESENTATIONCONTEXTERROR,
	     SRV_Message(SRV_PRESENTATIONCONTEXTERROR), "sendStoreRequest");
    }
    /* build a StoreRequest command object */
    cond = MSG_BuildCommand(storeRequest, &storeRequestObject);
    if (cond != MSG_NORMAL) {
	return COND_PushCondition(SRV_OBJECTBUILDFAILED, SRV_Message
				(SRV_OBJECTBUILDFAILED), "MSG_BuildCommand",
				  "sendStoreRequest");
    }
    /* Now send the store request on the same association */
    cond = SRV_SendCommand(association, storePresentationCtx,
			   &storeRequestObject);
    if (cond != SRV_NORMAL) {
	return COND_PushCondition(SRV_SENDFAILED, SRV_Message(SRV_SENDFAILED),
				  "SRV_SendCommand", "sendStoreRequest");
    }
    /* Now send the data set */
    cond = SRV_SendDataSet(association, storePresentationCtx,
			   &storeRequest->dataSet, NULL, NULL, 0);
    if (cond != SRV_NORMAL) {
	return COND_PushCondition(SRV_SENDFAILED, SRV_Message(SRV_SENDFAILED),
				  "SRV_SendDataSet", "sendStoreRequest");
    }
    return SRV_NORMAL;
}
