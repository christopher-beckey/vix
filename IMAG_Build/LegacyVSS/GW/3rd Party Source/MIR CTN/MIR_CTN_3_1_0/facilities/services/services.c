...
static char rcsid[] = "$Revision: 1 $ $RCSfile: services.c,v $";
...
BOOLEAN PRVSRV_debug = 0;
static CONDITION
writeCallback(void *buffer, unsigned long length,
	      int last, CALLBACK_STRUCTURE * callbackStructure);
static CONDITION
    createFile(char *dirName, char *qualifiedFile, int *fd);
static CONDITION
verifyCommandValidity(DUL_ASSOCIATESERVICEPARAMETERS * params,
		      DUL_PRESENTATIONCONTEXTID ctxid,
		      unsigned short command);
static CONDITION
dequeCommand(DUL_ASSOCIATIONKEY ** association,
	     DUL_PRESENTATIONCONTEXTID ctxID, MSG_TYPE messageType,
	     COMMAND_ENTRY * commandEntry);
static CONDITION enqueCommand(COMMAND_ENTRY * commandEntry);
	...
	* ctx;
	...
	if (ctx == NULL)
	    return COND_PushCondition(SRV_MALLOCFAILURE,
			       SRV_Message(SRV_MALLOCFAILURE), sizeof(*ctx),
				      "SRV_RequestServiceClass");
	...
	return COND_PushCondition(SRV_UNSUPPORTEDSERVICE,
				  SRV_Message(UPSUPPORTEDSERVICE), SOPClass,
				  "SRV_RequestServiceClass");
...
static SOPCLASSPERMITTEDSRV classPermittedSrvTable[] = {
    ...
    {DICOM_SOPCLASSDIGXRAYPRESENTATION, digXRayPresentationSOPClass,
    DIM_OF(disXRayPresentationSOPClass)},
    ...
    {DICOM_SOPCLASSENCAPSULATEDPDF, mrImageStorageSOPClass, DIM_OF(mrImageStorageSOPClass)}, /* Use the MR storage table; the commands are the same */
		   ...
		   MSG_TYPE * messageType, void **messageArg)
{
    ...
    *ctxID = 0;
    if (PRVSRV_debug)
	fprintf(stderr, "Beginning to read a COMMAND PDV\n");
    for (s = stream, last = FALSE, bytesRead = 0, type = DUL_COMMANDPDV;
	 type == DUL_COMMANDPDV && !last;) {
	cond = SRVPRV_ReadNextPDV(association, block, timeout, &pdv);
	if (cond != SRV_NORMAL) {
	    if (cond == SRV_READPDVFAILED)
		return COND_PushCondition(SRV_RECEIVEFAILED,
		      SRV_Message(SRV_RECEIVEFAILED), "SRV_ReceiveCommand");
	    else
		return cond;
	}
	if (bytesRead + pdv.fragmentLength > sizeof(stream)) {
	    (void) COND_PushCondition(SRV_OBJECTTOOLARGE,
				      SRV_Message(SRV_OBJECTTOOLARGE));
	    return COND_PushCondition(SRV_RECEIVEFAILED,
		      SRV_Message(SRV_RECEIVEFAILED), "SRV_ReceiveCommand");
	}
	if (*ctxID == 0)
	    *ctxID = pdv.presentationContextID;

	if (pdv.presentationContextID != *ctxID) {
	    (void) COND_PushCondition(SRV_UNEXPECTEDPRESENTATIONCONTEXTID,
			   SRV_Message(SRV_UNEXPECTEDPRESENTATIONCONTEXTID),
				      (long) pdv.presentationContextID);
	    return COND_PushCondition(SRV_RECEIVEFAILED,
		      SRV_Message(SRV_RECEIVEFAILED), "SRV_ReceiveCommand");
	}
	if (pdv.pdvType != DUL_COMMANDPDV) {
	    (void) COND_PushCondition(SRV_PDVRECEIVEDOUTOFSEQUENCE,
				  SRV_Message(SRV_PDVRECEIVEDOUTOFSEQUENCE),
			       "Command", "Data", pdv.presentationContextID,
				      "SRV_ReceiveCommand");
	    return COND_PushCondition(SRV_RECEIVEFAILED,
		      SRV_Message(SRV_RECEIVEFAILED), "SRV_ReceiveCommand");
	}
	...
	(void) COND_PushCondition(SRV_UNEXPECTEDPDVTYPE,
				  SRV_Message(SRV_UNEXPECTEDPDVTYPE));
	return COND_PushCondition(SRV_RECEIVEFAILED,
		      SRV_Message(SRV_RECEIVEFAILED), "SRV_ReceiveCommand");
    }
    /* repair */
	...
	void *ctx;
	U32 elementLength;
	static DCM_ELEMENT element = {DCM_CMDCOMMANDFIELD,
	DCM_US, "", 1, sizeof(dcmCommand), (void *) &dcmCommand};
	...
	if (command != NULL)
	    *command = dcmCommand;
	if (PRVSRV_debug)
	    fprintf(stderr, "Command received: %04x\n", (int) dcmCommand);
	/*
	 * we verify if the command is applicable for the SOP class that is
	 * negotiated for the association
	 */
	cond = verifyCommandValidity(params, *ctxID, dcmCommand);
	if (cond != SRV_NORMAL) {
	    return COND_PushCondition(SRV_UNSUPPORTEDCOMMAND,
		      SRV_Message(SRV_UNSUPPORTEDCOMMAND), (int) dcmCommand,
				      "SRV_ReceiveCommand");
	...
	case DCM_N_DELETE_RESPONSE:
	    *msg = NULL;
	    ...
	    (void) DCM_CloseObject(&object);
	    if (cond != MSG_NORMAL)
		return COND_PushCondition(SRV_PARSEFAILED,
			SRV_Message(SRV_PARSEFAILED), "SRV_ReceiveCommand");
		...
		unsigned long length)
{
    ...
    if (cond != DCM_EXPORTCOMMANDEND && cond != DCM_NORMAL) {
		   ...
		   DCM_OBJECT ** dataSet)
{
	...
	* object;
	...
	ctxID = 0;
    int
        fd = 0;			/* File descriptor if we need to create file */
    char
        qualifiedFile[1024];

    /*
     * First we deal with the various values possible for the argument
     * dirName
     */
	...
	if (dirName == NULL)
	    return COND_PushCondition(SRV_MALLOCFAILURE,
			   SRV_Message(SRV_MALLOCFAILURE), sizeof(*dirName),
				      "SRV_ReceiveDataSet");
	(void) strcpy(dirName, ".");	/* make the directory name as current
					 * working directory */
    }
    for (s = stream, last = FALSE, bytesRead = 0, type = DUL_DATASETPDV;
	 type == DUL_DATASETPDV && !last;) {
	cond = SRVPRV_ReadNextPDV(association, block, timeout, &pdv);
	if (cond != SRV_NORMAL) {
	    if (cond == SRV_READPDVFAILED)
		return COND_PushCondition(SRV_RECEIVEFAILED,
					  SRV_Message(SRV_RECEIVEFAILED),
					  "SRV_ReceiveDataSet");
	    else
		return cond;
	}
	if (bytesRead + pdv.fragmentLength > sizeof(stream)) {
	    ...
	    if (write(fd, stream, bytesRead) != bytesRead) {
		return COND_PushCondition(SRV_RECEIVEFAILED,
					  SRV_Message(SRV_RECEIVEFAILED),
					  "SRV_ReceiveDataSet");
	    }
	...
	(void) close(fd);
	cond = DCM_OpenFile(qualifiedFile, DCM_ORDERLITTLEENDIAN |
			 DCM_FORMATCONVERSION | DCM_DELETEONCLOSE, &object);
    } else {
	/* repair */
	cond = DCM_ImportStream(stream, bytesRead,
		     DCM_ORDERLITTLEENDIAN | DCM_FORMATCONVERSION, &object);
    }
    if (cond != DCM_NORMAL)
	return COND_PushCondition(SRV_RECEIVEFAILED,
				  SRV_Message(SRV_RECEIVEFAILED),
		  ...
		  MSG_TYPE * messageType, void **messageArg)
{
	...
	*messageArg = commandEntry.message;
	...
	if (localCtxID == 0)
	    localCtxID = pdv.presentationContextID;
		   ...
		   DUL_PDV * pdv)
{
	...
	if (cond != DUL_PDATAPDUARRIVED) {
		...
		return COND_PushCondition(SRV_ILLEGALASSOCIATION,
				       SRV_Message(SRV_ILLEGALASSOCIATION));
	    else if (cond == DUL_PEERREQUESTEDRELEASE)
		return COND_PushCondition(SRV_PEERREQUESTEDRELEASE,
				      SRV_Message(SRV_PEERREQUESTEDRELEASE),
					  "SRVPRV_ReadNextPDV");
	    else if (cond == DUL_PEERABORTEDASSOCIATION)
		return COND_PushCondition(SRV_PEERABORTEDASSOCIATION,
				    SRV_Message(SRV_PEERABORTEDASSOCIATION),
					  "SRVPRV_ReadNextPDV");
	    else if (cond == DUL_READTIMEOUT)
		return SRV_READTIMEOUT;
	    else
		return COND_PushCondition(SRV_READPDVFAILED,
					  SRV_Message(SRV_READPDVFAILED),
					  "SRVPRV_ReadNextPDV");
	}
	cond = DUL_NextPDV(association, pdv);
	if (cond != DUL_NORMAL)	/* repair */
	    return COND_PushCondition(SRV_READPDVFAILED,
				      SRV_Message(SRV_READPDVFAILED));
    }
    return SRV_NORMAL;
...
{
    static int
        fileIndex = 1;		/* For creating unique file names */
    int
        pid;
    ...
    *fd = open(qualifiedFile, O_CREAT | O_WRONLY | O_TRUNC, 0666);
    if (*fd < 0) {
	(void) COND_PushCondition(SRV_SYSTEMERROR,
			      SRV_Message(SRV_SYSTEMERROR), strerror(errno),
				  "createFile");
	return COND_PushCondition(SRV_FILECREATEFAILED,
			   SRV_Message(SRV_FILECREATEFAILED), qualifiedFile,
				  "createFile");
    } else
	return SRV_NORMAL;
}
	     ...
	     COMMAND_ENTRY * commandEntry)
{
    ...
    * c;
    ...
    while ((c != NULL) && !match) {
	match = TRUE;
	if (c->association != association)
	    match = FALSE;
	if ((ctxID != 0) && (c->ctxID != ctxID))
	    match = FALSE;
	if ((messageType != MSG_K_NONE) && (c->messageType != messageType))
	    match = FALSE;
	if (!match)
	    c = LST_Next(&commandList);
    }
    if (match) {
	*commandEntry = *c;
...
enqueCommand(COMMAND_ENTRY * commandEntry)
{
    ...
    * c, *tail;
    ...
    if (commandList == NULL)
	commandList = LST_Create();

    if (commandList == NULL)
	return COND_PushCondition(SRV_LISTFAILURE,
			      SRV_Message(SRV_LISTFAILURE), "enqueCommand");

    c = malloc(sizeof(*c));
    if (c == NULL)
	return COND_PushCondition(SRV_MALLOCFAILURE,
		SRV_Message(SRV_MALLOCFAILURE), sizeof(*c), "enqueCommand");

    *c = *commandEntry;

    tail = LST_Tail(&commandList);
    (void) LST_Position(&commandList, tail);
    if (LST_Insert(&commandList, c, LST_K_AFTER) != LST_NORMAL)
	return COND_PushCondition(SRV_LISTFAILURE,
			      SRV_Message(SRV_LISTFAILURE), "enqueCommand");
