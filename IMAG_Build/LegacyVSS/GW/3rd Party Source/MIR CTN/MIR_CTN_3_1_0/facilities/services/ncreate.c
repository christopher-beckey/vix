...
static char rcsid[] = "$Revision: 1 $ $RCSfile: ncreate.c,v $";
		   ...
		   char *dirName)
{
	...
	* localResponse;
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			 SRV_Message(SRV_NOCALLBACK), "SRV_NCreateRequest");
	...
	return COND_PushCondition(SRV_UNSUPPORTEDSERVICE,
			     ...
			     createRequest->classUID, "SRV_NCreateRequest");

    if (createRequest->type != MSG_K_N_CREATE_REQ)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
	      SRV_Message(SRV_ILLEGALPARAMETER), "type", "N-CREATE Request",
				  "SRV_NCreateRequest");

    cond = MSG_BuildCommand(createRequest, &commandObject);
    if (cond != MSG_NORMAL)
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
		     SRV_Message(SRV_OBJECTBUILDFAILED), "N-CREATE Request",
				  "SRV_NCreateRequest");
	...
	return COND_PushCondition(SRV_REQUESTFAILED,
		      SRV_Message(SRV_REQUESTFAILED), "SRV_NCreateRequest");

    if (createRequest->dataSetType == DCM_CMDDATANULL)
	/* sending attribute list is mandatory */
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
	SRV_Message(SRV_ILLEGALPARAMETER), "dataSetType", "N-CREATE Request",
				  "SRV_NCreateRequest");

    cond = SRV_SendDataSet(association, presentationCtx,
			   &createRequest->dataSet, NULL, NULL, 0);
    if (cond != SRV_NORMAL)
	return COND_PushCondition(SRV_REQUESTFAILED,
		      SRV_Message(SRV_REQUESTFAILED), "SRV_NCreateRequest");
	...
	*createResponse = *localResponse;
    (void) MSG_Free(&message);
    if (cond != SRV_NORMAL)
	return COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
	     SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_NCreateRequest");
...
SRV_NCreateResponse(DUL_ASSOCIATIONKEY ** association,
		    ...
		    MSG_N_CREATE_REQ ** createRequest,
		    MSG_N_CREATE_RESP * createResponse,
		    SRV_N_CREATE_RESP_CALLBACK * createCallback,
		    void *createCtx, char *dirName)
{
    CONDITION
	rtnCond = SRV_NORMAL,
	cond;
	...
	* responseObject = NULL;
	...
	rtnCond = COND_PushCondition(SRV_NOCALLBACK,
			SRV_Message(SRV_NOCALLBACK), "SRV_NCreateResponse");
	...
	cond = SRV_ReceiveDataSet(association, presentationCtx,
			 DUL_BLOCK, 0, dirName, &(*createRequest)->dataSet);
	if (cond != SRV_NORMAL) {
	...
	rtnCond = COND_PushCondition(SRV_OBJECTBUILDFAILED,
		 SRV_Message(SRV_OBJECTBUILDFAILED), "SRV_NCreateResponse");
	createResponse->status = MSG_K_PROCESSINGFAILURE;
	goto SEND_RESPONSE;
    }
    cond = createCallback(*createRequest, createResponse, createCtx,
			  presentationCtx);
    if (cond != SRV_NORMAL) {
	...
	rtnCond = COND_PushCondition(SRV_ILLEGALPARAMETER,
	     SRV_Message(SRV_ILLEGALPARAMETER), "type", "N-CREATE Response",
				     "SRV_NCreateResponse");
	goto SEND_RESPONSE;
    }
SEND_RESPONSE:
/*  Assume the callback function has set the status value.  If return
**  value is not normal, we will set a status value of processing failure
*/
	...
	if (createResponse->dataSet)
	    (void) DCM_CloseObject(&createResponse->dataSet);
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
		    SRV_Message(SRV_OBJECTBUILDFAILED), "N-CREATE Response",
				  "SRV_NCreateResponse");
    }
	...
	cond = SRV_SendDataSet(association, presentationCtx,
			       &createResponse->dataSet, NULL, NULL, 0);
	(void) DCM_CloseObject(&createResponse->dataSet);
	if (cond != SRV_NORMAL)
	    return COND_PushCondition(SRV_RESPONSEFAILED,
		    SRV_Message(SRV_RESPONSEFAILED), "SRV_NCreateResponse");
