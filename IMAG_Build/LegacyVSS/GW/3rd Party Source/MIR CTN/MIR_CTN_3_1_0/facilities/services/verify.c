...
static char rcsid[] = "$Revision: 1 $ $RCSfile: verify.c,v $";
...
{
    DCM_OBJECT
	* object = NULL;	/* Handle for an encoded object */
    CONDITION
	...
	* response;
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			   SRV_Message(SRV_NOCALLBACK), "SRV_CEchoRequest");
	...
	return COND_PushCondition(SRV_REQUESTFAILED,
			SRV_Message(SRV_REQUESTFAILED), "SRV_CEchoRequest");
	...
	*echoResponse = *response;
	...
	return COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
	       SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_CEchoRequest");
...
SRV_CEchoResponse(DUL_ASSOCIATIONKEY ** association,
		  ...
		  MSG_C_ECHO_RESP * echoResponse,
		  SRV_C_ECHO_RESP_CALLBACK* callback, void *ctx, char *dirName)
{
    DCM_OBJECT
	* object = NULL;	/* Handle for an encoded object */
    CONDITION
    ...
    if (callback == NULL)
	return COND_PushCondition(SRV_NOCALLBACK,
			  SRV_Message(SRV_NOCALLBACK), "SRV_CEchoResponse");
    if (echoResponse->type != MSG_K_C_ECHO_RESP)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
		 SRV_Message(SRV_ILLEGALPARAMETER), "type", "ECHO Response",
				  "SRV_CEchoResponse");
	...
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
	  SRV_Message(SRV_ILLEGALPARAMETER), "dataSetType", "ECHO Response",
				  "SRV_CEchoResponse");
    ...
    if (cond != MSG_NORMAL)
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
			SRV_Message(SRV_OBJECTBUILDFAILED), "ECHO Response",
				  "SRV_CEchoResponse");
	...
	return COND_PushCondition(SRV_RESPONSEFAILED,
		      SRV_Message(SRV_RESPONSEFAILED), "SRV_CEchoResponse");
