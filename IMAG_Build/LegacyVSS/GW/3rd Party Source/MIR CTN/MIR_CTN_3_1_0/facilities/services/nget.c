...
static char rcsid[] = "$Revision: 1 $ $RCSfile: nget.c,v $";
	  ...
	  SRV_N_GET_REQ_CALLBACK * getCallback, void *getCtx, char *dirName)
{
	...
	* localResponse;
	...
	return COND_PushCondition(SRV_NOCALLBACK,
			    SRV_Message(SRV_NOCALLBACK), "SRV_NGetRequest");
	...
	return COND_PushCondition(SRV_UNSUPPORTEDSERVICE,
				  ...
				  getRequest->classUID, "SRV_NGetRequest");

    if (getRequest->type != MSG_K_N_GET_REQ)
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
		 SRV_Message(SRV_ILLEGALPARAMETER), "type", "N-GET Request",
				  "SRV_NGetRequest");

    cond = MSG_BuildCommand(getRequest, &commandObject);
    if (cond != MSG_NORMAL)
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
			SRV_Message(SRV_OBJECTBUILDFAILED), "N-GET Request",
				  "SRV_NGetRequest");
	...
	return COND_PushCondition(SRV_REQUESTFAILED,
			 SRV_Message(SRV_REQUESTFAILED), "SRV_NGetRequest");
    ...
    if (messageType != MSG_K_N_GET_RESP)
	...
	/* attribute list must be present in the response */
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
	 SRV_Message(SRV_ILLEGALPARAMETER), "dataSetType", "N-GET Response",
				  "SRV_NGetRequest");
    ...
    if (cond != SRV_NORMAL)
	(void) COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
		SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_NGetRequest");
...
SRV_NGetResponse(DUL_ASSOCIATIONKEY ** association,
		 ...
		 MSG_N_GET_REQ ** getRequest, MSG_N_GET_RESP * getResponse,
		 SRV_N_GET_RESP_CALLBACK * getCallback, void *getCtx,
		 char *dirName)
{
    CONDITION
	rtnCond = SRV_NORMAL,
	...
	* responseObject = NULL;
	...
	rtnCond = COND_PushCondition(SRV_NOCALLBACK,
			   SRV_Message(SRV_NOCALLBACK), "SRV_NGetResponse");
	...
	rtnCond = COND_PushCondition(SRV_OBJECTBUILDFAILED,
				     SRV_Message(SRV_OBJECTBUILDFAILED),
				     "NGet Response Data Set",
				     "SRV_NGetResponse");
	getResponse->status = MSG_K_PROCESSINGFAILURE;
	goto SEND_RESPONSE;
    }
    cond = getCallback(*getRequest, getResponse, getCtx,
		       presentationCtx);
    if (cond != SRV_NORMAL) {
	rtnCond = COND_PushCondition(SRV_CALLBACKABORTEDSERVICE,
	       SRV_Message(SRV_CALLBACKABORTEDSERVICE), "SRV_NGetResponse");
	goto SEND_RESPONSE;
    }
    if (getResponse->type != MSG_K_N_GET_RESP) {
	rtnCond = COND_PushCondition(SRV_ILLEGALPARAMETER,
		SRV_Message(SRV_ILLEGALPARAMETER), "type", "N-GET Response",
				     "SRV_NGetResponse");
	goto SEND_RESPONSE;
    }
SEND_RESPONSE:
	...
	if (getResponse->dataSet)
	    (void) DCM_CloseObject(&getResponse->dataSet);
	return COND_PushCondition(SRV_OBJECTBUILDFAILED,
		       SRV_Message(SRV_OBJECTBUILDFAILED), "N-GET Response",
				  "SRV_NGetResponse");
    }
	...
	return COND_PushCondition(SRV_ILLEGALPARAMETER,
			   SRV_Message(SRV_ILLEGALPARAMETER), "dataSetType",
				  "N-GET Response", "SRV_NGetResponse");
    }
    cond = SRV_SendDataSet(association, presentationCtx,
			   &getResponse->dataSet, NULL, NULL, 0);
    (void) DCM_CloseObject(&getResponse->dataSet);
    if (cond != SRV_NORMAL)
	return COND_PushCondition(SRV_SENDFAILED,
				  SRV_Message(SRV_SENDFAILED), "DataSet",
				  "SRV_NGetResponse");
