...
static char rcsid[] = "$Revision: 1 $ $RCSfile: dcm.c,v $";
...
static int fileSizeWithStatus(int fd, U32* size);
#endif
...
/*    size = fileSize(fd);*/
    size = 0;
    status = fileSizeWithStatus(fd, &size);
    if (status != 0) {
      perror("");
		    ...
		    void *ctx)
{
	...
	* elementItem;
	...
	while (elementItem != NULL && !done) {
	    ...
	    if (!found) {
                // Save the current element and group, in case changed in callback
                LST_NODE* currentElement = LST_Current(&groupItem->elementList);
                LST_NODE* currentGroup = LST_Current(&((*object)->groupList));
		e = elementItem->element;
		cond = callback(&e, ctx);
		if (cond != DCM_NORMAL)
		    done = TRUE;
                // restore the saved element and group positions
                (void) LST_Position(&groupItem->elementList, currentElement);
                (void) LST_Position(&((*object)->groupList), currentGroup);
	    }
	    elementItem = LST_Next(&groupItem->elementList);
	}
	groupItem = LST_Next(&((*object)->groupList));
...
**
** Purpose:
**	Remove an entire group with the given group number from the object
**
** Parameter Dictionary:
...
**
** Purpose:
**	Remove all odd groups in the object.
**
** Parameter Dictionary:
**	callerObject		Handle to caller's object
**
...
CONDITION
DCM_StripOddGroups(DCM_OBJECT ** callerObject)
{
    PRIVATE_OBJECT
	** object;
    CONDITION
	...
	* elementItem;
    ...
    object = (PRIVATE_OBJECT **) callerObject;
    cond = checkObject(object, "DCM_StripOddGroups");
    if (cond != DCM_NORMAL)
	return cond;
    ...
    while (groupItem != NULL) {
      ...
      if ((group & 0x1) == 0x1) {	/* Then, this is an odd group */
	while ((elementItem = LST_Pop(&groupItem->elementList)) != NULL) {
	  CTN_FREE(elementItem);
	}
	groupItem = LST_Remove(&(*object)->groupList, LST_K_AFTER);
	...
	if (cond != LST_NORMAL)
	  return COND_PushCondition(DCM_LISTFAILURE,
			   DCM_Message(DCM_LISTFAILURE), "DCM_StripOddGroups");
	CTN_FREE(groupItem);
	/* The current node is now ready for processing; */
...
CONDITION
DCM_GetSequenceObject(DCM_OBJECT ** object, DCM_TAG seqTag, DCM_OBJECT** rtnObject)
{
  LST_HEAD* h = 0;
  CONDITION cond;
		  ...
		  size_t structureSize, long stringOffset, LST_HEAD ** list)
{
	...
	* elementItem;
    ...
    obj = (PRIVATE_OBJECT **) object;
    cond = checkObject(obj, "DCM_GetElementValueList");
    if (cond != DCM_NORMAL)
	return cond;
...
DCM_GetElement(DCM_OBJECT ** callerObject, DCM_TAG tag, DCM_ELEMENT * element)
{
	...
	* elementItem;
    ...
    cond = checkObject(obj, "DCM_GetElement");
    if (cond != DCM_NORMAL)
	return cond;
	       ...
	       U32 * rtnLength)
{
    ...
    *rtnLength += 2;
    ...
    if (vr == DCM_OB || vr == DCM_OW || vr == DCM_SQ || vr == DCM_UN || vr==DCM_UT) {
	*b++ = 0x00;
	*b++ = 0x00;
...
static int
fileSizeWithStatus(int fd, U32* size)
{
  int
    status;
  struct stat
