...
static char rcsid[] = "$Revision: 1 $ $RCSfile: ufs.c,v $";
...
static CONDITION
localSelect(TBL_PRV_HANDLE * handle, const TBL_CRITERIA * criteria,
       TBL_FIELD * fields, long *count, CONDITION(*callback) (), void *ctx);
...
TBL_Open(const char *databaseName, const char *tableName, TBL_HANDLE ** handle)
{
    ...
    *   masterFileHandle = NULL;
	...
	if (cond == TBL_NORMAL)
	    *handle = (TBL_HANDLE *) privateHandle;
    }
    return cond;
}
...
TBL_Select(TBL_HANDLE ** handle, const TBL_CRITERIA * criteriaList,
	   TBL_FIELD * fieldList, long *count, CONDITION(*callback) (),
	   void *ctx)
{
    TBL_PRV_HANDLE
	** privateHandle;
    ...
    cond = localSelect(*privateHandle, criteriaList, fieldList, count,
		       callback, ctx);
...
localOpen(char *fileName, TBL_PRV_HANDLE * handle)
{
	...
	* field;
    ...
    if (handle->file == NULL)
	return COND_PushCondition(TBL_ERROR(TBL_FILEOPENFAILED), "localOpen",
				  fileName);

    while (fgets(line, sizeof(line), handle->file) != NULL) {
	if (sscanf(line, "%s", scratch) == 1) {
	    if (strcmp(scratch, "DATA") == 0)
		break;
	    if (strcmp(scratch, "FIELDNAME") == 0) {
		if ((field = malloc(sizeof(*field))) == NULL)
		    return COND_PushCondition(TBL_ERROR(TBL_MALLOCFAILURE),
					      "localOpen");
		if (sscanf(line, "%s %s", scratch, field->fieldName) != 2)
		    return COND_PushCondition(TBL_ERROR(TBL_ILLEGALFORMAT),
					      "no field name", "localOpen");
		if (LST_Enqueue(&handle->fields, field) != LST_NORMAL)
		    return COND_PushCondition(TBL_ERROR(TBL_LISTFAILURE),
					      "localOpen");
	    }
	}
    }
...
handleCreate()
{
    ...
    * privateHandle;
    ...
    if (privateHandle == NULL) {
	(void) COND_PushCondition(TBL_ERROR(TBL_MALLOCFAILURE), "TBL_Open",
...
static CONDITION
localSelect(TBL_PRV_HANDLE * handle,
	    const TBL_CRITERIA * criteria, TBL_FIELD * fields, long *count,
	    CONDITION(*callback) (), void *ctx)
{
    ...
    * field;
    ...
    if (field != NULL)
	(void) LST_Position(&handle->fields, field);
	...
	if (match) {
	    ...
	    ++localCount;
	    if (callback != NULL)
		cond = callback(fields, localCount, ctx);
	}
    }
    if (count != NULL)
	*count = localCount;
...
{
    if (sscanf(value, "%d", field->Value.Value.Signed4) != 1)
	return 0;
    field->Value.IsNull = FALSE;
    field->Value.Size = 4;
    return TBL_NORMAL;
