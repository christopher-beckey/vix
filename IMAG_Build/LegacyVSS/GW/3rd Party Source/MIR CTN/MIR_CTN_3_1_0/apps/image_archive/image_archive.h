...
associationCheck(char *node, DUL_ASSOCIATESERVICEPARAMETERS * params,
	   DMAN_HANDLE ** handle, CTNBOOLEAN forgive, const char *genericAE,
...
serviceRequests(DUL_NETWORKKEY ** network, DUL_ASSOCIATIONKEY ** association,
	   DUL_ASSOCIATESERVICEPARAMETERS * service, CTNBOOLEAN reducedCap);
CONDITION
nextAssociationRequest(char *node, DUL_NETWORKKEY ** network,
	     DUL_ASSOCIATESERVICEPARAMETERS * service, unsigned long maxPDU,
		       ...
		       PROCESS_ELEMENT * processElement, CTNBOOLEAN useThreads);


CONDITION
parseImageForInsert(DCM_OBJECT ** object,
		    IDB_Insertion * insertStructure);
CONDITION parseQuery(DCM_OBJECT ** object, IDB_Query * queryStructure);
CONDITION verifyQuery(IDB_Query * queryStructure, char *SOPClass, int queryLevel);
CONDITION buildQuery(DCM_OBJECT ** object, IDB_Query * queryStructure);

CONDITION
parseImageForInsertMB(DCM_OBJECT ** object,
		    IDB_Insertion * insertStructure);
CONDITION parseQueryMB(DCM_OBJECT ** object, IDB_Query * queryStructure);
CONDITION verifyQueryMB(IDB_Query * queryStructure, char *SOPClass, int queryLevel);
CONDITION buildQueryMB(DCM_OBJECT ** object, IDB_Query * queryStructure);

void logMessage(char *controlString,...);
