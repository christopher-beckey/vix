...
static char rcsid[] = "$Revision: 1 $ $RCSfile: image_utils.c,v $";
#include	<stdio.h>
#include 	<stdlib.h>
#include	<fcntl.h>
#include	<string.h>
...
static CONDITION
checkCharacterSets(DCM_OBJECT** object, CHR_ENCODING* encoding)
{
  ...
  STRING_MATRIX firstElement[] = {
    { "EUCJP", "", CHR_ISO2022JP },
    { "EUCJP", "ISO_IR 100", CHR_ISO2022JP },
    { "EUCJP", "ISO_IR 100 ", CHR_ISO2022JP },
    { "EUCJP", "ISO 2022 IR 6", CHR_ISO2022JP },
    { "EUCJP", "ISO 2022 IR 6 ", CHR_ISO2022JP },
    { "EUCJP", "ISO 2022 IR 100", CHR_ISO2022JP },
    { "EUCJP", "ISO 2022 IR 100 ", CHR_ISO2022JP },
    { "EUCJPROMAJI", "ISO_IR 13", CHR_DEFAULTISOIR13 },
    { "EUCJPROMAJI", "ISO_IR 13 ", CHR_DEFAULTISOIR13 },
    { "EUCJPROMAJI", "ISO 2022 IR 13", CHR_DEFAULTISOIR13 },
    { "EUCJPROMAJI", "ISO 2022 IR 13 ", CHR_DEFAULTISOIR13 }
  };
  STRING_MATRIX secondaryElements[] = {
    { "EUCJP", "ISO 2022 IR 87", CHR_ISO2022JP },
    { "EUCJP", "ISO 2022 IR 87 ", CHR_ISO2022JP },
    { "EUCJP", "ISO 2022 IR 159", CHR_ISO2022JP },
    { "EUCJP", "ISO 2022 IR 159 ", CHR_ISO2022JP },
    { "EUCJPROMAJI", "ISO 2022 IR 87" , CHR_DEFAULTISOIR13 },
    { "EUCJPROMAJI", "ISO 2022 IR 87 ", CHR_DEFAULTISOIR13 },
    { "EUCJPROMAJI", "ISO 2022 IR 159", CHR_DEFAULTISOIR13 },
    { "EUCJPROMAJI", "ISO 2022 IR 159 ", CHR_DEFAULTISOIR13 },
    { "EUCKR", "ISO 2022 IR 149", CHR_ISO2022KR }
  };
  char* tok1;
  char* tok2;
  ...
  */
  ...
  for (index = 0;
    ...
    if (strcmp(firstElement[index].c2, tok1) == 0) {
      *encoding = firstElement[index].encoding;
      returnValue = 1;
    }
  }
  if (returnValue != 1)
  ...
  while(tok2 != NULL) {
	...
	index++) {
      ...
      if (strcmp(secondaryElements[index].c2, tok2) == 0) {
	*encoding = secondaryElements[index].encoding;
	returnValue = 1;
      }
    }
...
static void
getFileStream(const char* path, void **ptr, U32* fileSize)
{
  CONDITION cond;
  int fd;
  long bytesRead;
  ...
  cond = UTL_FileSize(path, fileSize);
  if (cond != UTL_NORMAL) {
    COND_DumpConditions();
    exit(1);
  }
  ...
  *ptr = malloc(*fileSize);
  if (*ptr == NULL) {
    fprintf(stderr, "Could not malloc %d bytes; exiting.\n",
        *fileSize);
    exit(1);
  }
  p = (unsigned char*)*ptr;
  ...
  if (bytesRead != *fileSize) {
    fprintf(stderr, "Error reading file: %s, bytes read = %d\n",
        path, *fileSize);
    perror("");
    exit(1);
  }
 ...
 */
    ...
    DCM_Debug(FALSE);
    if (useImport) {
      printf("Using import stream \n");
      getFileStream(filename, &ptr, &fileSize);
      cond = DCM_ImportStream(ptr, fileSize, options, &object);
    } else {
...
#ifdef CTN_MULTIBYTE
    cond = checkCharacterSets(&object, &idata->encoding);
    if (cond != 1) {
      fprintf(stderr, "Error in Specific Character Set (0008 0005) \n");
      idata->encoding = CHR_ISO2022JP;
    }
#endif
