...
#define ACC_PORT 0x0000		/* Dummy space filler - should be filled with
				 * acceptor port */
...
typedef struct {
    ...
    u_long seg_seq;		/* Segment's sequence number for 1st byte of
				 * data */
    u_long seg_ack;		/* Segment's acknowledgement number expected
				 * for last byte of data */
...
typedef struct {
    int fd_in;			/* File descriptor for incoming data STREAM */
        CONDITION(*callback_itoa) ();	/* Callback function for delivery of
					 * segment ini->acc */
    void *itoa_ctx;		/* Context pointer for callback_itoa call */
        CONDITION(*callback_atoi) ();	/* Callback function for delivery of
					 * segment acc->ini */
    void *atoi_ctx;		/* Context pointer for callback_atoi call */
        CONDITION(*callback_state) ();	/* Callback function for reporting
					 * state information */
...
CONDITION decode(char *ptr, int length);
void cleanLists();
void decodeDebug(CTNBOOLEAN flag);

/*
	Prototypes for DLPI operations found in dlroutines.c
*/

CONDITION dlbindreq(int fd, u_long sap, u_long max_conind, u_long service_mode, u_long conn_mgmt, u_long xidtest);
CONDITION dlunbindreq(int fd);
CONDITION dldetachreq(int fd);
CONDITION dlbindack(int fd, char *bufp);
CONDITION dlattachreq(int fd, u_long ppa);
CONDITION dlokack(int fd, char *bufp);
CONDITION dlpromisconreq(int fd, u_long level);
