/*---------------------------------------------------------------------
 * $Date: 11/04/10 9:24a $             $Revision: 1 $
 *---------------------------------------------------------------------
 *
 *
...
#ifdef _NO_PROTO
static int
handle_dialog_child(wgt, manage_func)
Widget wgt;
void (*manage_func) ();
#else
static int
...
#else
static void
popup_dialog(Widget wgt, XtGrabKind grab_flag)
#endif
{
...
#ifdef _NO_PROTO
int
UxPopupInterface(wgt, grab_flag)
Widget wgt;
XtGrabKind grab_flag;
#else
int
...
#ifdef _NO_PROTO
void
UxDeleteContextCB(wgt, client_data, _call_data)
Widget wgt;
XtPointer client_data,
    _call_data;
#else
void
UxDeleteContextCB(Widget wgt, XtPointer client_data,
...
#ifdef _NO_PROTO
int
UxPutContext(wgt, context)
Widget wgt;
caddr_t context;
#else
int
UxPutContext(Widget wgt, caddr_t context)
#endif
{
    int status;
...
#ifdef _NO_PROTO
static void
DelayedFreeData(client_data, _id)
XtPointer client_data;
XtIntervalId *_id;
#else
static void
DelayedFreeData(XtPointer client_data, XtIntervalId * _id)
#endif
{
    XtIntervalId *id = _id;
...
#ifdef _NO_PROTO
void
UxDestroyContextCB(_wgt, client_data, _call_data)
Widget _wgt;
XtPointer client_data,
    _call_data;
#else
void
UxDestroyContextCB(Widget _wgt, XtPointer client_data,
		   XtPointer _call_data)
#endif
{
    Widget wgt = _wgt;
...
#ifdef _NO_PROTO
void
UxFreeClientDataCB(_wgt, client_data, _call_data)
Widget _wgt;
XtPointer client_data,
    _call_data;
#else
void
UxFreeClientDataCB(Widget _wgt, XtPointer client_data,
		   XtPointer _call_data)
#endif
{
    Widget wgt = _wgt;
...
#ifdef _NO_PROTO
void
UxLoadResources(_fname)
char *_fname;
#else
void
UxLoadResources(char *_fname)
#endif
{
    char *fname = _fname;
}
...
#ifdef _NO_PROTO
XmFontList
UxConvertFontList(fontlist_str)
char *fontlist_str;
#else
XmFontList
UxConvertFontList(char *fontlist_str)
#endif
{
    XrmValue from,
        to;
...
#ifdef _NO_PROTO
wchar_t *
UxConvertValueWcs(value_str)
char *value_str;
#else
wchar_t *
UxConvertValueWcs(char *value_str)
#endif
{
...
#ifdef _NO_PROTO
Pixmap
UxConvert_bitmap(file_name)
char *file_name;
#else
Pixmap
UxConvert_bitmap(char *file_name)
#endif
{
...
#ifdef _NO_PROTO
Pixmap
UxConvertPixmap(file_name)
char *file_name;
#else
Pixmap
UxConvertPixmap(char *file_name)
#endif
{
...
#ifdef _NO_PROTO
XtArgVal
UxRemoveValueFromArgList(args, ptr_num_args, res_name)
Arg *args;
Cardinal *ptr_num_args;
String res_name;
#else
XtArgVal
UxRemoveValueFromArgList(Arg * args, Cardinal * ptr_num_args,
			 String res_name)
#endif
{
    Cardinal i,
	...
	if (!strcmp(args[i].name, res_name)) {
	    ...
	    /*
	     * We remove this arg from the array by moving the remaining args
	     * down one in the array and decreasng the arg count
	     */
...
#ifdef _NO_PROTO
void *
UxNewContext(size, isSubclass)
size_t size;
int isSubclass;
#else
void *
UxNewContext(size_t size, int isSubclass)
#endif				/* _NO_PROTO */
{
