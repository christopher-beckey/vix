...
**			This application requests an Association with a
**			single presentation context as an SCP of the Detached
**			Patient Management Meta SOP Class.  This demonstrates
**			SCU/SCP role negotiation.
**  Usage:
**	ex4_initiator [-c callingTitle][-d FAC][-m maxPDU][-t title][-v] node port\n\
**
** Last Update:		$Author: Vhaiswstarkm $, $Date: 11/04/10 9:24a $
** Source File:		$RCSfile: ex4_initiator.c,v $
** Revision:		$Revision: 1 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1 $ $RCSfile: ex4_initiator.c,v $";
...
main(int argc, char **argv)
{
	...
	* network = NULL;
    ...
    char
       *calledAPTitle = "RSNA_PACS_NODE",
       *callingAPTitle = "RSNA_HIS_NODE",
        localHost[40],
		      ...
		      DUL_ASSOCIATESERVICEPARAMETERS * params)
{
    ...
    * association = NULL;
    char
       *SOPClasses[] = {DICOM_SOPCLASSDETACHEDPATIENTMGMTMETA
    };
		   ...
		   int classCount)
{
    ...
    while (classCount-- > 0) {
	cond = SRV_RequestServiceClass(*SOPClasses++, DUL_SC_ROLE_SCP, params);
	if (cond != SRV_NORMAL) {
	    COND_DumpConditions();
	    return 0;
	}
