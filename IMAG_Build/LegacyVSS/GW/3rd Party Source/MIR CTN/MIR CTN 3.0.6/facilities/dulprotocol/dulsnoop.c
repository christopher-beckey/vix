...
**
** Last Update:         $Author: Vhaiswstarkm $, $Date: 11/04/10 9:28a $
** Source File:         $RCSfile: dulsnoop.c,v $
** Revision:            $Revision: 1 $
** Status:              $State: Exp $
*/

static char rcsid[] = "$Revision: 1 $ $RCSfile: dulsnoop.c,v $";
...
DUL_FileSnoop(char *itoa_file, char *atoi_file, char *initiator, char *acceptor)
{
    ...
    int read_atoi_flag;		/* Flag to indicate read from ini->acc file
				 * req'd */
    int read_itoa_flag;		/* Flag to indicate read from acc->ini file
				 * req'd */
    ...
    int fd_atoi;		/* File descriptor for initiator -> acceptor
				 * data */
    int fd_itoa;		/* File descriptor for acceptor -> initiator
				 * data */
	...
	return COND_PushCondition(DUL_SNPFILEOPEN, DUL_Message(DUL_SNPFILEOPEN),
				  itoa_file, "DUL_FileSnoop");
    if ((fd_atoi = open(atoi_file, O_RDONLY, 0)) == NULL)
	return COND_PushCondition(DUL_SNPFILEOPEN, DUL_Message(DUL_SNPFILEOPEN),
				  atoi_file, "DUL_FileSnoop");
    /*
	...
	if (read_itoa_flag) {
		...
		return COND_PushCondition(DUL_SNPFILEREAD,
					  DUL_Message(DUL_SNPFILEREAD),
					  itoa_file, "DUL_FileSnoop");
	}
	/*
	...
	if (read_atoi_flag) {
		...
		return COND_PushCondition(DUL_SNPFILEREAD, DUL_Message(DUL_SNPFILEREAD),
					  atoi_file, "DUL_FileSnoop");
	}
	/*
	 * Only if have a good header check against sequence number being
	...
	if (read_itoa || read_atoi)
	    return COND_PushCondition(DUL_SNPPREMATUREEOF,
			 DUL_Message(DUL_SNPPREMATUREEOF), "DUL_FileSnoop");
    }
    /*
		 ...
		 char *acceptor, int port, int bufsize, int associations)
{
    ...
    cond = SNP_Init();
    if (cond != SNP_NORMAL)
	return COND_PushCondition(DUL_SNPINIT, DUL_Message(DUL_SNPINIT),
				  "DUL_NetworkSnoop");
    /*
	...
	return COND_PushCondition(DUL_SNPBADSTATE,
				  DUL_Message(DUL_SNPBADSTATE),
			   SNP_StateMsg(current_state), "DUL_NetworkSnoop");

    /*
     * If finished up in a bad DUL association state, report it
     */
    if ((ini_state < 1) || (acc_state < 1)) {
	return COND_PushCondition(DUL_SNPBADASSOCSTATE,
				  DUL_Message(DUL_SNPBADASSOCSTATE));
    }
    return DUL_NORMAL;
}
...
callbackITOA(char *buffer, int bufsize, void *ctx)
{
...
#ifdef DEBUG
	if (debug)
	    fprintf(stderr, "Error: WRITE on ring buffer in callbackITOA\n");
#endif
...
#ifdef DEBUG
	    if (debug)
		fprintf(stderr, "Error: READ on ring buffer in callbackATOI\n");
#endif
	    return SNP_CALLBACKFAIL;
	}
	/*
	...
	if (avail_flag || (ring_buf[read_ptr] > DUL_MAXTYPE) || (!ring_buf[read_ptr])) {
		...
		if (cond != DUL_NORMAL)
		    return cond;
		callPDU.callITOA(ini_state, acc_state, DUL_TYPEASSOCIATERQ,
			   &ring_buf[read_ptr], (int) pdu_len + 6, &params);
		break;
		...
		callPDU.callITOA(ini_state, acc_state, DUL_TYPEASSOCIATEAC,
			   &ring_buf[read_ptr], (int) pdu_len + 6, &params);
		break;
		...
		callPDU.callITOA(ini_state, acc_state, DUL_TYPERELEASERQ,
			   &ring_buf[read_ptr], (int) pdu_len + 6, &params);
		break;
...
#ifdef DEBUG
		    if (debug)
			fprintf(stderr, "Error: FREE on ring buffer in callbackITOA\n");
#endif
		    return SNP_CALLBACKFAIL;
		}
	    }
...
callbackATOI(char *buffer, int bufsize, void *ctx)
{
...
#ifdef DEBUG
	if (debug)
	    fprintf(stderr, "Error: WRITE on ring buffer in callbackATOI\n");
#endif
	return SNP_CALLBACKFAIL;
    }
    do {
	...
	if (avail_flag || (ring_buf[read_ptr] > DUL_MAXTYPE) || (!ring_buf[read_ptr])) {
		...
		callPDU.callATOI(ini_state, acc_state, DUL_TYPEASSOCIATERQ,
			       &ring_buf[read_ptr], (int) pdu_len, &params);
		break;
	    case DUL_TYPEASSOCIATEAC:
		...
		if (cond != DUL_NORMAL)
		    return cond;
		callPDU.callATOI(ini_state, acc_state, DUL_TYPEASSOCIATEAC,
			   &ring_buf[read_ptr], (int) pdu_len + 6, &params);
		break;
	    case DUL_TYPEASSOCIATERJ:
...
#ifdef DEBUG
		    if (debug)
			fprintf(stderr, "Error: FREE on ring buffer in callbackATOI\n");
#endif
		    return SNP_CALLBACKFAIL;
		}
	    }
...
*/
static int
ring_write(unsigned char *ring_buf, unsigned char *buffer, int bufsize,
...
fill_rq_params(u_char * buf, u_long pdulen)
{
	...
	params.maximumOperationsPerformed =
	    assoc.userInfo.asyncOperations.maximumOperationsProvided;
	params.maximumOperationsInvoked =
	    assoc.userInfo.asyncOperations.maximumOperationsInvoked;
    }
...
fill_ac_params(u_char * buf, u_long pdulen)
{
	...
	*requestedPresentationCtx;
    ...
    if (prvCtx != NULL)
	(void) LST_Position(&assoc.presentationContextList, prvCtx);
    while (prvCtx != NULL) {
	...
	requestedPresentationCtx = (DUL_PRESENTATIONCONTEXT *)
	    findPresentationCtx(&params.requestedPresentationContext,
				prvCtx->contextID);
	...
	scuscpRole = (PRV_SCUSCPROLE *) findSCUSCPRole(&assoc.userInfo.SCUSCPRoleList,
	...
	if (LST_Enqueue(&params.acceptedPresentationContext,
			userPresentationCtx) != LST_NORMAL)
	    return COND_PushCondition(DUL_LISTERROR,
			      DUL_Message(DUL_LISTERROR), "fill_ac_params");
...
DUL_FileSnoop(char *itoa_file, char *atoi_file, char *initiator, char *acceptor)
{
    return COND_PushCondition(DUL_SNPUNIMPLEMENTED,
			      SNP_Message(DUL_SNPUNIMPLEMENTED),
			      "DUL_FileSnoop");
}
...
{
    return COND_PushCondition(DUL_SNPUNIMPLEMENTED,
			      DUL_Message(DUL_SNPUNIMPLEMENTED),
			      "DUL_NetworkSnoop");
}
