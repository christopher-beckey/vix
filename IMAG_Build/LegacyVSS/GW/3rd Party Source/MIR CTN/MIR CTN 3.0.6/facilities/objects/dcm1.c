...
static char rcsid[] = "$Revision: 1 $ $RCSfile: dcm1.c,v $";
...
DCM_ELEMENT metaRequired[] = {
    {DCM_METAINFORMATIONVERSION, DCM_OB, "", 1,
	sizeof(meta.fileMetaInformationVersion),
    (void *) meta.fileMetaInformationVersion},
    {DCM_METAMEDIASTORAGESOPCLASS, DCM_UI, "", 1,
	sizeof(meta.mediaStorageSOPClassUID),
    (void *) meta.mediaStorageSOPClassUID},
    {DCM_METAMEDIASTORAGESOPINSTANCE, DCM_UI, "", 1,
	sizeof(meta.mediaStorageSOPInstanceUID),
    (void *) meta.mediaStorageSOPInstanceUID},
    {DCM_METATRANSFERSYNTAX, DCM_UI, "", 1,
	sizeof(meta.transferSyntaxUID),
    (void *) meta.transferSyntaxUID},
    {DCM_METAIMPLEMENTATIONCLASS, DCM_UI, "", 1,
	sizeof(meta.implementationClassUID),
    (void *) meta.implementationClassUID}
};

DCM_FLAGGED_ELEMENT metaOptional[] = {
    {DCM_METAIMPLEMENTATIONVERSION, DCM_SH, "", 1,
	sizeof(meta.implementationVersionName),
	(void *) meta.implementationVersionName,
	DCM_FILEMETA_IMPLEMENTATIONVERSIONNAME,
    &meta.flag},
    {DCM_METASOURCEAETITLE, DCM_AE, "", 1,
	sizeof(meta.sourceApplicationEntityTitle),
	meta.sourceApplicationEntityTitle,
	DCM_FILEMETA_SOURCEAPPLICATIONENTITYTITLE,
    &meta.flag},
    {DCM_METAPRIVATEINFORMATIONCREATOR, DCM_UI, "", 1,
	sizeof(meta.privateInformationCreatorUID),
	(void *) meta.privateInformationCreatorUID,
	DCM_FILEMETA_PRIVATEINFORMATIONCREATORUID,
    &meta.flag}
};
CONDITION
DCM_GetFileMeta(DCM_OBJECT ** callerObject, DCM_FILE_META ** fileMeta)
{
    ...
    cond = DCM_ParseObject(callerObject,
			   metaRequired, (int) DIM_OF(metaRequired),
			   metaOptional, (int) DIM_OF(metaOptional), NULL);
    if (cond != DCM_NORMAL)
	return cond;		/* repair */
...
DCM_SetFileMeta(DCM_OBJECT ** callerObject, DCM_FILE_META * fileMeta)
{
    ...
    meta = *fileMeta;
    cond = DCM_ModifyElements(callerObject,
			      metaRequired, (int) DIM_OF(metaRequired),
			    metaOptional, (int) DIM_OF(metaOptional), NULL);
    if (cond != DCM_NORMAL)
	return cond;		/* repair */
...
DCM_FreeFileMeta(DCM_FILE_META ** fileMeta)
{
    ...
    if ((*fileMeta)->privateInformation != NULL)
	CTN_FREE((*fileMeta)->privateInformation);
