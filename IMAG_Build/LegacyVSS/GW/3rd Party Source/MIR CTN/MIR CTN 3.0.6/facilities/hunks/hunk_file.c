...
** Source File:		$Source: /sw2/prj/ctn/cvs/facilities/hunks/hunk_file.c,v $
** Revision:		$Revision: 1 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1 $ $RCSfile: hunk_file.c,v $";
...
static int
    Opened = 0;			/* has the database been opened, initially no */

static HunkFileHeader
    GS_hfh;			/* the hunk file header	 */
static Hunk
    GS_curhunk;			/* the hunk currently in memory */
static int
    GS_hunkfd;			/* the hunk file file descriptor */
static void
   *GS_hunkstorage = NULL;	/* the storage for the hunk data */
static void
   *GS_addstorage = NULL;	/* the storage for the address data */
...
CONDITION
HF_Create(char *fname, int num_hunks,
	  int hunk_record_length, int num_recs_per_hunk)
{

    int
        wb,			/* the  number of bytes written */
        addsize,		/* the size of the addresses	 */
    ...
    if (num_hunks < 1 || hunk_record_length < 1 || num_recs_per_hunk < 1)
	return (HF_BADPARMS);

    if ((GS_hunkfd = open(fname, O_RDWR | O_CREAT, 0777)) < 0)
	return (HF_CREATERROR);
 ...
 */
    ...
    if ((GS_hunkstorage = (void *) malloc(hunksize)) == (void *) NULL) {
	return (HF_NOMEMORY);
    }
    if ((GS_addstorage = (void *) malloc(addsize)) == (void *) NULL) {
	return (HF_NOMEMORY);
    }
...
HF_Open(char *fname)
{
	...
	hunksize += ((GS_hfh.hunk_record_length + sizeof(HunkBufAdd)) *
		     GS_hfh.num_recs_per_hunk);
    ...
    if ((lseek(GS_hunkfd, (off_t) seek_pos, SEEK_SET)) < 0) {
	return (HF_SEEKERROR);
    }
...
HF_Close(char *fname)
{
    ...
    if (HF_WriteFileHeader() != HF_NORMAL) {
	return (HF_WRITERROR);
    }
...
HF_AddHunk()
{
    ...
    if ((retval = HF_WriteCurrentHunk()) != HF_NORMAL)
	return (retval);
    if ((lseek(GS_hunkfd, (off_t) 0, SEEK_END)) < 0) {
	return (HF_SEEKERROR);
    }
...
HF_WriteCurrentHunk(void)
{
    ...
    if (GS_curhunk.hunk_head->hunk_dirty == 1) {	/* this page is
							 * dirty...write it out */
	seek_pos = sizeof(HunkFileHeader) +
	    (GS_curhunk.hunk_head->hunk_number * GS_hfh.hunk_length);
	if ((lseek(GS_hunkfd, (off_t) seek_pos, SEEK_SET)) < 0) {
	    return (HF_SEEKERROR);
	}
	GS_curhunk.hunk_head->hunk_dirty = 0;
...
HF_WriteFileHeader()
{
    ...
    if ((write(GS_hunkfd, (char *) &GS_hfh, sizeof(HunkFileHeader))) !=
	sizeof(HunkFileHeader)) {
	return (HF_WRITERROR);
    }
    return (HF_NORMAL);
...
HF_ReadCurrentHunk(int hunk_num)
{
    ...
    if (hunk_num >= GS_hfh.hunks_allocated)
	return (HF_BADHUNKNUM);

    if (GS_curhunk.hunk_head->hunk_number == hunk_num)
	return (HF_NORMAL);
...
HF_AllocateStaticRecord(int hunk_number, int size)
{
    ...
    if ((retval = HF_AllocateRecord(size, &add)) != HF_NORMAL) {
	return (retval);
    }
    if ((retval = HF_ReadCurrentHunk(hunk_number)) != HF_NORMAL)
	return (retval);

    if (GS_curhunk.hunk_head->static_buf_node.hunk_number != HUNK_PTR_NULL &&
	GS_curhunk.hunk_head->static_buf_node.node_number != HUNK_PTR_NULL) {
	return (HF_STATICINUSE);
    }
...
HF_AllocateRecord(int size, HunkBufAdd *address)
{
    ...
    if (size > (GS_hfh.hunk_record_length *
		GS_hfh.num_recs_per_hunk))
	return (HF_RECTOOBIG);

    num_bufs = size / GS_hfh.hunk_record_length;
    if ((size % GS_hfh.hunk_record_length) != 0)
	num_bufs++;
	...
	if ((retval = HF_ReadCurrentHunk(i)) != HF_NORMAL)
	    return (retval);

	if (num_bufs <= GS_curhunk.hunk_head->free_nodes) {	/* we can service it
								 * here... */
	...
	if ((retval = HF_AddHunk()) != HF_NORMAL)
	    ...
	    if ((retval = HF_ReadCurrentHunk(GS_hfh.hunks_allocated - 1)) !=
		HF_NORMAL)
		return (retval);
	    else {
		free_index = HF_FindFreeNode();
...
HF_DeallocateRecord(HunkBufAdd *address)
{
    ...
    if ((retval = HF_ReadCurrentHunk(address->hunk_number)) != HF_NORMAL)
	return (retval);
...
HF_DeallocateStaticRecord(int hunk_number)
{
    ...
    GS_curhunk.hunk_head->static_buf_node.node_number = HUNK_PTR_NULL;
    GS_curhunk.hunk_head->hunk_dirty = 1;
    if ((retval = HF_DeallocateRecord(&add)) != HF_NORMAL)
	return (retval);
...
HF_Dumper(void)
{
    ...
    printf("Allocated: %d, Length: %d, Rec Len: %d Recs/hunk: %d ID: %s\n\n",
	   GS_hfh.hunks_allocated, GS_hfh.hunk_length,
	   GS_hfh.hunk_record_length, GS_hfh.num_recs_per_hunk,
	   GS_hfh.hunk_file_id);
    for (i = 0; i < GS_hfh.hunks_allocated; i++) {
	...
	printf("Hunk static_buf_node: %d %d\n",
	       GS_curhunk.hunk_head->static_buf_node.hunk_number,
	       GS_curhunk.hunk_head->static_buf_node.node_number);
	printf("Hunk free_nodes: %d\n", GS_curhunk.hunk_head->free_nodes);
	if (GS_curhunk.hunk_head->free_nodes < 100) {
	    for (j = 0; j < 100; j++) {
		if (GS_curhunk.node_list[j] == HUNK_USED) {
		    printf("\tBuffer %3d [Link: %3d] ",
			   j, GS_curhunk.buf_pool[j].nextp->node_number);
		    for (k = 0; k < 15; k++)
			printf("%c", GS_curhunk.buf_pool[j].buf[k]);
		    printf(" ... ");
		    for (k = GS_hfh.hunk_record_length - 15;
			 k < GS_hfh.hunk_record_length; k++)
			printf("%c", GS_curhunk.buf_pool[j].buf[k]);
		    printf("\n");
		}
	    }
...
HF_ReadRecord(HunkBufAdd *add, int bufsize, void *buffer)
{
    ...
    if ((retval = HF_ReadCurrentHunk(add->hunk_number)) != HF_NORMAL)
	return (retval);

    if (add->node_number < 0)
	return (HF_BADNODENUM);

    if (bufsize <= GS_hfh.hunk_record_length) {
	memcpy((char *) buffer,
	       (char *) GS_curhunk.buf_pool[add->node_number].buf,
	       bufsize);
    } else {
	...
	while (left_to_transfer > GS_hfh.hunk_record_length) {
	    memcpy((char *) pbuffer,
		   (char *) GS_curhunk.buf_pool[current_node].buf,
		   GS_hfh.hunk_record_length);
