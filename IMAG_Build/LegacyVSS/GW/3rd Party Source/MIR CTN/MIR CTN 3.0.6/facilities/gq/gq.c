...
static char rcsid[] = "$Revision: 1 $ $RCSfile: gq.c,v $";
...
#define GQ_UNIMPLEMENTED_TXT	\
	"GQ Routine %s is unimplemented on this machine architecture"
...
GQ_KillQueue(int qid)
{
    ...
    if (ret != GQ_NORMAL) {
	COND_PushCondition(GQ_NOPENQUEUE,
			   "GQ_KillQueue: No open queue (ID : %d)", qid);
	return (GQ_NOPENQUEUE);
    }
    if ((shmdt(SHM_ADD)) == -1) {
	COND_PushCondition(GQ_SHAREDMEMORYFAIL,
			   "%s", "GQ_KillQueue: shmdt failed");
	...
	COND_PushCondition(GQ_SHAREDMEMORYFAIL,
			   "%s", "GQ_KillQueue: shmctl failed");
	return (GQ_SHAREDMEMORYFAIL);
    }
    if ((semctl(SEM_ID, 0, IPC_RMID, sem_buf)) == -1) {
	COND_PushCondition(GQ_SEMAPHOREFAIL,
			   "%s", "GQ_SemaphoreRelease: semctl failed");
    ...
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_KillQueue");
#endif
}
...
GQ_InitQueue(int qid, int num_elements, int element_size)
{
    ...
    struct shmid_ds
        shm_buf;		/* In case we need to abort...		 */

    struct stat
        sbuf;			/* for getting file status information */

/* check whether a generalized queue already exists for the specified queue
   ID */

    fname = GQ_MakeFilename(qid);
    if (fname == NULL) {
	COND_PushCondition(GQ_FILEACCESSFAIL, "%s",
		 "GQ_InitQueue: Cannot access GQ_QUEUEDIRECTORY/gq???.dat");
	return (GQ_FILEACCESSFAIL);
    }
    if (stat(fname, &sbuf) < 0) {
	if (errno == ENOENT) {
	    /* file does not exist and we need to create it */
	    if ((fd = fopen(fname, "w")) == NULL) {
		COND_PushCondition(GQ_FILECREATEFAILED, "%s: %s",
				   "GQ_InitQueue: Cannot create", fname);
		return (GQ_FILECREATEFAILED);
	    }
	} else {
	    return COND_PushCondition(GQ_FILEACCESSFAIL, "%s",
		 "GQ_InitQueue: Cannot access GQ_QUEUEDIRECTORY/gq???.dat");
	}
    } else {
	/* GQ already exists for the given identifier */
	return COND_PushCondition(GQ_MULTCREATEREQUEST,
		       "GQ_InitQueue: GQ with ID (%d) already exists", qid);
    }
/*
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_InitQueue");
#endif
}
...
GQ_Enqueue(int qid, void *element)
{
    ...
    if (ret != GQ_NORMAL) {
	COND_PushCondition(GQ_NOPENQUEUE,
			   "GQ_Enqueue: No open queue (ID : %d)", qid);
	return (GQ_NOPENQUEUE);
    }
	...
	COND_PushCondition(ret,
			   "%s", "GQ_Enqueue: Semaphore failure");
	return (ret);
    }
    if ((QPTRS->GQ_head == (QPTRS->GQ_tail + 1)) ||
	((QPTRS->GQ_head == 0) &&
	 (QPTRS->GQ_tail == (QPTRS->GQ_numelements - 1)))) {
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_Enqueue");
#endif
}
 ...
 */
    ...
    if (ret != GQ_NORMAL) {
	COND_PushCondition(GQ_NOPENQUEUE,
			   "GQ_Dequeue: No open queue (ID : %d)", qid);
	return (GQ_NOPENQUEUE);
    }
    if (SM_attached == FALSE) {
    ...
    if ((ret = GQ_Wait()) != GQ_NORMAL) {
	COND_PushCondition(ret,
			   "%s", "GQ_Dequeue: Semaphore failure");
	return (ret);
    }
	...
	COND_PushCondition(GQ_QUEUEEMPTY, "GQ_Dequeue: Queue (ID %d) empty",
			   qid);
 ...
 */
    if (++QPTRS->GQ_head >= QPTRS->GQ_numelements)
	QPTRS->GQ_head = 0;
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_Dequeue");
#endif
}
...
GQ_GetQueue(int qid, int element_size)
{
	...
	COND_PushCondition(GQ_FILEACCESSFAIL,
	    "%s", "GQ_GetQueue: Cannot access GQ_QUEUEDIRECTORY/gq???.dat");
	return (GQ_FILEACCESSFAIL);
    }
 ...
 */
	...
	shmdt(SHM_ADD);
	COND_PushCondition(GQ_BADELEMSIZE,
			   "GQ_GetQueue: Bad element size specification");
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_GetQueue");
#endif
}
...
GQ_PrintQueue(int qid, void (print_func(void *)))
{
    ...
    if (ret != GQ_NORMAL) {
	COND_PushCondition(GQ_NOPENQUEUE,
			   "GQ_PrintQueue: No open queue (ID : %d)", qid);
	return (GQ_NOPENQUEUE);
    }
    if (SM_attached == FALSE) {
    ...
    if ((ret = GQ_Wait()) != GQ_NORMAL) {
	COND_PushCondition(ret,
			   "%s", "GQ_PrintQueue: Semaphore failure");
	return (ret);
    }
    if (QPTRS->GQ_head == QPTRS->GQ_tail) {
	if ((ret = GQ_Signal()) != GQ_NORMAL) {
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_PrintQueue");
#endif
}
 ...
 */
    ...
    if (ret != GQ_NORMAL) {
	COND_PushCondition(GQ_NOPENQUEUE,
			   "GQ_Peek: No open queue (ID : %d)", qid);
	return (GQ_NOPENQUEUE);
    }
    if (SM_attached == FALSE) {
 ...
 */
    if (!element) {
	return COND_PushCondition(GQ_NOMEMORY, "%s",
		       "GQ_PeekQueue, No memory for allocated for element");
    }
    ep = (char *) GQ;
    ep += (QPTRS->GQ_head * QPTRS->GQ_elementsize);
    memcpy((char *) element, ep, QPTRS->GQ_elementsize);
 ...
 */
    ...
    if (ret != GQ_NORMAL) {
	COND_PushCondition(GQ_NOPENQUEUE,
			   "GQ_Modify: No open queue (ID : %d)", qid);
	return (GQ_NOPENQUEUE);
    }
    if (SM_attached == FALSE) {
 ...
 */
    if (!element) {
	return COND_PushCondition(GQ_NOMEMORY, "%s",
			     "GQ_ModifyHeadElement: No memory for element");
    }
    ep = (char *) GQ;
    ep += (QPTRS->GQ_head * QPTRS->GQ_elementsize);
    memcpy((char *) element, ep, QPTRS->GQ_elementsize);
...
/* operate on this element */
    if (func)
	func(element);		/* user defined function */
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_Modify");
#endif
}
/* GQ_Wait
**
...
GQ_Wait(void)
{
	...
	COND_PushCondition(GQ_SEMAPHOREFAIL,
			   "%s", "GQ_Wait: semop failed");
	return (GQ_SEMAPHOREFAIL);
    }
    return (GQ_NORMAL);
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_Wait");
#endif
}
...
GQ_MakeFilename(int qid)
{
	...
	(void) COND_PushCondition(GQ_MAXQUEUEEXCEEDED,
		     "GQ QID Exceeded maximum allowed queue number (%d %d)",
				  GQ_MAXNAMEDQUEUES, qid);
	return (NULL);
...
GQ_GetQueueSize(int qid, int *size)
{
    ...
    if (!size)
	return COND_PushCondition(GQ_NOMEMORY, "GQ_GetQueueSize: No memory allocated for returning length");
    *size = 0;
    ret = access_queue(qid);
    if (ret != GQ_NORMAL) {
	COND_PushCondition(GQ_NOPENQUEUE,
			   "GQ_GetQueueSize: No open queue (ID : %d)", qid);
	return (GQ_NOPENQUEUE);
    }
    if (SM_attached == FALSE) {
    ...
    if ((ret = GQ_Wait()) != GQ_NORMAL) {
	COND_PushCondition(ret,
			   "%s", "GQ_GetQueueSize: Semaphore failure");
	return (ret);
    }
    if (QPTRS->GQ_head == QPTRS->GQ_tail) {
	if ((ret = GQ_Signal()) != GQ_NORMAL) {
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "GQ_GetQueueSize");
#endif
}
...
access_queue(int qid)
{
    ...
    if ((fname = GQ_MakeFilename(qid)) == NULL) {
	COND_PushCondition(GQ_FILEACCESSFAIL,
	   "%s", "access_queue: Cannot access GQ_QUEUEDIRECTORY/gq???.dat");
	return (GQ_FILEACCESSFAIL);
    }
    if ((fd = fopen(fname, "r")) == NULL) {
	COND_PushCondition(GQ_FILEACCESSFAIL,
			   "access_queue: Cannot access file (%s)", fname);
	return (GQ_FILEACCESSFAIL);
    }
    fscanf(fd, "%d", &SEM_ID);
    fscanf(fd, "%d", &SHM_ID);
...
#else
    return COND_PushCondition(GQ_UNIMPLEMENTED,
			      GQ_UNIMPLEMENTED_TXT, "access_queue");
#endif
}
