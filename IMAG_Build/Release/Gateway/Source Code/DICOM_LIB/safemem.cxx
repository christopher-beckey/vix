...
void	* operator	new(size_t	x)
	...
	if ( ! x )
		{
		fprintf(stdout, "Attempt to allocate zero length block in %s/%d\n",
...
void	 operator	delete	( void	*vptr )
	...
	if ( ! vptr )
		{
		fprintf(stdout, "Attempt to delete NULL pointer %s/%d\n",
			TempFileName, TempLineNumber);
		return;
		}
	if (!UnRegisterMemory(vptr))
		{
		printf("Possible Duplicate Delete in File: %s Line %d vptr=%x\n",
